<?php
/**
 * Handles the functionality of admin emails
 *
 ...
 * Class start.
 */
if (!class_exists('KA_Osm_Admin_Email')) {
	/**
	 * Define Class
	 */
	class KA_Osm_Admin_Email extends WC_Email {
		protected $map = array();
		public function __construct() {
			// Override the necessary class members of WC_Email.
			$this->map            = $this->get_email_status_data();
			$this->id             = 'custom_order_admin_statuses_emails';
			$this->recipient      = $this->get_option('recipient', get_option('admin_email'));
			$this->title          = __('Custom Order Status Admin Emails', 'addify_osm');
			$this->description    = __('Custom order status change notification will be sent to admin when order status has been changed.', 'addify_osm');
			...
			$this->template_html  = 'templates/emails/admin-custom-order-status.php';
			$this->template_plain = 'templates/emails/plain/admin-custom-order-status.php';
			...
			if (is_array($this->map) || is_object($this->map)) {
				foreach ($this->map as $key => $status_email) {
					...
					if ('admin' == $status_email['email_recipient']) {
						if ('any' == $status_email['from_status']) {
							// Check if from status is any.
							add_action('woocommerce_order_status_' . $to_status . '_email_notification', array( $this, 'trigger' ), 10, 2);
						} else {
							...
							if (is_array($this->get_auto_status_data()) || is_object($this->get_auto_status_data())) {
								...
								foreach ($this->get_auto_status_data() as $rule_data) {
									...
									if ($rule_data['rule_from'] == $status_email['from_status'] && $rule_data['rule_to'] == $status_email['to_status']) {
										// Check if disable admin notification is enabled i automation rules.
										if ('yes' == $rule_data['admin_notify']) {
							...
							if (true == $flag) {
								add_action('woocommerce_order_status_' . $from_status . '_to_' . $to_status . '_email_notification', array( $this, 'trigger' ), 10, 2);
							}
						}
					}
				}
			...
			add_filter('woocommerce_email_subject_custom_order_admin_statuses_emails', array( $this, 'change_email_subject' ), 10, 2);
			// Change email heading.
			add_filter('woocommerce_email_heading_custom_order_admin_statuses_emails', array( $this, 'change_email_heading' ), 10, 2);
			// Change email additional content.
			add_filter('woocommerce_email_additional_content_custom_order_admin_statuses_emails', array( $this, 'change_email_content' ), 10, 2);
		}
		...
		public function get_default_subject() {
			return __('Order by {order_billing_full_name} on {order_date}, currently status is {order_status}.', 'addify_osm');
		}
		/**
		 * Override default heading
		 */
		public function get_default_heading() {
			return __('The status of order is {order_status}.', 'addify_osm');
		}
		/**
		 * Override default content
		 */
		public function get_default_additional_content() {
			return __('Below are the order details', 'addify_osm');
		}
		/**
		 * Trigger email
		 *
		...
		public function init_form_fields() {
			...
			$this->form_fields = array(
				...
				'subject'            => array(
					...
					'description' => sprintf(__('Available placeholders: %s', 'addify_osm'), '<code>{order_billing_full_name}, {site_title}, {order_date}, {order_number}, {order_status}</code>'),
		...
		public function get_auto_status_data() {
			...
			if (!empty($status_rules)) {
				foreach ($status_rules  as $key =>  $status_rule) {
					$order_status_emails[ $key ]['admin_notify'] = get_post_meta($status_rule, 'osm_disable_admin_notification', true);
