...
if ( ! BWFAN_PRO_Common::is_lite_3_0() && ! class_exists( 'BWFAN_Contact_Address' ) ) {
	class BWFAN_Contact_Address extends BWFAN_Merge_Tag {
		...
		public function __construct() {
			...
			$this->tag_description = __( 'Contact Address', 'wp-marketing-automations-pro' );
		...
		public function get_address( $cid ) {
			...
			if ( ! empty( $contact->contact->get_country() ) ) {
				$address[] = BWFAN_PRO_Common::get_country_name( $contact->contact->get_country() );
			}
	 ...
	 */
	BWFAN_Merge_Tag_Loader::register( 'bwf_contact', 'BWFAN_Contact_Address', null, __( 'Contact', 'wp-marketing-automations-pro' ) );
}