...
class BWFAN_Caldera_Form_Field extends BWFAN_Merge_Tag {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_bwfan_get_automation_caldera_form_fields', array( $this, 'bwfan_get_automation_caldera_form_fields' ) );
	}
	...
	public function get_view() {
        ...
        <select id="" class="bwfan-input-wrapper bwfan-mb-15 bwfan_tag_select bwfan_caldera_form_fields"
	...
	public function bwfan_get_automation_caldera_form_fields() {
		...
		if ( empty( $automation_obj->error ) ) {
			...
			if ( isset( $automation_meta['event_meta'] ) && isset( $automation_meta['event_meta']['bwfan-caldera_form_submit_form_id'] ) ) {
				$form_id = sanitize_text_field( $automation_meta['event_meta']['bwfan-caldera_form_submit_form_id'] );
				$fields  = [];
				if ( ! empty( $form_id ) ) {
					$obj    = BWFAN_Caldera_Form_Submit::get_instance();
	...
	public function get_setting_schema() {
		return [
			[
				...
				"description" => "",
				"ajax_cb"     => 'bwfan_get_automation_caldera_form_fields',
			]
		];
	}
}
...
if ( bwfan_is_caldera_forms_active() ) {
	BWFAN_Merge_Tag_Loader::register( 'calderaforms', 'BWFAN_Caldera_Form_Field', null, __( 'Caldera Form', 'wp-marketing-automations-pro' ) );
}
