...
class BWFAN_WPFORMS_Form_Field extends BWFAN_Merge_Tag {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_bwfan_get_automation_wpform_form_fields', array( $this, 'bwfan_get_automation_wpform_form_fields' ) );
	}
	...
	public function get_view() {
        ...
        <select id="" class="bwfan-input-wrapper bwfan-mb-15 bwfan_tag_select bwfan_wpforms_form_fields" style="padding-left:10px;" name="field"></select>
		<?php
	...
	public function parse_shortcode( $attr ) {
		...
		if ( is_array( $field_value ) ) {
			$field_value = isset( $field_value['first'] ) ? implode( ' ', $field_value ) : ( isset( $field_value['primary'] ) ? $field_value['primary'] : implode( ', ', $field_value ) );
		}

		return $this->parse_shortcode_output( $field_value, $attr );
	}

	public function bwfan_get_automation_wpform_form_fields() {
		...
		if ( empty( $automation_obj->error ) ) {
			...
			if ( isset( $automation_meta['event_meta'] ) && isset( $automation_meta['event_meta']['bwfan-wpforms_form_submit_form_id'] ) ) {
				$form_id = sanitize_text_field( $automation_meta['event_meta']['bwfan-wpforms_form_submit_form_id'] );
				$fields  = [];
				if ( ! empty( $form_id ) ) {
					$obj    = BWFAN_WPFORMS_Form_Submit::get_instance();
	...
	public function get_setting_schema() {
		return [
			[
				...
				"description" => "",
				"ajax_cb"     => 'bwfan_get_automation_wpform_form_fields',
			]
		];
	}
