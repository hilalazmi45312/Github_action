...
if ( ! BWFAN_PRO_Common::is_lite_3_0() && ! class_exists( 'BWFCRM_Fields' ) ) {
	...
	class BWFCRM_Fields {
		...
		public static function get_groups_with_fields( $show_address_fields = false, $add_email_field = false, $editable_by_user = null, $mapping = false ) {
			...
			$field_data[0]['name'] = __( 'General', 'wp-marketing-automations-pro' );
			$field_data[0]['id']   = 0;

			/** Email Field */
			if ( true === $add_email_field ) {
				$field_data[0]['fields']['email'] = __( 'Email', 'wp-marketing-automations-pro' );
			}
			...
			foreach ( $fields as $field_key => $field ) {
				...
				$group_id   = 0;
				$group_name = __( 'General', 'wp-marketing-automations-pro' );
				if ( ! empty( $field['gid'] ) ) {
					$group_id   = $field['gid'];
			...
			/** 'Creation Date' Field */
			if ( true === $mapping ) {
				$field_data[0]['fields']['creation_date'] = __( 'Creation Date (Y-m-d)', 'wp-marketing-automations-pro' );
			}
		...
		public static function add_field( $field_name, $type, $options, $placeholder, $mode, $vmode, $search, $group_id = 0, $view = 1 ) {
			...
			if ( ! empty( $field ) ) {
				return BWFCRM_Common::crm_error( __( 'Field already exists', 'wp-marketing-automations-pro' ) );
			}
			$meta = array();
			if ( ! empty( $options ) ) {
		...
		public static function update_field( $field_id, $group_id, $field_name, $type, $options, $placeholder, $slug, $mode, $vmode, $search ) {
			...
			if ( empty( $already_exists ) ) {
				...
				return array(
					'status'  => 404,
					'message' => __( 'Field slug already exists, kindly change', 'wp-marketing-automations-pro' ),
				);
			}
