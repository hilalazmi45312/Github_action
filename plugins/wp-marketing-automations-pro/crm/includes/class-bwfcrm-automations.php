...
if ( ! BWFAN_PRO_Common::is_lite_3_0() && ! class_exists( 'BWFCRM_Automations' ) ) {
	/**
	...
	class BWFCRM_Automations {
		...
		public function get_contact_tasks( $contact_id ) {
			...
			$scheduled_task = BWFCRM_Model_Automations::get_tasks_for_contact( $contact_id );
			$contact_logs   = BWFCRM_Model_Automations::get_logs_for_contact( $contact_id );
		...
		public function get_tasks_items( $active_automations, $tasks, $type = 'tasks' ) {
			...
			foreach ( $tasks as $task_id => $task ) {
				...
				$items[ $type ][ $task_id ] = array(
					...
					'automation_source'       => ! is_null( $source_instance ) ? $source_instance->get_name() : __( 'Data unavailable. Contact Support.', 'wp-marketing-automations-pro' ),
					'automation_event'        => ! is_null( $event_instance ) ? $event_instance->get_name() : __( 'Data unavailable. Contact Support.', 'wp-marketing-automations-pro' ),
					'task_integration'        => esc_html__( 'Not Found', 'wp-marketing-automations-pro' ),
					'task_integration_action' => esc_html__( 'Not Found', 'wp-marketing-automations-pro' ),
				...
				$items[ $type ][ $task_id ]['task_details']   = ! is_null( $event_instance ) ? $event_instance->get_task_view( $task_details ) : '<b>' . __( 'Data unavailable. Contact Support.', 'wp-marketing-automations-pro' ) . '</b>';
		...
		public function get_abandoned_phone( $checkout_data ) {
			...
			$phone_value   = ( is_array( $checkout_data ) && isset( $checkout_data['fields'] ) && is_array( $checkout_data['fields'] ) && isset( $checkout_data['fields']['billing_phone'] ) && ! empty( $checkout_data['fields']['billing_phone'] ) ) ? $checkout_data['fields']['billing_phone'] : __( 'N.A.', 'wp-marketing-automations-pro' );
		...
		public function get_last_abandoned_cart( $contact_email ) {
			...
			$contact_abandoned_carts = BWFCRM_Model_Automations::get_last_abandoned_cart( $contact_email );
		...
		public static function get_top_automations() {
			$top_automations = BWFCRM_Model_Automations::get_top_automations();
