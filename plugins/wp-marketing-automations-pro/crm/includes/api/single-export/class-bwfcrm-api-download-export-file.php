...
class BWFCRM_API_Download_Single_Export_File extends BWFCRM_API_Base {
    public static $ins;
    ...
    public function process_api_call() {
        ...
        $user_data = get_user_meta( $user_id, 'bwfcrm_single_export_status', true );
        if ( ! isset( $user_data[ $type ] ) || ! isset( $user_data[ $type ][ 'url' ] ) ) {
            $this->response_code = 404;
            $response = __( 'Unable to download the exported file', 'wp-marketing-automations-pro' );
...
BWFCRM_API_Loader::register( 'BWFCRM_API_Download_Single_Export_File' );
