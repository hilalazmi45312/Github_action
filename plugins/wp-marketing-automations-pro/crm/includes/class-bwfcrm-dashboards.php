...
class BWFCRM_Dashboards {
	 ...
	 * @return array|object|null
	 */
	public static function get_total_email_sents( $start_date, $end_date, $is_interval, $interval ) {
		global $wpdb;
		...
		$base_query  = "SELECT  count( id ) as email_sents" . $interval_query . "  FROM `" . $table . "` WHERE 1 = 1  " . $where . " and c_status = 2 " . $group_by . " ORDER BY " . $order_by . " ASC";
		$email_sents = $wpdb->get_results( $base_query, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function get_total_email_open( $start_date, $end_date, $is_interval, $interval ) {
		...
		$open_counts = $wpdb->get_results( $base_query, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function get_total_email_click( $start_date, $end_date, $is_interval, $interval ) {
		...
		$email_click = $wpdb->get_results( $base_query, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function get_total_orders( $start_date, $end_date, $is_interval, $interval ) {
		...
		$base_query   = "SELECT  count( id ) as total_orders" . $interval_query . "  FROM `" . $table . "` WHERE 1 = 1 " . $where . " and wcid != 0 " . $group_by . " ORDER BY " . $order_by . " ASC";
		$total_orders = $wpdb->get_results( $base_query, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	 ...
	 * @return array|object|null
	 */
	public static function get_total_revenue( $start_date, $end_date, $is_interval, $interval ) {
		global $wpdb;
		$table          = $wpdb->prefix . 'bwfan_conversions';
		$date_col       = "date";
		$interval_query = '';
		...
		$base_query    = "SELECT  sum( wctotal ) as total_revenue" . $interval_query . "  FROM `" . $table . "` WHERE 1 = 1 " . $where . " " . $group_by . " ORDER BY " . $order_by . " ASC";
		$total_revenue = $wpdb->get_results( $base_query, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	 ...
	 * @return array|object|null
	 */
	public static function get_email_trends( $start_date, $end_date, $by = 'day' ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'bwfan_engagement_tracking';
		// $saved_last_id  = get_option( 'bwfan_show_contacts_from' );
		// $start_id_query = ! empty( $saved_last_id ) ? " AND cid > $saved_last_id " : '';
		$start_id_query = '';
		$query          = " SELECT $by, COUNT(ID) as opens from $table_name where f_open BETWEEN '" . $start_date . "' AND  '" . $end_date . "' AND $by!='' $start_id_query GROUP BY $by ";
		$response       = $wpdb->get_results( $query, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

		if ( empty( $response ) ) {
			return [];
		}

		$opens      = array_map( function ( $open_data ) {
			return isset( $open_data['opens'] ) ? $open_data['opens'] : 0;
		}, $response );
		$total_open = array_sum( $opens );

		$prepare_data = [];
		foreach ( $response as $data ) {
			$mean           = empty( $data['opens'] ) ? 0 : $data['opens'] / $total_open;
			$prepare_data[] = [
				$by     => $data[ $by ],
				'mean'  => number_format( $mean, 4 ),
				'open'  => $data['opens'],
				'total' => $total_open
			];
		}
