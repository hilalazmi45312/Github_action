...
if ( ! BWFAN_PRO_Common::is_lite_3_0() && ! class_exists( 'BWFCRM_Contact' ) ) {
	/**
	...
	class BWFCRM_Contact {
		...
		public function show_contact_terms_by_name() {
			if ( ! is_null( BWFAN_PRO_Common::$show_tags_list_by_name ) ) {
				return BWFAN_PRO_Common::$show_tags_list_by_name;
			}

			BWFAN_PRO_Common::$show_tags_list_by_name = apply_filters( 'bwfan_contact_show_terms_by_name', false );
		...
		public function get_customer_as_array( $slim_data = false ) {
			...
			$wc_data['aov_store']    = BWFCRM_Common::get_store_aov();
		...
		public function get_offer_as_array() {
			...
			if ( class_exists( 'WFOB_Core' ) && bwfcrm_is_woocommerce_active() ) {
				$contact_offer_data['bump'] = BWFCRM_Model_Contact::get_bump_details( $contact_id );
			}

			/** checking oder bump activated  */
			if ( class_exists( 'WFACP_Core' ) && bwfcrm_is_woocommerce_active() ) {
			...
			if ( class_exists( 'WFOCU_Core' ) && bwfcrm_is_woocommerce_active() ) {
		...
		public function create_wp_user( $email, $first_name = '', $last_name = '', $password = '', $notify_new_user = false ) {
			if ( ! is_email( $email ) ) {
				return BWFCRM_Common::crm_error( __( 'Can\'t create WP User as provided email is not valid', 'wp-marketing-automations-pro' ) );
			}
		...
		public function add_tags( $tags, $use_cache = false, $stop_hooks = false, $return_skipped = false ) {
			if ( ! $this->is_contact_exists() ) {
				return BWFCRM_Common::crm_error( __( 'Contact doesn\'t exists', 'wp-marketing-automations-pro' ) );
			}

			$tags = $this->set_tags( $tags, $use_cache, $stop_hooks, $return_skipped );
			if ( false === $tags ) {
				return BWFCRM_Common::crm_error( __( 'Provided tags are empty / invalid', 'wp-marketing-automations-pro' ) );
			}
		...
		public function add_lists( $lists, $use_cache = false, $stop_hooks = false, $return_skipped = false ) {
			...
			if ( false === $lists ) {
				return BWFCRM_Common::crm_error( __( 'Provided lists are empty / invalid', 'wp-marketing-automations-pro' ) );
			}
		...
		public static function get_contacts( $search, $offset, $limit, $filters = array(), $additional_info = array(), $return_type = ARRAY_N, $update_fields = false ) {
			...
			if ( $exclude_unsubs_lists ) {
				$filters = BWFCRM_Filters::maybe_add_unsubscribe_lists_filter( $filters );
			}

			$filter_match = isset( $filters['match'] ) && ! empty( $filters['match'] ) ? $filters['match'] : 'all';
			$filter_match = ( 'any' === $filter_match ? ' OR ' : ' AND ' );
			$filters      = BWFCRM_Filters::_normalize_input_filters( $filters );
		...
		public function get_conversations( $mode, $offset = 0, $limit = 0 ) {
			if ( ! class_exists( 'BWFAN_Email_Conversations' ) || ! isset( BWFAN_Core()->conversations ) || ! BWFAN_Core()->conversations instanceof BWFAN_Email_Conversations ) {
				return BWFCRM_Common::crm_error( 'Conversations module not found' );
			}
		...
		public function get_conversation_email( $con_id = 0 ) {
			...
			if ( ! is_array( $cons ) || empty( $cons ) || ! is_array( $cons[0] ) || ! isset( $cons[0]['ID'] ) ) {
				return BWFCRM_Common::crm_error( 'No Conversation for this email found' );
			}
		...
		public function get_subscriptions_array( $offset = 0, $limit = 10 ) {
			...
			foreach ( $contact_subscriptions as $subscriptions ) {
				...
				$user_renewal_order                                                    = $subscriptions->get_related_orders( 'ids', array( 'renewal', 'resubscribe' ) );
				$contact_subscription_data['subscriptions'][ $co_id ]['total_renewal'] = BWFAN_Pro_Common::get_paid_orders_count( $user_renewal_order );
		...
		public function get_contact_funnels() {
			$contact_id    = $this->get_id();
			$funnel_object = new BWFCRM_Funnels();
		...
		public function send_note_conversation( $note_id, $note_data ) {
			...
			/** Create Engagement */
			$conversation    = BWFCRM_Core()->conversation->create_campaign_conversation( $this, $note_id, 0, $author_id, BWFAN_Email_Conversations::$MODE_EMAIL, true, array(
			...
			$unsubscribe_link = BWFAN_PRO_Common::get_unsubscribe_link( [ 'uid' => $this->contact->get_uid() ] );
			if ( ! empty( $unsubscribe_link ) ) {
				$headers[] = "List-Unsubscribe: <$unsubscribe_link>";
				$headers[] = "List-Unsubscribe-Post: List-Unsubscribe=One-Click";
			}

			$email_subject = BWFCRM_Core()->conversation->prepare_email_subject( $note_data['title'], array() );

			try {
				$email_body = BWFCRM_Core()->conversation->prepare_email_body( $conversation['conversation_id'], $note_data['cid'], $conversation['hash_code'], 'rich', $message, array() );
			} catch ( Error $e ) {
		...
		public function get_display_status() {
			if ( ! $this->is_contact_exists() ) {
				return BWFCRM_Common::crm_error( __( 'Contact not valid' ), 'wp-marketing-automations-pro' );
			}
		...
		public function get_marketing_status() {
			...
			switch ( $this->get_display_status() ) {
				case BWFCRM_Contact::$DISPLAY_STATUS_UNSUBSCRIBED:
					$status = __( 'Unsubscribed', 'wp-marketing-automations-pro' );
					break;
				case BWFCRM_Contact::$DISPLAY_STATUS_SUBSCRIBED:
					$status = __( 'Subscribed', 'wp-marketing-automations-pro' );
					break;
				case BWFCRM_Contact::$DISPLAY_STATUS_UNVERIFIED:
					$status = __( 'Unverified', 'wp-marketing-automations-pro' );
					break;
				case BWFCRM_Contact::$DISPLAY_STATUS_BOUNCED:
					$status = __( 'Bounced', 'wp-marketing-automations-pro' );
					break;
			}
