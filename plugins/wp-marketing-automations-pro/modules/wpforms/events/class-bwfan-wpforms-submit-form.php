...
final class BWFAN_WPFORMS_Form_Submit extends BWFAN_Event {
	...
	private function __construct() {
		$this->event_merge_tag_groups = array( 'wpforms', 'bwf_contact' );
		...
		$this->optgroup_label         = esc_html__( 'WPForms', 'wp-marketing-automations-pro' );
		$this->priority               = 10;
	...
	public function pre_executable_actions( $automation_data ) {
		...
		$this->email = is_array( $this->email ) && isset( $this->email['primary'] ) && is_email( $this->email['primary'] ) ? $this->email['primary'] : ( is_email( $this->email ) ? $this->email : '' );
	...
	public function validate_v2_event_settings( $automation_data ) {
		if ( absint( $automation_data['form_id'] ) !== absint( $automation_data['event_meta']['bwfan-wpforms_form_submit_form_id'] ) ) {
			return false;
		}
	...
	public function capture_v2_data( $automation_data ) {
		...
		$this->fields        = BWFAN_Common::$events_async_data['fields'];
		$this->entry_id      = BWFAN_Common::$events_async_data['entry_id'];
		$this->email         = ( ! empty( $email_map ) && isset( $this->entry['fields'][ $email_map ] ) ) ? $this->entry['fields'][ $email_map ] : '';
		$firstname           = ( ! empty( $first_name_map ) && isset( $this->entry['fields'][ $first_name_map ] ) ) ? $this->entry['fields'][ $first_name_map ] : '';
		...
		$this->first_name = is_array( $firstname ) && isset( $firstname['first'] ) ? $firstname['first'] : $firstname;
		$last_name        = is_array( $firstname ) && isset( $firstname['last'] ) ? $firstname['last'] : '';
		$this->last_name  = is_array( $this->last_name ) && isset( $this->last_name['last'] ) ? $this->last_name['last'] : ( ! empty( $this->last_name ) ? $this->last_name : $last_name );
		$this->email      = is_array( $this->email ) && isset( $this->email['primary'] ) && is_email( $this->email['primary'] ) ? $this->email['primary'] : ( is_email( $this->email ) ? $this->email : '' );
	...
	public function get_fields_schema() {
		...
		return [
			[
				'id'          => 'bwfan-wpforms_form_submit_form_id',
				'type'        => 'select',
				'options'     => $forms,
			...
			[
				...
				"description" => "",
				"ajax_cb"     => 'bwfan_get_wpforms_form_fields',
				"ajax_field"  => [
					'id' => 'bwfan-wpforms_form_submit_form_id'
				],
				"fieldChange" => 'bwfan-wpforms_form_submit_form_id',
				"toggler"     => [
					'fields'   => array(
						array(
							'id'    => 'bwfan-wpforms_form_submit_form_id',
