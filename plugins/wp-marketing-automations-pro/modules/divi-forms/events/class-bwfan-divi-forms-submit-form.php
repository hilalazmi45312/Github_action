...
final class BWFAN_Divi_Form_Submit extends BWFAN_Event {
	...
	private function __construct() {
		...
		$this->event_merge_tag_groups = array( 'divi_forms', 'bwf_contact' );
		...
		$this->optgroup_label     = esc_html__( 'Divi Forms', 'wp-marketing-automations-pro' );
		$this->priority           = 10;
	...
	public function load_hooks() {
		add_filter( 'bwfan_get_form_submit_events', array( $this, 'add_divi_to_form_submit_events' ), 10, 1 );
		add_action( 'wp_ajax_bwfan_get_divi_form_fields', array( $this, 'bwfan_get_divi_form_fields' ) );
		add_action( 'et_pb_contact_form_submit', array( $this, 'process' ), 10, 3 );
	}

	public function bwfan_get_divi_form_fields() {
		...
		$results = [];
		if ( ! empty( $fields ) ) {
			$results = array_map( function ( $f_key, $f_value ) {
				return array(
					'key'   => $f_key,
					'value' => $f_value
				);
	...
	public function get_view_data() {
		$options = [];
		$forms   = BWFAN_Divi_Forms_Common::extract_forms_and_fields();
	...
	public function get_form_fields( $form_id ) {
		...
		$form_fields = BWFAN_Divi_Forms_Common::extract_forms_and_fields();
		$fields      = array();
		...
		foreach ( $form_fields as $id => $form ) {
			if ( absint( $form_id ) !== absint( $id ) ) {
				continue;
			}
			...
			foreach ( $form['fields'] as $field ) {
				...
				$fields[ $field['field_id'] ] = $field['field_title'];
			}
		}
	 ...
	 * @return void
	 */
	public function process( $fields_values, $et_contact_error, $contact_form_info ) {
		...
		$form_number        = $contact_form_info['contact_form_number'];
		$form_id            = "$post_id-$unique_id-$form_number";
		$data['form_id']    = $form_id;
		$data['form_title'] = isset( BWFAN_Divi_Forms_Common::extract_forms_and_fields()[ $form_id ] ) && isset( BWFAN_Divi_Forms_Common::extract_forms_and_fields()[ $form_id ]['title'] ) ? BWFAN_Divi_Forms_Common::extract_forms_and_fields()[ $form_id ]['title'] : '';

		// fetching the entries and fields
		$entries      = array();
		$fields_array = [];
		foreach ( $fields_values as $id => $fields ) {
			$entries[ $id ]      = $fields['value'];
			$fields_array[ $id ] = $fields['label'];
		}
	...
	public function validate_v2_event_settings( $automation_data ) {
		...
		if ( empty( $automation_data ) || ! isset( $automation_data['form_id'] ) || ! isset( $automation_data['event_meta']['bwfan-divi_form_submit_form_id'] ) ) {
			return false;
		}
		...
		if ( ! empty( $is_form_global ) ) {
			$submitted_unique_id = BWFAN_Divi_Forms_Common::extract_unique_form_id( $automation_data['form_id'] );
			$saved_unique_id     = BWFAN_Divi_Forms_Common::extract_unique_form_id( $automation_data['event_meta']['bwfan-divi_form_submit_form_id'] );

			return strval( $saved_unique_id ) === strval( $submitted_unique_id );
		}

		return ( strval( $automation_data['form_id'] ) === strval( $automation_data['event_meta']['bwfan-divi_form_submit_form_id'] ) );
	}
	...
	public function get_fields_schema() {
		...
		return [
			[
				'id'          => 'bwfan-divi_form_submit_form_id',
				'type'        => 'select',
				'options'     => $forms,
			...
			[
				...
				'placeholder' => __( 'Select', 'wp-marketing-automations-pro' ),
				"description" => "",
				"ajax_cb"     => 'bwfan_get_divi_form_fields',
				"ajax_field"  => [
					'id' => 'bwfan-divi_form_submit_form_id'
				],
				"fieldChange" => 'bwfan-divi_form_submit_form_id',
				"toggler"     => [
					'fields'   => array(
						array(
							'id'    => 'bwfan-divi_form_submit_form_id',
	...
	public function add_divi_to_form_submit_events( $events ) {
		$events[] = 'BWFAN_Divi_Form_Submit';
