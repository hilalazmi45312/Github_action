...
final class BWFAN_GF_Form_Submit extends BWFAN_Event {
	...
	private function __construct() {
		$this->event_merge_tag_groups = array( 'gforms', 'bwf_contact' );
		...
		$this->optgroup_label         = esc_html__( 'Gravity Form', 'wp-marketing-automations-pro' );
		$this->priority               = 10;
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( is_array( $inputs ) ) {
				foreach ( $inputs as $input ) {
					...
					/*
					 * just making sure that array index does not get alter in case of float input id by conversing it to a string
					 */
					$fields[ (string) $input['id'] ] = $field->label . ': ' . $input['label'];
				}
			} else {
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( is_array( $inputs ) ) {
				foreach ( $inputs as $input ) {
					...
					if ( $field['choices'] && 'checkbox' === $field['type'] ) {
						$data['entry'][ $field->id ][] = $entry[ $input['id'] ];
					}
	...
	public function validate_v2_event_settings( $automation_data ) {
		if ( absint( $automation_data['form_id'] ) !== absint( $automation_data['event_meta']['bwfan-gf_form_submit_form_id'] ) ) {
			return false;
		}
	...
	public function get_fields_schema() {
		...
		return [
			[
				'id'          => 'bwfan-gf_form_submit_form_id',
				'type'        => 'select',
				'options'     => $forms,
			...
			[
				...
				'placeholder' => __( 'Select', 'wp-marketing-automations-pro' ),
				"description" => "",
				"ajax_cb"     => 'bwfan_get_gforms_form_fields',
				"ajax_field"  => [
					'id' => 'bwfan-gf_form_submit_form_id'
				],
				"fieldChange" => 'bwfan-gf_form_submit_form_id',
				"toggler"     => [
					'fields'   => array(
						array(
							'id'    => 'bwfan-gf_form_submit_form_id',
