...
class SubscriptionPage {
	...
	public function __construct() {
		...
		// HPOS
		add_action( 'handle_bulk_actions-woocommerce_page_wc-orders--shop_subscription',
			function ( $redirect_to, $action, $subscription_ids ) {
				...
				if ( 'dfws_apply_subscription_discounts' === $action ) {
					...
					if ( $changed > 0 ) {
						...
						$this->getContainer()->getAdminNotifier()->flash( sprintf( __( 'Discounts have been applied to %1$s %2$s',
							'discounts-for-woocommerce-subscriptions' ), $changed,
							_n( 'subscription', 'subscriptions', $changed,
								'discounts-for-woocommerce-subscriptions' ) ) );
					}
					
				}
				
				if ( 'dfws_remove_subscription_discounts' === $action ) {
					foreach ( $subscription_ids as $subscription_id ) {
						$subscription = wcs_get_subscription( $subscription_id );
					...
					if ( $changed > 0 ) {
						// translators: %s: number of subscriptions, %2s: subscription/subscriptions
						$this->getContainer()->getAdminNotifier()->flash( sprintf( __( 'Discounts have been removed from %1$s %2$s',
							'discounts-for-woocommerce-subscriptions' ), $changed,
							_n( 'subscription', 'subscriptions', $changed,
								'discounts-for-woocommerce-subscriptions' ) ) );
					}
