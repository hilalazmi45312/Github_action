...
class ProductManager extends ProductManagerAbstract {
		...
		<div id="subscription-discounts-data" class="panel woocommerce_options_panel">
			...
			<?php
				woocommerce_wp_checkbox( array(
					'id'          => '_dfwc_do_not_display_discounts_table',
					'label'       => __( 'Do not display discounts table', 'discounts-for-woocommerce-subscriptions' ),
					'description' => __( 'Check this box if you do not want to display discounts table on the product page.',
						'discounts-for-woocommerce-subscriptions' ),
					'value'       => DiscountsManager::isHideDisplayPricingTable( $post->ID ) ? 'yes' : 'no',
				) );
			?>
	...
	public function updatePriceRules( $product_id ) {
		...
		if ( wp_verify_nonce( $nonce, 'save_simple_product_subscription_discount__data' ) ) {
			...
			DiscountsManager::setDisplayPricingTable( $product_id,
				isset( $data['_dfwc_do_not_display_discounts_table'] ) );
		...
		if ( $product_object instanceof WC_Product ) {
			...
			<div class="hidden <?php echo esc_attr( $supportedSimpleTypesClasses ); ?>">
				<?php
					...
					$this->getContainer()->getFileManager()->includeTemplate( 'admin/add-price-rules.php', array(
						'price_rules_fixed'      => DiscountsManager::getFixedDiscounts( $product_object->get_id(),
							'edit' ),
						'price_rules_percentage' => DiscountsManager::getPercentageDiscounts( $product_object->get_id(),
							'edit' ),
						'type'                   => $type,
						'prefix'                 => 'simple',
					) );
