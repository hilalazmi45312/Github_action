	...
	public function __construct() {
		...
		add_filter( 'woocommerce_subscriptions_product_price_string_inclusions',
			function ( $include, WC_Product $product ) {
				...
				if ( is_cart() || is_checkout() || $product->is_type( 'variable-subscription' ) ) {
					return $include;
				}
				...
				if ( ! $price || $price === $product->get_price() || '' === $product->get_price() ) {
					return $include;
				}
				
				$include['price'] = wc_format_sale_price( wc_get_price_to_display( $product,
						array( 'price' => $product->get_regular_price() ) ), wc_get_price_to_display( $product,
		...
		add_filter( 'woocommerce_before_calculate_totals', function ( \WC_Cart $cart ) {
			...
			foreach ( $cart->get_cart_contents() as $cartItem ) {
				...
				$updatePrice = apply_filters( 'subscription_discounts/first_payment_discount/update_price', true,
					$cartItem, $cart );
