...
class DiscountsManager {
	...
	public static function isHideDisplayPricingTable( $productId, $context = 'view' ) {
		...
		$parentProductId = $product->get_parent_id() ? $product->get_parent_id() : $productId;
		
		return 'yes' === get_post_meta( $parentProductId, '_dfwc_do_not_display_discounts_table', 'no' );
	}
	
	public static function setDisplayPricingTable( $productId, $value ) {
		$value = boolval( $value );
		
		if ( $value ) {
			update_post_meta( $productId, '_dfwc_do_not_display_discounts_table', 'yes' );
		} else {
			delete_post_meta( $productId, '_dfwc_do_not_display_discounts_table' );
		}
	}
}
