	...
	public function __construct() {
				...
				foreach ( $cart->get_cart_contents() as $key => $cartItem ) {
					...
					if ( array_key_exists( 2, $discounts ) ) {
						...
						if ( 'fixed' === $discountsType ) {
							...
							$newPrice = apply_filters( 'subscription_discounts/cart/next_renewal_discount_price',
								$newPrice, $productId, $discounts, $discountsType, $cartItem );
							
							$newCartContents[ $key ]['data']->add_meta_data( '_dfws_original_product_price',
								$newCartContents[ $key ]['data']->get_price() );
		...
		add_action( 'woocommerce_checkout_create_order_line_item', function ( $item, $cart_item_key, $values, $order ) {
			...
			if ( $item instanceof WC_Order_Item_Product ) {
				...
				$productOriginalPrice = $product->meta_exists( '_dfws_original_product_price' ) ? $product->get_meta( '_dfws_original_product_price',
					true ) : null;
				
				DiscountedOrderItem::addDiscounts( $item, null, null, $productOriginalPrice );
			}
		}, 10, 4 );
