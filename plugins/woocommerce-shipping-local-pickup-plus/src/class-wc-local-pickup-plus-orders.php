...
class WC_Local_Pickup_Plus_Orders {
	...
	public function __construct() {
		...
		$this->order_items = wc_local_pickup_plus()->load_class( '/src/class-wc-local-pickup-plus-order-items.php', 'WC_Local_Pickup_Plus_Order_Items' );
	...
	public function get_orders_by_appointment_range( $start = 0, $end = 0 ) {
		...
		// filter order item meta for new meta key (`_pickup_appointment_start`) or legacy meta key `_pickup_date`
		// NOTE: we intentionally do not check the appointment end time here because it can lead to unexpected results
		// if the start time is within start/end, but the end time is not
	 ...
	 * @since 2.0.0
	 *
	 * @param string|string[] $recipients the email recipients, as a comma-separated string
	 * @param \WC_Order|null $order the order as the email object
	...
	public function notify_pickup_locations_recipients( $recipients, $order = null ) {
		...
		if (! $pickupLocations = $this->get_order_pickup_locations($order)) {
			return $recipients;
		}

		$isList = is_array($recipients);
		$updatedRecipients = $isList ?
			array_map('sanitize_email', $recipients) :
			array_map('sanitize_email', explode(',', $recipients));
		...
		foreach ($pickupLocations as $location) {
			...
			if (! $locationRecipients = array_filter((array) $location->get_email_recipients())) {
				continue;
			}

			$updatedRecipients = array_values(array_unique([...$updatedRecipients, ...$locationRecipients]));
		}

		return $isList ? $updatedRecipients : implode(',', $updatedRecipients);
	}
	...
	private function use_pickup_location_taxable_address() {
		...
		if (    $local_pickup_plus
			...
			if ( is_admin() ) {
				 ...
				 * LPP circumstances, causing an issue where the user's chosen rate/method is reset.
				 */
