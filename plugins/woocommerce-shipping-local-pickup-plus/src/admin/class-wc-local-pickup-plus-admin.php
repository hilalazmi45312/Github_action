...
class WC_Local_Pickup_Plus_Admin {
	...
	private function includes() {
		...
		$this->pickup_locations = wc_local_pickup_plus()->load_class( '/src/admin/class-wc-local-pickup-plus-pickup-locations-admin.php', 'WC_Local_Pickup_Plus_Pickup_Locations_Admin' );

		// Pickup Locations handler for WC orders
		$this->orders = wc_local_pickup_plus()->load_class( '/src/admin/class-wc-local-pickup-plus-orders-admin.php', 'WC_Local_Pickup_Plus_Orders_Admin' );

		// Products and Product Categories handler class
		$this->products = wc_local_pickup_plus()->load_class( '/src/admin/class-wc-local-pickup-plus-products-admin.php', 'WC_Local_Pickup_Plus_Products_Admin' );

		// Pickup Locations Import and Export handlers
		require_once( wc_local_pickup_plus()->get_plugin_path() . '/src/admin/abstract-class-wc-local-pickup-plus-import-export.php' );

		$this->import = wc_local_pickup_plus()->load_class( '/src/admin/class-wc-local-pickup-plus-import.php', 'WC_Local_Pickup_Plus_Import' );
		$this->export = wc_local_pickup_plus()->load_class( '/src/admin/class-wc-local-pickup-plus-export.php', 'WC_Local_Pickup_Plus_Export' );
	}
	...
	public function add_import_export_admin_pages() {
		...
		foreach ( $pages as $key => $page_name ) {
			add_submenu_page(
				...
				$page_name,
				$page_name,
				$this->get_import_export_capability(),
				$key,
	...
	private function load_meta_boxes() {
		...
		require_once( wc_local_pickup_plus()->get_plugin_path() . '/src/admin/meta-boxes/abstract-class-wc-local-pickup-plus-meta-box.php' );
		...
		if ( ! empty( $meta_boxes_classes ) ) {
			...
			foreach ( $meta_boxes_classes as $class ) {
				...
				$file_path = wc_local_pickup_plus()->get_plugin_path() . '/src/admin/meta-boxes/' . $file_name;
		...
		<table
			...
			<tr>
				<th colspan="3" data-export-label="Local Pickup Plus">
					<h2>
						<?php esc_html_e( 'Local Pickup Plus', 'woocommerce-shipping-local-pickup-plus' ); ?>
