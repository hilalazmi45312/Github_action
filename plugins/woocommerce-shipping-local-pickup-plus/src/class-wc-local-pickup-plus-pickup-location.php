...
class WC_Local_Pickup_Plus_Pickup_Location {
	...
	/**
	 * Gets the value of the `_pickup_location_products` meta key, while ensuring the two expected array keys are set.
	 *
	 * @return array<string, array>
	 */
	protected function getProductsMetaValue() : array
	{
		$products_set = get_post_meta($this->id, $this->products_meta, true);
		if (! is_array($products_set)) {
			$products_set = [];
		}

		if (! isset($products_set['product_categories'])) {
			$products_set['product_categories'] = [];
		}

		if (! isset($products_set['products'])) {
	...
	/**
	 * Gets the IDs of the products that have been selected for this location. This is intended for use in the "Edit"
	 * context. It differs from {@see static::get_products()} because this only retrieves the raw values, as stored
	 * in the database. Whereas {@see static::get_products()} combines both the product and category settings to return
	 * a complete list of all relevant products.
	 *
	 * @return int[]
	 */
	public function getProductsForEdit() : array
	{
		return array_filter(array_map(
			'absint',
			$this->getProductsMetaValue()['products']
		));
	}
