				...
				if ( $pickup_date && 'disabled' !== wc_local_pickup_plus_appointments_mode() ) {
					...
					if ( $start_date ) {
						...
						try {
							...
							wc_local_pickup_plus()->log( sprintf( 'Could not create appointment for shipping item #%1$s in order #%2$s: %3$s', $shipping_item->get_id(), $shipping_item->get_order_id(), $e->getMessage() ) );
						}
					}
				}
			}
