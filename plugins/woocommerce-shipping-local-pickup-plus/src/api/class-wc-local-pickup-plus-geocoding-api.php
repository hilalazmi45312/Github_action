...
class WC_Local_Pickup_Plus_Geocoding_API extends Framework\SV_WC_API_Base {
	...
	private function parse_geocode_request_args( array $args ) {
		...
		if ( '' !== $args['address_1'] ) {
			$address .= trim( (string) $args['address_1'] );
			if ( '' !== $args['address_2'] ) {
				$address .= ' ' . trim( (string) $args['address_2'] );
			}
		}

		$address        = trim( $address );
		$map_components = [
			'locality'            => trim( (string) $args['city'] ),
			'administrative_area' => trim( (string) $args['state'] ),
			'postal_code'         => trim( (string) $args['postcode'] ),
