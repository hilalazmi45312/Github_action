...
class Appointments {
	...
	public function is_appointment_time_available( $calendar_day, $pickup_location, $appointment_duration, $start_date, $end_date ) {
		...
		if ( $pickup_location->get_appointments()->is_anytime_appointments_enabled() ) {
			...
			// we add a 60 second grace period to this check to account for small delays during the checkout process
			// e.g. the start date is set to 10:00:12 (H:i:s)
			// but the first available time is calculated 4 seconds later at 10:00:16
			if (DateTimeComparison::isGreaterThanOrEqualTo($start_date, $pickup_location->get_appointments()->get_first_available_pickup_time($calendar_day), 60)) {
