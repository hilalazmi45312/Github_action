...
class WC_Local_Pickup_Plus_Pickup_Locations {
	...
	private function load_classes() {
		...
		$classes     = array(
			...
			'/src/pickup-locations/class-wc-local-pickup-plus-address.php',
			'/src/pickup-locations/class-wc-local-pickup-plus-price-adjustment.php',
			'/src/pickup-locations/class-wc-local-pickup-plus-business-hours.php',
			'/src/pickup-locations/class-wc-local-pickup-plus-public-holidays.php',
			'/src/pickup-locations/class-wc-local-pickup-plus-schedule-adjustment.php',
			'/src/pickup-locations/Appointments.php',
			// the main pickup location object that uses all of the above:
			'/src/class-wc-local-pickup-plus-pickup-location.php',
		);
	 ...
	 * @param null|string $radius radius to search within, or null to not impose any radius limits
	 * @return \WC_Local_Pickup_Plus_Pickup_Location[] array of locations
	 */
	public function get_pickup_locations_by_distance( array $coordinates, $args = array(), $radius = null ) {
		...
		if (is_null($radius)) {
			// if no radius is defined, we always want to sort by distance, otherwise the results will be meaningless
			// note: despite the wording here, we'll actually be ordering by the result of our query (distance from coordinates) -- not necessarily the distance from the customer
			$args['orderby'] = 'distance_customer';

			/**
			 * Filters the default radius to use in the query. We only filter the default (`null`) value here
			 * so that we don't also apply the filter on subsequent calls and get stuck in an infinite loop.
			 *
			 * Setting this to `50km` will revert to previous plugin behaviour (pre-2.11.5)
			 *
			 * @param null|string $radius radius to search within, or null to not impose any radius limits
			 *
			 * @since 2.11.5
			 */
			$radius = apply_filters('woocommerce_pickup_locations_by_distance_default_radius', $radius);
		}

		$radius_km    = $radius ? max( 0, $this->parse_distance( $radius ) ) : null;
		$cache_key    = http_build_query( array_merge( array( 'lat' => $latitude, 'lon' => $longitude ), array( $radius ), $args ) );

		// if we already have the cached results, return them immediately
		...
		if (($radius_km > 0 && $radius_km <= 40075) || is_null($radius_km)) {

			// we only include a distance limit if a radius was provided
			$distanceSql = ! is_null($radius_km) ? $wpdb->prepare('HAVING distance < %d', $radius_km) : '';
			...
			if (! empty($results)) {
				...
				// we only do this if a radius was provided in the first place
				if (empty($location_ids) && $radius_km) {
				...
				// if no results, and a radius was provided, progressively expand the search by expanding the queried radius
				return $this->get_pickup_locations_by_distance($coordinates, $args, (string) ($radius_km * 2) . 'km');
			}
	...
	public function get_sorted_pickup_locations( $query_args = array() ) {
		...
		switch ( wc_local_pickup_plus_shipping_method()->pickup_locations_sort_order() ) {
			...
			case 'distance_customer' :
				...
				$pickup_locations = $this->get_pickup_locations_by_distance( $coordinates, array_merge( array( 'post_status' => 'publish', 'orderby' => 'distance_customer' ), $query_args ) );
			break;
