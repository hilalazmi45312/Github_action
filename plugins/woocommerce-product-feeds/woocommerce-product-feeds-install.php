...
function woocommerce_gpf_install() {
	...
	if ( false === $settings ) {
		...
		// @TODO - remove version check when minimum supported version is 9.6 or higher.
		if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '9.6.0', 'ge' ) ) {
			$settings['product_prepopulate']['brand'] = 'tax:product_brand';
		}
	...
	// Other updates to apply, but only if this is a genuinely fresh installation.
	// Otherwise, the DB upgrade should take care of things
