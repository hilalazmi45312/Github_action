...
class StructuredData {
	...
	// Dependencies.
	private ProductFeedItemFactory $product_feed_item_factory;
	 ...
	 * Store dependencies.
	 *
	 * @param ProductFeedItemFactory $product_feed_item_factory
	 */
	public function __construct( ProductFeedItemFactory $product_feed_item_factory ) {
		$this->product_feed_item_factory = $product_feed_item_factory;
	}

	/**
	 * Add the filters so that we can modify the structured data.
	 */
	public function initialise(): void {
		// Add hook to modify JSON+LD data.
		add_filter(
			'woocommerce_structured_data_product',
			[ $this, 'structured_data_product' ],
			8,
			2
		);
		add_filter(
			'woocommerce_structured_data_product_offer',
			[ $this, 'structured_data_product_offer' ],
			10,
			2
		);
	}
	 ...
	 * @throws Exception
	 */
	public function structured_data_product( array $markup, WC_Product $product ): array {
	 ...
	 * @throws Exception
	 */
	public function structured_data_product_offer( array $markup, WC_Product $offer_product ): array {
		...
		$parent_product = wc_get_product( $offer_product->get_id() );
		...
		$feed_item = $this->product_feed_item_factory->create( 'google', $offer_product, $parent_product, false );
	 ...
	 * @throws Exception
	 */
	private function structured_data_variable_product( array $markup, WC_Product $product ): array {

		$reloaded_product = wc_get_product( $product->get_id() );

		// Get the feed information for this product.
		// Note: We do not calculate pricing to avoid having to query all variations.
		$feed_item = $this->product_feed_item_factory->create(
			'google',
			$reloaded_product,
			$reloaded_product,
			false
	 ...
	 * @throws Exception
	 */
	private function structured_data_simple_product( array $markup, WC_Product $product ): array {
		...
		// GTIN.
		if ( isset( $feed_item->additional_elements['gtin'][0] ) ) {
			$raw_gtin    = str_replace( [ ' ', '-' ], '', $feed_item->additional_elements['gtin'][0] );
	...
	/*
	 * Map the condition to the corresponding schema value.
	 */
	private function schemaize_condition( string $condition ): string {
	 ...
	 * @return array
	 */
	private function generate_brand_markup( string $brand ): array {
