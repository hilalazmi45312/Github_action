    ...
    class WPH_PluginUpdate
         {
             ...
             public function __construct( $api_url, $slug, $plugin )
                 {
                     ...
                     $this->licence     =   WPH_licence::get_licence_data();   
                 }
                     ...
                     if ( $response ) 
                        {
                            ...
                            if ( $refresh )
                                {
                                    ...
                                    WPH_licence::update_licence_data( $this->licence );
                                    
                                    $this->postprocess_response_run( $response_block );
                                }
                            ...
                            if ( isset ( $response->new_version )   &&  version_compare( $response->new_version, WPH_CORE_VERSION, '<='  ) )
                                return $checked_data;
             ...
             public function plugins_api_call( $def, $action, $args )
                 {
                     ...
                     if($request_string === FALSE)
                        return new WP_Error('plugins_api_failed', __('An error occour when try to identify the pluguin.' , 'wp-hide-security-enhancer') . '&lt;/p> &lt;p>&lt;a href=&quot;?&quot; onclick=&quot;document.location.reload(); return false;&quot;>'. __( 'Try again', 'wp-hide-security-enhancer' ) .'&lt;/a>');;
                     ...
                     if  ( $data    === FALSE )
                         {
                             ...
                             if(is_wp_error( $data ) || $data['response']['code'] != 200)
                                return new WP_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.' , 'wp-hide-security-enhancer') . '&lt;/p> &lt;p>&lt;a href=&quot;?&quot; onclick=&quot;document.location.reload(); return false;&quot;>'. __( 'Try again', 'wp-hide-security-enhancer' ) .'&lt;/a>', $data->get_error_message());
             ...
             private function postprocess_response_run ( $response_block )
                {
                    if ( isset ( $response_block->status_code )  &&  in_array( $response_block->status_code, array ( 'e001', 'e002', 'e003', 's403'  ) ) )
