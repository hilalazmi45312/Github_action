    ...
    class WPH_module_general_document_loaded_assets_postprocessing extends WPH_module_component
        {
            ...
            function get_component_menu_title()
                {
                    return "Doc Loaded Assets PP";
                }
                
            function module_component_is_visible()
                {
                    
                    if ( defined ( 'WPH_DOCUMENT_LOADED_ASSETS_POSTPROCESSING' ) )
                        return TRUE;
                    
                    $html_css_js_replacements   =   $this->wph->functions->get_site_module_saved_value('html_css_js_replacements',   $this->wph->functions->get_blog_id_setting_to_use(), 'display');
                    
                    if ( empty ( $html_css_js_replacements ) )
                        return FALSE;
                        else
                        {
                            $found  =   FALSE;
                            foreach ( $html_css_js_replacements   as  $block )
                                {
                    ...
                    foreach ( $component_settings   as  $component_key  =>  $component_setting )
                        {
                            switch ( $component_setting['id'] )
                                {
                                    case 'document_loaded_assets_postprocessing' :
                    ...
                    foreach ( $data as $url )
                        {
                            ...
                            if ( strlen ( $this->wph->default_variables['site_relative_path'] ) > 2 &&  strpos( $rewrited_url, $this->wph->default_variables['site_relative_path']) === 0 )
                                $rewrited_url = str_replace( $this->wph->default_variables['site_relative_path'], '', $rewrited_url );  
