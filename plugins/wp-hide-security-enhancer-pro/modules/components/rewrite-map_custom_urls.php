                    ...
                    foreach ( $values   as  $key    =>  $block)
                        {
                            ...
                            $rewrite_base   =   $this->wph->functions->get_rewrite_path( $replaced_parsed_path, [ 'left_slash'  =>  FALSE, 'right_slash' =>  FALSE, 'exclude_wp_dir'    =>  TRUE ] );
                            $rewrite_to     =   $this->wph->functions->get_rewrite_path( $replacement_parsed['path'], [ 'left_slash'  =>  TRUE, 'right_slash' =>  FALSE, 'exclude_wp_dir'    =>  TRUE  ] );
                            ...
                            if($this->wph->server_nginx_config   === TRUE)           
                                {
                                    $rewrite_base   =   $this->wph->functions->get_rewrite_path( $replaced_parsed_path, [ 'left_slash'  =>  FALSE, 'right_slash' =>  FALSE, 'include_full_path' =>  TRUE, 'exclude_wp_dir'    =>  TRUE, 'type' =>  'nginx' ] );
                                    if ( $global_match  === TRUE )
                                        {
                                            $rewrite_base   .=  '/(.+)'; 
                                            $rewrite_to     .=  '/$2';  
