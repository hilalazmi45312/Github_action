                                    ...
                                    case 'new_xml_rpc_path':
                                        $component_setting = array_merge($component_setting, array(
                                            ...
                                            'help'          => array(
                                                ...
                                                'description'   => __("XML-RPC is a remote procedure call (RPC) protocol which uses XML to encode its calls and HTTP as a transport mechanism. This service allows other applications to talk to your WordPress site.", 'wp-hide-security-enhancer') . "<br /><br />" .
                                                                  __("As default the path to XML-RPC file is:", 'wp-hide-security-enhancer') . 
                                                                  "<code>https://-domain-name-/xmlrpc.php</code><br /><br />" .
                                                                  __("Through this option, it can be changed to anything else. This ensures the protocol will not be called by anyone who doesn't know the actual path.", 'wp-hide-security-enhancer'),
                                    ...
                                    case 'block_xml_rpc':
                                        $component_setting = array_merge($component_setting, array(
                                            ...
                                            'help'          => array(
                                                ...
                                                'description'   => __("This blocks the default XML-RPC service. The functionality applies if <b>New XML-RPC Path</b> option is filled in.", 'wp-hide-security-enhancer') . "<br /><br />" .
                                                                  __("Keep in mind that some third-party services, like Jetpack, rely on XML-RPC to connect to WordPress sites for features such as monitoring, statistics, and site management.", 'wp-hide-security-enhancer'),
                                                'option_documentation_url' => 'https://wp-hide.com/documentation/rewrite-xml-rpc/'
                                            ),
                                            'options'       => array(
            ...
            function _init_new_xml_rpc_path($saved_field_data)
                {
                    ...
                    $url            =   $this->wph->functions->prepare_path (    site_url() . '/' .  'xmlrpc.php' );
                    $replacement    =   $this->wph->functions->prepare_path (    home_url() . '/' .  $saved_field_data );
                    ...
                    if($this->wph->server_nginx_config   === TRUE)           
                        {
                            ...
                            $rewrite_base   =   $this->wph->functions->get_rewrite_path( 'xmlrpc.php', [ 'left_slash'  =>  TRUE, 'right_slash' =>  FALSE, 'include_full_path' =>  TRUE, 'exclude_wp_dir'    =>  TRUE, 'type' =>  'nginx' ] );
