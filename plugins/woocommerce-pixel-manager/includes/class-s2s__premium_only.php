...
//      }
	...
	private static function get_current_identifiers() {
		...
		$identifiers_from_session = array();
		if (Shop::is_woocommerce_session_active()) {
			$identifiers_from_session = WC()->session->get(static::get_identifiers_key(), array());
		}
	...
	private static function get_identifiers_from_browser() {
		...
		// Note: No logging for missing IP - this is expected in iframe/health check scenarios
	...
	public static function hook_into_purchase_events() {
		foreach (static::get_purchase_trigger_hooks() as $hook) {
			add_action($hook, array( static::class, 'send_purchase_hit_order_id' ));
		}
	}
	...
	public static function hook_into_order_created_events() {
		add_action('woocommerce_checkout_order_created', array( static::class, 'set_identifiers_on_order' ));
	}
