...
class Facebook_CAPI extends S2S {
	...
	private static function get_api_version() {
		...
		$api_version = apply_filters_deprecated('wooptpm_facebook_capi_api_version', array( $api_version ), '1.13.0', 'pmw_facebook_capi_api_version');
		$api_version = apply_filters_deprecated('wpm_facebook_capi_api_version', array( $api_version ), '1.31.2', 'pmw_facebook_capi_api_version');
	...
	public static function send_event_hit( $event_data ) {
		...
		// Only set client_ip_address if not already provided in the event data (e.g., from frontend)
		// Priority:
		// 1. IP already set in user_data from frontend (most accurate, comes from browser)
		// 2. Client IP stored in WC session (captured earlier from browser via external IP service)
		// 3. Geolocation class (server-side detection, fallback)
		if (!isset($event_data['user_data']['client_ip_address']) || empty($event_data['user_data']['client_ip_address'])) {
			$session_ip = null;
			if (Shop::is_woocommerce_session_active()) {
				$session_ip = WC()->session->get('client_ip');
			}

			if ($session_ip) {
				$event_data['user_data']['client_ip_address'] = $session_ip;
			} elseif (Geolocation::get_user_ip()) {
				$event_data['user_data']['client_ip_address'] = Geolocation::get_user_ip();
			}
			// Note: Missing IP is handled gracefully - Meta CAPI will still work without it
		}
...
//      update_post_meta($order->get_id(), self::get_purchase_hit_key(), true);
	...
	private static function hook_into_subscriptions_events() {
		...
		add_action('woocommerce_subscription_payment_complete', array( __CLASS__, 'facebook_capi_report_subscription_payment_complete' ), 10, 1);
		add_action('woocommerce_subscription_status_cancelled', array( __CLASS__, 'facebook_capi_report_subscription_cancellation' ), 10, 1);
		add_action('woocommerce_subscription_status_updated', array( __CLASS__, 'facebook_capi_report_subscription_update' ), 10, 3);

		if (self::track_facebook_capi_subscription_renewal()) {
			add_action('woocommerce_subscription_renewal_payment_complete', array( __CLASS__, 'facebook_capi_report_subscription_purchase_renewal' ), 10, 2);
		}
	}
...
//      }
	...
	private static function is_ads_delivery_opt_out_active() {
		...
		$opt_out = apply_filters_deprecated('wooptpm_facebook_capi_ads_delivery_opt_out', array( false ), '1.13.0', 'pmw_facebook_capi_ads_delivery_opt_out');
		$opt_out = apply_filters_deprecated('wpm_facebook_capi_ads_delivery_opt_out', array( $opt_out ), '1.31.2', 'pmw_facebook_capi_ads_delivery_opt_out');
	...
	private static function add_data_processing_options( $capi_event_data ) {
		...
		$processing_options = apply_filters_deprecated('wooptpm_facebook_capi_data_processing_options', array( array() ), '1.13.0', 'pmw_facebook_capi_data_processing_options');
		$processing_options = apply_filters_deprecated('wpm_facebook_capi_data_processing_options', array( $processing_options ), '1.31.2', 'pmw_facebook_capi_data_processing_options');
	...
	private static function get_user_data( $identifiers, $order ) {
		...
		if (Options::is_facebook_capi_advanced_matching_enabled()) {
			...
			// Priority: client_ip_address (may contain IPv6 from session) > ip_address (server-side)
			if (isset($identifiers['client_ip_address'])) {
				$user_data['client_ip_address'] = $identifiers['client_ip_address'];
	...
	protected static function add_pixel_specific_identifiers( $identifiers ) {
		...
		// Check for client IP in WC session
		// If not available, use the IP from identifiers array (server-side captured)
		$session_ip = null;
		if (Shop::is_woocommerce_session_active()) {
			$session_ip = WC()->session->get('client_ip');
		}
		...
		if ($session_ip) {
			$identifiers['client_ip_address'] = $session_ip;
		} elseif (isset($identifiers['ip_address'])) {
