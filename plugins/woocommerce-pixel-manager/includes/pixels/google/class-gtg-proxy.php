...
class GTG_Proxy {
	 ...
	 * @var array
	 */
	private static $reserved_headers = array(
	 ...
	 * @var array
	 */
	private static $valid_tag_prefixes = array(
		'AW',  // Google Ads
		'G',   // GA4
		'GT',  // Google Tag
	...
	public static function init() {
		...
		add_filter('do_parse_request', array( __CLASS__, 'maybe_handle_proxy_request' ), 10, 3);

		// Flush rewrite rules when the PMW options change (measurement path might have changed)
		add_action('update_option_wgact_plugin_options', array( __CLASS__, 'maybe_flush_rewrite_rules' ), 10, 2);
		...
		add_action('init', array( __CLASS__, 'update_proxy_config_cache' ), 20);
		
		// Ensure isolated proxy file exists and is up to date
		add_action('init', array( __CLASS__, 'ensure_isolated_proxy_file' ), 21);
	}
	...
	public static function register_rest_routes() {
		...
		register_rest_route(self::$rest_namespace, '/gtg-proxy/health/', array(
			'methods'             => 'GET',
		...
		// nosemgrep: audit.php.wp.security.rest-route.permission-callback.return-true
		register_rest_route(self::$rest_namespace, '/gtg-proxy/', array(
			'methods'             => array( 'GET', 'POST' ),
			'callback'            => array( __CLASS__, 'handle_proxy_request' ),
			'permission_callback' => '__return_true',
		));
	}
	...
	public static function handle_proxy_request( $request ) {
		...
		if (!$tag_id && !$path) {
			self::log_proxy_event( 'Missing required parameters in REST API request', array(), 'warning' );
			return new \WP_Error('missing_parameters', 'Missing required parameters: id and s', array( 'status' => 400 ));
		}
	...
	private static function process_and_return_proxy_response( $tag_id, $path, $geo, $method, $body, $mpath = '' ) {
		...
		if ( self::is_rate_limited() ) {
			self::log_proxy_event( 'Request rate limited', array( 'client_ip' => self::get_client_ip() ), 'warning' );
		...
		if ( strlen( $body ) > self::$max_request_body_size ) {
			self::log_proxy_event( 'Request body too large', array( 'size' => strlen( $body ) ) );
			return new \WP_Error( 'body_too_large', 'Request body exceeds maximum size', array( 'status' => 413 ) );
		}

		// Validate tag ID
		if (!self::validate_tag_id($tag_id)) {
			self::log_proxy_event( 'Invalid tag ID format', array( 'tag_id' => $tag_id ), 'warning' );
			return new \WP_Error('invalid_tag_id', 'Invalid Google Tag ID format', array( 'status' => 400 ));
		}
		...
		if (!$path) {
			self::log_proxy_event( 'Invalid path after sanitization', array( 'original_path' => $original_path ), 'warning' );
		...
		if ( ! self::is_valid_fps_url( $fps_url ) ) {
			self::log_proxy_event( 'Invalid FPS URL constructed', array( 'url' => $fps_url ), 'error' );
			return new \WP_Error( 'invalid_fps_url', 'Invalid FPS URL', array( 'status' => 400 ) );
		}
		...
		if ( isset( $response['body'] ) && strlen( $response['body'] ) > self::$max_response_body_size ) {
			self::log_proxy_event( 'Response body too large', array( 'size' => strlen( $response['body'] ) ), 'error' );
			return new \WP_Error( 'response_too_large', 'Response exceeds maximum size', array( 'status' => 502 ) );
		}
	...
	private static function output_proxy_response( $response, $allow_empty_response = true ) {
		...
		if ( ! isset( $data['body'] ) || empty( $data['body'] ) ) {
			if ( $allow_empty_response ) {
				...
				self::log_proxy_event( 'Empty response from upstream', array( 'status_code' => isset( $data['status_code'] ) ? $data['status_code'] : 'unknown' ), 'error' );
				status_header( 502 );
		...
		if ( isset( $data['headers'] ) ) {
			$skip_headers = array( 'transfer-encoding', 'connection', 'content-encoding', 'content-length', 'cache-control', 'expires', 'pragma' );
	...
	private static function is_rate_limited() {
		...
		if ( $request_count >= $max_requests ) {
			self::log_proxy_event( 'Rate limit exceeded', array( 'ip' => $client_ip, 'count' => $request_count ) );
			return true;
		}
	 ...
	 * @return void
	 */
	private static function log_proxy_event( $message, $context = array(), $level = 'debug' ) {
	...
	public static function ensure_isolated_proxy_file() {
		...
		if ( ! file_exists( $proxy_file ) ) {
			self::log_proxy_event( 'Isolated proxy file not found', array( 'path' => $proxy_file ), 'error' );
			return false;
		}
