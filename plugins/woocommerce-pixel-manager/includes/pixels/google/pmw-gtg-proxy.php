<?php
/**
 * Pixel Manager Pro for WooCommerce - Isolated Google Tag Gateway Proxy
 *
...
if ( isset( $_GET['healthCheck'] ) ) {
	...
	if ( file_exists( $config_file ) ) {
		...
		if ( $config && ! empty( $config['logging_enabled'] ) && ! empty( $config['log_directory'] ) ) {
			...
			$level_priority      = array( 'debug' => 7, 'info' => 6, 'notice' => 5, 'warning' => 4, 'error' => 3 );
...
final class PMW_GTG_Proxy_Standalone {
	 ...
	 * @var array
	 */
	const VALID_TAG_PREFIXES = array( 'AW', 'G', 'GT', 'DC' );
	...
	public static function run() {
		try {
			...
			if ( ! $config['enabled'] ) {
				self::log( 'GTG proxy not enabled', array(), 'debug' );
			...
			self::log( 'Processing GTG request', array( 'uri' => $request_uri, 'method' => $method ), 'debug' );
			...
			if ( $is_direct_access ) {
				...
				self::log( 'Direct proxy access detected', array(), 'debug' );
			...
			if ( 0 !== strpos( $request_uri, $measurement_path ) ) {
				...
				self::log( 'Request does not match measurement path', array( 'uri' => $request_uri, 'path' => $measurement_path ), 'debug' );
				http_response_code( 404 );
				exit;
			}
			...
			if ( empty( $tag_id ) ) {
				self::log( 'Missing tag ID', array(), 'warning' );
				http_response_code( 400 );
				exit( 'Missing tag ID' );
			}

			if ( ! self::validate_tag_id( $tag_id ) ) {
				self::log( 'Invalid tag ID format', array( 'tag_id' => $tag_id ), 'warning' );
				http_response_code( 400 );
				exit( 'Invalid tag ID' );
			}
			...
			if ( 'POST' === $method ) {
				...
				if ( strlen( $body ) > self::MAX_REQUEST_BODY_SIZE ) {
					self::log( 'Request body too large', array( 'size' => strlen( $body ) ), 'warning' );
			...
			if ( ! self::is_valid_fps_url( $fps_url ) ) {
				self::log( 'Invalid FPS URL generated', array( 'url' => $fps_url ), 'error' );
				http_response_code( 400 );
				exit( 'Invalid request' );
			}

			self::log( 'Proxying to FPS', array( 'url' => $fps_url, 'tag_id' => $tag_id ), 'info' );
			...
			if ( false === $response ) {
				self::log( 'Failed to proxy request to FPS', array( 'url' => $fps_url ), 'error' );
			...
			self::log( 'GTG Proxy exception', array( 'message' => $e->getMessage(), 'trace' => $e->getTraceAsString() ), 'error' );
	...
	private static function handle_secondary_request() {
		...
		self::log( 'Handling secondary Google request', array( 'tag_id' => $tag_id ), 'info' );
		...
		if ( ! self::is_valid_fps_url( $fps_url ) ) {
			self::log( 'Invalid FPS URL for secondary request', array( 'url' => $fps_url ), 'error' );
			http_response_code( 400 );
			exit( 'Invalid request' );
		}
		...
		if ( false === $response ) {
			self::log( 'Failed secondary request to FPS', array( 'url' => $fps_url ), 'error' );
			http_response_code( 502 );
			exit( 'Proxy error' );
		}
	...
	private static function handle_direct_access_request( $method ) {
		...
		// Validate tag ID
		if ( empty( $tag_id ) ) {
			self::log( 'Missing tag ID in direct access', array(), 'warning' );
			http_response_code( 400 );
			exit( 'Missing tag ID' );
		}

		if ( ! self::validate_tag_id( $tag_id ) ) {
			self::log( 'Invalid tag ID format in direct access', array( 'tag_id' => $tag_id ), 'warning' );
			http_response_code( 400 );
			exit( 'Invalid tag ID' );
		}
		...
		if ( 'POST' === $method ) {
			$body = file_get_contents( 'php://input' );
			if ( strlen( $body ) > self::MAX_REQUEST_BODY_SIZE ) {
				self::log( 'Request body too large in direct access', array( 'size' => strlen( $body ) ), 'warning' );
				http_response_code( 413 );
				exit( 'Request too large' );
			}
		}
		...
		if ( ! self::is_valid_fps_url( $fps_url ) ) {
			self::log( 'Invalid FPS URL in direct access', array( 'url' => $fps_url ), 'error' );
			http_response_code( 400 );
			exit( 'Invalid request' );
		}

		self::log( 'Proxying to FPS (direct access)', array( 'url' => $fps_url, 'tag_id' => $tag_id ), 'info' );
		...
		if ( false === $response ) {
			self::log( 'FPS request failed in direct access', array( 'url' => $fps_url ), 'error' );
			http_response_code( 502 );
			exit( 'Proxy error' );
		}
		...
		self::log( 'Direct access request completed successfully', array( 'tag_id' => $tag_id, 'status_code' => $response['status_code'] ), 'info' );
	}
	 ...
	 * @return void
	 */
	private static function log( $message, $context = array(), $level = 'info' ) {
		// Get configuration to check log level
		$config           = isset( self::$config ) ? self::$config : array( 'log_level' => 'error', 'logging_enabled' => false );
	...
	private static function get_config_from_wp() {
		...
		if ( $wp_load_path && file_exists( $wp_load_path ) ) {
			...
			$options          = get_option( 'wgact_plugin_options', array() );
	...
	private static function send_curl_request( $method, $url, $headers, $body = '' ) {
		...
		if ( false === $response ) {
			self::log( 'cURL request failed', array( 'error' => $error, 'url' => $url ), 'error' );
			return false;
		}
		...
		if ( self::MAX_RESPONSE_BODY_SIZE < strlen( $response_body ) ) {
			self::log( 'Response too large', array( 'size' => strlen( $response_body ) ), 'warning' );
			return false;
		}
	...
	private static function send_stream_request( $method, $url, $headers, $body = '' ) {
		...
		if ( false === $response ) {
			self::log( 'Stream request failed', array( 'url' => $url ), 'error' );
			return false;
		}
	...
	private static function rewrite_response_paths( $response, $tag_id, $geo, $direct_access = false ) {
		if ( ! isset( $response['body'] ) || empty( $response['body'] ) ) {
			self::log( 'Rewrite skipped - empty body', array(), 'debug' );
			return $response;
		}
		...
		if ( $direct_access && ! empty( $config['proxy_url'] ) ) {
			...
			if ( ! empty( $base_path ) ) {
				...
				self::log( 'Using proxy_url for path rewrite (direct access mode)', array( 'base_path' => $base_path ), 'debug' );
			} else {
				self::log( 'Rewrite skipped - invalid proxy_url path', array(), 'debug' );
				return $response;
			}
		} else {
			...
			if ( ! empty( $base_path ) ) {
				...
				self::log( 'Rewrite skipped - no measurement path', array(), 'debug' );
				return $response;
			}
		}
		...
		if ( 0 === strpos( strtolower( str_replace( ' ', '', $content_type ) ), 'application/javascript' ) ) {
			...
			self::log( 'Rewrite JS response', array(
			...
			if ( $ccm_count > 0 ) {
				self::log( 'Rewrite CCM paths', array( 'count' => $ccm_count ), 'debug' );
			}
			...
			if ( isset( $response['headers']['location'] ) ) {
				...
				self::log( 'Rewrite redirect location', array( 'location' => $response['headers']['location'] ), 'debug' );
			}
		} else {
			self::log( 'Rewrite skipped - not JS or redirect', array( 'content_type' => $content_type ), 'debug' );
		}
	...
	private static function output_response( $response, $method ) {
		...
		$skip_headers = array( 'transfer-encoding', 'connection', 'content-encoding', 'content-length' );
		...
		if ( empty( $body ) && ! $allow_empty ) {
			self::log( 'Empty response from upstream', array( 'status_code' => $status_code ), 'warning' );
			http_response_code( 502 );
		...
		self::log( 'Response sent', array( 'status' => $status_code, 'size' => strlen( $body ) ), 'debug' );
