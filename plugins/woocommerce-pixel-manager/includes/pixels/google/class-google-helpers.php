...
class Google_Helpers {
	...
	public static function is_ga4_debug_mode_active() {
		...
		$debug_mode = apply_filters_deprecated('wooptpm_enable_ga_4_mp_event_debug_mode', array( false ), '1.13.0', 'pmw_enable_ga_4_mp_event_debug_mode');
		$debug_mode = apply_filters_deprecated('wpm_enable_ga_4_mp_event_debug_mode', array( $debug_mode ), '1.31.2', 'pmw_enable_ga_4_mp_event_debug_mode');
	...
	public static function get_ga_id_type() {
		...
		$ga_id_type = apply_filters_deprecated('wooptpm_product_id_type_for_google_analytics', array( $ga_id_type ), '1.13.0', 'pmw_product_id_type_for_google_analytics');
		$ga_id_type = apply_filters_deprecated('wpm_product_id_type_for_google_analytics', array( $ga_id_type ), '1.31.2', 'pmw_product_id_type_for_google_analytics');
	...
	public static function get_google_ads_conversion_ids() {
		...
		$google_ads_conversion_identifiers = apply_filters_deprecated('wgact_google_ads_conversion_identifiers', array( $google_ads_conversion_identifiers ), '1.10.2', 'pmw_google_ads_conversion_identifiers');
		$google_ads_conversion_identifiers = apply_filters_deprecated('wooptpm_google_ads_conversion_identifiers', array( $google_ads_conversion_identifiers ), '1.13.0', 'pmw_google_ads_conversion_identifiers');
		$google_ads_conversion_identifiers = apply_filters_deprecated('wpm_google_ads_conversion_identifiers', array( $google_ads_conversion_identifiers ), '1.31.2', 'pmw_google_ads_conversion_identifiers');

		$google_ads_conversion_identifiers = apply_filters('pmw_google_ads_conversion_identifiers', $google_ads_conversion_identifiers);

		$formatted_conversion_ids = array();
	...
	private static function address_requirements_are_met( $billing_address ) {
		...
		$required_keys        = array( 'first_name', 'last_name', 'postal_code', 'country' );
		$required_keys_sha256 = array( 'sha256_first_name', 'sha256_last_name', 'postal_code', 'country' );
	...
	public static function get_google_linker_settings() {
		$linker_settings = apply_filters_deprecated('wooptpm_google_cross_domain_linker_settings', array( null ), '1.13.0', 'pmw_google_cross_domain_linker_settings');
		$linker_settings = apply_filters_deprecated('wpm_google_cross_domain_linker_settings', array( $linker_settings ), '1.31.2', 'pmw_google_cross_domain_linker_settings');
	...
	public static function get_google_consent_mode_ads_data_redaction_setting() {
		 ...
		 */
		$ads_data_redaction = apply_filters_deprecated('wooptpm_google_ads_data_redaction', array( false ), '1.13.0', 'wpm_google_ads_data_redaction');
		$ads_data_redaction = apply_filters_deprecated('wooptpm_google_ads_data_redaction', array( $ads_data_redaction ), '1.31.0', 'pmw_google_ads_data_redaction');
	...
	public static function get_google_consent_mode_url_passthrough_setting() {
		$url_passthrough = apply_filters_deprecated('wooptpm_google_url_passthrough', array( true ), '1.13.0', 'wpm_google_url_passthrough');
		$url_passthrough = apply_filters_deprecated('wpm_google_url_passthrough', array( $url_passthrough ), '1.31.0', 'pmw_google_url_passthrough');
		return (bool) apply_filters('pmw_google_url_passthrough', $url_passthrough);
	}

	public static function get_ga4_parameters( $id ) {

		$ga_4_parameters = array();
		...
		$ga_4_parameters = apply_filters_deprecated('wooptpm_ga_4_parameters', array( $ga_4_parameters, $id ), '1.13.0', 'pmw_ga_4_parameters');
		$ga_4_parameters = apply_filters_deprecated('wpm_ga_4_parameters', array( $ga_4_parameters, $id ), '1.31.2', 'pmw_ga_4_parameters');
	...
	public static function get_ga_ua_parameters( $id ) {
		...
		$ga_ua_parameters = apply_filters_deprecated('woopt_pm_analytics_parameters', array( $ga_ua_parameters, $id ), '1.10.10', 'pmw_ga_ua_parameters');
		$ga_ua_parameters = apply_filters_deprecated('wooptpm_ga_ua_parameters', array( $ga_ua_parameters, $id ), '1.13.0', 'pmw_ga_ua_parameters');
		$ga_ua_parameters = apply_filters_deprecated('wpm_ga_ua_parameters', array( $ga_ua_parameters, $id ), '1.31.2', 'pmw_ga_ua_parameters');
...
//            $product = new WC_Product($refund_item->get_product_id());
	...
	private static function reformat_email_for_enhanced_conversions( $email ) {
		...
		if (isset($parts[1]) && in_array($parts[1], array( 'gmail.com', 'googlemail.com' ))) {
	...
	public static function save_gclid_in_order__premium_only( $order ) {
		...
		$gclid = self::get_gclid__premium_only();

		if (null === $gclid) {
			return;
		}

		$order->update_meta_data('_wpm_gclid', $gclid);
		$order->save();
	}

	private static function get_gclid__premium_only() {

		$_cookie = Helpers::get_input_vars(INPUT_COOKIE);
		$_get    = Helpers::get_input_vars(INPUT_GET);

		if (isset($_cookie['_gcl_aw'])) {
			return self::extract_gclid_from_string__premium_only($_cookie['_gcl_aw']);
		}

		if (is_array($_get) && array_key_exists('gclid', $_get)) {
			return self::extract_gclid_from_string__premium_only($_get['gclid']);
		}

		return '';
	}

	private static function extract_gclid_from_string__premium_only( $string ) {
	...
	public static function determine_google_tag_id_information() {
		...
		$tag_id_information = array(
			'active'     => null,
			'suppressed' => array(), // 'suppressed' is used to store IDs that are not accessible
		);
	...
	public static function gtag_gateway_health_check() {
		...
		$response = wp_remote_get($gateway_url, array(
			'timeout'   => 3,
			'headers'   => array(
