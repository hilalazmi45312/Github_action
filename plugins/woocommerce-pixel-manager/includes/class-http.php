...
class HTTP {
	...
	public function __construct( $request_args_override = array() ) {

		// Set the default options and allow these to be overridden upon instantiation
		$this->request_args = array_merge(array(
			'body'        => '',
			'timeout'     => 5,
			'redirection' => 5,
			'httpversion' => '1.0',
	...
	private function filter_options() {
		$this->request_args = apply_filters_deprecated('wooptpm_http_post_request_args', array( $this->request_args ), '1.13.0', 'pmw_http_post_request_args');
		$this->request_args = apply_filters_deprecated('wpm_http_post_request_args', array( $this->request_args ), '1.31.2', 'pmw_http_post_request_args');

		return apply_filters('pmw_http_post_request_args', $this->request_args);
	}

	public function post( $url, $payload = null, $request_args_override = array() ) {
	...
	private function log_request( $url, $response, $payload = null, $request_args_override = array() ) {
