...
//  }
	...
	public static function purge_cache_on_plugin_changes() {
		...
		add_action('update_option_' . PMW_DB_OPTIONS_NAME, array( __CLASS__, 'purge_entire_cache' ), 10, 3);
		add_action('add_option_' . PMW_DB_OPTIONS_NAME, array( __CLASS__, 'purge_entire_cache' ), 10, 3);
...
//        add_filter('upgrader_post_install', [__CLASS__, 'purge_cache_of_all_cache_plugins'], 10, 3);
		...
		add_action('upgrader_process_complete', array( __CLASS__, 'upgrader_process_complete_tasks' ), 10, 2);
	}
	...
	private static function purge_kinsta_cache() {
		...
		try {
			wp_remote_get('https://' . self::localhost_domain() . '/kinsta-clear-cache-all', array(
...
// works for WP Rocket >= 3.9
	...
	public static function third_party_plugin_tweaks_on_init() {
		...
		if (self::is_wp_cookie_consent_active()) {
			add_filter('option_wpl_options_custom-scripts', array( __CLASS__, 'add_wpl_options_custom_scripts' ));
			add_filter('default_option_wpl_options_custom-scripts', array( __CLASS__, 'add_wpl_options_custom_scripts' ));
		}
		...
		if (self::is_wp_rocket_active() && Options::is_lazy_load_pmw_active()) {
			...
			add_filter('rocket_delay_js_exclusions', array( __CLASS__, 'exclude_pmw_lazy_from_wp_rocket' ));
			add_filter('rocket_defer_inline_exclusions', array( __CLASS__, 'exclude_pmw_lazy_from_wp_rocket' ));
			add_filter('rocket_exclude_defer_js', array( __CLASS__, 'exclude_pmw_lazy_from_wp_rocket' ));
			add_filter('rocket_exclude_js', array( __CLASS__, 'exclude_pmw_lazy_from_wp_rocket' ));
			add_filter('rocket_minify_excluded_external_js', array( __CLASS__, 'exclude_pmw_lazy_from_wp_rocket' ));
			add_filter('rocket_excluded_inline_js_content', array( __CLASS__, 'exclude_pmw_lazy_from_wp_rocket' ));
		}
...
//              }
	...
	public static function add_wpl_options_custom_scripts( $custom_scripts ) {
		...
		if (!isset($custom_scripts['whitelist_script'])) {
			$custom_scripts['whitelist_script'] = array();
		}
...
//          error_log('PMW: RCB integration for ' . $type . ' was disabled.');
	...
	protected static function disable_sg_optimizer_js_optimization() {
		...
		add_filter('sgo_javascript_combine_excluded_inline_content', array( __CLASS__, 'sg_optimizer_js_exclude_combine_inline_content' ));
		add_filter('sgo_javascript_combine_exclude', array( __CLASS__, 'sgo_javascript_combine_exclude_move_after' ));
		add_filter('sgo_javascript_combine_exclude_move_after', array( __CLASS__, 'sgo_javascript_combine_exclude_move_after' ));
		add_filter('sgo_js_minify_exclude', array( __CLASS__, 'sg_optimizer_js_minify_exclude' ));
		add_filter('sgo_js_async_exclude', array( __CLASS__, 'sgo_javascript_combine_exclude_move_after' ));
	}

	protected static function disable_litespeed_js_optimization() {
		add_filter('litespeed_optimize_js_excludes', array( __CLASS__, 'litespeed_optimize_js_excludes' ));
		add_filter('litespeed_optm_js_defer_exc', array( __CLASS__, 'litespeed_cache_js_defer_exc' ));
		add_filter('litespeed_optm_cssjs', array( __CLASS__, 'litespeed_optm_cssjs' ));
		add_filter('option_litespeed.conf.optm-js_inline_defer', array( __CLASS__, 'disable_litespeed_js_inline_after_dom' ));
	}

	protected static function disable_autoptimze_js_optimization() {
		add_filter('autoptimize_filter_js_consider_minified', array( __CLASS__, 'autoptimize_filter_js_consider_minified' ));
		add_filter('autoptimize_filter_js_dontmove', array( __CLASS__, 'autoptimize_filter_js_dontmove' ));
	}

	protected static function disable_wp_optimize_js_optimization() {
		// add_filter('wpo_minify_inline_js', '__return_false');
		add_filter('wp-optimize-minify-default-exclusions', array( __CLASS__, 'wp_optimize_minify_default_exclusions' ));
	}
	...
	protected static function disable_async_javascript_js_optimization() {
		add_filter('option_aj_plugin_exclusions', function ( $options ) {
			...
			return array_unique(array_merge($options, array(
...
//      }
	...
	protected static function disable_wp_rocket_js_optimization() {
		...
		add_filter('rocket_delay_js_exclusions', array( __CLASS__, 'add_wp_rocket_exclusions' ));
		add_filter('rocket_defer_inline_exclusions', array( __CLASS__, 'add_wp_rocket_exclusions' ));
		add_filter('rocket_exclude_defer_js', array( __CLASS__, 'add_wp_rocket_exclusions' ));
		add_filter('rocket_exclude_js', array( __CLASS__, 'add_wp_rocket_exclusions' ));
		add_filter('rocket_minify_excluded_external_js', array( __CLASS__, 'add_wp_rocket_exclusions' ));
		add_filter('rocket_excluded_inline_js_content', array( __CLASS__, 'add_wp_rocket_exclusions' ));
	}
