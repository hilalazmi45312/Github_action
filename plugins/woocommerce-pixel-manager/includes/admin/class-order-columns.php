...
class Order_Columns {
	...
	private function __construct() {
		...
		// pre HPOS
		add_action('pre_get_posts', array( $this, 'view_pixels_not_fired' ), 1000, 1);
		...
		// HPOS only
		add_action('manage_woocommerce_page_wc-orders_custom_column', array( $this, 'render_order_column_content' ), 20, 2);
		add_action('woocommerce_order_list_table_prepare_items_query_args', array( $this, 'hpos_view_query_adjustment' ), 20);
		add_filter('views_woocommerce_page_wc-orders', array( $this, 'hpos_menu_view' ), 20);
	}

	public function hpos_menu_view( $views ) {

		$count_missing_pixel_fires = count(wc_get_orders(array(
			'return'         => 'ids',
	...
	public function hpos_view_query_adjustment( $query_args ) {
		...
		$query_args['status']       = array( 'wc-completed', 'wc-processing', 'wc-on-hold', 'wc-pending' );
	...
	public function view_pixels_not_fired( $query ) {
		...
		if ($is_pmw_pixels_not_fired_view) {
			...
			$query->set('post_status', array( 'wc-completed', 'wc-processing', 'wc-on-hold', 'wc-pending' ));
		...
		$count_missing_pixel_fires = count(get_posts(array(
			'fields'         => 'ids',
