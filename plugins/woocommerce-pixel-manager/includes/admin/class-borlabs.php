...
class Borlabs {
	...
	private static $cookie_groups_db     = array();
	private static $cookies_in_db        = array();
	private static $cookie_presets       = array();
	private static $cookie_group_presets = array();
	...
	private static function can_the_setup_be_run() {
		...
		if (!Helpers::is_admin_page(array( 'borlabs-cookie', 'borlabs-cookie-cookies', 'borlabs-cookie-settings', 'wpm', 'pmw', 'wgact' ))) {
			return false;
		}
	...
	private static function add_cookies_for_remaining_languages_in_cookie_groups() {
		...
		$cookie_group_languages = self::get_filtered_languages_from_cookie_group_db(array( 'en', 'de' ));
	...
	private static function get_filtered_languages_from_cookie_group_db( $exclude_languages = array() ) {

		// Don't exclude any languages
		$cookie_group_languages = array();
	...
	private static function update_borlabs_cookie_settings() {
		...
		foreach (self::$cookie_presets as $cookie_id => $cookie_value) {
			...
			$cookie_languages_from_groups = self::get_filtered_languages_from_cookie_group_db(array());
	...
	private static function get_pmw_info_suffix() {
		...
		$message = esc_html__('Automatically added by the Pixel Manager Pro for WooCommerce', 'woocommerce-google-adwords-conversion-tracking-tag');
		return ' (' . $message . ')';
	}
