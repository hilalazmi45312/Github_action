...
//        $html                                .= 'wc_get_page_permalink(\'checkout\'): ' . wc_get_page_permalink('checkout') . PHP_EOL;
				...
				if (true) {
					...
					$html .= self::get_acr_analysis_for_debug_info__premium_only() . PHP_EOL;
				}
...
//        $html .= PHP_EOL;
	...
	public static function run_tracking_accuracy_analysis() {
		...
		$amount_of_orders_to_analyze = min(
			...
			max(
				count(self::get_enabled_payment_gateways()) * 100,
				self::get_count_of_pmw_tracked_orders_for_one_month()
			)
		);
	...
	protected static function get_maximum_orders_to_analyze() {
		...
		if (get_transient('pmw_tracking_accuracy_analysis_running')) { // If available means that last run failed or timed out.
			...
			$maximum_orders_to_analyze = min(
				...
				self::get_default_maximum_orders_to_analyse()
			);
		} else {
			// Default value
	...
	public static function get_acr_analysis_for_debug_info__premium_only() {
		...
		foreach ($per_gateway_analysis as $gateway_analysis) {
			...
			// if $gateway_analysis['order_count_measured_including_acr'] or $gateway_analysis['percentage_including_acr_recovered'] are not set
			// return following text in the HTML "collecting data..."
			if (!isset($gateway_analysis['order_count_measured_including_acr']) || !isset($gateway_analysis['percentage_including_acr_recovered'])) {
				return 'collecting data...' . PHP_EOL;
			}
...
//      }
	...
	private static function pmw_remote_get_response( $url ) {
		$response = wp_remote_get($url, array(
			...
			'redirection'         => 0,      // We don't want to follow redirects
		));
	...
	private static function pmw_get_final_url( $url ) {
		$response = wp_remote_get($url, array(
			...
			'limit_response_size' => 5000,
			'blocking'            => true,    // We need to wait to see if we get a server response 200
			'redirection'         => 10,      // We don't want to follow redirects
		));
	...
	private static function set_transient_with_verification( $transient, $value, $expiration ) {
		...
		if (false === $stored_value) {
			Logger::debug(sprintf(
				'Transient verification failed for %s. External object cache (Redis/Memcached) may have rejected the data. Cache type: %s',
				$transient,
				Environment::get_external_object_cache()
			));
			return false;
		}
