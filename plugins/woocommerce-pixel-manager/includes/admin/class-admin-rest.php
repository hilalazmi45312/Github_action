...
class Admin_REST {
	...
	public function register_routes() {
		...
		register_rest_route(self::$rest_namespace, '/notifications/', array(
			'methods'             => 'POST',
		...
		register_rest_route(self::$rest_namespace, '/options-backup/(?P<timestamp>\d+)', array(
			'methods'             => 'GET',
			'callback'            => function ( $request ) {
				...
				if (!$timestamp) {
					return new \WP_Error('invalid_timestamp', 'Invalid timestamp provided', array( 'status' => 400 ));
				}
				...
				if (empty($backup)) {
					return new \WP_Error('backup_not_found', 'Backup not found for the specified timestamp', array( 'status' => 404 ));
				}
		...
		register_rest_route(self::$rest_namespace, '/ltv/', array(
			'methods'             => 'POST',
			'callback'            => function ( $request ) {
		...
		register_rest_route(self::$rest_namespace, '/options-backup/(?P<timestamp>\d+)/restore', array(
			'methods'             => 'POST',
			'callback'            => function ( $request ) {
				$timestamp = $request->get_param('timestamp');
		...
		register_rest_route(self::$rest_namespace, '/logs/download', array(
			'methods'             => 'POST',
			'callback'            => function ( $request ) {
				...
				if (empty($logs)) {
					wp_send_json_error(array( 'message' => esc_html__('No log files found.', 'woocommerce-google-adwords-conversion-tracking-tag') ));
				}
				...
				if (empty($filtered_logs)) {
					wp_send_json_error(array( 'message' => esc_html__('No log files found for the specified source.', 'woocommerce-google-adwords-conversion-tracking-tag') ));
				}
				...
				if ($zip->open($zip_file, \ZipArchive::CREATE) !== true) {
					wp_send_json_error(array( 'message' => esc_html__('Could not create ZIP archive.', 'woocommerce-google-adwords-conversion-tracking-tag') ));
				}
				...
				if (0 === $files_added) {
					...
					rmdir($temp_dir);
					wp_send_json_error(array( 'message' => esc_html__('No files could be added to the ZIP archive.', 'woocommerce-google-adwords-conversion-tracking-tag') ));
				}
