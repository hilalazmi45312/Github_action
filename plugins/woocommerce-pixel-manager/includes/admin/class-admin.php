...
class Admin {
	...
	public static function init_admin() {
		...
		add_action('admin_enqueue_scripts', array( __CLASS__, 'wpm_admin_scripts' ));

		add_action('admin_enqueue_scripts', array( __CLASS__, 'pmw_edit_order_scripts' ));

		add_action('admin_enqueue_scripts', array( __CLASS__, 'wpm_admin_css' ));

		// add the admin options page
		add_action('admin_menu', array( __CLASS__, 'plugin_admin_add_page' ), 99);

		// install a settings page in the admin console
		add_action('admin_init', array( __CLASS__, 'plugin_admin_init' ));

		add_action('admin_init', array( __CLASS__, 'add_order_extra_details' ));
		...
		if (true) {
			...
			// if self::google->is_ga4_data_api_active() is not true or if Options::is_order_level_ltv_calculation_active() is not true then don't add the meta box
			if (Helpers::can_order_modal_be_shown()) {
				...
				add_action('add_meta_boxes', function () {
					...
					add_meta_box(
						...
						esc_html__('Pixel Manager', 'woocommerce-google-adwords-conversion-tracking-tag') . self::html_beta(),
						array( __CLASS__, 'pmw_order_modal__premium_only' ),
						$screen,
						'side',
						'core'
					);
		...
		add_action('admin_head', array( __CLASS__, 'output_cody_availability' ));

		// output some info into the wpmDataLayer object in the header
		add_action('admin_head', array( __CLASS__, 'wpm_data_layer' ));
	}
	...
	public static function output_cody_availability() {
		...
		<script>
			var pmw_cody = {
				available: <?php echo esc_html($is_available ? 'true' : 'false'); ?>,
				url      : '<?php echo esc_html($chatbase_url); ?>',
			};
		</script>
		<?php
		...
		if ($is_available) :
			...
			<div id="pmw-chatbot-widget" style="display: none;">
				<button type="button" id="pmw-chatbot-toggle" class="pmw-chatbot-toggle" aria-label="<?php esc_attr_e('Open chat', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>">
			...
			<div id="pmw-chatbot-panel" class="pmw-chatbot-panel">
				<div class="pmw-chatbot-panel-header">
					<h3><?php esc_html_e('Chat with Pixie', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></h3>
					<button type="button" id="pmw-chatbot-close" class="pmw-chatbot-close" aria-label="<?php esc_attr_e('Close chat', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>">
	...
	public static function pmw_order_modal__premium_only( $post_or_order_object ) {
		...
		<div class="pmw order-modal">
			<?php if (Options::is_ga4_data_api_active()) : ?>
				<div class="section">
					<div class="title">
						<h1><?php esc_html_e('GA4 Attribution', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></h1>
						<?php self::get_documentation_html_by_key('ga4_data_api'); ?>
					</div>
			...
			<?php if (LTV::get_total_ltv_from_order($order)) : ?>
				<div class="section">
					...
					<div class="data" id="order-modal-ltv-data">
						...
						<span><?php echo wp_kses_post(wc_price(Shop::get_order_value_in_base_currency($order, LTV::get_total_ltv_from_order($order)))); ?></span>
						<h4><?php esc_html_e('by marketing order value', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></h4>
						<span><?php echo wp_kses_post(wc_price(Shop::get_order_value_in_base_currency($order, LTV::get_marketing_ltv_from_order($order)))); ?></span>
					</div>
				</div>
			<?php endif; ?>
		</div>
		...
		wp_localize_script(
			...
			array(
				...
				'timezone' => array(
					...
					'offset' => wp_timezone()->getOffset(new \DateTime()) / 3600, // Hours offset from UTC
				),
			)
		);
	...
	public static function add_order_extra_details() {
		...
		add_action('woocommerce_admin_order_data_after_order_details', function ( $order ) {
			...
			<div class="form-field form-field-wide">
				<h4><?php esc_html_e('Pixel Manager Order Details', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></h4>
				<div>
					<p>
						<strong><a href="<?php echo esc_url($order->get_checkout_order_received_url() . '&nodedupe&pmwloggeron'); ?>"><?php esc_html_e('order URL', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></a></strong>
					</p>
				</div>
			</div>
			<?php
	...
	public static function plugin_admin_add_page() {
		...
		add_submenu_page(
			...
			'wpm',
			array( __CLASS__, 'plugin_options_page' )
		);
	}
	...
	public static function plugin_admin_init() {
		...
		register_setting(
			...
			array( '\SweetCode\Pixel_Manager\Admin\Validations', 'options_validate' )
		);
	...
	public static function add_section_main() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_main_description' ),
		...
		// pro version
		if (true || Options::is_pro_version_demo_active()) {
	...
	public static function add_section_main_subsection_statistics( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_google_analytics_4_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_hotjar_site_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (true || Options::is_pro_version_demo_active()) {
			add_settings_field(
				...
				esc_html__(
					'Contentsquare tag ID',
					'woocommerce-google-adwords-conversion-tracking-tag'
				) . self::html_beta(),
				array( __CLASS__, 'option_html_contentsquare_tag_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
	...
	public static function add_section_main_subsection_marketing( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_google_ads_conversion_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (Environment::is_woocommerce_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_google_ads_conversion_label' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_facebook_pixel_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_adroll_advertiser_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_adroll_pixel_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_linkedin_partner_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_bing_uet_tag_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_outbrain_advertiser_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_pinterest_pixel_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_reddit_pixel_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_snapchat_pixel_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_taboola_account_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_tiktok_pixel_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_twitter_pixel_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
	...
	public static function add_section_main_subsection_optimization( $section_ids ) {
		...
		if (Helpers::is_experiment()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_ab_tasty_account_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'option_html_optimizely_project_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_vwo_account_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced() {
		...
		add_settings_section(
			$section_ids['settings_name'],
			esc_html($section_ids['title']),
			array( __CLASS__, 'plugin_section_advanced_description' ),
			'wpm_plugin_options_page'
		);
	...
	public static function add_section_advanced_subsection_general( $section_ids ) {
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_disable_tracking_for_user_roles' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'info_html_scroll_tracker_thresholds' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'html_lazy_load_pmw' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'html_track_pageview_events_s2s' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_maximum_compatibility_mode' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'html_load_deprecated_functions' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_shop() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_shop_html' ),
			'wpm_plugin_options_page'
		);
		...
		add_settings_field(
			...
			esc_html__(
				...
				'woocommerce-google-adwords-conversion-tracking-tag'
			) . self::get_documentation_html_e(Documentation::get_link('marketing_value_logic')),
			array( __CLASS__, 'html_marketing_value_logic' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_dynamic_remarketing' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'plugin_option_product_identifier' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_variations_output' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'plugin_option_google_business_vertical' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_order_duplication_prevention' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'info_html_order_list_info' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'html_subscription_value_multiplier' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'html_ltv_calculation_on_orders' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (Helpers::is_experiment()) {
			...
			add_settings_field(
				...
				esc_html__(
					...
					'woocommerce-google-adwords-conversion-tracking-tag'
				) . self::html_experiment(),
				array( __CLASS__, 'html_ltv_automatic_recalculation' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'ltv_manual_recalculation' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			esc_html__(
				'Order Extra Details Output',
				'woocommerce-google-adwords-conversion-tracking-tag'
			) . self::html_beta(),
			array( __CLASS__, 'order_extra_details_output' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_google( $section_ids ) {
		...
		if (Environment::is_woocommerce_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'plugin_setting_google_tag_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'plugin_setting_google_gateway_measurement_path' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'plugin_setting_aw_merchant_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'info_html_google_analytics_eec' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			if (true || Options::is_pro_version_demo_active()) {
				...
				add_settings_field(
					...
					array( __CLASS__, 'setting_html_google_analytics_4_api_secret' ),
					'wpm_plugin_options_page',
					$section_ids['settings_name']
				);
				...
				add_settings_field(
					...
					array( __CLASS__, 'setting_html_ga4_property_id' ),
					'wpm_plugin_options_page',
					$section_ids['settings_name']
				);
				...
				add_settings_field(
					...
					array( __CLASS__, 'setting_html_g4_data_api_credentials' ),
					'wpm_plugin_options_page',
					$section_ids['settings_name']
				);
			}
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_g4_page_load_time_tracking' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_google_user_id' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			if (Environment::is_woocommerce_active()) {
				...
				add_settings_field(
					...
					array( __CLASS__, 'setting_html_google_enhanced_conversions' ),
					'wpm_plugin_options_page',
					$section_ids['settings_name']
				);
			}
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_google_ads_phone_conversion_number' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_google_ads_phone_conversion_label' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			if (Environment::is_woocommerce_active()) {
				...
				add_settings_field(
					...
					array( __CLASS__, 'setting_html_google_ads_conversion_adjustments_conversion_name' ),
					'wpm_plugin_options_page',
					$section_ids['settings_name']
				);
				...
				add_settings_field(
					...
					array( __CLASS__, 'setting_html_google_ads_conversion_adjustments_feed' ),
					'wpm_plugin_options_page',
					$section_ids['settings_name']
				);
			}
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'info_html_automatic_email_link_tracking' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
			...
			add_settings_field(
				...
				array( __CLASS__, 'info_html_automatic_phone_link_click_tracking' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
	...
	public static function add_section_advanced_subsection_facebook( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_facebook_capi_token' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_facebook_capi_test_event_code' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_facebook_advanced_matching' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			esc_html__(
				'Meta (Facebook): Domain Verification Meta Tag ID',
				'woocommerce-google-adwords-conversion-tracking-tag'
			) . self::html_beta(),
			array( __CLASS__, 'setting_html_facebook_domain_verification_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_pinterest( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_pinterest_ad_account_id' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_pinterest_apic_token' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_pinterest_enhanced_match' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_pinterest_advanced_matching' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_snapchat( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_snapchat_capi_token' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_snapchat_advanced_matching' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_reddit( $section_ids ) {
		...
		add_settings_field(
			...
			esc_html__(
				'Reddit Conversions API: token',
				'woocommerce-google-adwords-conversion-tracking-tag'
			),
			array( __CLASS__, 'option_html_reddit_capi_token' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			esc_html__(
				'Reddit Conversions API: test event code',
				'woocommerce-google-adwords-conversion-tracking-tag'
			),
			array( __CLASS__, 'option_html_reddit_capi_test_event_code' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_reddit_advanced_matching' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_tiktok( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_tiktok_eapi_token' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_tiktok_eapi_test_event_code' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_tiktok_advanced_matching' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_bing( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'option_html_bing_enhanced_conversions' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_linkedin( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_linkedin_view_content' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_linkedin_add_to_cart' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_linkedin_purchase' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_advanced_subsection_twitter( $section_ids ) {
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_twitter_add_to_cart' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_twitter_add_to_wishlist' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_twitter_view_content' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_twitter_search' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_twitter_initiate_checkout' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			esc_html__(
				'Add Payment Info Event ID',
				'woocommerce-google-adwords-conversion-tracking-tag'
			),
			array( __CLASS__, 'setting_twitter_add_payment_info' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_twitter_purchase' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
	}
	...
	public static function add_section_consent_management() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_consent_management_html' ),
			'wpm_plugin_options_page'
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_google_consent_mode_active' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);

		if (true) {
			// Add fields for the Microsoft Ads Consent Mode
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_microsoft_ads_consent_mode_active' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_explicit_consent_mode' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'setting_html_explicit_consent_regions' ),
			'wpm_plugin_options_page',
			$section_ids['settings_name']
		);
		...
		if (true || Options::is_pro_version_demo_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_google_tcf_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_cookiebot_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_cookiebot_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_complianz_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_complianz_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_cookie_notice_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_cookie_notice_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_cookie_script_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_cookie_script_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_moove_gdpr_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_moove_gdpr_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_cookieyes_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_cookieyes_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
		...
		if (Environment::is_termly_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'setting_html_termly_support' ),
				'wpm_plugin_options_page',
				$section_ids['settings_name']
			);
		}
	...
	public static function add_section_logs() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_logger' ),
			'wpm_plugin_options_page'
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'html_logger_activation' ),
			'wpm_plugin_options_page',
			'wpm_plugin_log_section'
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'html_log_level' ),
			'wpm_plugin_options_page',
			'wpm_plugin_log_section'
		);
		...
		add_settings_field(
			...
			array( __CLASS__, 'html_log_outgoing_http_requests' ),
			'wpm_plugin_options_page',
			'wpm_plugin_log_section'
		);
		...
		if (Environment::is_woocommerce_active()) {
			...
			add_settings_field(
				...
				array( __CLASS__, 'html_log_files' ),
				'wpm_plugin_options_page',
				'wpm_plugin_log_section'
			);
		}
	...
	public static function add_section_diagnostics() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_diagnostics_html' ),
			'wpm_plugin_options_page'
		);
	}
	...
	public static function add_section_opportunities() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_opportunities_html' ),
			'wpm_plugin_options_page'
		);
	}
	...
	public static function add_section_support() {
		...
		add_settings_section(
			...
			array( __CLASS__, 'plugin_section_support_description' ),
			'wpm_plugin_options_page'
		);
	}
					...
					<table>
						...
						<?php foreach (Debug_Info::get_payment_gateways() as $gateway) : ?>
							<tr>
								...
								<td><?php echo esc_html($gateway->method_title); ?></td>
								<td><?php echo esc_html(get_class($gateway)); ?></td>
							</tr>
						<?php endforeach; ?>
						</tbody>
					</table>
				...
				<div style="margin-left: 10px; font-family: Courier;">
					<table>
						...
						<?php foreach ($per_gateway_analysis as $gateway_analysis) : ?>
							...
							?>
							<tr>
								<td><?php echo esc_html($gateway_analysis['gateway_id']); ?></td>
								<td><?php echo esc_html($gateway_analysis['order_count_measured']); ?></td>
								<td>of</td>
								<td><?php echo esc_html($gateway_analysis['order_count_total']); ?></td>
								<td>=</td>
								<td><?php echo esc_html($gateway_analysis['percentage']); ?>%</td>
								<td><?php self::get_gateway_accuracy_warning_status($gateway_analysis['percentage']); ?></td>
							</tr>
						<?php endforeach; ?>
			...
			<div style="margin-top: 10px">
				...
				<div style="margin-left: 10px; font-family: Courier;">
					<table>
						...
						<tr>
							<td>Total</td>
							<td><?php echo esc_html($order_count_measured); ?></td>
							<td>of</td>
							<td><?php echo esc_html($order_count_total); ?></td>
							<td>=</td>
							<td>
								<?php
							...
							<td><?php self::get_gateway_accuracy_warning_status($percent); ?></td>
				...
				<div style="margin-left: 10px; font-family: Courier;">
					...
					<table>
						...
						<?php foreach ($per_gateway_analysis as $gateway_analysis) : ?>
							...
							if (!isset($gateway_analysis['order_count_total'])) {
								// skip
								continue;
							}

							$order_count_total_value = (int) $gateway_analysis['order_count_total'];

							$order_count_measured_including_acr = isset($gateway_analysis['order_count_measured_including_acr'])
								? (int) $gateway_analysis['order_count_measured_including_acr']
								: ( isset($gateway_analysis['order_count_measured'])
									? (int) $gateway_analysis['order_count_measured']
									: 0 );

							$order_count_measured_raw = isset($gateway_analysis['order_count_measured'])
								? (int) $gateway_analysis['order_count_measured']
								: $order_count_measured_including_acr;

							$acr_recovered = isset($gateway_analysis['acr_recovered'])
								? (int) $gateway_analysis['acr_recovered']
								: 0;

							$percentage_including_acr_recovered = isset($gateway_analysis['percentage_including_acr_recovered'])
								? $gateway_analysis['percentage_including_acr_recovered']
								: Helpers::get_percentage($order_count_measured_including_acr, $order_count_total_value);

							$gateway_id = isset($gateway_analysis['gateway_id'])
								? $gateway_analysis['gateway_id']
								: __('Unknown gateway', 'woocommerce-google-adwords-conversion-tracking-tag');

							$order_count_total    += $order_count_total_value;
							$order_count_measured += $order_count_measured_including_acr;
							$missing_conversions  += max($order_count_total_value - $order_count_measured_raw, 0);
							$acr_total_recovered  += $acr_recovered;
							?>
							<tr>
								<td><?php echo esc_html($gateway_id); ?></td>
								<td><?php echo esc_html($order_count_measured_including_acr . ' (' . $acr_recovered . ')'); ?></td>

								<td>of</td>
								<td><?php echo esc_html($order_count_total_value); ?></td>
								<td>=</td>
								<td><?php echo esc_html($percentage_including_acr_recovered); ?>%
								</td>

								<td><?php self::get_gateway_accuracy_warning_status($percentage_including_acr_recovered); ?></td>
						...
						<tr>
							<td>Total</td>
							<td><?php echo esc_html($order_count_measured); ?>
								(<?php echo esc_html($acr_total_recovered); ?>)
							</td>
							<td>of</td>
							<td><?php echo esc_html($order_count_total); ?></td>
							<td>=</td>
				...
				<div style="margin-top: 10px">
					...
					<div style="margin-left: 10px">
						<p>
							...
							<?php esc_html_e('Get the pro version of the Pixel Manager Pro for WooCommerce over here', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
	...
	public static function plugin_section_support_description() {
		...
		<div class="pmw-debug-info-section">
			...
			<div>
				<textarea id="debug-info-textarea" class=""
							...
							cols="100%" rows="30" readonly><?php echo esc_html(Debug_Info::get_debug_info()); ?>
				</textarea>
				...
				<span id="debug-info-success" class="success-message"
						style="display:none;margin-left: 10px;color: #2e7d32;font-weight: 500;"><?php esc_html_e('Copied!', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></span>
			</div>
		...
		<div class="pmw-export-section">
			...
			<div>
				<textarea id="export-settings-json" class=""
							...
							cols="100%" rows="10" readonly><?php echo wp_json_encode(Options::get_options()); ?>
				</textarea>
				<div class="pmw-export-buttons">
					<button id="export-to-disk-button" class="button button-primary"
							type="button"
							onclick="wpm.saveSettingsToDisk()">
						<?php esc_html_e('Export to disk', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
					</button>
					<button id="export-copy-clipboard-button" class="button button-primary"
							type="button">
						<?php esc_html_e('Copy to Clipboard', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
					</button>
		...
		<div class="pmw-import-section">
			<h2 class="pmw-import-heading"><?php esc_html_e('Import settings', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></h2>
			<div class="pmw-import-content">
				<div class="pmw-file-upload-container">
					<a href="#" class="pmw-file-upload-button button button-primary">
						<?php esc_html_e('Choose File', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
					</a>
		...
		<div class="pmw-backup-section">
			<h2 class="pmw-backup-heading"><?php esc_html_e('Automatic Settings Backup', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></h2>
			<p class="pmw-backup-description"><?php esc_html_e('Manage your settings backups. You can restore previous configurations or download backup files.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></p>
			...
			<div class="pmw-backup-content">
				...
				if (empty($options_backup_array)) {
					...
					echo '</tbody>';
					echo '</table>';
					echo '</div>'; // Close .pmw-table-container
				}
				?>
	...
	public static function option_html_google_analytics_4_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_ga4_measurement_id()); ?>"
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_google_ads_conversion_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_google_ads_conversion_id()); ?>"
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_google_ads_conversion_label() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_google_ads_conversion_label()); ?>"
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_vwo_account_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_vwo_account_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_optimizely_project_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_optimizely_project_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_ab_tasty_account_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_ab_tasty_account_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_contentsquare_tag_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_contentsquare_tag_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_facebook_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_facebook_pixel_id()); ?>"
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_adroll_advertiser_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_adroll_advertiser_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_adroll_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_adroll_pixel_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_bing_enhanced_conversions() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_bing_enhanced_conversions_enabled()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php esc_html_e('Enable Microsoft Enhanced Conversions', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php
	...
	public static function option_html_linkedin_partner_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_linkedin_partner_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function setting_linkedin_view_content() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_linkedin_conversion_id('view_content')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function setting_linkedin_add_to_cart() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_linkedin_conversion_id('add_to_cart')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
	...
	public static function setting_linkedin_purchase() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_linkedin_conversion_id('purchase')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
	...
	public static function option_html_bing_uet_tag_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_bing_uet_tag_id()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php
	...
	public static function option_html_twitter_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_pixel_id()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php
	...
	public static function setting_twitter_add_to_cart() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('add_to_cart')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_twitter_add_to_wishlist() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('add_to_wishlist')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_twitter_view_content() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('view_content')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_twitter_search() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('search')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_twitter_initiate_checkout() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('initiate_checkout')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_twitter_add_payment_info() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('add_payment_info')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_twitter_purchase() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_twitter_event_id('purchase')); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function option_html_outbrain_advertiser_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_outbrain_advertiser_id()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php
	...
	public static function option_html_pinterest_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_pinterest_pixel_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_pinterest_enhanced_match() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_pinterest_enhanced_match_enabled()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
	...
	public static function option_html_snapchat_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_snapchat_pixel_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_taboola_account_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_taboola_account_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_tiktok_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_tiktok_pixel_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_hotjar_site_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_hotjar_site_id()); ?>"
				onclick="this.select();"
		/>
		<?php
	...
	public static function option_html_reddit_pixel_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_reddit_advertiser_id()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		...
		<p style="margin-top:16px">
			...
			<?php echo '&nbsp;<code>t2_gvnawxpb</code>'; ?>
		</p>
		<?php
	}
	...
	public static function option_html_snapchat_capi_token() {
		...
		<textarea class="pmw mono"
					...
					rows="6"
					onfocus="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>><?php echo esc_html(Options::get_snapchat_capi_token()); ?></textarea>
		<?php self::display_status_icon(Options::get_snapchat_capi_token(), Options::is_snapchat_active()); ?>
		<?php self::get_documentation_html_by_key('snapchat_capi_token'); ?>
		<?php self::html_pro_feature(); ?>
		<?php if (!Options::is_snapchat_active()) : ?>
			<p>
				<span class="dashicons dashicons-info"></span>
				<?php esc_html_e('You need to activate the Snapchat pixel', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			</p>
		<?php endif; ?>
		<?php
	}
	...
	public static function option_html_snapchat_advanced_matching() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_snapchat_advanced_matching_enabled()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php esc_html_e('Enable Snapchat Advanced Matching', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php
	...
	public static function option_html_reddit_advanced_matching() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_reddit_advanced_matching_enabled()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
	...
	public static function option_html_reddit_capi_token() {
		...
		<textarea class="pmw mono"
					...
					rows="6"
					onfocus="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>><?php echo esc_html(Options::get_reddit_capi_token()); ?></textarea>
		<?php self::display_status_icon(Options::get_reddit_capi_token(), Options::is_reddit_active()); ?>
		<?php self::get_documentation_html_by_key('reddit_capi_token'); ?>
		<?php self::html_pro_feature(); ?>
		<?php if (!Options::is_reddit_active()) : ?>
			<p>
				<span class="dashicons dashicons-info"></span>
				<?php esc_html_e('You need to activate the Reddit pixel', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			</p>
		<?php endif; ?>
		<?php
	}
	...
	public static function option_html_reddit_capi_test_event_code() {
		...
		<input class="pmw mono"
				...
				value="<?php echo esc_html(Options::get_reddit_capi_test_event_code()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php self::display_status_icon(Options::get_reddit_capi_test_event_code(), Options::is_reddit_active()); ?>
		<?php self::get_documentation_html_by_key('reddit_capi_test_event_code'); ?>
		<?php self::html_pro_feature(); ?>
		<?php if (!Options::is_reddit_active()) : ?>
			<p>
				<span class="dashicons dashicons-info"></span>
	...
	public static function html_marketing_value_logic() {
		...
		<label>
			<input type="radio"
					...
					name="<?php echo esc_html(Options::get_marketing_value_logic_input_field_name()); ?>"
					value="0"
		...
		<?php if (true || '2' === Options::get_marketing_value_logic() || Options::is_pro_version_demo_active()) : ?>
			<br>
			<label>
				<input type="radio"
					...
					<?php if (!Environment::is_a_cog_plugin_active() || !true) : ?>
						disabled
					<?php endif; ?>
				>
		...
		<div style="margin-top: 10px">
			<div>
				...
				<?php if ( true && !Environment::is_a_cog_plugin_active() ) : ?>
			</div>
			<div style="margin-top: 10px">
				<span class="dashicons dashicons-info"></span>
	...
	private static function output_advanced_section_cog_html( $subsection ) {
		...
		<a href="#" class="advanced-section-link pmw-tooltip" data-as-section="advanced"
			data-as-subsection="<?php echo esc_html($subsection); ?>">
	...
	public static function setting_html_google_consent_mode_active() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_google_consent_mode_active()); ?>
			/>
			<?php esc_html_e('Enable Google Consent Mode v2 with standard settings', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>

		</label>
		<?php self::display_status_icon(Options::is_google_consent_mode_active(), true, true); ?>
		<?php
	...
	public static function setting_html_microsoft_ads_consent_mode_active() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_bing_consent_mode_active()); ?>
			/>
			<?php esc_html_e('Enable Microsoft Ads Consent Mode with standard settings', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>

		</label>
		<?php self::display_status_icon(Options::is_bing_consent_mode_active(), true, true); ?>
		<?php
	...
	public static function setting_html_explicit_consent_mode() {
		...
		<label>
			<input type="hidden"
					value="0"
					name="<?php echo esc_html(Options::get_cookie_consent_explicit_consent_input_field_name()); ?>"
			>
			<input type="checkbox"
					id="setting_explicit_consent_mode"
					name="<?php echo esc_html(Options::get_cookie_consent_explicit_consent_input_field_name()); ?>"
					value="1"
				<?php checked(Options::is_consent_management_explicit_consent_active()); ?>
				<?php disabled(Options::consent_management_is_explicit_consent_active_override()); ?>
			/>
			<?php
			esc_html_e('Enable Explicit Consent Mode', 'woocommerce-google-adwords-conversion-tracking-tag');
			?>
		</label>
		<?php self::display_status_icon(Options::is_consent_management_explicit_consent_active(), true, true); ?>
		<?php if (Options::consent_management_is_explicit_consent_active_override()) : ?>
			<?php self::html_status_icon_override(); ?>
		<?php endif; ?>
		<?php self::get_documentation_html_by_key('explicit_consent_mode'); ?>
		<p style="margin-top:10px">
		<div>

			<?php esc_html_e('While the Explicit Consent Mode is active no pixels will be fired until the visitor gives consent.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			<br>
		</div>
		<div style="margin-top: 5px">
			<span class="dashicons dashicons-info"></span>
			<?php esc_html_e("Only activate the Explicit Consent Mode if you are also using a Consent Management Platform (a cookie banner) that is compatible with the Pixel Manager. Here's a list of compatible plugins:", 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			<a
					href="https://sweetcode.com/docs/wpm/consent-management/platforms"
					target="_blank"><?php esc_html_e('Compatible Consent Management Platforms (CMPs)', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></a>
			<br>
			<?php esc_html_e('You can also use our Consent API to make your custom cookie banner compatible with the Pixel Manager:', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			<a
					href="https://sweetcode.com/docs/wpm/consent-management/api"
					target="_blank"><?php esc_html_e('Consent API', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></a>
		</div>
		</p>
		<?php
	}
	...
	public static function setting_html_explicit_consent_regions() {
		...
		>
			<?php foreach (Consent_Mode_Regions::get_consent_mode_regions() as $region_code => $region_name) : ?>
				<option value="<?php echo esc_html($region_code); ?>"
					<?php
		...
		<?php self::get_documentation_html_by_key('restricted_consent_regions'); ?>
		<?php self::display_status_icon(Options::is_consent_management_explicit_consent_active(), true, true); ?>
		<p>
			<?php
			...
			<br>
		<div style="margin-top: 5px">
			<?php if (!Options::is_consent_management_explicit_consent_active()) : ?>
				<span class="dashicons dashicons-info"></span>
				<?php esc_html_e('Requires the Explicit Consent Mode to be active.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
				<br>
			<?php endif; ?>
			<span class="dashicons dashicons-info"></span>
			<?php
	...
	public static function setting_html_google_tcf_support() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_google_tcf_support_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
	...
	public static function setting_html_google_analytics_4_api_secret() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_ga4_mp_api_secret()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php
	...
	public static function setting_html_ga4_property_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_ga4_data_api_property_id()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php
	...
	public static function setting_html_g4_data_api_credentials() {
		...
		$client_email
			= isset(Options::get_ga4_data_api_credentials()['client_email'])
			? Options::get_ga4_data_api_credentials()['client_email']
			: '';
		...
		<div style="margin-top: 5px">
			...
			<input class="pmw mono readonly"
					...
					type="text"
					style="width:<?php echo esc_html($text_length); ?>ch"
					value="<?php echo esc_html($client_email); ?>"
					readonly
					onclick="this.select();"
			/>

			<?php self::render_copy_button('pmw_setting_ga4_data_api_client_email'); ?>


			<?php self::display_status_icon(isset(Options::get_ga4_data_api_credentials()['client_email']), Options::get_ga4_data_api_property_id()); ?>
			<?php self::get_documentation_html_by_key('ga4_data_api_credentials'); ?>
			<?php self::wistia_video_icon('sm0n75srdu'); ?>
			<?php self::html_pro_feature(); ?>
	...
	public static function setting_html_g4_page_load_time_tracking() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_ga4_page_load_time_tracking_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php
	...
	public static function setting_html_google_user_id() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_google_user_id_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php
	...
	public static function setting_html_google_enhanced_conversions() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_google_enhanced_conversions_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php
	...
	public static function setting_html_google_ads_phone_conversion_number() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_google_ads_phone_conversion_number()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function setting_html_google_ads_phone_conversion_label() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_google_ads_phone_conversion_label()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php
	...
	public static function setting_html_google_ads_conversion_adjustments_conversion_name() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(Options::get_google_ads_conversion_adjustments_conversion_name()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
				onclick="this.select();"
		/>
		<?php self::display_status_icon(Options::is_google_ads_conversion_adjustments_active(), Options::is_google_ads_active()); ?>
	...
	public static function setting_html_google_ads_conversion_adjustments_feed() {
		...
		<div style="margin-top: 5px">
			...
			<?php self::html_pro_feature(); ?>
			<div style="margin-top: 20px">
				<?php if (!Options::is_google_ads_conversion_adjustments_active()) : ?>
					<span class="dashicons dashicons-info" style="padding-right: 10px"></span>
	...
	public static function setting_html_facebook_capi_token() {
		...
		<textarea class="pmw mono"
					...
					rows="5"
					onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>><?php echo esc_html(Options::get_facebook_capi_token()); ?></textarea>
		<?php
	...
	public static function setting_html_facebook_capi_test_event_code() {
		...
		<input class="pmw mono"
				...
				type="text"
				style="width:<?php echo esc_html($text_length); ?>ch"
				value="<?php echo esc_html(Options::get_facebook_capi_test_event_code()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php self::display_status_icon(Options::get_facebook_capi_test_event_code(), Options::get_facebook_capi_token() && Options::get_facebook_capi_test_event_code(), true); ?>
		<?php
	...
	public static function setting_facebook_advanced_matching() {
		...
		<label>
			...
			<input type="checkbox"
					...
					name="wgact_plugin_options[facebook][capi][user_transparency][send_additional_client_identifiers]"
					value="1"
				<?php checked(Options::is_facebook_capi_advanced_matching_enabled()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php
	...
	public static function setting_html_facebook_domain_verification_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				value="<?php echo esc_html(Options::get_facebook_domain_verification_id()); ?>"
				onclick="this.select();"
		/>
		<?php self::display_status_icon(Options::get_facebook_domain_verification_id()); ?>
		<?php self::get_documentation_html_by_key('facebook_domain_verification_id'); ?>
		<?php self::html_pro_feature(); ?>
		<br><br>
		<?php
	...
	public static function setting_html_pinterest_ad_account_id() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_pinterest_ad_account_id()); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function setting_html_pinterest_apic_token() {
		...
		<textarea class="pmw mono"
					...
					rows="2"
					onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>><?php echo esc_html(Options::get_pinterest_apic_token()); ?></textarea>
	...
	public static function setting_pinterest_advanced_matching() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_pinterest_advanced_matching_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php
			esc_html_e('Send events with additional visitor identifiers, such as email and phone number, if available.', 'woocommerce-google-adwords-conversion-tracking-tag');
	...
	public static function setting_html_tiktok_eapi_token() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($text_length); ?>"
				type="text"
				value="<?php echo esc_html(Options::get_tiktok_eapi_token()); ?>"
				style="width:<?php echo esc_html($text_length); ?>ch"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php
	...
	public static function setting_html_tiktok_eapi_test_event_code() {
		...
		<input class="pmw mono"
				...
				type="text"
				style="width:<?php echo esc_html($text_length); ?>ch"
				value="<?php echo esc_html(Options::get_tiktok_eapi_test_event_code()); ?>"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php self::display_status_icon(Options::get_tiktok_eapi_test_event_code(), Options::get_tiktok_eapi_token() && Options::get_tiktok_eapi_test_event_code(), true); ?>
	...
	public static function setting_tiktok_advanced_matching() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_tiktok_advanced_matching_enabled()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>
			<?php
			esc_html_e('Send events with additional visitor identifiers, such as email and phone number, if available.', 'woocommerce-google-adwords-conversion-tracking-tag');
	...
	public static function setting_html_order_duplication_prevention() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_order_duplication_prevention_option_active()); ?>
			/>
			<?php
			self::get_order_duplication_prevention_text();
			?>
		</label>
		<?php self::display_status_icon(Options::is_order_duplication_prevention_option_active()); ?>
	...
	public static function setting_html_maximum_compatibility_mode() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_maximum_compatiblity_mode_active()); ?>
			/>
			<?php
	...
	public static function setting_html_disable_tracking_for_user_roles() {
		...
		>
			<?php foreach (get_editable_roles() as $role => $details) : ?>
				<option value="<?php echo esc_html($role); ?>" <?php echo esc_html(in_array($role, Options::get_excluded_roles(), true) ? 'selected' : ''); ?>><?php echo esc_html($details['name']); ?></option>
			<?php endforeach; ?>
	...
	public static function info_html_order_list_info() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_shop_order_list_info_enabled()); ?>
			/>
	...
	public static function info_html_scroll_tracker_thresholds() {
		...
		<input class="pmw mono"
				...
				size="40"
				type="text"
				value="<?php echo esc_html(implode(',', Options::get_scroll_tracking_thresholds())); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
		<?php self::display_status_icon(!empty(Options::get_scroll_tracking_thresholds()), true, true); ?>
	...
	public static function html_subscription_value_multiplier() {
		...
		<input class="pmw mono"
				...
				size="<?php echo esc_html($field_width); ?>"
				type="text"
				style="width:<?php echo esc_html($field_width); ?>ch"
				value="<?php echo esc_html(Options::get_subscription_multiplier()); ?>"
				onclick="this.select();"
			<?php echo esc_html(self::disable_if_demo()); ?>
		/>
	...
	public static function html_ltv_calculation_on_orders() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_order_level_ltv_calculation_active()); ?>
			/>
	...
	public static function html_ltv_automatic_recalculation() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_automatic_ltv_recalculation_active()); ?>
			/>
		...
		<?php self::display_status_icon(Options::is_automatic_ltv_recalculation_active()); ?>
		<?php
	...
	public static function order_extra_details_output() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_order_extra_details_active()); ?>
			/>
		...
		<?php self::display_status_icon(Options::is_order_extra_details_active(), true, true); ?>
		<?php
	...
	public static function html_lazy_load_pmw() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_lazy_load_pmw_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>

			<?php esc_html_e('Lazy load the Pixel Manager', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php self::display_status_icon(Options::is_lazy_load_pmw_active(), Options::lazy_load_requirements(), true); ?>
		<?php self::get_documentation_html_by_key('lazy_load_pmw'); ?>
		<?php self::html_pro_feature(); ?>
		<?php if (Options::is_lazy_load_pmw_active() && !Options::lazy_load_requirements()) : ?>
			<p>
				<span class="dashicons dashicons-info"></span>
				<?php esc_html_e('Google Optimize is active, but the Google Optimize anti flicker snippet is not. You need to activate the Google Optimize anti flicker snippet, or deactivate Google Optimize.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			</p>
		<?php endif; ?>

		<p>
			<span class="dashicons dashicons-info"></span>
			<?php esc_html_e('Enabling this feature will give you better page speed scores. Please read the documentation to learn more about the full implications while using this feature.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</p>
	...
	public static function html_track_pageview_events_s2s() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_pageview_events_s2s_active()); ?>
				<?php echo esc_html(self::disable_if_demo()); ?>
			/>

			<?php esc_html_e('Send PageView events through the server-2-server protocol', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php self::display_status_icon(Options::is_pageview_events_s2s_active(), Options::server_2_server_enabled(), true); ?>
		<?php self::get_documentation_html_by_key('pageview_events_s2s'); ?>
		<?php self::html_pro_feature(); ?>

		<?php if (Options::is_pageview_events_s2s_active() && !Options::server_2_server_enabled()) : ?>
			<p>
				<span class="dashicons dashicons-info"></span>
				<?php esc_html_e('For this feature to be used, at least one server-to-server feature, like Facebook CAPI must be enabled.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			</p>
		<?php endif; ?>
		<p style="margin-top: 10px">
			<span class="dashicons dashicons-info"></span>
			<?php esc_html_e('Enabling this feature is encouraged by some platforms like Meta (Facebook). But, it will add a lot of stress to your server, because it will have to run on every PageView.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</p>
		<?php
	}
	...
	public static function html_load_deprecated_functions() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::should_load_deprecated_functions()); ?>
			/>

			<?php esc_html_e('Load deprecated functions for backward compatibility', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php self::display_status_icon(Options::should_load_deprecated_functions(), true, true); ?>
		<?php self::get_documentation_html_by_key('load_deprecated_functions'); ?>

		<p style="margin-top: 10px">
			<span class="dashicons dashicons-info"></span>
			<?php esc_html_e('Deprecated functions provide backward compatibility for older integrations. If you are not using any custom code that relies on deprecated Pixel Manager function names or event names (such as wpmBeginCheckout, wpmAddToCart, etc.), you can safely disable this to reduce the amount of JavaScript loaded.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</p>
		<?php
	}
	...
	public static function html_logger_activation() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_logging_enabled()); ?>
			/>

			<?php esc_html_e('Enable logger', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php self::display_status_icon(Options::is_logging_enabled(), true, true); ?>
	...
	public static function html_log_level() {
		...
		<select id="wpm_plugin_option_log_level" name="wgact_plugin_options[general][logger][level]">
			<?php foreach (Logger::get_log_levels() as $log_level_number => $log_level_name) : ?>
				<option value="<?php echo esc_html($log_level_name); ?>"
					<?php selected($log_level_name, Options::get_log_level()); ?>
				>
					<?php echo esc_html($log_level_number . ' - ' . $log_level_name); ?>
				</option>
			<?php endforeach; ?>
		</select>
	...
	public static function html_log_outgoing_http_requests() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_http_request_logging_enabled()); ?>
			/>
			<?php esc_html_e('Enable HTTP request logging', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php self::display_status_icon(Options::is_http_request_logging_enabled(), true, true); ?>
	...
	public static function html_log_files() {
		...
		<button id="wgact_show_recent_log_file"
			...
			<?php disabled(!$admin_url_link_recent_wc_log); ?>
		>
		...
		<button id="wgact_download_logs_zip"
			...
			<?php disabled(empty($all_log_files)); ?>
		>
			<?php if (!empty($all_log_files)) : ?>
				<?php esc_html_e('Download all log files', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			<?php else : ?>
				<?php esc_html_e('No log files found to download', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			<?php endif; ?>
		</button>
	...
	public static function option_html_variations_output() {
		...
		<label>
			...
			<input type="checkbox"
					...
					value="1"
				<?php checked(Options::is_shop_variations_output_active()); ?>
			/>
			<?php esc_html_e('Enable variations output', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</label>
		<?php self::display_status_icon(Options::is_shop_variations_output_active(), true, true); ?>
		<?php self::get_documentation_html_by_key('variations_output'); ?>
		<p>
			<span class="dashicons dashicons-info"></span>
			<?php esc_html_e('In order for this to work you need to upload your product feed including product variations and the item_group_id. Disable it, if you choose only to upload the parent product for variable products.', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</p>
		<?php
	}
		...
		<div style="display: inline-block;">
			...
			<label>
				<input type="radio"
						...
						value="0"
					<?php echo( checked(0, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
			...
			<label>
				<input type="radio"
						...
						name="wgact_plugin_options[google][ads][google_business_vertical]"
						value="1"
					<?php echo( checked(1, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
			...
			<label>
				<input type="radio"
						...
						name="wgact_plugin_options[google][ads][google_business_vertical]"
						value="3"
					<?php echo( checked(3, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
			...
			<label>
				<input type="radio"
						...
						name="wgact_plugin_options[google][ads][google_business_vertical]"
						value="4"
					<?php echo( checked(4, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
			...
			<label>
				<input type="radio"
						...
						name="wgact_plugin_options[google][ads][google_business_vertical]"
						value="5"
					<?php echo( checked(5, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
			...
			<label>
				<input type="radio"
						...
						name="wgact_plugin_options[google][ads][google_business_vertical]"
						value="6"
					<?php echo( checked(6, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
			...
			<label>
				<input type="radio"
						...
						name="wgact_plugin_options[google][ads][google_business_vertical]"
						value="8"
					<?php echo( checked(8, Options::get_google_ads_business_vertical_id(), false) ); ?>
					<?php echo esc_html(self::disable_if_demo()); ?>
				/>
	...
	public static function plugin_setting_google_tag_id() {
		...
		<input class="pmw mono readonly"
				...
				size="40"
				value="<?php echo esc_html($tag_id); ?>"
				readonly
				onclick="this.select();"
		/>

		<?php self::render_copy_button('pmw_plugin_google_tag_id', 'pmwGoogleTagIdTooltip'); ?>
		<?php self::get_documentation_html_by_key('google_tag_id'); ?>
		<br><br>
		<?php
	...
	public static function plugin_setting_google_gateway_measurement_path() {
		...
		<input class="pmw mono"
				...
				size="40"
				value="<?php echo esc_html(Options::get_google_tag_gateway_measurement_path()); ?>"
				onclick="this.select();"
		/>
		<?php self::render_copy_button('pmw_google_tag_gateway_measurement_path', 'pmwGoogleTagIdTooltip'); ?>

		<!-- Pre-rendered status icons (initially invisible) -->
		<span id="pmw-status-icon-inactive"
				class="pmw-status-icon inactive <?php echo $has_measurement_path ? 'invisible' : ''; ?>">
			<?php esc_html_e('inactive', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</span>
		<span id="pmw-status-icon-active" class="pmw-status-icon active invisible">
			<?php esc_html_e('active', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</span>
		<span id="pmw-status-icon-partially-active" class="pmw-status-icon partially-active invisible">
			<?php esc_html_e('partially active', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</span>
		...
		<span id="pmw-health-check-passed" class="pmw-status-icon active invisible">
			<?php esc_html_e('health check passed', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</span>
		<span id="pmw-health-check-failed" class="pmw-status-icon inactive invisible">
			<?php esc_html_e('health check failed', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
		</span>

		<?php self::get_documentation_html_by_key('google_tag_gateway_measurement_path'); ?>
		<?php // self::wistia_video_icon('d3ehc3lurt'); ?>
		<p style="margin-top:10px">
			<?php esc_html_e('Your Google Gateway Measurement Path. It should look like this:', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
			<code>/metrics</code>
		</p>

		<?php if ($has_measurement_path) : ?>
			<script>
				document.addEventListener("DOMContentLoaded", function () {
					// Get the gateway URL from PHP
					const gatewayUrlHealthy = '<?php echo esc_js($gateway_url_healthy); ?>';
	...
	public static function plugin_setting_aw_merchant_id() {
		...
		<input class="pmw mono"
				...
				size="40"
				value="<?php echo esc_html(Options::get_google_ads_merchant_id()); ?>"
				onclick="this.select();"
		/>
		<?php self::display_status_icon(Options::get_google_ads_merchant_id()); ?>
	...
	public static function plugin_option_product_identifier() {
		...
		<label>
			<input type="radio"
					...
					value="0"
				<?php echo( checked(0, Options::get_google_ads_product_identifier(), false) ); ?>
			/>
		...
		<label>
			<input type="radio"
					...
					name="wgact_plugin_options[google][ads][product_identifier]"
					value="2"
				<?php echo( checked(2, Options::get_google_ads_product_identifier(), false) ); ?>
			/>
		...
		<label>
			<input type="radio"
					...
					name="wgact_plugin_options[google][ads][product_identifier]"
					value="1"
				<?php echo( checked(1, Options::get_google_ads_product_identifier(), false) ); ?>
			/>
		...
		<label>
			<input type="radio"
					...
					name="wgact_plugin_options[google][ads][product_identifier]"
					value="3"
				<?php echo( checked(3, Options::get_google_ads_product_identifier(), false) ); ?>
			/>
	...
	private static function html_beta_e( $margin_top = '1px' ) {
		?>
		<div class="pmw-status-icon beta" style="margin-top: <?php echo esc_html($margin_top); ?>">
	...
	private static function html_status_icon_override() {
		?>
		<div class="pmw-status-icon partially-active"><?php esc_html_e('override', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></div>
		<?php
	}

	private static function html_status_icon_deprecated() {
		?>
		<div class="pmw-status-icon inactive"><?php esc_html_e('deprecated', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></div>
		<?php
	}

	private static function html_status_health_check_passed() {
		?>
		<div class="pmw-status-icon active"><?php esc_html_e('health check passed', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></div>
		<?php
	}

	private static function html_status_health_check_failed() {
		?>
		<div class="pmw-status-icon inactive"><?php esc_html_e('health check failed', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></div>
		<?php
	}
	...
	private static function wistia_video_icon( $wistia_id, $tooltip_text = '' ) {
		...
		<script>
			(function () {
				const video_id = '<?php echo esc_html($wistia_id); ?>';
		...
		<div class="pmw wistia_embed wistia_async_<?php echo esc_html($wistia_id); ?> popover=true popoverContent=link videoFoam=false"
			data-wistia-id="<?php echo esc_html($wistia_id); ?>">
	...
	public static function output_copy_button_js() {
		...
		<script>
			const pmwCopyToClipboard = (inputId, tooltipId) => {
				...
				pmwCaFeedTooltip.innerHTML = "<?php esc_html_e('Copied', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>";
			};
	...
	public static function render_copy_button( $input_id, $tooltip_id = null ) {
		...
		<div class="pmwCaTooltip">
			<a href="javascript:void(0)" class="pmw-copy-icon pmwCaTooltip"
				onclick="pmwCopyToClipboard('<?php echo esc_attr($input_id); ?>', '<?php echo esc_attr($tooltip_id); ?>')"
				onmouseout="pmwResetCopyButton('<?php echo esc_attr($tooltip_id); ?>')"></a>
			<span class="pmwCaTooltiptext"
					id="<?php echo esc_attr($tooltip_id); ?>"><?php esc_html_e('Copy to clipboard', 'woocommerce-google-adwords-conversion-tracking-tag'); ?></span>
		</div>
		<?php
	}
}
