...
class Shop {
	...
	public static function get_order_value_total_marketing( $order, $apply_multipliers = false ) {
		...
		if (in_array(Options::get_options_obj()->shop->order_total_logic, array( '0', 'order_subtotal' ), true)) {
		...
		} elseif (in_array(Options::get_options_obj()->shop->order_total_logic, array( '1', 'order_total' ), true)) {

			// Order total
			$order_total = $order->get_total() - $order->get_total_refunded();

		} elseif (in_array(Options::get_options_obj()->shop->order_total_logic, array( '2', 'order_profit_margin' ), true)) {
		...
		$order_total = apply_filters_deprecated('wgact_conversion_value_filter', array( $order_total, $order ), '1.10.2', 'pmw_marketing_conversion_value_filter');
		$order_total = apply_filters_deprecated('wooptpm_conversion_value_filter', array( $order_total, $order ), '1.13.0', 'pmw_marketing_conversion_value_filter');
		$order_total = apply_filters_deprecated('wpm_conversion_value_filter', array( $order_total, $order ), '1.31.2', 'pmw_marketing_conversion_value_filter');
	...
	public static function conversion_pixels_already_fired_html() {
		...
		<!--	----------------------------------------------------------------------------------------------------
				...
				More info on testing: <?php echo esc_html(Documentation::get_link('test_order')); ?>
				----------------------------------------------------------------------------------------------------
		-->
		<?php
	}
	...
	public static function can_order_confirmation_be_processed( $order ) {
		...
		$conversion_prevention = apply_filters_deprecated('wgact_conversion_prevention', array( false, $order ), '1.10.2', 'pmw_conversion_prevention');
		$conversion_prevention = apply_filters_deprecated('wooptpm_conversion_prevention', array( $conversion_prevention, $order ), '1.13.0', 'pmw_conversion_prevention');
		$conversion_prevention = apply_filters_deprecated('wpm_conversion_prevention', array( $conversion_prevention, $order ), '1.31.2', 'pmw_conversion_prevention');
...
//      }
	...
	public static function apply_subscription_multiplier__premium_only( $order_total, $order ) {
		...
		// we return early if the subscription value is zero.
		if (0 === $value_subscription + $value_rest) {
			return $order_total;
		}
	...
	public static function view_item_list_trigger_settings() {
		...
		$settings = apply_filters_deprecated('wooptpm_view_item_list_trigger_settings', array( $settings ), '1.13.0', 'pmw_view_item_list_trigger_settings');
		$settings = apply_filters_deprecated('wpm_view_item_list_trigger_settings', array( $settings ), '1.31.2', 'pmw_view_item_list_trigger_settings');
	...
	public static function get_order_paid_statuses() {
		...
		return apply_filters_deprecated('wpm_register_custom_order_confirmation_statuses', array( $statuses ), '1.30.3', 'Use the woocommerce_order_is_paid_statuses filter instead');
	}
	...
	public static function get_transient_identifiers_from_session() {
		...
		self::$transient_identifiers = self::get_value_from_woocommerce_session(self::get_transient_identifiers_key(), array());
	...
	public static function get_custom_order_parameters( $order ) {
		return (array) apply_filters('pmw_custom_order_parameters', array(), $order);
	}
	...
	public static function get_custom_order_item_parameters( $order_item, $order ) {
		return (array) apply_filters('pmw_custom_order_item_parameters', array(), $order_item, $order);
	}
