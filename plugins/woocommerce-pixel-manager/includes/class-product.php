...
class Product {
	...
	public static function pmw_get_order_items( $order ) {
		...
		$order_items = apply_filters_deprecated('wooptpm_order_items', array( $order->get_items(), $order ), '1.13.0', 'wpm_order_items');
		$order_items = apply_filters_deprecated('wpm_order_items', array( $order_items ), '1.31.2', 'pmw_order_items');
	...
	public static function get_dyn_r_ids( $product ) {
		...
		$dyn_r_ids = apply_filters_deprecated('wooptpm_product_ids', array( $dyn_r_ids, $product ), '1.13.0', 'pmw_product_ids');
		$dyn_r_ids = apply_filters_deprecated('wpm_product_ids', array( $dyn_r_ids, $product ), '1.31.2', 'pmw_product_ids');
	...
	public static function get_dyn_r_id_type( $pixel_name ) {
		...
		$dyn_r_id_type = apply_filters_deprecated('wooptpm_product_id_type_for_' . $pixel_name, array( $dyn_r_id_type ), '1.13.0', 'pmw_product_id_type_for_');
		$dyn_r_id_type = apply_filters_deprecated('wpm_product_id_type_for_' . $pixel_name, array( $dyn_r_id_type ), '1.31.2', 'pmw_product_id_type_for_');
	...
	public static function get_brand_name( $product_id ) {
		...
		$brand_taxonomy = apply_filters_deprecated('wooptpm_custom_brand_taxonomy', array( $brand_taxonomy ), '1.13.0', 'pmw_custom_brand_taxonomy');
		$brand_taxonomy = apply_filters_deprecated('wpm_custom_brand_taxonomy', array( $brand_taxonomy ), '1.31.2', 'pmw_custom_brand_taxonomy');
	...
	public static function get_product_category( $product_id ) {
		...
		if (!empty($prod_cats)) {
			...
			$prod_cats_output = apply_filters_deprecated('wgact_filter', array( $prod_cats_output ), '1.10.2', '', 'This filter has been deprecated without replacement.');
		}
