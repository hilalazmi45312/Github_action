...
class Options {
		...
		return array(
			...
			'google'     => array(
				'ads'          => array(
					...
					'enhanced_conversions'     => false,
					'conversion_adjustments'   => array(
						'conversion_name' => '',
					),
				),
				'analytics'    => array(
					'universal'        => array(                // TODO remove
															'property_id' => '',
					),
					'ga4'              => array(
						'measurement_id'          => '',
				...
				'consent_mode' => array(
					'active'  => true,
					'regions' => array(),  // TODO: Move to the consent management section
				),
				'tcf_support'  => false,
				'user_id'      => false,
				'tag_gateway'  => array(
			...
			'shop'       => array(
				...
				'cookie_consent_mgmt'           => array(
					'explicit_consent' => false,
				),
				'order_deduplication'           => true,
				'disable_tracking_for'          => array(),
				'order_list_info'               => true,
				'subscription_value_multiplier' => 1.00,
				'ltv'                           => array(
					'order_calculation'       => array(
						'is_active' => false,
					),
					'automatic_recalculation' => array(
						'is_active' => false,
					),
				),
				'order_extra_details'           => array(
					'is_active' => false,
				),
			),
			'general'    => array(
				...
				'scroll_tracker_thresholds'  => array(),
				'lazy_load_pmw'              => false,
				'logger'                     => array(
					'is_active'         => false,
					'level'             => 'warning',
	...
	public static function get_automatic_options_backup() {
		...
		return isset($options_backup['auto']) ? $options_backup['auto'] : array();
	}
...
//      error_log(print_r($target_array, true));
	...
	public static function get_restricted_consent_regions() {
		...
		if (in_array('EU', $regions, true)) {
			$regions = array_diff(array_merge($regions, WC()->countries->get_european_union_countries()), array( 'EU' ));
		}
	...
	public static function is_pro_version_demo_active() {
		...
		if (!Helpers::is_pmw_wcm_distro() && true) {
			return false;
		}
...
//      }
	...
	public static function save_automatic_options_backup_with_timestamp( $timestamp, $options = null ) {
		...
		$options_backup = get_option(self::$options_backup_name, array());

		// Ensure the auto backup section exists
		if (!isset($options_backup['auto'])) {
			$options_backup['auto'] = array();
		}
		...
		foreach ($backup_timestamps as $timestamp) {
			...
			if ($timestamp >= $monthly_cutoff) {
				...
				if (!isset($monthly_backups[$month_key]) || $timestamp > $monthly_backups[$month_key]['timestamp']) {
					$monthly_backups[$month_key] = array(
						'timestamp' => $timestamp,
						'data'      => $backups[$timestamp],
					);
				}
