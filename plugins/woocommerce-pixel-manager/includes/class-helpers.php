...
class Helpers {
	...
	public static function is_orders_page() {
		...
		if (
			'woocommerce_page_wc-orders' === self::pmw_current_screen('id')
			&& 'shop_order' === self::pmw_current_screen('post_type')
			&& !isset($_get['action'])
		) {
			return true;
		}
	...
	public static function lazy_load_pmw__premium_only() {
		...
		if (
			...
			&& Options::is_lazy_load_pmw_active()
		) {
			return true;
		}
...
//      $data = $email ? self::get_user_object_email($email) : $data;
	...
	public static function get_raw_user_data( $order = null, $current_user = null ) {
		...
		// Roles
		$roles = is_user_logged_in() && $current_user ? $current_user->roles : array();
...
//      $email = self::normalize_google_email_address($email);
	...
	public static function get_version_info() {
		return array(
			...
			'pro'                  => true,
			'eligible_for_updates' => true,
			'distro'               => PMW_DISTRO,
			//          'beta'               => PMW_DISTRO === 'fms' ? false : false,
			'beta'                 => self::is_beta(),
	 ...
	 */
	public static function pmw_as_has_scheduled_action( $hook, $args = array(), $group = '', $partial_matching = 'off' ) {
		...
		return (bool) as_get_scheduled_actions(array(
			'hook'                  => $hook,
			'args'                  => $args,
			'group'                 => $group,
...
//          'data-cookieconsent' => [],
	...
	public static function get_opening_script_string() {
		...
		if (Environment::is_cookiebot_active()) {
			$attributes['data-cookieconsent'] = array( 'ignore' );
			$attributes['data-uc-allowed']    = array( 'true' ); // The Cookiebot plugin now seems to also load the Usercentrics CMP script in some cases.
		}

		if (Environment::is_usercentrics_cmp_active()) {
			$attributes['data-uc-allowed'] = array( 'true' );
		}
		...
		if (
			Environment::is_elementor_active() &&
			Environment::is_server_behind_cloudflare()
		) {
			$attributes['data-cfasync'] = array( 'false' );
		}
