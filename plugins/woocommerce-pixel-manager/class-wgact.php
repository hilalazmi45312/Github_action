...
/**
 * Defines the main class WCPM for the Pixel Manager Pro for WooCommerce plugin.
...
//          require_once __DIR__ . '/vendor/sweetcode-com/wp-flush/wp-flush.php';
		...
		if ($this->are_requirements_not_met()) {
			add_action('admin_menu', array( $this, 'add_empty_admin_page' ), 99);
		...
		if (Environment::is_woocommerce_active()) {
			add_action('before_woocommerce_init', array( __CLASS__, 'declare_woocommerce_compatibilities' ));
			add_action('init', array( $this, 'register_hooks_for_woocommerce' ), 10, 2);
			add_action('init', array( $this, 'register_generic_hooks' ), 10, 2);
			add_action('init', array( $this, 'run_woocommerce_reports' ), 10, 2);
	...
	public function init() {
		...
		if (is_admin()) {
			...
			add_filter('plugin_action_links_' . PMW_PLUGIN_BASENAME, array( $this, 'pmw_settings_link' ));
		}
	...
	public function inject_pixels() {
		...
		$cookie_prevention = apply_filters_deprecated(
			'wooptpm_cookie_prevention', array( $cookie_prevention ),
			'1.12.1',
			'',
