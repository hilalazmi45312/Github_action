<?php
/**
 * This template displays the checkout order tip.
 *
 * This template can be overridden by copying it to yourtheme/delivery-slots-for-woocommerce/checkout-order-tip.php
 *
 ...
 * @since 1.0.0
 */
do_action( 'dey_before_checkout_order_tip_wrapper' );
?>
<div class="<?php echo esc_attr( implode( ' ', dey_get_checkout_order_tip_wrapper_classes() ) ); ?>">
	<h4><?php echo wp_kses_post( dey_get_order_tip_title_label() ); ?></h4>
	...
	if ( ( dey_is_predefined_order_tip_type() || dey_is_mixed_order_tip_type() ) && dey_check_is_array( $buttons ) ) :
		?>
		<div class="dey-order-tip-predefined-buttons">
			<?php
			foreach ( $buttons as $button_value => $button_label ) :
				?>
				<button type="button" 
 ...
 * @since 1.0
 */
do_action( 'dey_after_checkout_order_tip_wrapper' );
