<?php
/**
 * This template displays the local pickup field.
 *
 * This template can be overridden by copying it to yourtheme/delivery-slots-for-woocommerce/order/local-pickup-fields.php
 *
 ...
 *
 * @since 3.0.0
 * @var array $fields Order local pickup fields.
 */
...
?>
<div class='dey-order-local-pickup-slots-wrapper dey-order-scheduler-fields'>
	<?php
		...
		switch ( $field['type'] ) :
			case 'show_calender':
				...
				<p id="<?php echo esc_attr( $key ); ?>_field" class="form-row <?php echo esc_attr( implode( ' ', $field['class'] ) ); ?>">
					...
					<input id='dey-order-local-pickup-date-picker-field' class='dey-order-local-pickup-date-picker-field' value="<?php echo esc_attr( $field['default'] ); ?>"/>
			...
			default:
				...
				if ( 'dey_local_pickup_date' === $key && dey_can_display_order_calendar_color_info() ) :
					?>
					<div class='dey-calender-color-info'>
						<?php foreach ( dey_get_order_calender_colors_labels() as $name => $label ) : ?>
							<span class="dey-calender-dot-color dey-calender-<?php echo esc_attr( $name ); ?>-dot"><?php echo esc_html( $label ); ?></span>
	...
	endforeach;
	?>
	<input type='hidden' name='dey_local_pickup_date' class='dey-order-local-pickup-date' data-selected_date="<?php echo esc_attr( dey_get_selected_order_scheduler_data_from_session( 'order_local_pickup_date' ) ); ?>" value=""/>
</div>  
