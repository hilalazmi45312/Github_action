...
/**
 * Handles the order delivery exports.
 */
if ( ! defined( 'ABSPATH' ) ) {
...
if ( ! class_exists( 'DEY_Order_Delivery_Export_CSV' ) ) {

	/**
	 * DEY_Order_Delivery_Export_CSV.
	 */
	class DEY_Order_Delivery_Export_CSV extends WC_CSV_Exporter {
		 ...
		 * @var string
		 */
		protected $export_type = 'order_delivery' ;
		 ...
		 * @var string
		 */
		protected $filename = 'order-delivery.csv' ;
		...
		public function get_default_column_names() {
			...
			/**
			 * This hook is used to alter the order delivery heading.
			 * 
			 * @since 1.0
			 */
			return apply_filters( 'dey_order_delivery_export_heading', $headings ) ;
		}
		...
		public function prepare_data_to_export() {
			...
			$order_delivery_ids = $this->get_order_delivery_ids() ;

			foreach ( $order_delivery_ids as $order_delivery_id ) {
				...
				$this->row_data[] = self::generate_row_data( $order_delivery ) ;
			}
		}
		...
		protected function generate_row_data( $order_delivery ) {
			...
			$row = array(
				...
				'product_name'  => dey_get_order_products_link( $order_delivery->get_product_ids(), false ),
				'delivery_date' => $order_delivery->get_formatted_delivery_date(),
				'mode'          => dey_display_delivery_mode( $order_delivery->get_delivery_mode() ),
				'time_slots'    => $order_delivery->get_formatted_time_slots(),
				'currency'      => $order_delivery->get_currency(),
				'delivery_fee'  => $order_delivery->get_delivery_charge(),
				'status'        => dey_display_post_status( $order_delivery->get_status(), false ),
				'created_date'  => $order_delivery->get_formatted_created_date(),
					) ;
			/**
			 * This hook is used to alter the order delivery row data.
			 * 
			 * @since 1.0
			 */
			return apply_filters( 'dey_order_delivery_export_row_data', $row ) ;
		}

		/**
		 * Get the order delivery IDs.
		 * 
		 * @return array
		 */
		protected function get_order_delivery_ids() {
			$args = array(
