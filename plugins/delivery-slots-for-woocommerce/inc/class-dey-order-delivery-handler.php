...
if ( ! class_exists( 'DEY_Order_Delivery_Handler' ) ) {

	/**
	 * Class.
	 * */
	class DEY_Order_Delivery_Handler {
		...
		public function __construct( $scheduler_rule ) {
			if ( ! empty( $scheduler_rule ) && ! is_object( $scheduler_rule ) ) {
				$scheduler_rule = dey_get_scheduler_rule( $scheduler_rule );
			}
		 ...
		 * @return array.
		 */
		public static function get_expected_first_date() {
			return self::prepare_expected_first_date();
		}
		 ...
		 * @return array.
		 */
		public static function get_expected_last_date() {
			return self::prepare_expected_last_date();
		}
		...
		public static function render() {
			if ( '2' === self::$scheduler_rule->get_delivery_slot_mode() ) {
				self::render_expected_days();
			} else {
				self::render_fields();
			}
		}
		...
		private static function render_fields() {
			$fields             = array();
			$selected_date      = dey_get_selected_order_scheduler_data_from_session( 'order_delivery_date' );
			$formatted_datetime = '2' === self::$scheduler_rule->get_delivery_time_mode() ? DEY_Date_Time::get_wp_format_datetime( $selected_date, 'date' ) . ' ' . DEY_Date_Time::get_wp_format_datetime( $selected_date, 'H:i' ) : DEY_Date_Time::get_wp_format_datetime( $selected_date, 'date' );

			$fields['dey_delivery_date'] = array(
				'type'        => '2' === get_option( 'dey_advanced_calender_display_mode' ) ? 'show_calender' : 'text',
				'label'       => dey_get_order_delivery_date_field_label(),
				'required'    => 'no' !== self::$scheduler_rule->get_delivery_calender_required(),
				'class'       => array( 'form-row-wide', 'dey-form-row' ),
				'input_class' => array( 'dey-order-delivery-date-field' ),
				'default'     => ! empty( $selected_date ) ? $formatted_datetime : '',
			);

			switch ( self::$scheduler_rule->get_delivery_time_mode() ) {
				case '3': // Time slots.
					$fields['dey_delivery_date']['input_class'][] = 'dey-datepicker';

					$selected_time_slot_data                      = dey_get_selected_order_scheduler_data_from_session( 'order_delivery_time_slot' );
					...
					$fields['dey_order_delivery_date_time_slots'] = array(
						'type'        => 'time_slots',
						'label'       => dey_get_order_delivery_time_slots_field_label(),
						'required'    => 'no' !== self::$scheduler_rule->get_delivery_time_slot_required(),
						'class'       => array( 'form-row-wide', 'dey-form-row' ),
						'input_class' => array( 'dey-order-delivery-time-slots', 'dey_select2' ),
			...
			dey_get_template(
				'order/delivery-fields.php',
				array(
					'fields'            => $fields,
					'scheduler_rule_id' => self::$scheduler_rule->get_id(),
				)
			);
		}
		 ...
		 * @return void.
		 */
		private static function render_expected_days() {
			$msg = dey_get_expected_order_delivery_date_message( self::get_expected_first_date(), self::get_expected_last_date() );

			dey_get_template( 'order/expected-days.php', array( 'msg' => $msg ) );
		}
		...
		public static function enqueue_default_scripts() {
			...
			DEY_Frontend_Assets::enqueue_script( 'dey-order-delivery' );

			DEY_Frontend_Assets::set_additional_localize_scripts(
				'dey-order-delivery',
				'dey_order_delivery_params',
				array(
		...
		public static function get_script_data() {
			return array(
				...
				'min_date'         => self::get_calender_first_date(), // For client date object.
				'max_date'         => self::get_calender_last_date(), // For client date object.
				'first_time_slot'  => self::$scheduler_rule->get_delivery_first_available_time_slot(),
		 ...
		 * @return string.
		 */
		private static function prepare_expected_first_date() {
			if ( isset( self::$expected_first_date ) ) {
				return self::$expected_first_date;
			}
		 ...
		 * @return string.
		 */
		private static function prepare_expected_last_date() {
			if ( isset( self::$expected_last_date ) ) {
				return self::$expected_last_date;
			}
		 ...
		 * @return void.
		 */
		private static function prepare_expected_dates() {
			self::populate_data();

			// Expected first date.
			$first_date_object = self::get_shipping_date_object();
			$start_count       = 0;
			$from_range        = intval( self::$scheduler_rule->get_delivery_expected_date_from() );
			while ( isset( self::$holidays[ $first_date_object->format( 'Y-m-d' ) ] ) || isset( self::$holidays[ $first_date_object->format( 'm-d' ) ] ) || ( $start_count < $from_range ) ) {
				if ( isset( self::$holidays[ $first_date_object->format( 'Y-m-d' ) ] ) || isset( self::$holidays[ $first_date_object->format( 'm-d' ) ] ) ) {
			...
			self::$expected_first_date = $first_date_object->format( 'Y-m-d' );

			// Expected last date.
			$last_date_object = self::get_shipping_date_object();
			$start_count      = 0;
			$to_range         = intval( self::$scheduler_rule->get_delivery_expected_date_to() );
			while ( $start_count < $to_range ) {
				if ( isset( self::$holidays[ $last_date_object->format( 'Y-m-d' ) ] ) || isset( self::$holidays[ $last_date_object->format( 'm-d' ) ] ) ) {
			...
			self::$expected_last_date = $last_date_object->format( 'Y-m-d' );
		}
		...
		public static function get_available_times() {
			...
			$time_from = self::$scheduler_rule->get_delivery_available_time_from();
			if ( $time_from ) {
			...
			$time_to = self::$scheduler_rule->get_delivery_available_time_to();
			if ( $time_to ) {
			...
			/**
			 * This hook is used to alter the order delivery available times.
			 *
			 * @since 1.0.0
			 */
			return apply_filters( 'dey_order_delivery_available_times', $available_times );
		}
		...
		public static function get_shipping_date_object() {
			...
			$cutoff_time          = self::$scheduler_rule->get_delivery_same_day_cutoff_time();
			$cutoff_time          = ( empty( $cutoff_time ) ) ? '23:59' : $cutoff_time;
			$next_day_cutoff_time = self::$scheduler_rule->get_delivery_next_day_cutoff_time();
			$present_date_object  = DEY_Date_Time::get_date_time_object( 'now' );

			// Consider the business days.
			$business_days   = array_filter( (array) get_option( 'dey_order_delivery_business_days' ) );
			$business_days   = dey_parse_business_day_option( $business_days, dey_business_day_default_option() );
			$processing_time = self::$scheduler_rule->get_delivery_processing_time();
			...
			$delivery_days = array_filter( (array) self::$scheduler_rule->get_delivery_days() );
		...
		public static function get_weekday_price( $date_object ) {
			...
			$weekdays_prices = array_filter( (array) self::$scheduler_rule->get_delivery_weekdays_prices() );
		...
		public static function get_same_day_price( $date_object ) {
			$same_day_price = ( $date_object->format( 'Y-m-d' ) === DEY_Date_Time::get_date_time_object( 'now' )->format( 'Y-m-d' ) ) ? self::$scheduler_rule->get_delivery_same_day_fee() : 0;

			/**
			 * This hook is to alter the same day price for Order Delivery.
			 *
			 * @param float $same_day_price Same Day Price.
			 * @since 3.2.0
			 */
			return apply_filters( 'dey_order_delivery_same_day_price', floatval( $same_day_price ) );
		}
		...
		public static function get_next_day_price( $date_object ) {
			$next_day_price = ( $date_object->format( 'Y-m-d' ) === DEY_Date_Time::get_date_time_object( 'now' )->modify( '+1 days' )->format( 'Y-m-d' ) ) ? self::$scheduler_rule->get_delivery_next_day_fee() : 0;

			/**
			 * This hook is to alter the next day price for Order Delivery.
			 *
			 * @param float $same_day_price Same Day Price.
			 * @since 3.2.0
			 */
			return apply_filters( 'dey_order_delivery_next_day_price', floatval( $next_day_price ) );
		}
		...
		private static function get_time_slot_options() {
			$time_slot_options = array( '' => dey_get_order_delivery_default_time_slot_label() );
			$selected_date     = dey_get_selected_order_scheduler_data_from_session( 'order_delivery_date' );
			if ( empty( $selected_date ) ) {
				return $time_slot_options;
			}

			$time_slot_option_labels = self::$scheduler_rule->get_order_delivery_time_slot_option_labels( $selected_date );
		...
		private static function prepare_available_dates() {
			...
			$i             = 0;
			$max_date      = intval( self::$scheduler_rule->get_delivery_days_availability() );
			$delivery_days = array_filter( (array) self::$scheduler_rule->get_delivery_days() );
			$date_object   = self::get_shipping_date_object();

			do {
				$per_day = self::$scheduler_rule->get_delivery_max_per_day();
				// Consider the delivery days.
				if ( ! isset( self::$special_dates[ $date_object->format( 'Y-m-d' ) ] ) && dey_check_is_array( $delivery_days ) && ! in_array( $date_object->format( 'w' ) + 1, $delivery_days ) ) {
				...
				if ( ! isset( self::$special_dates[ $date_object->format( 'Y-m-d' ) ] ) && isset( self::$holidays[ $date_object->format( 'Y-m-d' ) ] ) ) {
					...
					if ( '' == $per_day ) {
						$available_date = array(
							'l' => dey_get_order_delivery_partially_booked_label(),
							't' => 'pb',
						);
						...
						$available_date  = array(
							...
							'l' => sprintf( dey_get_order_delivery_remaining_count_label(), $remaining_count ),
							't' => 'pb',
						);
					} else {
						$available_date = array(
							'l' => dey_get_order_delivery_booked_label(),
							't' => 'fb',
						);
					}
					// Special day.
			...
			/**
			 * This hook is used to alter the order delivery available dates.
			 *
			 * @since 1.0
			 */
			self::$available_dates = apply_filters( 'dey_order_delivery_available_dates', self::$available_dates );
		...
		private static function prepare_booked_dates() {
			...
			$post_query->select( 'COUNT(p.ID) AS count, pm.meta_value AS date' )
				...
				->leftJoin( $wpdb->prefix . 'postmeta', 'pm2', 'p.ID = pm2.post_id' )
				->where( 'p.post_type', DEY_Register_Post_Types::ORDER_DELIVERY_POSTTYPE )
				->whereIn( 'p.post_status', dey_get_order_delivery_statuses() )
				->where( 'pm.meta_key', 'dey_delivery_date' )
				->groupBy( 'pm.meta_value' )
				->where( 'pm1.meta_key', 'dey_delivery_mode' )
				->whereNot( 'pm1.meta_value', '2' )
				->where( 'pm2.meta_key', 'dey_scheduler_rule_id' )
				->where( 'pm2.meta_value', $scheduler_rule->get_id() );

			$delivery_ids = $post_query->fetchArray();
			if ( ! dey_check_is_array( $delivery_ids ) ) {
				return self::$booked_dates;
			}

			foreach ( $delivery_ids as $key => $delivery_data ) {
				self::$booked_dates[ DEY_Date_Time::get_date_time_object( $delivery_data['date'] )->format( 'Y-m-d' ) ] = intval( $delivery_data['count'] );
			}

			/**
			 * This hook is used to alter the order delivery booked dates.
			 *
			 * @since 1.0
			 */
			self::$booked_dates = apply_filters( 'dey_order_delivery_booked_dates', self::$booked_dates );
		...
		private static function prepare_holidays() {
			...
			self::$holidays = array();
			if ( '1' == self::$scheduler_rule->get_holidays_mode() ) { // Global level.
				$args = array(
				...
				foreach ( $holidays as $key => $holiday ) {
					...
					if ( isset( $holiday['schedule_type'] ) && '2' === $holiday['schedule_type'] ) {
						continue;
					}
			...
			/**
			 * This hook is used to alter the order delivery holiday dates.
			 *
			 * @since 1.0
			 */
			self::$holidays = apply_filters( 'dey_order_delivery_holiday_dates', self::$holidays );
		...
		private static function prepare_special_dates() {
			...
			if ( '2' === self::$scheduler_rule->get_special_days_mode() ) { // Rule level.
				...
				foreach ( $special_dates as $key => $special_day ) {
					...
					if ( isset( $special_day['schedule_type'] ) && '2' === $special_day['schedule_type'] ) {
						continue;
					}
			...
			/**
			 * This hook is used to alter the order delivery special dates.
			 *
			 * @since 1.0.0
			 */
			self::$special_dates = apply_filters( 'dey_order_delivery_special_dates', self::$special_dates );
