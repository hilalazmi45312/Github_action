<?php
/**
 * Handles the Order Local Pickup - Pickup location.
 * 
 * @since 4.0.0
 * */
...
if ( ! class_exists( 'DEY_Pickup_Location_Order_Local_Pickup_Handler' ) ) {

	/**
	 * Class.
	 * */
	class DEY_Pickup_Location_Order_Local_Pickup_Handler {
		 ...
		 * @var object
		 */
		protected static $pickup_location;

		/**
		 * Constructor.
		 *
		 * @since 4.0.0
		 * @param object $pickup_location Selected pickup location object.
		 */
		public function __construct( $pickup_location ) {
			self::$pickup_location = is_object( $pickup_location ) ? $pickup_location : dey_get_pickup_location( $pickup_location );
		}

		/**
		 * Get the pickup selected pickup location.
		 *
		 * @since 4.0.0
		 * @return object $pickup_location
		 */
		...
		private static function render_fields() {
			...
			$fields['dey_pickup_location'] = array(
				...
				'required'    => 'yes' === self::$pickup_location->get_pickup_location_required(),
			...
			$formatted_datetime = '2' === self::$pickup_location->get_pickup_time_mode() ? DEY_Date_Time::get_wp_format_datetime( $selected_date, 'date' ) . ' ' . DEY_Date_Time::get_wp_format_datetime( $selected_date, 'H:i' ) : DEY_Date_Time::get_wp_format_datetime( $selected_date, 'date' );
			...
			$fields['dey_local_pickup_date'] = array(
				...
				'required'    => 'yes' === self::$pickup_location->get_pickup_calender_required(),
				'class'       => array( 'form-row-wide', 'dey-form-row' ),
				'input_class' => array( 'dey-order-local-pickup-date-field' ),
				'default'     => ! empty( $selected_date ) ? $formatted_datetime : '',
			);

			switch ( self::$pickup_location->get_pickup_time_mode() ) {
				case '3': // Time slots.
					...
					$fields['dey_order_local_pickup_date_time_slots'] = array(
						...
						'required'    => 'yes' === self::$pickup_location->get_pickup_time_slot_required(),
						'class'       => array( 'form-row-wide', 'dey-form-row' ),
		...
		private static function enqueue_assets() {
			...
			DEY_Frontend_Assets::set_additional_localize_scripts( 'dey-order-local-pickup', 'dey_order_local_pickup_params', self::get_script_data() );
		}
		...
		public static function get_script_data() {
			return array(
				...
				'first_time_slot'  => self::$pickup_location->get_pickup_first_available_time_slot(),
		...
		public static function get_available_times() {
			...
			$time_from = self::$pickup_location->get_pickup_available_time_from();
			if ( $time_from ) {
			...
			$time_to = self::$pickup_location->get_pickup_available_time_to();
			if ( $time_to ) {
		...
		public static function get_shipping_date_object() {
			...
			$cutoff_time          = self::$pickup_location->get_pickup_same_day_cutoff_time();
			$cutoff_time          = ( empty( $cutoff_time ) ) ? '23:59' : $cutoff_time;
			$next_day_cutoff_time = self::$pickup_location->get_pickup_next_day_cutoff_time();
			...
			$processing_time = self::$pickup_location->get_pickup_processing_time();
			...
			$pickup_days = array_filter( (array) self::$pickup_location->get_pickup_days() );
		...
		public static function get_weekday_price( $date_object ) {
			$price           = 0;
			$weekdays_prices = array_filter( (array) self::$pickup_location->get_pickup_weekdays_prices() );
		...
		public static function get_same_day_price( $date_object ) {
			$same_day_fee   = self::$pickup_location->get_pickup_same_day_fee();
		...
		public static function get_next_day_price( $date_object ) {
			$next_day_fee   = self::$pickup_location->get_pickup_next_day_fee();
			$next_day_price = ( $date_object->format( 'Y-m-d' ) === DEY_Date_Time::get_date_time_object( 'now' )->modify( '+1 days' )->format( 'Y-m-d' ) ) ? self::$pickup_location->get_pickup_next_day_fee() : 0;
		...
		private static function get_time_slot_options() {
			...
			$time_slot_option_labels = self::$pickup_location->get_order_local_pickup_time_slot_option_labels( $selected_date );
		...
		private static function prepare_available_dates() {
			...
			$i           = 0;
			$max_date    = intval( self::$pickup_location->get_pickup_days_availability() );
			$pickup_days = array_filter( (array) self::$pickup_location->get_pickup_days() );
			$date_object = self::get_shipping_date_object();

			do {
				$per_day = self::$pickup_location->get_pickup_max_per_day();
		...
		private static function prepare_holidays() {
			...
			self::$holidays = array();
			if ( '2' === self::$pickup_location->get_holidays_mode() ) { // Rule level.
				$holidays = self::$pickup_location->get_holidays();
		...
		private static function prepare_special_dates() {
			...
			if ( '2' === self::$pickup_location->get_special_days_mode() ) { // Rule level.
				$special_dates = self::$pickup_location->get_special_days();
