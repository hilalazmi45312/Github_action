<?php
/**
 * Customer - Today Order Pickup With Slot.
 *
 * @since 1.0.0
 * */
...
if ( ! class_exists( 'DEY_Customer_Today_Order_Pickup_With_Slot_Notification' ) ) {
	 ...
	 * @since 1.0.0
	 * */
	class DEY_Customer_Today_Order_Pickup_With_Slot_Notification extends DEY_Notifications {
		...
		public function __construct() {
			$this->id          = 'customer_today_order_pickup_with_slot';
			$this->type        = 'customer';
			$this->title       = __( 'Customer Pickup Notification - Order Pickup With Time Slots', 'delivery-slots-for-woocommerce' );
		...
		public function trigger_automatic_email( $order_local_pickup ) {
			...
			$this->trigger( $order_local_pickup );
		}
		...
		public function trigger( $order_local_pickup ) {
			...
			$this->placeholders['{time_slot}']     = $order_local_pickup->get_formatted_time_slots();
			$this->placeholders['{customer_name}'] = $order_local_pickup->get_user_name();
		...
		public function get_settings_array() {
			...
			$section_fields[] = array(
				'type'  => 'title',
				'title' => __( 'Order Pickup With Time Slots', 'delivery-slots-for-woocommerce' ),
				'id'    => 'dey_today_order_local_pickup_with_slot_email_options',
			);
			$section_fields[] = array(
