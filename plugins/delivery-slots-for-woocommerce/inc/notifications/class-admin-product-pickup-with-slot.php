<?php
/**
 * Location Admin - Product Pickup With Slot.
 *
 * @since 3.5.0
 * */
...
if ( ! class_exists( 'DEY_Admin_Product_Pickup_With_Slot_Notification' ) ) {
	 ...
	 * @since 3.5.0
	 * */
	class DEY_Admin_Product_Pickup_With_Slot_Notification extends DEY_Notifications {
		...
		public function __construct() {
			$this->id          = 'admin_product_pickup_with_slot';
			$this->type        = 'admin';
			$this->title       = __( 'Location Admin Product Pickup Notification - With Time Slots', 'delivery-slots-for-woocommerce' );
			$this->description = __( 'This email will be sent to location admin on the day of pickup.', 'delivery-slots-for-woocommerce' );
		...
		public function get_default_message() {
			...
			The Product {product_link} associated Order {order_id} on {site_name} should be pickup on({pickup_date}) between {time_slot} by {customer_name}.
		...
		public function trigger( $product_pickup ) {
			...
			$this->placeholders['{time_slot}']     = $product_pickup->get_formatted_time_slots();
		...
		public function get_settings_array() {
			...
			$section_fields[] = array(
				'type'  => 'title',
				'title' => __( 'Product Pickup With Time Slots', 'delivery-slots-for-woocommerce' ),
				'id'    => 'dey_product_pickup_with_slot_email_options',
			);
			$section_fields[] = array(
