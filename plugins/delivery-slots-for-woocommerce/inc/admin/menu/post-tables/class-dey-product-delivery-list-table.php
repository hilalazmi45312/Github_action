...
if ( ! class_exists( 'DEY_Product_Delivery_List_Table' ) ) {

	/**
	 * DEY_Product_Delivery_List_Table Class.
	 * */
	class DEY_Product_Delivery_List_Table extends DEY_Post_List_Table {
		...
		protected function init_extra_hooks() {
			// Mark as delivered.
			add_action( 'post_action_delivered', array( $this, 'update_delivered_status' ) );
			// Send manual email.
		...
		public function define_columns( $columns ) {
			...
			$columns['delivery_date']    = __( 'Delivery Date', 'delivery-slots-for-woocommerce' );
			...
			$columns['delivery_charge']  = __( 'Delivery Fee', 'delivery-slots-for-woocommerce' );
		...
		public function define_sortable_columns( $columns ) {
			$custom_columns = array(
				...
				'delivery_charge' => array( 'delivery_charge', true ),
		...
		public function define_bulk_actions( $actions ) {
			...
			$actions['delivered'] = __( 'Mark as Delivered', 'delivery-slots-for-woocommerce' );
		...
		public function extra_tablenav( $which ) {
			...
			$export_url = add_query_arg(
				array(
					...
					'dey_export_csv' => 'product_delivery',
				),
				admin_url( 'edit.php' )
			);
			?>
			...
			<a href="#" data-type="product_delivery" class="dey-print-data button button-primary"><?php esc_html_e( 'Print', 'delivery-slots-for-woocommerce' ); ?></a>
			<?php
		}
		...
		public function bulk_post_updated_messages( $bulk_messages, $bulk_counts ) {
			...
			$bulk_messages[ $this->post_type ] = array(
				/* translators: %s: product delivery count */
				'updated' => _n( '%s product delivery updated.', '%s product deliveries updated.', $bulk_counts['updated'], 'delivery-slots-for-woocommerce' ),
				/* translators: %s: product delivery count */
				'locked'  => _n( '%s product delivery not updated, somebody is editing it.', '%s product deliveries not updated, somebody is editing them.', $bulk_counts['locked'], 'delivery-slots-for-woocommerce' ),
				/* translators: %s: product delivery count */
				'deleted' => _n( '%s product delivery permanently deleted.', '%s product deliveries permanently deleted.', $bulk_counts['deleted'], 'delivery-slots-for-woocommerce' ),
			);
		 ...
		 * @return void
		 */
		public function update_delivered_status( $post_id ) {
			check_admin_referer( 'delivered-post_' . $post_id );
			...
			if ( ! $product_delivery->exists() || ! $product_delivery->has_status( 'dey_upcoming' ) ) {
				return;
			}

			dey_update_product_delivery( $post_id, array(), array( 'post_status' => 'dey_delivered' ) );

			$sendback = add_query_arg( 'post_type', $this->post_type, admin_url( 'edit.php' ) );

			wp_safe_redirect( add_query_arg( 'delivered', 1, $sendback ) );
			exit();
		}
		...
		protected function prepare_row_data( $post_id ) {
			if ( empty( $this->object ) || $this->object->get_id() !== $post_id ) {
				$this->object = dey_get_product_delivery( $post_id );
			}
		}
		...
		/**
		 * Render the delivery date column.
		 *
		 * @return void
		 */
		public function render_delivery_date_column() {

			echo wp_kses_post( $this->object->get_formatted_delivery_date() );
		}
		...
		public function render_mode_column() {
			...
			echo esc_html( dey_display_delivery_mode( $this->object->get_delivery_mode() ) );
		}
		...
		/**
		 * Render the delivery charge column.
		 *
		 * @since 1.0.0
		 * @return void
		 */
		public function render_delivery_charge_column() {
			echo wp_kses_post( $this->object->get_formatted_delivery_charge() );
		}
		...
		public function render_actions_column() {
			...
			if ( $this->object->has_status( 'dey_upcoming' ) ) {
				...
				$actions   = sprintf( '<a class="dey-delivered-post button" href=%s>%s</a>', wp_nonce_url( $link, 'delivered-post_' . $this->object->get_id() ), __( 'Mark as Delivered', 'delivery-slots-for-woocommerce' ) );
		...
		protected function query_filters( $query_vars ) {
			if ( isset( $_GET['dey_delivery_day_filter'] ) && ! empty( $_GET['dey_delivery_day_filter'] ) ) {
				...
				$query_vars['meta_query'] = dey_get_delivery_meta_query_args( $day_filter, $from_date, $to_date );
			}
		...
		protected function get_search_post_ids( $terms ) {
			...
			foreach ( $terms as $term ) {
				...
				$post_query->select( 'DISTINCT `p`.ID' )
						...
						->where( '`p`.post_type', DEY_Register_Post_Types::PRODUCT_DELIVERY_POSTTYPE )
						->whereIn( '`p`.post_status', dey_get_product_delivery_statuses() )
		...
		public function send_manual_email( $post_id ) {
			...
			/**
			 * This hook is used to do extra product delivery manual email.
			 *
			 * @since 2.6
			 */
			do_action( 'dey_product_delivery_manual_email', $product_delivery );
		...
		protected function orderby_filters( $query_vars ) {
			...
			switch ( strtolower( $query_vars['orderby'] ) ) {
				...
				case 'delivery_charge':
					$query_vars['meta_key'] = 'dey_delivery_charge';
					$query_vars['orderby']  = 'meta_value_num';
					break;
				case 'mode':
					$query_vars['meta_key'] = 'dey_delivery_mode';
					$query_vars['orderby']  = 'meta_value_num';
					break;
				...
				case 'delivery_date':
					$query_vars['meta_key']  = 'dey_delivery_date';
