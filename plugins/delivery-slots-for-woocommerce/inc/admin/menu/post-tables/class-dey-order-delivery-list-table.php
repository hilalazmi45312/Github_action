...
if ( ! class_exists( 'DEY_Order_Delivery_List_Table' ) ) {

	/**
	 * DEY_Order_Delivery_List_Table Class.
	 * */
	class DEY_Order_Delivery_List_Table extends DEY_Post_List_Table {
		...
		public function extra_tablenav( $which ) {
			...
			<a href="#" data-type="order_delivery" class="dey-print-data button button-primary"><?php esc_html_e( 'Print', 'delivery-slots-for-woocommerce' ); ?></a>
			<?php
		}
		...
		public function bulk_post_updated_messages( $bulk_messages, $bulk_counts ) {
			...
			$bulk_messages[ $this->post_type ] = array(
				/* translators: %s: order delivery count */
				'updated' => _n( '%s order delivery updated.', '%s order deliveries updated.', $bulk_counts['updated'], 'delivery-slots-for-woocommerce' ),
				/* translators: %s: order delivery count */
				'locked'  => _n( '%s order delivery not updated, somebody is editing it.', '%s order deliveries not updated, somebody is editing them.', $bulk_counts['locked'], 'delivery-slots-for-woocommerce' ),
				/* translators: %s: order delivery count */
				'deleted' => _n( '%s order delivery permanently deleted.', '%s order deliveries permanently deleted.', $bulk_counts['deleted'], 'delivery-slots-for-woocommerce' ),
			);
		...
		public function update_delivered_status( $post_id ) {
			...
			dey_update_order_delivery( $post_id, array(), array( 'post_status' => 'dey_delivered' ) );
		...
		protected function prepare_row_data( $post_id ) {
			if ( empty( $this->object ) || $this->object->get_id() !== $post_id ) {
				$this->object = dey_get_order_delivery( $post_id );
			}
		}
		...
		public function send_manual_email( $post_id ) {
			...
			/**
			 * This hook is used to do extra order delivery manual email.
			 *
			 * @since 2.6
			 */
			do_action( 'dey_order_delivery_manual_email', $order_delivery );
