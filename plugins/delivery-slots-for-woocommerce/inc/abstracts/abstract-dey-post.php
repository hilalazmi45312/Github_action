...
if ( ! class_exists( 'DEY_Post' ) ) {

	/**
	 * DEY_Post Class.
	 * */
	abstract class DEY_Post {
		 ...
		 * @var string
		 * */
		protected $duplicate_status = 'dey_inactive' ;
		...
		protected function load_metadata() {
			...
			if ( ! dey_check_is_array( $meta_data_array ) ) {
				return ;
			}
		...
		public function duplicate() {
			if ( ! dey_check_is_array( $this->duplicate_meta_keys ) ) {
				return false;
			}

			$meta_data = array();
			foreach ( $this->duplicate_meta_keys as $key => $value ) {
			...
			$post_data = array(
				'post_title'   => $this->post->post_title . '- Duplicate',
