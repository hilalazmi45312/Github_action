jQuery(document).ready(function ($) {
    ...
    $('.jagif-override-type-wrap select').on('change', function () {
        jagifOverride($(this).val());
    });

    function jagifOverride(val) {
        let override_wrap = $('.jagif-override-type-wrap'),
            description_wrap = $(override_wrap).find('.description');
        $.each(description_wrap, function (index, value) {
            if ($(value).hasClass('des-override-' + val)) {
                $(value).removeClass('jagif-hidden');
            } else {
                $(value).addClass('jagif-hidden');
            }
        })
    }
