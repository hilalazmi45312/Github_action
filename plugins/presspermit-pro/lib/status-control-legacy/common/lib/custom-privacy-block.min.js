...
var sideEffectL10nPrivacyManipulation = function sideEffectL10nPrivacyManipulation(status) {
  setTimeout(function () {
    ...
    if (privacyLabel) {
      ...
      if (!$('.editor-post-publish-panel__header-cancel-button button:visible').length) {
        ...
        if (status === 'publish') {
          ...
          if (privacyLabel) {
            ...
            if ($('#pp_save_draft_label').filter(':visible').length) {
              ...
              privacyLabel = rvy__('Save %s').replace('%s', privacyLabel);

              var buttonWidth = 18 + parseInt(privacyLabel.length) * 6.5;
              ...
              $('#pp_save_draft_label').html(privacyLabel).css('left', spanPosLeft + 'px');
            }
          }
        }
...
var PPCustomPostPrivacyInfo = function PPCustomPostPrivacyInfo(_ref) {
  ...
  if (PPCustomPrivacy.customPrivacyEnabled) {
    ...
    }, forceDefault ? rvy__('Visibility is locked by default settings.', 'presspermit-pro') : force ? rvy__('Visibility is locked by parent page setting.','presspermit-pro'): getPrivacyLabel(status) && -1 == ['draft'].indexOf(status) ? '' : rvy__('To select a custom visibility, first privately publish the post.', 'presspermit-pro')));
  }
};
