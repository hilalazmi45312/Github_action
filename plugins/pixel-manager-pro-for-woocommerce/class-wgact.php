...
class WCPM {
	...
	public function __construct() {
		...
		if (Environment::is_woocommerce_active()) {
			...
			add_action('init', [ $this, 'register_generic_hooks' ], 10, 2);
	...
	public function run_woocommerce_reports() {
		...
		if (
			Environment::is_action_scheduler_active()
			&& Environment::is_transients_enabled()
		) {
...
//			return false;
	...
	public function requirements_error() {
		...
		<div class="error">
			<p>
				<strong>
					<?php esc_html_e('Pixel Manager for WooCommerce error', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
				</strong>:
			...
			<ul class="ul-disc">
				<li><?php esc_html_e('The WooCommerce plugin needs to be activated', 'woocommerce-google-adwords-conversion-tracking-tag'); ?>
					:
	...
	public function init() {
		...
		if (is_admin()) {
			...
			if (
				...
				&& Options::is_shop_order_list_info_enabled()
			) {
	...
	private static function is_admin_trial_promo_active() {
		...
		$admin_trial_promo_active = apply_filters_deprecated('wooptpm_show_admin_trial_promo', [ true ], '1.13.0', 'pmw_show_admin_trial_promo');
		$admin_trial_promo_active = apply_filters_deprecated('wpm_show_admin_trial_promo', [ $admin_trial_promo_active ], '1.31.2', 'pmw_show_admin_trial_promo');
	...
	private static function is_admin_notifications_active() {
		...
		$admin_notifications_active = apply_filters_deprecated('wooptpm_show_admin_notifications', [ true ], '1.13.0', 'pmw_show_admin_notifications');
		$admin_notifications_active = apply_filters_deprecated('wpm_show_admin_notifications', [ $admin_notifications_active ], '1.31.2', 'pmw_show_admin_notifications');
