...
/**
 * TikTok Events API adapter for server-side event processing
 *
 * Handles filtering and sending events to TikTok Events API.
 * Also implements Pixel_Descriptor for unified pixel registry.
 *
 * @package SweetCode\Pixel_Manager\Pixels\Adapters\TikTok
 * @since 1.51.0
 */
class TikTok_Adapter extends Abstract_Pixel_Adapter implements Pixel_Descriptor {
	...
	public function is_active() {
		return \SweetCode\Pixel_Manager\Options::is_tiktok_active();
	}
	...
	/**
	 * Check if TikTok EAPI is available
	 *
	 * @return bool
	 */
	public function is_available() {
		return class_exists( 'SweetCode\Pixel_Manager\Pixels\TikTok\TikTok_EAPI' );
	}

	/**
	 * Send event data to TikTok Events API
	 *
	 * @param array $pixel_data Filtered event data ready to send
	 * @return void
	 */
	protected function send( $pixel_data ) {
		TikTok_EAPI::send_s2s_event( $pixel_data );
	}
}
