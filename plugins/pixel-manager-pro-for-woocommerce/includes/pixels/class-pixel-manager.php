...
class Pixel_Manager {
	...
	private function __construct() {
		...
		if (wpm_fs()->can_use_premium_code__premium_only()) {
			add_action('wp_ajax_pmw_server_to_server_event', [ $this, 'capture_ajax_server_to_server_event__premium_only' ]);
			add_action('wp_ajax_nopriv_pmw_server_to_server_event', [ $this, 'capture_ajax_server_to_server_event__premium_only' ]);

			add_action('wp_ajax_pmw_process_transient_session_identifiers', [ $this, 'pmw_process_transient_session_identifiers__premium_only' ]);
			add_action('wp_ajax_nopriv_pmw_process_transient_session_identifiers', [ $this, 'pmw_process_transient_session_identifiers__premium_only' ]);
		}
...
//				}
		...
		if (wpm_fs()->can_use_premium_code__premium_only()) {
			...
			register_rest_route($this->rest_namespace, '/ga4/data-api/credentials/', [
					...
					'permission_callback' => [ __CLASS__, 'protected_rest_route_permission_callback' ],
			]);
	...
	public static function protected_rest_route_permission_callback() {
		...
		return new WP_Error('rest_forbidden', esc_html__('You can not view private data.', 'woocommerce-google-adwords-conversion-tracking-tag'), [ 'status' => 401 ]);
	}
...
//			$order_id = $order->get_id();
	...
	public function pmw_process_transient_session_identifiers__premium_only() {
		...
		Logger::debug('pmw_process_transient_session_identifiers');
