...
if ( ! class_exists( 'WC_SC_Ajax' ) ) {
	...
	class WC_SC_Ajax {
		...
		private function __construct() {
			...
			add_action( 'wp_ajax_sc_json_search_store_credit_coupons', array( $this, 'sc_json_search_store_credit_coupons' ) );
						...
						switch ( $post['meta_key'] ) {
							...
							case 'customer_email':
								if ( is_serialized( $post['meta_value'] ) ) {
									// nosemgrep: audit.php.lang.security.object-injection .
									$meta_value                                    = unserialize( $post['meta_value'], array( 'allowed_classes' => false ) ); // phpcs:ignore
									$coupon_posts[ $post_id ][ $post['meta_key'] ] = is_array( $meta_value ) ? $meta_value : ''; // phpcs:ignore
								} else {
									$coupon_posts[ $post_id ][ $post['meta_key'] ] = ''; // phpcs:ignore
								}
								break;
						}
					}
		...
		/**
		 * Function to search store credit coupons only
		 *
		 * @param string $x Search term.
		 * @param array  $post_types Post types.
		 */
		public function sc_json_search_store_credit_coupons( $x = '', $post_types = array( 'shop_coupon' ) ) {
			try {
				global $wpdb;
