...
if ( ! class_exists( 'WC_SC_Purchase_Credit' ) ) {
	...
	class WC_SC_Purchase_Credit {
				...
				if ( is_admin() || wp_doing_ajax() || wp_doing_cron() || wp_is_rest_endpoint() || defined( 'REST_REQUEST' ) ) {
					if ( function_exists( 'get_current_screen' ) ) {
						...
						if ( ! empty( $screen->post_type ) && 'product' === $screen->post_type ) {
							return $price; // No need to run the following code when working with products.
						}
					}
						...
						jQuery('form.woocommerce-checkout').on('change', 'input#wc_sc_schedule_gift_sending', function(){
							if ( jQuery(this).is(':checked') && false === jQuery('.gift_sending_date_time').hasClass('hasDatepicker') ) {
								setTimeout(function(){
									...
									.datetimepicker({ 
														dateFormat: '<?php echo esc_html( $this->php_to_jquery_date_or_time_format( get_option( 'date_format' ) ) ); ?>', 
														minDate: 0,
														inline: true,
														changeMonth: true, 
														changeYear: true,
														...
														timeFormat: '<?php echo esc_html( $this->php_to_jquery_date_or_time_format( get_option( 'time_format' ) ) ); ?>',
														timeSuffix: '',
		 ...
		 * @param array|WC_Order_Item $values Associative array containing item's details.
		 */
		...
		public function call_for_credit_cart_item_data( $cart_item_data = array(), $product_id = '', $variation_id = '' ) {
			try {
				...
				if ( ! empty( $coupons ) && $this->is_coupon_amount_pick_from_product_price( $coupons ) && ! ( $_product->get_price() > 0 ) ) {
					...
					$request_add_to_cart             = ! empty( $request_product_id ) ? $request_product_id : $request_add_to_cart;
							...
							jQuery.ajax({
								...
								success: function( response ) {
									if ( 'yes' !== response.success ) {
										console.log(
											'<?php echo esc_js( _x( 'Failed to update coupon receiver details in session.', 'Error message when AJAX fails to update coupon receiver details', 'woocommerce-smart-coupons' ) ); ?>'
										);
									}
								}
							});
		...
		public function maybe_required_coupon_delivery_date_time( $fields = array(), $errors = object ) {
			try {
				...
				if ( ! empty( $request_gift_sending_date_time ) ) {
					$gift_sending_timestamp = $this->convert_gift_sending_date_time_to_timestamps( $request_gift_sending_date_time );
				}
		...
		public function set_coupon_receiver_details_session( $posted_data = array() ) {
			try {
				if ( is_callable( 'WC' ) && is_object( WC() ) && is_object( WC()->session ) && is_callable( array( WC()->session, 'set' ) ) ) {
					...
					if ( isset( $coupon_receiver_details['gift_sending_date_time'] ) ) {
						$coupon_receiver_details['gift_sending_timestamp'] = $this->convert_gift_sending_date_time_to_timestamps( $coupon_receiver_details['gift_sending_date_time'] );
					}
					...
					WC()->session->set( 'wc_sc_coupon_receiver_details_session', ! empty( $coupon_receiver_details ) ? $coupon_receiver_details : null );
				}
			} catch ( \Throwable $e ) {
				$this->sc_block_catch_error( $e );
			}
		...
		public function ajax_save_coupon_receiver_details() {
			try {
				...
				$this->sc_block_catch_error( $e );
				wp_send_json( array( 'message' => __( 'Failed to save coupon receiver details.', 'woocommerce-smart-coupons' ) ) );
			}
		...
		/**
		 * Convert WordPress date/time format to jQuery Timepicker format.
		 *
		 * @param string $wp_format WordPress date or time format.
		 * @return string string jQuery Timepicker compatible format.
		 */
		public static function php_to_jquery_date_or_time_format( $wp_format ) {
			$replacements = array(
				// Date.
				'd' => 'dd', // Day with leading zero (01–31).
				'j' => 'd',  // Day without leading zero (1–31).
				'D' => 'D',  // Short weekday (Mon–Sun).
				'l' => 'DD', // Full weekday (Monday–Sunday).
				'm' => 'mm', // Month with leading zero (01–12).
				'n' => 'm',  // Month without leading zero (1–12).
				'M' => 'M',  // Short month (Jan–Dec).
				'F' => 'MM', // Full month (January–December).
				'Y' => 'yy', // Full year (e.g., 2025).
				'y' => 'y',  // Short year (e.g., 25).
			...
			return strtr( $wp_format, $replacements );
		}
	}
