...
if ( ! class_exists( 'WC_SC_Coupon_Import' ) ) {
	...
	class WC_SC_Coupon_Import extends WP_Importer {
		...
		public function process_bulk_generate_action() {
			try {
				global $woocommerce_smart_coupon, $wp_filesystem;
							...
							if ( function_exists( 'mime_content_type' ) ) {
								...
								if ( in_array( $file_mime_type, $allowed_mime_types, true ) ) {
									...
									if ( $wp_filesystem->exists( $real_full_path ) && $wp_filesystem->is_readable( $real_full_path ) ) {
										$file_contents = $wp_filesystem->get_contents( $real_full_path );
		...
		public function process_coupon( $post ) {
			try {
				...
				if ( 0 === $post_author ) {
					$post_author = get_option( 'wc_sc_import_coupons_author' );
				}
							...
							if ( true === $is_import_meta ) {
								if ( $this->is_wc_gte_30() ) {
									if ( 'expiry_date' === $meta_key ) {
										...
										if ( true === $has_setter ) {
											if ( in_array( $key, array( 'individual_use', 'free_shipping', 'exclude_sale_items', 'apply_before_tax' ), true ) ) {
		...
		public function handle_upload() {
			...
			$importing_user_id = get_option( 'wc_sc_import_coupons_author', false );

			// Only proceed if another user is importing.
			if ( $importing_user_id && $importing_user_id !== $current_user_id ) {
				...
				switch ( $bulk_action ) {
					...
					case 'import':
						$bulk_text = esc_html__( 'Import process is already in progress. You’ll be able to import again only after it is completed.', 'woocommerce-smart-coupons' );
						break;

					case 'generate':
					default:
						$bulk_text = esc_html__( 'Coupon generation is already in progress. You’ll be able to generate again only after it is completed.', 'woocommerce-smart-coupons' );
						break;

				}

				printf( '<p><strong>%s</strong></p>', esc_html( $bulk_text ) );
		...
		public function format_data_from_csv( $data, $enc ) {
			return ( 'UTF-8' === $enc ) ? $data : mb_convert_encoding( $data, 'UTF-8', mb_detect_encoding( $data ) );
		}
