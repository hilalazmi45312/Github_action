...
if ( ! class_exists( 'WC_SC_Order_Fields' ) ) {
	...
	class WC_SC_Order_Fields {
		...
		private function __construct() {
			...
			add_action( 'woocommerce_email_order_details', array( $this, 'sc_flag_admin_email_for_discount_adjustment' ), 5, 4 );
			add_filter( 'woocommerce_get_order_item_totals', array( $this, 'add_smart_coupons_discount_details' ), 10, 3 );
			add_action( 'woocommerce_email_after_order_table', array( $this, 'sc_cleanup_admin_email_flag' ) );
		...
		public function admin_order_totals_add_smart_coupons_discount_details( $order_id = 0 ) {
			try {
				...
				// Check if the order is a subscription and return early.
		...
		/**
		 * Flags that the current email is an admin new order email.
		 *
		 * @param WC_Order $order Order object.
		 * @param bool     $sent_to_admin Whether it's sent to admin.
		 * @param bool     $plain_text Whether it's plain text.
		 * @param WC_Email $email Email object.
		 */
		public function sc_flag_admin_email_for_discount_adjustment( $order, $sent_to_admin, $plain_text, $email ) {
			if ( $email instanceof WC_Email_New_Order ) {
				global $is_smart_coupon_admin_email;
				$is_smart_coupon_admin_email = true;
			}
		}
		...
		public function add_smart_coupons_discount_details( $total_rows = array(), $order = null, $tax_display = '' ) {
			try {
				global $store_credit_label, $woocommerce_smart_coupon, $is_smart_coupon_admin_email;
				...
				// Calculate actual coupon-only discount.
				$full_discount   = (float) $order->get_discount_total();
				$coupon_discount = $full_discount - ( (float) $total_credit_used - $credit_used['discount_tax'] );

				// Modify discount row for admin emails only.
				if ( isset( $total_rows['discount'] ) ) {
					if ( $coupon_discount > 0 ) {
						if ( ! empty( $is_smart_coupon_admin_email ) ) {
							$total_rows['discount']['value'] = '-' . wc_price( $coupon_discount );
						}
					} else {
		...
		/**
		 * Unsets the global flag after email is sent to avoid affecting other logic.
		 */
		public function sc_cleanup_admin_email_flag() {
			global $is_smart_coupon_admin_email;
			unset( $is_smart_coupon_admin_email );
		}
		...
		public function smart_coupons_order_amount_total_discount( $total_discount, $order = null ) {
			try {
				...
				$order_actions = array( 'woocommerce_add_coupon_discount', 'woocommerce_save_order_items', 'woocommerce_add_order_item', 'woocommerce_calc_line_taxes' );
				$post_action = ( ! empty( $_POST['action'] ) ) ? wc_clean( wp_unslash( $_POST['action'] ) ) : ''; // phpcs:ignore
				// Check if the current context is NOT Order Edit Page, Thank You Page, or Email Template.
				if (
					! ( is_admin() && isset( $_GET['action'] ) && 'edit' === wc_clean( wp_unslash( $_GET['action'] ) ) ) && //phpcs:ignore
					// Order Edit Page in admin.
					! ( ! is_admin() && is_wc_endpoint_url( 'order-received' ) ) && // Thank You Page on the frontend.
					! ( wp_doing_ajax() && in_array( $post_action, $order_actions, true ) ) && // Allow AJAX requests when order edit page hit an action.
					did_action( 'woocommerce_email_order_meta' ) === 0 // Email context not triggered.
				) {
					...
					if ( is_admin() || wp_doing_ajax() || wp_doing_cron() || wp_is_rest_endpoint() || defined( 'REST_REQUEST' ) ) {
						if ( function_exists( 'get_current_screen' ) ) {
							...
							if ( ! empty( $screen->post_type ) && 'product' === $screen->post_type ) {
								return $id; // No need to run the following code when working with products.
							}
						}
