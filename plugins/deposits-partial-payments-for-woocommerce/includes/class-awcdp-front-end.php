...
class AWCDP_Front_End
{
    ...
    function __construct($file = '', $version = '1.0.0') {
        ...
        if ($this->check_woocommerce_active()) {
            ...
            add_filter('woocommerce_add_cart_item_data', array($this, 'awcdp_add_cart_item_data'), 20, 3);
            ...
            add_filter('woocommerce_payment_complete_order_status', array($this, 'awcdp_payment_complete_order_status'), 20, 2);
  ...
  function awcdp_get_deposit_type($product_id){
    ...
    // return $type;
    return apply_filters('awcdp_product_deposit_type', $type, $product_id);
  ...
  function user_restriction(){
    ...
    function awcdp_add_cart_item_data($cart_item_meta, $product_id, $variation_id){
      ...
      $enabled = isset( $_REQUEST['data']['awcdp_deposit_option'] ) ? (sanitize_text_field( $_REQUEST['data']['awcdp_deposit_option'] )) : $enabled;
            ...
            if ($parent) {
              ...
              if (!empty($schedule) && is_array($schedule)) {
                foreach ($schedule as $payment) {
                  ...
                  if ( array_key_exists( 'id', $payment ) && $payment['id'] == $order->get_id() ) {  
                    $suffix .= $count;
                    break;
                  }
                }
