jQuery(function ($) {
		...
		$(window).on("elementor/frontend/init", function () {
			if (window.elementorFrontend && window.elementorFrontend.hooks) {
				...
				window.elementorFrontend.hooks.addAction(
					"frontend/element_ready/sh_product_variation.default",
					function ($scope) {
	...
	function initSelectedValueDisplay($scope) {
		...
		var $mainForm = $("form.cart, form.variations_form")
			.not($widget.find("form"))
			.first();
		if (!$mainForm.length) {
		...
		$widget
			.off("click.editor-swatches", ".wd-swatch")
			.on("click.editor-swatches", ".wd-swatch", function (e) {
				e.preventDefault();
		...
		$widget
			.off("change.editor-dropdown", "select[data-attribute_name]")
			.on("change.editor-dropdown", "select[data-attribute_name]", function () {
				var $select = $(this);
	...
	if (window.elementorFrontend && window.elementorFrontend.isEditMode()) {
		...
		$(document).on("elementor/editor/change", function (event, model) {
			if (
				model &&
				model.get &&
				model.get("widgetType") === "sh_product_variation"
			) {
				// Small delay to ensure the widget has re-rendered
				setTimeout(function () {
					var $widget = $(
						".elementor-widget-sh_product_variation .sh-variation-form"
					);
					if ($widget.length) {
						...
						initSelectedValueDisplay(
							$widget.closest(".elementor-widget-sh_product_variation")
						);
					}
				}, 100);
			}
		...
		$(document).on(
			"elementor/panel/open_editor/widget",
		...
		// MutationObserver removed to prevent performance issues in Elementor editor
		// We rely on Elementor's native hooks (frontend/element_ready/widget) instead
	}
