...
(function ($) {
	...
	var ProductExtrasWidget = {
		...
		bindEvents: function () {
			...
			this.cache.$document.on(
				"click.productExtras change.productExtras input.productExtras",
				this.handleDelegatedEvents.bind(this)
			);
		...
		handleClickEvents: function ($target, e) {
			...
			if (
				$target.hasClass("pewc-checkbox-label") ||
				$target.hasClass("pewc-radio-label")
			) {
			...
			if (
				$target.closest(".pewc-checkbox-wrapper").length &&
				!$target.is('input[type="checkbox"], input[type="number"]')
			) {
			...
			if (
				$target.closest(".sh-select-option-card").length &&
		...
		handleChangeEvents: function ($target, e) {
			...
			if (
				...
				$target.closest(".sh-product-extras-container").length
			) {
			...
			if (
				$target.closest(
					".sh-product-extras-container, .sh-product-extras-fields"
				).length
			) {
		...
		initializeFields: function () {
			...
			$(".pewc-checkboxes-list-wrapper .pewc-checkbox-wrapper").each(
				function () {
					var $wrapper = $(this);
			...
			if (form.length > 0 && checkbox.length > 0) {
				...
				if (checkboxName && checkbox.val() !== undefined) {
					...
					if (checkbox.is(":checked")) {
						...
						var hiddenQuantity = form.find(
							'input[type="hidden"][name="' + quantityName + '"]'
						);
						...
						var quantityName = checkboxName.replace("[]", "_quantity[]");
						form
							.find('input[type="hidden"][name="' + quantityName + '"]')
							.remove();
					}
				}
			}
		...
		handleQuantityInputChange: function ($input) {
			...
			this.batchDOMUpdates($card, function () {
				...
				if ($checkbox.is(":checked")) {
					...
					var productType = $checkbox.data("product-type");
					if (productId) {
						ProductExtrasWidget.storeSelectedProduct(
							productId,
							quantity,
							productType,
							$card
			...
			this.batchDOMUpdates($card, function () {
				...
				if (radioName) {
					$('input[name="' + radioName + '"]').each(function () {
						$(this)
							.closest(".sh-select-option-card")
							.removeClass("sh-product-selected");
					});
				}
		...
		handleFieldUpdate: function ($field) {
			...
			if (
				fieldName &&
				this.cache.$cartForm.length > 0 &&
				$field.val() !== undefined
			) {
				var fieldValue = $field.val();
				// Update or create hidden field in the cart form
				var $hiddenField = this.cache.$cartForm.find(
					'input[type="hidden"][name="' + fieldName + '"]'
				);
				if ($hiddenField.length > 0) {
					$hiddenField.val(fieldValue);
		...
		updateCardPrices: function ($card) {
			...
			if (
				isNaN(basePriceOriginal) &&
				isNaN(basePriceSale) &&
				isNaN(basePriceRegular)
			) {
			...
			if (!isNaN(basePriceOriginal) && !isNaN(basePriceSale)) {
				...
				$priceSection
					.find(".sh-price-original")
					.html(this.formatPrice(newOriginalPrice));
				$priceSection
					.find(".sh-price-sale")
					.html(this.formatPrice(newSalePrice));
			} else if (!isNaN(basePriceRegular)) {
				// Update regular pricing
				var newRegularPrice = basePriceRegular * quantity;

				$priceSection
					.find(".sh-price-regular")
					.html(this.formatPrice(newRegularPrice));
			} else {
		...
		resetCardPrices: function ($card) {
			...
			$card.removeData(
				"base-price-original base-price-sale base-price-regular"
			);
		...
		extractPriceFromElement: function ($element) {
			...
			if (priceText.includes(",") && priceText.includes(".")) {
				...
				if (lastPeriod > lastComma) {
					...
					priceText =
						priceText.substring(0, lastComma).replace(/\./g, "") +
						"." +
						priceText.substring(lastComma + 1);
				}
				...
				if (
					commaCount === 1 &&
					priceText.indexOf(",") === priceText.length - 3
				) {
		...
		formatPrice: function (price) {
			...
			return (
				'<span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">RM</span>' +
				formattedPrice +
				"</bdi></span>"
			);
		},
		...
		handleFileUpload: function (e) {
			...
			if (files.length > 0) {
				...
				$feedback.html(
					'<i class="fa fa-check-circle" style="color: green;"></i> File selected: ' +
					fileName
				);
			}
		},
			...
			$(".sh-product-extras-container [data-tooltip]").each(function () {
				...
				$element
					.on("mouseenter", function () {
						var $tooltip = $(
							'<div class="field-tooltip">' + tooltipText + "</div>"
						);
						$("body").append($tooltip);
		...
		areAllVariationsSelected: function ($card) {
			...
			$card.find(".sh-variation-attribute").each(function () {
				...
				var hasActiveSelection =
					$container.find(".wd-swatch.wd-active").length > 0;
		...
		handleVariationChange: function ($card) {
			...
			// Use widgetId_productId key pattern to match storeSelectedProduct
			var widgetId = $card.closest(".sh-product-extras-container").data("widget-id") || "default";
			var key = widgetId + "_" + productId;

			if (window.shSelectedProducts && window.shSelectedProducts[key]) {
				// Check if variation data has actually changed
				var currentVariationData =
					window.shSelectedProducts[key].variationData || {};
				var variationChanged =
					JSON.stringify(currentVariationData) !== JSON.stringify(variations);

				if (variationChanged) {
					window.shSelectedProducts[key].variationData = variations;
					// Also update variationId from card data
					window.shSelectedProducts[key].variationId = $card.data("variation-id") || null;
		...
		resetVariationDisplay: function ($card) {
			...
			if (window.shSelectedProducts && window.shSelectedProducts[key]) {
				window.shSelectedProducts[key].variationData = {};
				window.shSelectedProducts[key].variationId = null;
		...
		findMatchingVariation: function (productId, selectedVariations, $card) {
			...
			const matchingVariation = this.findLocalMatchingVariation(
				variationData,
				selectedVariations
			);
					...
					if (data && Array.isArray(data)) {
						...
						if (selectedVariations) {
							...
							for (const selectedAttr in selectedVariations) {
								const selectedValue =
									selectedVariations[selectedAttr].toLowerCase();
									...
									if (variation.attributes) {
										for (const attrName in variation.attributes) {
											...
											if (attrValue) {
												...
												if (
													attrValueLower === selectedValue ||
													attrValueLower.includes(selectedValue)
												) {
													matchScore++;
													foundMatch = true;
													break;
				...
				for (const selectedAttr in selectedVariations) {
					...
					if (!variationValue) {
						for (const varAttr in variationAttributes) {
							...
							if (
								varValue &&
								varValue.toLowerCase() === selectedValue.toLowerCase()
							) {
		...
		updateVariationData: function ($card, variationData) {
			...
			if (variationData.price_html) {
				...
				var $regularElement = $tempDiv.find(
					"del .woocommerce-Price-amount, del.amount, .price del"
				);
				...
				if (!salePrice) {
					var $normalElement = $tempDiv.find(
				...
				if (salePrice && regularPrice) {
					...
					formattedPriceHtml =
						'<span class="sh-price-sale">' + salePrice + "</span>";
					formattedPriceHtml +=
						'<span class="sh-price-original">' + regularPrice + "</span>";
				} else if (normalPrice) {
					formattedPriceHtml =
						'<span class="sh-price-regular">' + normalPrice + "</span>";
				} else {
			...
			var productId = $card.data("product-id");
			var widgetId =
				$card.closest(".sh-product-extras-container").data("widget-id") ||
				"default";
			var key = widgetId + "_" + productId;

			if (window.shSelectedProducts && window.shSelectedProducts[key]) {
				var quantity = window.shSelectedProducts[key].quantity || 1;
				var productType =
					window.shSelectedProducts[key].productType || "variable";
		...
		updateCardSwatches: function ($card) {
			...
			if (
				...
				!cached.list.length ||
				!variationIndex ||
				!Object.keys(variationIndex).length
			)
				return;
			...
			$card.find(".wd-swatches-product").each(function () {
				...
				const attrName =
					$swatchContainer.data("id") || $swatchContainer.attr("data-id");
				$swatchContainer.find(".wd-swatch").each(function () {
					const $option = $(this);
					const value = $option.data("value") || $option.attr("data-value");
					const candidates =
						variationIndex[attrName] && variationIndex[attrName][value]
							? variationIndex[attrName][value]
							: variationIndex["attribute_" + attrName] &&
								variationIndex["attribute_" + attrName][value]
								? variationIndex["attribute_" + attrName][value]
								: [];
					let totalQty = 0;
					for (let i = 0; i < candidates.length; i++) {
						...
						for (const key in selectedMap) {
							if (!Object.prototype.hasOwnProperty.call(selectedMap, key))
								continue;
					...
					if (totalQty <= 0) {
						$option
							...
							.removeClass("wd-enabled")
							.prop("disabled", true)
							.attr("data-stock-qty", 0);
					} else {
						$option
							.removeClass("wd-disabled")
							.addClass("wd-enabled")
							.prop("disabled", false)
							.attr("data-stock-qty", totalQty);
					}
				});
			});
		},
		...
		initializeVariationAvailability: function () {
			$(".sh-product-extras-container .sh-product-extra-card").each(
				function () {
					var $card = $(this);
					var productId = $card.data("product-id");
					if (productId) {
		...
		updateMainProductPrice: function (variationData) {
			...
			if (!this.originalMainPriceHtml) {
				var $mainPriceElements = $(
					".product-price .price, .woocommerce-Price-amount, .senheng-price-wrapper"
				);
			...
			if ($mainPriceElements.length > 0 && variationData.price_html) {
				...
				$mainPriceElements.each(function () {
					...
					if (salePrice && regularPrice) {
						...
						var $originalPriceRow = $priceElement.siblings(
							".original-price-row"
						);
		...
		handleCartSelection: function (checkbox, isSelected) {
			...
			// Handle regular option checkboxes (fallback for options without specific classes)
				...
				if (productId) {
					...
					var optionLabel =
						card.find(".sh-product-title").text() || "Option " + optionKey;
		...
		handleProductCartSelection: function ($checkbox, isSelected) {
			...
			if (isSelected) {
				...
				this.removeSelectedProduct(productId, card);
			}
		},
		...
		storeSelectedProduct: function (productId, quantity, productType, card) {
			...
			// Ensure productId is treated consistently as a string
			...
			var childDiscount = $checkbox.data("child-discount") || 0;
			var discountType = $checkbox.data("discount-type") || "";
			...
			if (!fieldLabel) {
				...
				if ($cardsWrapper.length) {
					var $fieldLabelElement = $cardsWrapper.prev(
						".sh-product-extras-field-label"
					);
			...
			// Capture main product ID to prevent leakage across different products
			var $form = card.closest("form.cart, form.variations_form");
			var mainProductId = null;
			if ($form.length) {
				// Try variable product hidden input
				var $pidInput = $form.find('input[name="product_id"]');
				if ($pidInput.length) {
					mainProductId = $pidInput.val();
				}
				// Try simple product button
				if (!mainProductId) {
					var $btn = $form.find('button[name="add-to-cart"]');
					if ($btn.length) {
						mainProductId = $btn.val();
					}
				}
				// Try form data attribute
				if (!mainProductId) {
					mainProductId = $form.data("product_id");
				}
			}
			...
			var productData = {
				...
				imageUrl: "",
				mainProductId: mainProductId, // Store main product ID
			};
			...
			// Store key on card for reliable removal
			card.data("sh-store-key", key);

			window.shSelectedProducts[key] = productData;

			this.updateCartFormData();
		},

		removeSelectedProduct: function (productId, card) {
			// Ensure productId is treated consistently as a string
			productId = String(productId);

			var key = null;
			if (card && card.data("sh-store-key")) {
				key = card.data("sh-store-key");
			} else {
				var widgetId = "default";
				if (card) {
					var $container = card.closest(".sh-product-extras-container");
					if ($container.length && $container.data("widget-id")) {
						widgetId = $container.data("widget-id");
					}
				}
				key = widgetId + "_" + productId;
			}

			var removed = false;
			if (window.shSelectedProducts && window.shSelectedProducts[key]) {
				delete window.shSelectedProducts[key];
				removed = true;
			}

			// Fallback: If strict key lookup failed, try to find by productId
			if (!removed && window.shSelectedProducts) {
				// 1. Try "default" prefix if we tried a specific widgetId
				if (window.shSelectedProducts["default_" + productId]) {
					delete window.shSelectedProducts["default_" + productId];
					removed = true;
				}

				// 2. Scan for any key ending in _productId if we still haven't found it
				// This handles cases where widgetId might be lost or mismatched (e.g. DOM updates)
				if (!removed) {
					var keys = Object.keys(window.shSelectedProducts);
					for (var i = 0; i < keys.length; i++) {
						var k = keys[i];
						// Check if key ends with _productId (format is widgetId_productId)
						if (k.endsWith("_" + productId)) {
							// Found a candidate.
							delete window.shSelectedProducts[k];
							removed = true;
							// Break after first match to avoid deleting other widgets' same product
							// (though in "uncheck" scenario, we usually want to remove the specific instance)
							break;
						}
					}
				}
			...
			// Immediately remove the specific hidden field for this product to avoid race conditions
			// The updateFormFields debounce might be too slow for immediate add-to-cart clicks
			if (card) {
				var $checkbox = card.find(".sh-checkbox-input");
				var checkboxName = $checkbox.attr("name");
				if (checkboxName) {
					...
					$("form.cart, form.variations_form")
						.find('input[type="hidden"][name="' + checkboxName + '"]')
						.remove();
				}
			}
		...
		storeSelectedInformation: function (
			infoId,
			infoLabel,
			infoPrice,
			imageUrl
			...
			var $checkbox = $(
				'.sh-information-checkbox-input[data-info-id="' + infoId + '"]'
			);
			var originalPrice = $checkbox.length
				? parseFloat($checkbox.data("info-original-price") || 0)
				: 0;
			...
			if (!fieldLabel) {
				// Look for the field label element that precedes the cards wrapper
				var $cardsWrapper = $checkbox.closest(
					".sh-product-extras-cards-wrapper"
				);
				if ($cardsWrapper.length) {
					var $fieldLabelElement = $cardsWrapper.prev(
						".sh-product-extras-field-label"
			...
			// Capture main product ID to prevent leakage across different products
			var $form = $checkbox.closest("form.cart, form.variations_form");
			var mainProductId = null;
			if ($form.length) {
				// Try variable product hidden input
				var $pidInput = $form.find('input[name="product_id"]');
		...
		updateCartFormData: function () {
			...
			// Iterate through each form to handle multiple products on the same page correctly
			$forms.each(function () {
				var $thisForm = $(this);

				// Determine the main product ID for this specific form
				var mainProductId =
					$thisForm.find('input[name="product_id"]').val() ||
					$thisForm.data("product_id") ||
					$thisForm.find('button[name="add-to-cart"]').val();

				// Remove existing product extras hidden fields FROM THIS FORM
				$thisForm
					.find(
						'input[name^="sh_selected_"], input[name^="product_extras_"], input[name^="extra_product_"], input[name^="extra_info_"], input[name="has_product_extras"]'
					)
					.remove();
				...
				if (productKeys.length > 0) {
					for (var i = 0; i < productKeys.length; i++) {
						...
						// FILTER: Only include if mainProductId matches
						if (
							mainProductId &&
							productData.mainProductId &&
							String(productData.mainProductId) !== String(mainProductId)
						) {
							continue;
						}
						...
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_product_ids[]" value="' +
							productData.productId +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_product_qty[' +
							productData.productId +
							']" value="' +
							productData.quantity +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_product_type[' +
							productData.productId +
							']" value="' +
						...
						if (productData.variationData) {
							...
							for (var j = 0; j < variationKeys.length; j++) {
								...
								hiddenFieldsHTML +=
									'<input type="hidden" name="extra_product_variation[' +
									productData.productId +
									"][" +
									attrName +
									']" value="' +
						...
						if (productData.variationId) {
							hiddenFieldsHTML +=
								'<input type="hidden" name="extra_product_variation_id[' +
								productData.productId +
								']" value="' +
						...
						if (productData.fieldLabel) {
							hiddenFieldsHTML +=
								'<input type="hidden" name="extra_product_field_labels[' +
								productData.productId +
								']" value="' +
								productData.fieldLabel +
								'">';
						}

						if (productData.childDiscount) {
							hiddenFieldsHTML +=
								'<input type="hidden" name="extra_product_child_discount[' +
								productData.productId +
								']" value="' +
								productData.childDiscount +
								'">';
						}
						if (productData.discountType) {
							hiddenFieldsHTML +=
								'<input type="hidden" name="extra_product_discount_type[' +
								productData.productId +
								']" value="' +
				...
				if (infoKeys.length > 0) {
					for (var i = 0; i < infoKeys.length; i++) {
						...
						if (
							mainProductId &&
							infoData.mainProductId &&
							String(infoData.mainProductId) !== String(mainProductId)
						) {
							continue;
						}
						...
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_info_ids[]" value="' +
							infoData.infoId +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_info_labels[' +
							infoData.infoId +
							']" value="' +
							infoData.infoLabel +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_info_prices[' +
							infoData.infoId +
							']" value="' +
							infoData.infoPrice +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_info_original_prices[' +
							infoData.infoId +
							']" value="' +
							(infoData.infoOriginalPrice || 0) +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_info_images[' +
							infoData.infoId +
							']" value="' +
							(infoData.imageUrl || "") +
							'">';
						hiddenFieldsHTML +=
							'<input type="hidden" name="extra_info_field_labels[' +
							infoData.infoId +
							']" value="' +
							(infoData.fieldLabel || "") +
							'">';
				...
				if (hasData) {
					hiddenFieldsHTML +=
						'<input type="hidden" name="has_product_extras" value="1">';
				}

				// Single DOM operation to add all hidden fields to THIS form
				if (hiddenFieldsHTML) {
					$thisForm.append(hiddenFieldsHTML);
				}
			});
		},
		 ...
		 * @param {string|number} filterMainProductId - Optional main product ID to filter extras
		 */
		getProductExtrasData: function (filterMainProductId) {
			var selectedProducts = window.shSelectedProducts || {};
			var selectedInformation = window.shSelectedInformation || {};
			...
			$.each(selectedProducts, function (productId, productData) {
				// Filter by main product ID if provided
				if (
					filterMainProductId &&
					productData.mainProductId &&
					String(productData.mainProductId) !== String(filterMainProductId)
				) {
					return;
				}
				...
				selectedProductsArray.push({
					...
					childDiscount: productData.childDiscount || 0,
			...
			$.each(selectedInformation, function (infoId, infoData) {
				...
				if (
					filterMainProductId &&
					infoData.mainProductId &&
					String(infoData.mainProductId) !== String(filterMainProductId)
				) {
					return;
				}
			...
			var productExtrasData = {
				...
				has_product_extras:
					selectedProductsArray.length > 0 || selectedInfoArray.length > 0
						? "1"
						: "0",
			};
			...
			if (selectedProductsArray.length > 0) {
				...
				$.each(selectedProductsArray, function (index, productData) {
					...
					productExtrasData.extra_product_qty[productData.productId] =
						productData.quantity;
					productExtrasData.extra_product_type[productData.productId] =
						productData.productType;

					if (
						productData.variationData &&
						Object.keys(productData.variationData).length > 0
					) {
						productExtrasData.extra_product_variation[productData.productId] =
							productData.variationData;
					}
			...
			if (selectedInfoArray.length > 0) {
				...
				$.each(selectedInfoArray, function (index, infoData) {
					...
					productExtrasData.extra_info_labels[infoData.infoId] =
						infoData.infoLabel;
					productExtrasData.extra_info_prices[infoData.infoId] =
						infoData.infoPrice;
					productExtrasData.extra_info_images[infoData.infoId] =
						infoData.imageUrl || "";
				});
			}
		...
		collectPEWCFields: function () {
			...
			$("form.cart, form.variations_form")
				...
				.each(function () {
					...
					if (
						name &&
						(name.indexOf("pewc_") === 0 ||
							name.indexOf("product_extras_") === 0)
					) {
						var value = "";
		...
		evaluateGroupConditions: function () {
			...
			$("[data-conditions]").each(function () {
				...
				var match = (
					$element.attr("data-condition-match") || "any"
				).toLowerCase();
				var action = (
					$element.attr("data-condition-action") || "show"
				).toLowerCase();
				var total = 0;
				var met = 0;
				...
				if (conditions && conditions.length > 0) {
					...
					var conditionsMet =
						match === "all" ? met === total && total > 0 : met > 0;
					var showGroup;
					if (action === "show") {
					...
					if (showGroup) {
						...
						if (
							$element.hasClass("sh-product-extras-group-title") ||
							$element.hasClass("sh-product-extras-group-placeholder")
						) {
							$element
								.nextUntil(
									"[data-conditions], .sh-product-extras-group-title, .sh-product-extras-group-placeholder"
								)
								.addClass("sh-conditions-met")
								.show();
						}
					} else {
			...
			$(".sh-product-extras-container").each(function () {
				var $container = $(this);
				var anyVisible =
					$container.find("[data-conditions].sh-conditions-met").length > 0;
				if (anyVisible) {
	...
	$(window).on("load", function () {
		...
		setTimeout(function () {
			if (
				ProductExtrasWidget.initialized &&
				$("form.cart, form.variations_form").length > 0
			) {
	...
	$(window).on("elementor/frontend/init", function () {
		if (typeof elementorFrontend !== "undefined") {
			elementorFrontend.hooks.addAction(
				"frontend/element_ready/sh_product_extras.default",
				function ($scope) {
					ProductExtrasWidget.init();
					ProductExtrasWidget.evaluateGroupConditions();
				}
	...
	$(document).on(
		"change",
		'[name="awcdp_deposit_option"], [name="trade_in"]',
		function () {
