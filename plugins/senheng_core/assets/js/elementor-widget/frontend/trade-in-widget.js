...
(function ($) {
    ...
    class TradeInWidget {
        ...
        bindEvents() {
            ...
            $(document).on('click', '.sh-trade-in-wrapper .wd-swatch', this.handleSwatchClick.bind(this));

            // Handle variation swatch clicks to clear errors
            $(document).on('click', '.variations .wd-swatch', this.handleVariationSwatchClick.bind(this));

            // Handle variation select changes to clear errors
            $(document).on('change', '.variations select', this.handleVariationSelectChange.bind(this));

            // Handle deposit option change (fallback for legacy)
            $(document).on('change', '.awcdp-deposits-wrapper input[name="awcdp_deposit_option"]', this.handleDepositChange.bind(this));
            ...
            $(document).on('sh_trade_in_changed', this.handleTradeInRestriction.bind(this));
            ...
            $(document).on('quantity_selector_updated', (event, data) => {
                ...
                if ($tradeInSwatches.length) {
                    const tradeInSelected = $tradeInSwatches.find('.wd-swatch.selected').length > 0;
                    if (tradeInSelected) {
                        setTimeout(() => {
        ...
        handleSwatchClick(event) {
            ...
            const $hiddenSelect = $swatchContainer.closest('.sh-value').find('select');
            ...
            $swatchContainer.closest('.sh-trade-in-wrapper').find('.sh-error-message').remove();
            ...
            const productId = $hiddenSelect.data('product-id') || $swatchContainer.closest('[data-product-id]').data('product-id');
        ...
        handleTradeInRestriction(event, data) {
            ...
            const $paymentContainer = $(`[data-id="awcdp_deposit_option_${productId}"]`);
        ...
        restrictToDepositOnly($paymentContainer, productId) {
            ...
            const $fullPaymentSwatch = $paymentContainer.find('.wd-swatch[data-value="no"]');
            ...
            const $depositSwatch = $paymentContainer.find('.wd-swatch[data-value="yes"]');
            ...
            // Update the hidden select element
            const $hiddenSelect = $paymentContainer.closest('.sh-value').find('select');
            if ($hiddenSelect.length) {
        ...
        addRestrictionNotice($container, message) {
            ...
            // Add new notice
            const $notice = $('<div class="sh-payment-restriction-notice" style="font-size: 12px; color: #666; margin-top: 5px; font-style: italic;">' + message + '</div>');
        ...
        removeRestrictionNotice($container) {
            $container.closest('.sh-value').find('.sh-payment-restriction-notice').remove();
        }
        ...
        onPageLoad() {
            ...
            if ($tradeInWrappers.length) {
                ...
                $tradeInWrappers.find('.sh-trade-in-select').each(function () {
                    ...
                    if ($label.length) {
                        const labelId = 'trade-in-label-' + Math.random().toString(36).substr(2, 9);
        ...
        initializeDepositSections() {
            ...
            $depositWrappers.each(function () {
                const $container = $(this);
                const $checkedRadio = $container.find('input[name="awcdp_deposit_option"]:checked');
        ...
        validateTradeInBeforeSubmission($form) {
            ...
            let $tradeInSwatches = $form.find('.wd-swatches-product[data-id*="trade_in"]');
            if (!$tradeInSwatches.length) {
            ...
            if ($tradeInSwatches.length) {
                ...
                if (tradeInSelected) {
                    ...
                    $selectedSwatches.each(function () {
                        const swatchValue = $(this).data('value') || $(this).text().trim().toLowerCase();
                        if (swatchValue === 'yes' || swatchValue === 'Yes' || swatchValue === 'YES') {
            ...
            let $paymentSwatches = $form.find('.wd-swatches-product[data-id*="awcdp_deposit_option"]');

            // Also check broader page context if not found in form
            if ($paymentSwatches.length === 0) {
                $paymentSwatches = $('.wd-swatches-product[data-id*="awcdp_deposit_option"]');
            }
            ...
            if ($paymentSwatches.length) {
                const $selectedPaymentSwatches = $paymentSwatches.find('.wd-swatch.selected');
            ...
            if (!tradeInSelected && !paymentSelected) {
                ...
                hasErrors = true;
            } else if (tradeInSelected && !paymentSelected && $paymentSwatches.length > 0) {
        ...
        updateFormData() {
            ...
            $forms.find('input[name="trade_in"], input[name="awcdp_deposit_option"]').remove();

            // Get current trade-in selection
            const $tradeInSwatch = $('.wd-swatches-product[data-id*="trade_in"] .wd-swatch.selected');
            if ($tradeInSwatch.length) {
                const tradeInValue = $tradeInSwatch.data('value');
                if (tradeInValue) {
                    $forms.append('<input type="hidden" name="trade_in" value="' + tradeInValue + '">');
                }
            }

            // Get current deposit selection
            const $depositSwatch = $('.wd-swatches-product[data-id*="awcdp_deposit_option"] .wd-swatch.selected');
            if ($depositSwatch.length) {
                const depositValue = $depositSwatch.data('value');
                if (depositValue) {
                    $forms.append('<input type="hidden" name="awcdp_deposit_option" value="' + depositValue + '">');
                }
            }
        }
        ...
        static getTradeInValue() {
            const $selectedSwatch = $('.wd-swatches-product[data-id*="trade_in"] .wd-swatch.selected');
        ...
        static setTradeInValue(value) {
            ...
            if ($swatchContainer.length) {
                ...
                const $targetSwatch = $swatchContainer.find('.wd-swatch[data-value="' + value + '"]');
