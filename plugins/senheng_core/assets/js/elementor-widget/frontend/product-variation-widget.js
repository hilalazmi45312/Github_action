jQuery(document).ready(function ($) {
	...
	// Helper: build variation index with simple caching to improve performance
	function buildVariationIndex($form) {
		// Return cached index if valid
		var cached = $form.data("variationIndexCache");
		if (cached && cached.list && cached.index) {
			return cached;
		}
		...
		// Cache the result
		$form.data("variationIndexCache", result);
	...
	function updateSwatches($form) {
		...
		$form.find(".variations select").each(function () {
			...
			$select.find("option").each(function () {
				...
				// Check if this option leads to any valid, in-stock variation given OTHER selections
				for (var i = 0; i < candidates.length; i++) {
					var v = candidates[i];
					var match = true;

					// Check compatibility with other selected attributes
					for (var otherAttr in selectedMap) {
						if (otherAttr === attrName) continue;
						var otherVal = selectedMap[otherAttr];
						// If user has selected a value for otherAttr
						if (otherVal) {
							// Variation attribute value for otherAttr
							var vVal = v.attributes[otherAttr];
							// If variation is specific (not Any/Empty) and doesn't match selection
							if (vVal !== "" && vVal !== undefined && vVal !== otherVal) {
								match = false;
								break;
							}
						}
					...
					if (match) {
						// Found a matching variation, check stock
						// Use strict stock checking if managing_stock is true
						if (v.managing_stock && typeof v.stock_quantity === "number") {
							if (v.stock_quantity > 0) {
								isAvailable = true;
							}
						} else {
							// Fallback to general stock status
							if (v.is_in_stock) {
								isAvailable = true;
							}
						}
				...
				if (isAvailable) {
					...
					$option.prop("disabled", true).removeClass("enabled");
					$swatch.addClass("wd-disabled").removeClass("wd-enabled");
				}
			});
		});
	}
	...
	function initVariationWidget() {
		$(
			...
			var $form = $(this);
			var $widget = $form.closest(
				".product-variation-widget, .sh-variation-form"
			);
			...
			$form
				.off("woocommerce_update_variation_values.widget")
				.on("woocommerce_update_variation_values.widget", function () {
					updateSwatches($form);
				});
			...
			if (
				...
				typeof $.fn.wc_variation_form !== "undefined"
			) {
				...
				var $mainBtn =
					$mainForm && $mainForm.length
						? $mainForm.find(".single_add_to_cart_button")
						: $();
				var $mainCustomBtn =
					$mainForm && $mainForm.length
						? $mainForm.find(".custom-add-to-basket-btn")
						: $();
				...
				if (!anyInStock && list.length > 0) {
					...
					$(document).trigger("sh_all_variations_out_of_stock", {
						form: targetForm,
					});
				}
			}
			...
			$widget
				.off("click.swatches", ".wd-swatch")
				.on("click.swatches", ".wd-swatch", function (e) {
					e.preventDefault();
			...
			$form
				.off("found_variation.widget")
				.on("found_variation.widget", function (event, variation) {
			...
			function getTradeDepositStatus($form) {
				// Use more specific selectors and cache lookups if possible
				var $tradeScope = $form.find(
				...
				var depositRequired =
					$depositSwatchScope.length > 0 ||
					$form.find('input[name="deposit_option"]').length > 0;

				var depositSelected = false;
				if (depositRequired) {
					if ($depositSwatchScope.length > 0) {
						depositSelected =
					...
					if (!depositSelected) {
						depositSelected =
							$form.find('input[name="deposit_option"]:checked').length > 0;
					}
				}
			...
			document.addEventListener(
				...
				function (ev) {
					...
					var clickable = target.closest(
						".single_add_to_cart_button, .custom-add-to-basket-btn, .wd-buy-now-btn"
					);
					if (!clickable) return;
					var $btn = $(clickable);
			...
			// Use native addEventListener for touchstart with passive: true to avoid scroll-blocking
			var widgetElement = $widget.get(0);
			if (widgetElement) {
				// Remove existing handlers first
				$widget.off("mousedown.userTouch", ".variations select");

				// Mousedown can stay with jQuery (not scroll-blocking)
				$widget.on("mousedown.userTouch", ".variations select", function () {
					$(this).attr("data-user-touched", "1").data("userTouched", true);
				});

				// Touchstart needs passive: true to avoid scroll-blocking warning
				widgetElement.addEventListener('touchstart', function (e) {
					var target = e.target;
					if (target && target.matches && target.matches('.variations select')) {
						$(target).attr("data-user-touched", "1").data("userTouched", true);
					} else if (target && target.closest) {
						var select = target.closest('.variations select');
						if (select) {
							$(select).attr("data-user-touched", "1").data("userTouched", true);
						}
					}
				}, { passive: true });
			}
	...
	$(document.body).on(
		"wc_fragments_loaded wc_fragments_refreshed updated_wc_div",
		function () {
			initVariationWidget();
		}
	);
			...
			if ($mainForm && $mainForm.length && attributeName) {
				...
				if ($mainSelect.length && $mainSelect.val() !== selectedValue) {
					...
					setTimeout(function () {
						...
						var widgetVariationId = $widgetForm
							.find('input[name="variation_id"]')
							.val();
						if (widgetVariationId && widgetVariationId !== "0") {
							$mainForm
								.find('input[name="variation_id"]')
								.val(widgetVariationId);
							...
							if (variationData) {
								...
								if (
									variationData.is_in_stock &&
									variationData.stock_quantity !== undefined
								) {
									completeVariation.stock_quantity =
