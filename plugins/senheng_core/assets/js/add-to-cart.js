(function ($) {
	...
	var cachedAjaxUrl =
		window.woodmart_settings && woodmart_settings.ajaxurl
	...
	jQuery(document).on(
		"sh_trade_in_changed sh_payment_option_changed",
		function () {
			if (
				window.fragmentRefreshControl &&
				typeof window.fragmentRefreshControl.block === "function"
			) {
	...
	jQuery(document).ajaxSend(function (event, xhr, settings) {
		if (
			...
			if (
				window.fragmentRefreshControl &&
				window.fragmentRefreshControl.isBlocked &&
				window.fragmentRefreshControl.isBlocked()
			) {
				try {
					xhr.abort();
	...
	function getRequirementStatus($form) {
		...
		var tradeRequired =
			$tradeScope.length > 0 ||
			$form.find('select[name*="trade_in"]').length > 0 ||
			$form.find('input[name="trade_in"]').length > 0;
		var hasTrade = false;
		if ($tradeScope.length) {
			hasTrade =
				$tradeScope.find(".wd-swatch.wd-active, .wd-swatch.selected").length >
				0;
			if (!hasTrade) {
		...
		if (!hasTrade) {
			hasTrade =
				$form.find('input[name="trade_in"]').length > 0 ||
				jQuery('input[name="trade_in"]').length > 0;
		}
		var $depScope = $form.find(
		...
		var $tradeWrapper = $form
			.closest(".custom-cart-wrapper")
			.find(".sh-trade-in-wrapper");
		...
		var depReq =
			depEnabled &&
			($depScope.length > 0 ||
				$form.find('input[name="deposit_option"]').length > 0 ||
				$form.find('select[name*="awcdp_deposit_option"]').length > 0);
		var hasDep = false;
		if ($depScope.length) {
			hasDep =
				$depScope.find(".wd-swatch.wd-active, .wd-swatch.selected").length > 0;
			if (!hasDep) {
		...
		if (!hasDep) {
			...
			hasDep =
				$depRadio.filter(":checked").length > 0 ||
				$form.find('input[name="awcdp_deposit_option"]').length > 0 ||
				jQuery('input[name="awcdp_deposit_option"]').length > 0;
		}
		return {
			vid: vid,
	...
	function updateQuantityOptions($dropdown, maxQty, minQty) {
		...
		var maxPerAdd = 5;
		var max = Math.min(maxPerAdd, Math.max(1, parseInt(maxQty, 10) || 1));
	...
	function setOutState($form) {
		...
		$qty.empty();
		// Don't show any value or symbol for out of stock
		$qty.append('<option value="" disabled selected></option>');
		$qty.val("");
		$qty.closest(".quantity-selector").addClass("no-variation out-of-stock");
		$form.find(".custom-cart-wrapper").attr("data-out-of-stock", "true");
	}

	function setSelectState($form) {
		if ($form.data("sh_all_oos") || allVariationsOutOfStock($form)) {
	...
	function setReadyState($form, variation) {
		...
		$button
			.prop("disabled", false)
			.removeClass(
				"wc-variation-selection-needed wc-variation-is-unavailable disabled"
			);
		$qty.prop("disabled", false);
		$qty.closest(".quantity-selector").removeClass("no-variation");
		var maxQ =
			typeof variation.max_qty === "number" && variation.max_qty > 0
				? variation.max_qty
	...
	$(document).on(
		...
		function (e, variation) {
			...
			var ok =
				vid !== 0 &&
				(!req.tradeRequired || req.hasTrade) &&
				(!req.depositRequired || req.hasDeposit);
			if (!ok) {
				setSelectState($form);
				return;
			}
	...
	function isSimpleProductOutOfStock($form) {
		...
		var $btn = $form.find(
			".single_add_to_cart_button, .custom-add-to-basket-btn"
		);
		if ($btn.length) {
	...
	$(document).on("sh_all_variations_out_of_stock", function (e, data) {
		var $form =
			data && data.form ? $(data.form) : $("form.variations_form").first();
		if ($form && $form.length) {
			$form.data("sh_all_oos", true);
			setOutState($form);
		}
	...
	$(document).on(
		"click",
		".custom-add-to-basket-btn.wc-variation-selection-needed",
		function (ev) {
			ev.preventDefault();
			var $btn = $(this);
	...
	function ensureTradeDepositInputs($form) {
		try {
			...
			if (tSel.length && tSel.data("value")) {
				$form.append(
					'<input type="hidden" name="trade_in" value="' +
					tSel.data("value") +
					'">'
				);
			}
	...
	function clearErrorNotices() {
		jQuery(
			".woocommerce-error, .woocommerce-message.error, .notice-error"
		).remove();
	}
	...
	function woodmart_ajax_add_to_cart($form, $button, $cartWrapper) {
		...
		// OPTIMIZATION: Serialize to object for better performance
		...
		var pid =
			...
			$button.val() ||
			"";
		var hasIndividualExtras =
			$form.find(
				'input[name^="extra_info_ids"], input[name^="extra_product_ids"]'
			).length > 0;
		if (!hasIndividualExtras) {
			try {
				if (
					window.ProductExtrasWidget &&
					typeof window.ProductExtrasWidget.getProductExtrasData === "function"
				) {
					var ped = window.ProductExtrasWidget.getProductExtrasData(pid);
					if (
						ped &&
						((ped.selectedProducts && ped.selectedProducts.length > 0) ||
							(ped.selectedInfo && ped.selectedInfo.length > 0))
					) {
						var productsJson = JSON.stringify(ped.selectedProducts);
						var infoJson = JSON.stringify(ped.selectedInfo);
						params["product_extras_products"] = productsJson;
						params["product_extras_info"] = infoJson;
						params["has_product_extras"] = "1";
					}
				}
			} catch (e) { }
		} else {
		...
		if (pid) {
			params["product_id"] = String(pid);
		}
		if (addVal) {
			params["add-to-cart"] = addVal;
		}
		var qty =
			$form.find(".custom-qty-dropdown").val() ||
			$form.find('input[name="quantity"]').val();
		if (qty) {
			params["quantity"] = String(qty);
		}
		params["action"] = "woodmart_ajax_add_to_cart";

		var data = jQuery.param(params);

		$button.removeClass("added not-added").addClass("loading");
		var $body =
			window.woodmartThemeModule && woodmartThemeModule.$body
				? woodmartThemeModule.$body
				: jQuery(document.body);
		$body.trigger("adding_to_cart", [$button, data]);

		jQuery
			.ajax({ url: cachedAjaxUrl, data: data, method: "POST" })
			.done(function (response) {
				...
				if (response.notices && response.notices.indexOf("error") > 0) {
					...
					jQuery(document.body).trigger("not_added_to_cart", [
						fragments,
						cart_hash,
						$button,
					]);
					...
					if (
						...
						window.woodmart_settings &&
						woodmart_settings.add_to_cart_action === "widget"
					) {
	...
	jQuery(document)
		...
		.on(
			...
			function (e) {
				...
				var isVariableProduct = $form.hasClass("variations_form");
				var vid = parseInt(
					$form.find('input[name="variation_id"]').val() || "0",
					10
				);
				...
				// Only check vid === 0 for variable products, not simple products
				var variationNotSelected = isVariableProduct && vid === 0;
				var needs =
				...
				if (needs) {
					...
					// Scroll to .sh-variation-form first, then fall back to form
					var $target = $(".sh-variation-form").first();
					if (!$target.length) {
						$target = $form.closest(".sh-variation-form");
					}
					if (!$target.length) {
						$target = $form;
					}
					var headerHeight =
						jQuery(".whb-header .whb-row.whb-sticky-row").length > 0
							? jQuery(".whb-header .whb-main-header").outerHeight()
							: 0;
	...
	jQuery(document.body).on("wc_add_to_cart_error", function () {
		jQuery(".custom-add-to-basket-btn.loading, .custom-add-to-basket-btn").each(
			function () {
				var $b = jQuery(this);
	...
	jQuery(document).on("click", ".custom-add-to-basket-btn", function (e) {
		...
		// Only require vid !== 0 for variable products, not simple products
		var variationNotSelected = isVariableProduct && vid === 0;
		var needs =
			variationNotSelected ||
			!(!req.tradeRequired || req.hasTrade) ||
	...
	// Fix: Prevent WooCommerce from adding disabled class to container
	// WooCommerce adds 'woocommerce-variation-add-to-cart-disabled' to the container div
	// We want this only on the button, not the container
	$(document).on('check_variations update_variation_values found_variation reset_data hide_variation show_variation', '.variations_form', function () {
		var $form = $(this);
		var $container = $form.find('.woocommerce-variation-add-to-cart');

		// Remove disabled class from container after WooCommerce adds it
		setTimeout(function () {
			$container.removeClass('woocommerce-variation-add-to-cart-disabled');
		}, 10);
	});

	// Also handle the AJAX complete for get_variation
