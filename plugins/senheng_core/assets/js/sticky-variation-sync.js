...
(function($) {
  ...
  function findWidgetVariationsForm() {
    ...
    var $form = $('.product-variation-widget .variations_form, .sh-variation-form .variations_form').first();
  ...
  function updateStickyUIOnFoundVariation(variation, ctx) {
    ...
    if ($form.length && variation && variation.attributes) {
      var currentAttrs = {};
      $form.find('.variations select').each(function(){ var n=$(this).attr('name'); var v=$(this).val(); if(n) currentAttrs[n]=v; });
      var mismatch = false;
      for (var a in currentAttrs) {
        var cur = currentAttrs[a];
        if (cur && cur !== '' && cur !== '0') {
          if (variation.attributes[a] !== cur) { mismatch = true; break; }
        }
      }
      if (mismatch) {
        if (ctx.$stickyCart.length) { ctx.$stickyCart.find('.wd-sticky-stock.stock.out-of-stock.wd-style-default').remove(); }
        if (ctx.$stickyAddBtn.length) { ctx.$stickyAddBtn.parent().find('> .wd-sticky-stock.stock.out-of-stock.wd-style-default').remove(); }
        ctx.$stickyAddBtn.removeClass('wd-enabled disabled');
        var selectText0 = (window.woodmart_settings && window.woodmart_settings.texts && window.woodmart_settings.texts.select_options) || ctx.initialStickyBtnText || 'Select options';
    ...
    function getTradeDepositStatus($form) {
      ...
      if (!$tradeScope.length) { $tradeScope = jQuery('.wd-swatches-product[data-id*="trade_in"]'); }
      ...
      if (!$depositSwatchScope.length) { $depositSwatchScope = jQuery('.wd-swatches-product[data-id*="awcdp_deposit_option"]'); }
      ...
      var depositEnabled = $tradeWrapper.length && (($tradeWrapper.data('deposit-enabled') === 'yes') || ($tradeWrapper.attr('data-deposit-enabled') === 'yes'));
      var depositRequired = depositEnabled && (($depositSwatchScope.length > 0) || depositRadioPresent);
    ...
    if (ctx.$stickyAddBtn.length) {
      var summaryBtnText = $('.summary-inner .single_add_to_cart_button').text() || '';
      var baseEnabled = !!(variation && (variation.is_purchasable !== false) && (variation.is_in_stock !== false));
      ...
      if (shouldEnable) {
        ...
        if (ctx.$stickyCart.length) {
          ctx.$stickyCart.find('.wd-sticky-stock.stock.out-of-stock.wd-style-default').remove();
        }
        if (ctx.$stickyAddBtn.length) {
          ctx.$stickyAddBtn.parent().find('> .wd-sticky-stock.stock.out-of-stock.wd-style-default').remove();
        }
        ctx.$stickyAddBtn.show();
        ctx.$stickyAddBtn.removeClass('disabled');
        ctx.$stickyAddBtn.text(summaryBtnText || (window.woodmart_settings && window.woodmart_settings.texts && window.woodmart_settings.texts.add_to_cart) || 'Add to cart');
        ...
        ctx.$stickyAddBtn.removeClass('wd-enabled disabled');
        var selectText = (window.woodmart_settings && window.woodmart_settings.texts && window.woodmart_settings.texts.select_options) || ctx.initialStickyBtnText || 'Select options';
        ...
        var oosText = (window.woodmart_settings && window.woodmart_settings.texts && window.woodmart_settings.texts.out_of_stock) ? window.woodmart_settings.texts.out_of_stock : 'Out of stock';
        if (ctx.$stickyAddBtn.length && !ctx.$stickyAddBtn.parent().find('> .wd-sticky-stock.stock.out-of-stock.wd-style-default').length) {
          ctx.$stickyAddBtn.after('<p class="wd-sticky-stock stock out-of-stock wd-style-default">' + oosText + '</p>');
        }
        ctx.$stickyAddBtn.removeClass('wd-enabled');
        ctx.$stickyAddBtn.addClass('disabled');
        ctx.$stickyAddBtn.hide();
      }
    }
  }
  ...
  function syncQuantityBetweenStickyAndWidget() {
    ...
    if ($widgetQty.length && $stickyQty.length) {
      $stickyQty.off('change.senhengSync').on('change.senhengSync', function() {
        $widgetQty.val($(this).val()).trigger('change');
      });
      $widgetQty.off('change.senhengSync').on('change.senhengSync', function() {
  ...
  function removeStickyCompareButton() {
    try {
      ...
      $scope.find('a.product-compare-button.wd-action-btn.wd-style-icon.wd-compare-icon').remove();
      // Clean up any tooltips tied to compare button.
      $scope.find('.wd-tooltip.wd-compare-icon, .wd-tooltip-inited.wd-compare-icon').remove();
  ...
  function init() {
    ...
    if ($widgetForm.length) {
      ...
      $widgetForm.on('variation_data_synced.senhengSticky', function(e, variation) {
        lastVariation = variation;
        updateStickyUIOnFoundVariation(variation, ctx);
      });
    ...
    $(document).on('click.senhengStickyReq', '.wd-swatches-product[data-id*="trade_in"] .wd-swatch, .wd-swatches-product[data-id*="awcdp_deposit_option"] .wd-swatch', function() {
      if (lastVariation) {
        updateStickyUIOnFoundVariation(lastVariation, ctx);
      }
    });
    $(document).on('change.senhengStickyReq', 'input[name="deposit_option"]', function() {
      if (lastVariation) {
        updateStickyUIOnFoundVariation(lastVariation, ctx);
      }
    });
    ...
    function detachThemeStickyClickHandlers() {
      try {
        ...
        if (window.woodmartThemeModule && woodmartThemeModule.$body) {
          ...
          woodmartThemeModule.$body.off('click', '.wd-sticky-btn-cart > .wd-buy-now-btn');
        }
      } catch (err) {
        // silent
      }
    ...
    function installStickyClickInterceptors() {
      ...
      document.addEventListener('click', function(ev) {
        ...
        if (!target) return;
        var clickable = target.closest('.wd-sticky-add-to-cart, .wd-sticky-btn-cart > .wd-buy-now-btn');
        if (!clickable) return;
        ...
        if (isStickyEnabled && $addToCartBtn.length && !$addToCartBtn.hasClass('disabled')) {
          // Loading state until AJAX completes.
          $el.addClass('loading').prop('disabled', true).attr('aria-disabled', 'true');
          var clearLoading = function() {
            $el.removeClass('loading').prop('disabled', false).attr('aria-disabled', 'false');
          };
          $(document.body).one('added_to_cart.senhengStickyLoading', clearLoading);
          $(document.body).one('wc_add_to_cart_error.senhengStickyLoading', clearLoading);
          $(document.body).one('not_added_to_cart.senhengStickyLoading', clearLoading);
    ...
    $(document).on('DOMNodeInserted', function(e) {
      if ($(e.target).find('.wd-sticky-btn').length || $(e.target).find('.variations_form').length) {
        syncQuantityBetweenStickyAndWidget();
        removeStickyCompareButton();
      }
    });
