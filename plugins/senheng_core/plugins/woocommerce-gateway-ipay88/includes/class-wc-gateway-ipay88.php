...
class WC_Gateway_iPay88 extends WC_Payment_Gateway
{
	...
	public $MerchantCode;
	public $MerchantKey;
	public $SecondMerchantCode;
	public $SecondMerchantKey;
	public $use_css;
	...
	public function __construct()
	{
		...
		$this->SecondMerchantCode       = isset($this->settings['SecondMerchantCode']) ? $this->settings['SecondMerchantCode'] : '';
		$this->SecondMerchantKey        = isset($this->settings['SecondMerchantKey']) ? $this->settings['SecondMerchantKey'] : '';
...
?>
		...
		<script>
			! function(e) {
				e(document).ready(function() {
					e("#woocommerce_ipay88_gateway").change(function() {
						var o = e(this).val().toLowerCase();
						console.log(o), e(".depend_on_gateway").closest("tr").hide(), e(".show_if_gateway_" + o).closest("tr").slideDown()
					}).change()
				})
			}(jQuery);
		</script>
	...
	function init_form_fields()
	{
		...
		$this->form_fields = array(
			...
			'gateway'                  => array(
				...
				'type'        => 'select',
				'description' => __('Which iPay88 gateway are you using? Currently supported are Malaysia.', 'wc_ipay88'),
				'css'         => 'min-width:350px;',
				'class'       => 'chosen_select',
				'default'     => 'MY',
				'options'     => array(
			...
			'second_merchant_section' => array(
				'title' => __('Second Merchant (BNPL / Special Payments)', 'wc_ipay88'),
				'type'  => 'title',
				'description' => __('Configure secondary iPay88 merchant credentials.', 'wc_ipay88'),
			),
			'SecondMerchantCode' => array(
				'title'       => __('Second Merchant Code', 'wc_ipay88'),
				'type'        => 'text',
				'description' => __('Merchant Code for BNPL / installment payments.', 'wc_ipay88'),
				'default'     => '',
			),
			'SecondMerchantKey' => array(
				'title'       => __('Second Merchant Key', 'wc_ipay88'),
				'type'        => 'password',
				'description' => __('Merchant Key for BNPL / installment payments.', 'wc_ipay88'),
				'default'     => '',
			),
	...
	function generate_ipay88_form($order_id)
	{
		...
		if (apply_filters('wc_ipay88_receipt_page_js_display', true)) {
			wc_enqueue_js(
				apply_filters(
					'wc_ipay88_receipt_page_js_output',
					'jQuery("body").block({
	...
	function process_payment($order_id)
	{
		...
		try {
			...
			if (
				...
				&& (int) $this->posted_payment_plan > 0
			) {
				if ($this->switch_to_second_merchant()) {
					update_post_meta($order_id, '_ipay88_merchant_mode', 'second');
				}
			}
	...
	function check_status_response_ipay88()
	{
		...
		if ($refno) {
			...
			if ($order) {
				$merchant_mode = get_post_meta($order_id, '_ipay88_merchant_mode', true);

				if ($merchant_mode === 'second') {
					$this->MerchantCode = $this->SecondMerchantCode;
					$this->MerchantKey  = $this->SecondMerchantKey;
				}
			}
		}
	...
	private function check_payment_fields($payment_type = '0', $payment_plan = '0')
	{
		...
		if (! empty($this->paymenttype_available)) {
			...
			$bnpl_options = apply_filters('wc_ipay88_bnpl_payment_types', array(
				...
				'727',
				'891',
				// '523', dont put as now GRAB can do both BNPL and non-BNPL
			));
	...
	private function switch_to_second_merchant()
	{
		if (empty($this->SecondMerchantCode) || empty($this->SecondMerchantKey)) {
			return false;
		}
