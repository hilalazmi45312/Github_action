	...
	wps_price_slider.updateForm = function( query, container, args, href ) {
		...
		if ( form.length > 0 ) {
			...
			if ( typeof args.title !== 'undefined' ) {
				var title = $( form ).find( 'input[name="wps-title"]' );
				if ( title.length > 0 ) {
					title.val( args.title );
				} else {
					$( form ).append( '<input type="hidden" name="wps-title" value="' + args.title + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-title"]' ).remove();
			}
			if ( typeof args.excerpt !== 'undefined' ) {
				var excerpt = $( form ).find( 'input[name="wps-excerpt"]' );
				if ( excerpt.length > 0 ) {
					excerpt.val( args.excerpt );
				} else {
					$( form ).append( '<input type="hidden" name="wps-excerpt" value="' + args.excerpt + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-excerpt"]' ).remove();
			}
			if ( typeof args.content !== 'undefined' ) {
				var content = $( form ).find( 'input[name="wps-content"]' );
				if ( content.length > 0 ) {
					content.val( args.content );
				} else {
					$( form ).append( '<input type="hidden" name="wps-content" value="' + args.content + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-content"]' ).remove();
			}
			if ( typeof args.categories !== 'undefined' ) {
				var categories = $( form ).find( 'input[name="wps-categories"]' );
				if ( categories.length > 0 ) {
					categories.val( args.categories );
				} else {
					$( form ).append( '<input type="hidden" name="wps-categories" value="' + args.categories + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-categories"]' ).remove();
			}
			if ( typeof args.attributes !== 'undefined' ) {
				var attributes = $( form ).find( 'input[name="wps-attributes"]' );
				if ( attributes.length > 0 ) {
					attributes.val( args.attributes );
				} else {
					$( form ).append( '<input type="hidden" name="wps-attributes" value="' + args.attributes + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-attributes"]' ).remove();
			}
			if ( typeof args.tags !== 'undefined' ) {
				var tags = $( form ).find( 'input[name="wps-tags"]' );
				if ( tags.length > 0 ) {
					tags.val( args.tags );
				} else {
					$( form ).append( '<input type="hidden" name="wps-tags" value="' + args.tags + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-tags"]' ).remove();
			}
			if ( typeof args.sku !== 'undefined' ) {
				var sku = $( form ).find( 'input[name="wps-sku"]' );
				if ( sku.length > 0 ) {
					sku.val( args.sku );
				} else {
					$( form ).append( '<input type="hidden" name="wps-sku" value="' + args.sku + '"/>' );
				}
			} else {
				$( form ).find( 'input[name="wps-sku"]' ).remove();
			}
