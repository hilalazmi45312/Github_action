...
( function( $ ) {
	...
	ixwpsf.productFilter = function( query, containers, args ) {
		var href                = ixwpsf.decodeURI( window.location.href ),
			hrefIsAlt           = false,
		...
		if ( typeof args.title !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-title', args.title, href );
		}
		if ( typeof args.excerpt !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-excerpt', args.excerpt, href );
		}
		if ( typeof args.content !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-content', args.content, href );
		}
		if ( typeof args.categories !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-categories', args.categories, href );
		}
		if ( typeof args.attributes !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-attributes', args.attributes, href );
		}
		if ( typeof args.tags !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-tags', args.tags, href );
		}
		if ( typeof args.sku !== 'undefined' ) {
			href = ixwpsf.updateQueryArg( 'wps-sku', args.sku, href );
		}
		...
		if ( ixwpsf.taxonomy.length > 0 ) {
			...
			for ( var i = 0; i < ixwpsf.taxonomy.length; i++ ) {
				...
				if ( typeof ixwpsf.taxonomy[i].op !== 'undefined' ) {
					href = ixwpsf.updateQueryArg( 'ixwpsf[taxonomy][' + ixwpsf.taxonomy[i].taxonomy + '][op]', ixwpsf.taxonomy[i].op, href );
				}
			}
		}
		...
		href = href.replace( /[?#]+$/, '' ); // @since 5.6.0 remove any trailing ? or #
		...
		ixwpsf.xhr = $.ajax({
			...
			beforeSend : function() {
				...
				$( document ).triggerHandler(
					'ixProductFilterRequestBeforeSend',
					{
						url : e_href,
						unpage_url : typeof args.unpage_url !== 'undefined' ? args.unpage_url : false // @since 6.0.0
					}
				);
			},
			...
			success : function( data, textStatus, jqXHR ) {
				...
				if ( warn_products_container || warn_product_container ) {
					...
					var has_product_collection_block = $( document ).find( '.wp-block-woocommerce-product-collection' ).length > 0;
					if ( has_product_collection_block ) {
						warn_product_container = false;
						warn_products_container = false;
					}
				}
				...
				$( '.product-search-filter-reset' ).trigger( 'ixProductFilterRequestDone', { url : e_href } );
	...
	ixwpsf.updateOrderingForm = function( query, containers, args, href ) {
		...
		if ( orderingForm.length > 0 ) {
			...
			if ( typeof args.title !== 'undefined' ) {
				var title = $( orderingForm ).find( 'input[name="wps-title"]' );
				if ( title.length > 0 ) {
					title.val( args.title );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-title" value="' + args.title + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-title"]' ).remove();
			}
			if ( typeof args.excerpt !== 'undefined' ) {
				var excerpt = $( orderingForm ).find( 'input[name="wps-excerpt"]' );
				if ( excerpt.length > 0 ) {
					excerpt.val( args.excerpt );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-excerpt" value="' + args.excerpt + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-excerpt"]' ).remove();
			}
			if ( typeof args.content !== 'undefined' ) {
				var content = $( orderingForm ).find( 'input[name="wps-content"]' );
				if ( content.length > 0 ) {
					content.val( args.content );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-content" value="' + args.content + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-content"]' ).remove();
			}
			if ( typeof args.categories !== 'undefined' ) {
				var categories = $( orderingForm ).find( 'input[name="wps-categories"]' );
				if ( categories.length > 0 ) {
					categories.val( args.categories );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-categories" value="' + args.categories + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-categories"]' ).remove();
			}
			if ( typeof args.attributes !== 'undefined' ) {
				var attributes = $( orderingForm ).find( 'input[name="wps-attributes"]' );
				if ( attributes.length > 0 ) {
					attributes.val( args.attributes );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-attributes" value="' + args.attributes + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-attributes"]' ).remove();
			}
			if ( typeof args.tags !== 'undefined' ) {
				var tags = $( orderingForm ).find( 'input[name="wps-tags"]' );
				if ( tags.length > 0 ) {
					tags.val( args.tags );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-tags" value="' + args.tags + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-tags"]' ).remove();
			}
			if ( typeof args.sku !== 'undefined' ) {
				var sku = $( orderingForm ).find( 'input[name="wps-sku"]' );
				if ( sku.length > 0 ) {
					sku.val( args.sku );
				} else {
					$( orderingForm ).append( '<input type="hidden" name="wps-sku" value="' + args.sku + '"/>' );
				}
			} else {
				$( orderingForm ).find( 'input[name="wps-sku"]' ).remove();
			}
			if ( typeof args.wpml !== 'undefined' ) {
			...
			if ( params.length > 0 ) {
				for ( var i = 0; i < params.length; i++ ) {
					...
					if (
						...
						key !== 'wps-title' && key !== 'wps-excerpt' && key !== 'wps-content' && key !== 'wps-categories' && key !== 'wps-attributes' && key !== 'wps-tags' && key !== 'wps-sku' &&
						key !== 'wpml' &&
						(
							key === 'min_price' ||
							key === 'max_price' ||
	...
	ixwpsf.brandFilterItemOnClick = function( e ) {
		...
		if ( $( this ).parent().hasClass( 'current-product_brand' ) ) {
			$( this ).addClass( 'loading-term-filter' );
			if ( multiple ) {
				$( this ).parent().removeClass( 'current-product_brand' );
				$( '.product-filter-field' ).first().trigger( 'ixTermFilter', [ $( this ).parent().data( 'term' ), 'product_brand', 'remove', origin_id ] );
			} else {
				$( '.product-search-product_brand-filter-item' ).removeClass( 'current-product_brand' );
				$( '.product-filter-field' ).first().trigger( 'ixTermFilter', [ '', 'product_brand', null, origin_id ] );
			}
		} else {
			if ( !multiple ) {
				$( '.product-search-product_brand-filter-item' ).removeClass( 'current-product_brand' );
			}
			$( this ).parent().addClass( 'current-product_brand' );
			$( this ).addClass( 'loading-term-filter' );
			$( '.product-filter-field' ).first().trigger( 'ixTermFilter', [ $( this ).parent().data( 'term' ), 'product_brand', action, origin_id ] );
		}
		return false;
	};

	ixwpsf.brandFilterSelectOnChange = function( e ) {
		var container = $( this ).closest( '.product-search-filter-terms' ),
			origin_id = $( container ).attr( 'id' ),
			multiple  = $( container ).data( 'multiple' ),
			clear     = false; // $( this ).val() === ''; <-- this won't do when the select is empty
		...
		$( '.product-filter-field' ).first().trigger( 'ixTermFilter', [ selected_terms, 'product_brand', action, origin_id ] );
		return false;
	};
	...
	$( document ).ready( function() {
		...
		$( 'select.product-search-filter-product_brand' ).prop( 'disabled', false ); // @since 6.1.0
		...
		$( document ).on( 'click', '.product-search-filter-terms:not(.filter-dead) .product-search-product_brand-filter-item a', ixwpsf.brandFilterItemOnClick ); // @since 6.1.0
		$( document ).on( 'change', 'select.product-search-filter-product_brand', ixwpsf.brandFilterSelectOnChange ); // @since 6.1.0
		...
		$( document ).on( 'ixProductFilterRequestDone', '.product-search-filter-reset', function( e, params ) {

			var has_filter = false;

			if ( typeof params !== 'undefined' ) {
				if ( typeof params.url !== 'undefined' ) {
					var pairs = ixwpsf.getQueryArgs( params.url );
					for ( var i = 0; !has_filter && i < pairs.length; i++ ) {
						let key = pairs[i][0];
						let value = pairs[i][1];
						let base = key.split( '[' ).shift();
						switch ( base ) {
							case 'ixwpss':
							case 'ixwpst':
							case 'ixwpsp':
		...
		$( '.product-search-filter-reset' ).trigger( 'ixProductFilterRequestDone', { url : window.location.href } );
		...
		$( document ).on( 'click ', 'a.wc-link-clear-any-filters', function () {
			var reset_url = '';
			if ( typeof woocommerce_product_search_context !== 'undefined' ) {
				reset_url = woocommerce_product_search_context.reset_url;
			}
			$(this).attr( 'href', reset_url )
		} );
	} );
