...
class WooCommerce_Product_Search {
	...
	public static function init() {
		...
		require_once WOO_PS_REST_LIB . '/class-rest.php';
		...
		if ( !empty( $_REQUEST['ixmbd'] ) ) {
			...
			$_SERVER['REQUEST_URI'] = preg_replace( '/(\?|&)(ixmbd(=[^&\?])|ixmbd=|ixmbd)/i', '', $_SERVER['REQUEST_URI'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public static function template_redirect() {
		...
		if ( is_404() ) {
			$url = $_SERVER['REQUEST_URI']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

			$redirect_url = preg_replace( '~/' . self::get_pagination_base() . '/[0-9]+~i', '', $url );
			$redirect_url = remove_query_arg( 'paged', $redirect_url );
			if ( $url !== $redirect_url ) {
				wp_redirect( $redirect_url, 301 ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
				exit;
			}
		}
	}
	...
	public static function plugins_loaded() {
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			require_once WOO_PS_VIEWS_LIB . '/class-woocommerce-product-search-filter-brand.php';
	...
	/**
	 * Filter the MO file path for generic translations if no specific ones are available.
	 *
	 * @since 6.0.0
	 *
	 * @param string $mofile
	...
	public static function load_textdomain_mofile( $mofile, $domain ) {
		...
		if ( 'woocommerce-product-search' === $domain ) {
			$locale = str_replace( $domain . '-', '', basename( $mofile, '.mo' ) );
			...
			if ( !file_exists( $mofile ) ) {
				...
				$the_mofile = null;
				if ( $form !== '' ) {
					$the_mofile = WOO_PS_CORE_DIR . '/languages/' . $domain . '-' . $language . '_' . $form . '.mo';
				...
				if ( $the_mofile === null ) {
					$the_mofile = WOO_PS_CORE_DIR . '/languages/' . $domain . '-' . $language . '.mo';
					if ( !file_exists( $the_mofile ) ) {
						$the_mofile = null;
					}
				}
	...
	public static function needs_db_update() {
		...
		if (
			...
			$brands_db_version = get_option( 'woocommerce_product_search_db_brands_version', '0' );
	...
	private static function cleanup( $drop = false ) {
		...
		if ( $delete_data || $drop ) {
			...
			self::cleanup_v1_indexes();
			delete_option( 'woocommerce_product_search_db_brands_version' );
	...
	private static function cleanup_metas() {
		...
		if ( function_exists( 'delete_term_meta' ) ) {
			...
			$wpdb->query(
				"DELETE FROM $wpdb->termmeta WHERE meta_key = 'product_search_image_id' AND term_id IN ( SELECT term_id FROM $wpdb->term_taxonomy WHERE taxonomy IN ($product_taxonomies) )" // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			);
		} else {
