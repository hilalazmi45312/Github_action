...
class WooCommerce_Product_Search_Indexer {
	 ...
	 * @var string[]
	 */
	private static $unicode_scripts = null;
	...
	public function __construct( $params = array() ) {
		...
		if ( is_array( $params ) ) {
			if ( isset( $params['guardian'] ) ) {
				if ( $params['guardian'] instanceof WooCommerce_Product_Search_Guardian ) {
	...
	public function preprocess_terms() {
		...
		if ( WooCommerce_Product_Search_Controller::table_exists( 'object_term' ) ) {
			...
			if ( count( $term_ids ) > 0 ) {
				...
				$limit = apply_filters( 'woocommerce_product_search_indexer_preprocess_terms_limit', null );
				if ( is_numeric( $limit ) ) {
				...
				if ( $limit !== null && $limit < count( $term_ids ) ) {

					$term_ids = array_slice( $term_ids, 0, $limit, true );
				}
				...
				foreach ( $term_ids as $term_id => $taxonomy ) {
					...
					$size += strlen( sprintf( '( 0, %d, "%s", "%s" ),', intval( $term_id ), gmdate( 'Y-m-d H:i:s' ), $taxonomy ) );
					if ( $size > 0.9 * $max ) {
						break;
					}
				...
				$query = $wpdb->prepare(
					"INSERT INTO $object_term_table " . // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					"( object_id, term_id, modified, taxonomy ) " .
					"VALUES " . implode( ',', $tuples ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
					$values
				);
	...
	public function process_term_weights( $term_ids = null, $taxonomy = null ) {
		...
		if ( is_taxonomy_hierarchical( $taxonomy ) ) {
			...
			$entries = $wpdb->get_results( $wpdb->prepare( $query, $taxonomy ) );
	...
	/**
	 * Whether there are any processable products.
	 *
	 * @since 4.1.0
	 *
	 * @return boolean
	 */
	public function has_processable() {

		global $wpdb;

		if ( $this->has_processable !== null ) {
			return $this->has_processable;
		}
		...
		if (
			...
			foreach ( $object_types as $object_type ) {
				...
				$object_type_id = $wpdb->get_var( $wpdb->prepare(
					"SELECT object_type_id FROM $object_type_table WHERE object_type = %s AND context_table = 'posts' AND context_column IS NULL AND context_key IS NULL", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$object_type
				) );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				$query = $wpdb->prepare(
					...
					" ( " .
						...
						"AND ( " .
							"ID NOT IN (SELECT object_id FROM $index_table WHERE object_type_id = %d) " . // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
							$object_term_query . // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						" ) " .
						"LIMIT 1" .
					" ) tmp",
					$object_type,
		...
		if (
			...
			foreach ( $object_types as $object_type ) {
				...
				$_post_ids = $wpdb->get_col( $wpdb->prepare (
					...
					$object_term_query . // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					" ) " .
					"ORDER BY $index_order_by $index_order_dir " . // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					"LIMIT %d",
					$object_type,
					intval( $object_type_id ),
					intval( $this->limit )
	...
	private function get_dt() {
		...
		if ( $t === null && function_exists( 'microtime' ) ) {
			$t = microtime( true );
		}
		...
		if ( $this->xseconds === null ) {
			$this->xseconds = $t;
		}

		return $t - $this->xseconds;
	}

	public function work() {

		global $wpdb;
		if ( !self::has_processable() ) {
			return;
		}
		...
		if ( $this->check_memory_limit ) {
			...
			$memory_limit = WooCommerce_Product_Search_System::get_memory_limit();
		}

		if ( $this->check_execution_limit ) {
			$max_execution_time = WooCommerce_Product_Search_System::get_max_execution_time();
		...
		WooCommerce_Product_Search_System::set_time_limit( 0 );
		...
		foreach( $post_ids as $post_id ) {
			...
			$max_dt = max( $max_dt, $t1 - $t0 );

			if ( $this->check_execution_limit ) {

				$break = ( $t1 + self::BREAKBUF * $max_dt ) >= $max_execution_time;

				$break = apply_filters( 'woocommerce_product_search_indexer_work_break', $break, $i, $post_ids, $post_id, $max_dt, $t1 - $t0, $max_execution_time );
				if ( $break ) {
					wps_log_info(
			...
			if ( $this->check_execution_limit ) {
				if ( function_exists( 'getrusage' ) ) {
					...
					if ( isset( $resource_usage['ru_utime.tv_sec'] ) ) {
						...
						$d = ceil( $execution_time - $initial_execution_time ); // @phpstan-ignore variable.undefined ($initial_execution_time is defined under the same entry conditions)
	...
	public function index( $post_id ) {
		...
		if ( $this->raw ) {
			$product = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE ID = %d", intval( $post_id ) ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		} else {
		...
		if ( $product ) {
			...
			if ( method_exists( $product, 'get_global_unique_id' ) ) {
				$guids[] = $product->get_global_unique_id();
			}
			if ( $product->is_type( 'variable' ) ) {
					...
					foreach( $variation_ids as $variation_id ) {
						if ( $variation = wc_get_product( $variation_id ) ) {
							...
							if ( method_exists( $variation, 'get_global_unique_id' ) ) {
								$guids[] = $variation->get_global_unique_id();
							}
						}
						unset( $variation );
					}
			...
			if ( $product->is_type( 'variation' ) ) {
				if ( method_exists( $product, 'get_parent_id' ) ) {
					...
					if ( $parent = wc_get_product( $parent_id ) ) {
						...
						$skus[]         = $parent->get_sku();
						if ( method_exists( $parent, 'get_global_unique_id' ) ) {
							$guids[] = $parent->get_global_unique_id();
						}
					}
				}
			}
			...
			if ( $this->raw ) {
				$context_columns = array(
					...
					'post_excerpt' => $this->filter( $product->post_excerpt . $descriptions . $guids, 'post_excerpt', $post_id ),
				...
				if ( !$product->is_type( 'variation' ) ) {
					$context_columns = array(
						...
						'post_content' => $this->filter( wc_format_content( $product->get_description() ) . $descriptions . $guids, 'post_content', $post_id ),
						'sku'          => $this->filter( $product->get_sku() . $skus . $guids, 'sku', $post_id ),
					...
					if ( !empty( $attribute_taxonomies ) ) {
						foreach ( $attribute_taxonomies as $attribute ) {
							...
							if ( $product->is_type( 'variable' ) ) {
								...
								/**
								 * @var WC_Product_Attribute|null $attribute_object
								 */
					...
					if ( function_exists( 'wc_get_brands' ) && $use_brands ) {
						$context_columns['brand'] = $this->filter( wc_get_brands( $post_id, ' ' ), 'brand', $post_id );
					}
					...
					$context_columns = array(
						...
						'post_content' => $this->filter( wc_format_content( $product->get_description() ) . $descriptions . $guids, 'post_content', $product->get_id() ),
						'sku'          => $this->filter( $product->get_sku() . $skus . $guids, 'sku', $product->get_id() )
					);
					if ( $parent_id ) {
						$context_columns['tag'] = $this->filter( wc_get_product_tag_list( $parent_id, ' ' ), 'tag', $product->get_id() );
						$context_columns['category'] = $this->filter( wc_get_product_category_list( $parent_id, ' ' ), 'category', $product->get_id() );

						if ( function_exists( 'wc_get_brands' ) && $use_brands ) {
							$context_columns['brand'] = $this->filter( wc_get_brands( $parent_id, ' ' ), 'brand', $product->get_id() );
						}
					} else {
						wps_log_warning( sprintf( 'The indexer detected inconsistent data for the product variation %d: missing parent.', $product->get_id() ) );
					}
					$attribute_taxonomies = wc_get_attribute_taxonomies();
					if ( !empty( $attribute_taxonomies ) ) {
						foreach ( $attribute_taxonomies as $attribute ) {
			...
			foreach( $context_columns as $context_column => $content ) {
				switch( $context_column ) {
					...
					case 'sku' :
						...
						break;
					case 'brand' :
						$object_type_id = $this->get_object_type_id( $object_type, 'brand', 'term_taxonomy', 'taxonomy', 'product_brand' );
						if ( $object_type_id === null ) {
							$object_type_id = $this->get_or_add_object_type( $object_type, 'brand', 'term_taxonomy', 'taxonomy', 'product_brand' );
						}
						break;
					default :
			...
			$wpdb->query( $wpdb->prepare(
				"INSERT INTO $index_table (key_id, object_id, object_type_id, count, modified) VALUES (0, %d, %d, 0, %s)", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				intval( $post_id ),
				intval( $object_type_id ),
				gmdate( 'Y-m-d H:i:s' )
			) );
			...
			if ( WooCommerce_Product_Search_Controller::table_exists( 'object_term' ) ) {
				...
				if ( $product->is_type( 'variation' ) ) {
					...
					if ( $parent_product_id ) {
						...
						if ( ! ( $parent_product instanceof WC_Product ) ) {
							...
							if ( $use_brands ) {
								$brand_ids = array_merge( $brand_ids, wc_get_product_term_ids( $parent_product_id, 'product_brand' ) );
							}
						}
					}
					...
					foreach ( $attributes as $taxonomy => $attribute ) {
						...
						if ( is_string( $taxonomy ) && is_string( $attribute ) ) {
							...
							$term = get_term_by( 'slug', $attribute, WooCommerce_Product_Search_Utility::sanitize_taxonomy( $taxonomy ) );
							if ( $term ) {
					...
					$taxonomy_inherited[] = 'product_brand';
						...
						if ( is_string( $taxonomy ) && ( $wc_product_attribute instanceof WC_Product_Attribute ) ) {
							...
							if ( taxonomy_exists( $taxonomy_name ) ) {
								...
								if ( !$wc_product_attribute->get_variation() ) {
									...
									$taxonomy_inherited[] = WooCommerce_Product_Search_Utility::sanitize_taxonomy( $taxonomy );
								}
							}
						}
					}
				...
				$term_ids = array_merge( $category_ids, $tag_ids, $attribute_term_ids, $brand_ids );
				...
				foreach ( $term_ids as $term_id ) {
					...
					if ( $term instanceof WP_Term ) {
						...
						$values[] = gmdate( 'Y-m-d H:i:s' );
						$query = "INSERT INTO $object_term_table "; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private function get_or_add_object_type( $object_type = null, $context = null, $context_table = null, $context_column = null, $context_key = null ) {
		...
		if ( WooCommerce_Product_Search_Controller::table_exists( 'object_type' ) ) {
			...
			if ( count( $conditions ) > 0 ) {
				...
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				$object_type_id = $wpdb->get_var( $wpdb->prepare(
					"SELECT object_type_id FROM $object_type_table $where", $values // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				) );
				if ( !$object_type_id ) {
					// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
					$query = $wpdb->prepare(
						sprintf(
							"INSERT INTO $object_type_table (%s) VALUES (%s)", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
							implode( ',', $fields ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
							implode( ',', $placeholders ) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						),
						$values
					);
	...
	private function delete_indexes( $object_id, $object_type, $context = null, $context_table = null, $context_column = null, $context_key = null ) {
		...
		$object_type_ids = $wpdb->get_col( $wpdb->prepare(
			"SELECT object_type_id FROM $object_type_table " . $where, // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			$values
		) );
		if ( !empty( $object_type_ids ) && is_array( $object_type_ids ) ) {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$wpdb->query( $wpdb->prepare(
				"DELETE FROM $index_table WHERE object_id = %d AND object_type_id IN $object_type_ids", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				intval( $object_id )
			) );
		...
		if ( WooCommerce_Product_Search_Controller::table_exists( 'object_term' ) ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$wpdb->query( $wpdb->prepare(
				"DELETE FROM $object_term_table WHERE object_id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				intval( $object_id )
			) );
		}
	}
	...
	private function delete_unused_keys() {
		...
		$index_table = WooCommerce_Product_Search_Controller::get_tablename( 'index' );
		$wpdb->query( "DELETE FROM $key_table WHERE key_id NOT IN (SELECT key_id FROM $index_table)" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
	...
	private function add_key( $key ) {
		...
		$key = function_exists( 'mb_substr' ) ?
			mb_substr( $key, 0, WooCommerce_Product_Search_Controller::MAX_KEY_LENGTH ) :
			substr( $key, 0, WooCommerce_Product_Search_Controller::MAX_KEY_LENGTH );

		$key_table = WooCommerce_Product_Search_Controller::get_tablename( 'key' );
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$key_id = $wpdb->get_var( $wpdb->prepare(
			"SELECT key_id FROM $key_table WHERE `key` = %s", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$key
		) );
		if ( !$key_id ) {
			if (
				$wpdb->query( $wpdb->prepare( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					"INSERT INTO $key_table (`key`) VALUES (%s)", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$key
				) )
			) {
	...
	private function add_index( $key_id, $object_id, $object_type_id, $count ) {
		global $wpdb;
		$index_table = WooCommerce_Product_Search_Controller::get_tablename( 'index' );
		$index_id = $wpdb->get_var( $wpdb->prepare( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			"SELECT index_id FROM $index_table WHERE key_id = %d AND object_id = %d AND object_type_id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			intval( $key_id ),
			intval( $object_id ),
			intval( $object_type_id )
		) );
		if ( !$index_id ) {
			if ( $wpdb->query( $wpdb->prepare( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				"INSERT INTO $index_table (key_id, object_id, object_type_id, count, modified) VALUES (%d, %d, %d, %d, %s)", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				intval( $key_id ),
				intval( $object_id ),
				intval( $object_type_id ),
				intval( $count ),
			...
			$wpdb->query( $wpdb->prepare( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				"UPDATE $index_table SET count = %d, modified = %s WHERE key_id = %d AND object_id = %d AND object_type_id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				intval( $count ),
				gmdate( 'Y-m-d H:i:s' ),
				intval( $key_id ),
				intval( $object_id ),
	...
	public function tokenize( $s ) {
		...
		$dashes = self::get_conservative_dashes();
		...
		foreach ( $tokens as $token ) {
			...
			if ( function_exists( 'mb_strpos' ) ) {
				...
				foreach ( $dashes as $dash ) {
					$has_dash = mb_strpos( $token, $dash ) !== false;
					if ( $has_dash ) {
						break;
					}
				}
				...
				foreach ( $dashes as $dash ) {
					$has_dash = strpos( $token, $dash ) !== false;
					if ( $has_dash ) {
						break;
					}
				}
			...
			if ( $has_dash ) {
				if ( function_exists( 'mb_ereg_replace' ) ) {
					$token_ = mb_ereg_replace( sprintf( "(%s)+", implode( '|', $dashes ) ), ' ', $token );
					$tokens_[] = $token_;
					$tokens_ = array_merge( $tokens_, explode( ' ', $token_ ) );
					$tokens_[] = mb_ereg_replace( sprintf( "(%s)+", implode( '|', $dashes ) ), '', $token );
				} else {
					$token_ = preg_replace( sprintf( "/(%s)+/u", implode( '|', $dashes ) ), ' ', $token );
					$tokens_[] = $token_;
					$tokens_ = array_merge( $tokens_, explode( ' ', $token_ ) );
					$tokens_[] = preg_replace( sprintf( "/(%s)+/u", implode( '|', $dashes ) ), '', $token );
				}
			}
		}
	...
	public static function clean( $content, $context = '' ) {
		if ( is_string( $content ) ) {
			...
			if ( !empty( $content ) ) {
				...
				wps_log_warning( 'Invalid content received.' . ( !empty( $error_message ) ? ( ' ' . esc_html( $error_message ) ) : '' ) ); // @phpstan-ignore empty.variable
			}
			$content = '';
		}
		return $content;
	...
	public static function normalize( $s ) {
		...
		if ( class_exists( 'Normalizer' ) && method_exists( 'Normalizer', 'normalize' ) ) {
			$s_normalized = Normalizer::normalize( $s );
			if ( is_string( $s_normalized ) ) {
				$s = $s_normalized;
			}
		}

		$s_word_breaks = null;
		if ( class_exists( 'IntlBreakIterator' ) && method_exists( 'IntlBreakIterator', 'createWordInstance' ) ) {
			try {
				$words = array();
				$instance = IntlBreakIterator::createWordInstance();
				if ( $instance !== null ) {
					$instance->setText( $s );
					foreach ( $instance->getPartsIterator() as $word ) {
						$words[] = $word;
					}
					...
					if ( count( $dashes ) > 0 ) {
						...
						for ( $i = 0; $i < $n; $i++ ) {
							if ( count( $stack ) === 0 ) {
								if ( ! in_array( $words[$i], $dashes ) ) {
									array_push( $stack, $words[$i] );
									continue;
								}
							} else {
								if ( in_array( $words[$i], $dashes ) ) {
									if ( ! in_array( $stack[array_key_last( $stack )], $dashes ) ) {
										array_push( $stack, $words[$i] );
									}
									continue;
								} else {
									if ( in_array( $stack[array_key_last( $stack )], $dashes ) ) {
										...
										$_words[] = implode( '', $stack );
										$stack = array( $words[$i] );
									}
								}
							}
						}
						if ( count( $stack ) > 0 && in_array( $stack[array_key_last( $stack )], $dashes ) ) {
							array_pop( $stack );
						}
						if ( count( $stack ) > 0 ) {
							$_words[] = implode( '', $stack );
						...
						if ( count( $words ) > 0 && count( $_words ) === 0 ) {
							$_words = $words;
						}

						$s_word_breaks = implode( ' ', $_words );
					} else {
	...
	public static function equalize( $s ) {
		...
		if ( is_string( $s ) ) {
			...
			$scripts = '\p{L}\p{N}';
			$unicode_scripts = self::get_unicode_scripts();
			foreach ( $unicode_scripts as $script ) {
				$scripts .= sprintf( '\p{%s}', $script );
			}

			$dashes = self::get_conservative_dashes();
			$dashbars = implode( '|', $dashes );
			$dashsec = implode( '', $dashes );

			if ( function_exists ( 'mb_ereg_replace' ) ) {

				$dashsec = mb_ereg_replace( "\u{002d}", "\\\u{002d}", $dashsec );

				$s = mb_ereg_replace( "[^\P{P}$dashsec]+", ' ', $s );

				$s = mb_ereg_replace( "[^$scripts$dashsec]++", ' ', $s );

				if ( count( $dashes ) > 0 ) {
					$s = trim( mb_ereg_replace( "(^|\s)+(\s|$dashbars)+(\s|$)+", ' ', $s ) );
				}

				$s = trim( mb_ereg_replace( '\s+', ' ', $s ) );
			} else {

				$dashsec = preg_replace( "/\u{002d}/u", "\\\u{002d}", $dashsec );

				$s = preg_replace( "/[^\P{P}$dashsec]+/u", ' ', $s );

				$s = preg_replace( "/[^$scripts$dashsec]++/u", ' ', $s );

				if ( count( $dashes ) > 0 ) {
					$s = trim( preg_replace( "/(^|\s)+(\s|$dashbars)+(\s|$)+/u", ' ', $s ) );
				}
	...
	/**
	 * Provide the set of dashes.
	 *
	 * @return string[]
	 */
	public static function get_conservative_dashes() {
		$dashes = array(
			"\u{002d}",
			"\u{00ad}",
			"\u{2010}",
		...
		$dashes = apply_filters( 'woocommerce_product_search_indexer_conservative_dashes', $dashes );
		$dashes = array_keys( array_flip( array_filter( $dashes, 'is_string' ) ) );
		return $dashes;
	}
	...
	/**
	 * Provide Unicode scripts.
	 *
	 * @since 6.3.0
	 *
	 * @return string[]
	 */
	public static function get_unicode_scripts() {

		if ( self::$unicode_scripts !== null ) {
			return self::$unicode_scripts;
		}
		$_scripts = array(
			'Arabic',
			'Armenian',
		...
		$_scripts = apply_filters( 'woocommerce_product_search_indexer_unicode_scripts', $_scripts );
		if ( !is_array( $_scripts ) ) {
			$_scripts = array();
		} else {
			$_scripts = array_unique( $_scripts );
		}

		$scripts = array();
		foreach ( $_scripts as $_script ) {
			$script = preg_replace( '/[^a-zA-Z0-9_-]/', '', $_script );
			if ( $script !== $_script ) {
				continue;
			}
			$property = sprintf( '\p{%s}', $script );
			if ( function_exists( 'mb_ereg_replace' ) ) {
				$test = mb_ereg_replace( "[$property]++", ' ', ' ' );
			} else {
				$test = preg_replace( "/[$property]++/u", ' ', ' ' );
			}
			if ( $test !== null && $test !== false ) {
				$scripts[] = $script;
			}
		}
		self::$unicode_scripts = $scripts;
		return self::$unicode_scripts;
	}
}
