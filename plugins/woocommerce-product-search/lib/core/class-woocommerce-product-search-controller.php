...
class WooCommerce_Product_Search_Controller {
	...
	public static function table_exists( $name ) {
		...
		if ( in_array( $name, self::$tables ) ) {
			...
			$exists = $wpdb->get_var( "SHOW TABLES LIKE '$table'" ) === $table; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			if ( $exists ) {
				self::$tables[] = $name;
	...
	public static function get_object_limit() {
		...
		if ( $wps_object_limit === 'AUTO' || $wps_object_limit === 'AUTOREPORT' ) {
			...
			if ( is_numeric( WPS_OBJECT_LIMIT ) ) {
				...
				if ( $limit < 0 ) { // @phpstan-ignore smaller.invalid
					$limit = 0;
				}
			} else {
				$limit = 0;
	...
	public static function setup() {
		...
		$tables[] = $key_table;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$key_table'" ) != $key_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		$tables[] = $index_table;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$index_table'" ) != $index_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$object_type_table'" ) != $object_type_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$object_term_table'" ) != $object_term_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		$tables[] = $query_table;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$query_table'" ) != $query_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		$tables[] = $uri_table;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$uri_table'" ) != $uri_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$user_agent_table'" ) != $user_agent_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		$tables[] = $hit_table;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$hit_table'" ) != $hit_table ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$queries[] =
		...
		if (
			( $wpdb->get_var( "SHOW TABLES LIKE '$key_table'" ) === $key_table ) && // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			( $wpdb->get_var( "SHOW TABLES LIKE '$index_table'" ) === $index_table ) && // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			( $wpdb->get_var( "SHOW TABLES LIKE '$object_type_table'" ) === $object_type_table ) && // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared

			( $wpdb->get_var( "SHOW TABLES LIKE '$object_term_table'" ) === $object_term_table ) // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		) {
	...
	public static function update( $previous_version ) {
		...
		if ( empty( $previous_version ) ) {
			...
			if ( $use_brands ) {
				update_option( 'woocommerce_product_search_db_brands_version', WOO_PS_PLUGIN_VERSION, false );
			}
		}
	...
	public static function update_db() {
		...
		$lock_path = untrailingslashit( WP_CONTENT_DIR ) . DIRECTORY_SEPARATOR . '.controllerlock-' . $blog_id;
		$lock_path = apply_filters( 'woocommerce_product_search_controller_lock_path', $lock_path, $blog_id );
		try {
		...
		if (
			...
			$brands_db_version = get_option( 'woocommerce_product_search_db_brands', '0' );
			if ( $brands_db_version === '0' ) {
				...
				$brands = $wpdb->get_results( $brands_query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				if ( is_array( $brands ) && count( $brands ) > 0 ) {
					$indexer = new WooCommerce_Product_Search_Indexer();

					$indexer->preprocess_terms();

					$branded_product_ids = array();
					$branded_products_query = "SELECT tr.object_id FROM $wpdb->term_relationships tr LEFT JOIN $wpdb->term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id WHERE tt.taxonomy = 'product_brand'";
					$branded_products = $wpdb->get_results( $branded_products_query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					if ( is_array( $branded_products ) && count( $branded_products ) > 0 ) {
						foreach ( $branded_products as $branded_product ) {
							$product_id = intval( $branded_product->object_id );
							if ( $product_id ) {
								$branded_product_ids[] = $product_id;
							}
						}
					}

					if ( count( $branded_product_ids ) > 0 ) {

						$query =
							"DELETE FROM $object_term_table WHERE " .
							'term_id = 0 AND ' .
							'object_id IN ( ' . implode( ',', $branded_product_ids ) . ' ) ';
						$wpdb->query( $query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

						$query =
							"DELETE FROM $object_term_table WHERE " .
							'term_id = 0 AND ' .
							'parent_object_id IN ( ' . implode( ',', $branded_product_ids ) . ' ) ';
						$wpdb->query( $query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					}
	...
	private static function cleanup_cache() {
		...
		if ( function_exists( 'wp_cache_delete' ) ) {
			...
			wp_cache_delete( 'woocommerce_product_search_db_brands_version', 'options' );
		}
	}
	...
	public static function cleanup_index() {
		global $wpdb;
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'key' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'index' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'object_type' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'object_term' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
	...
	public static function cleanup_stats() {
		global $wpdb;
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'query' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'uri' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'user_agent' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$wpdb->query( 'DROP TABLE IF EXISTS ' . self::get_tablename( 'hit' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
