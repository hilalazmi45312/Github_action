...
class WooCommerce_Product_Search_Hit {
		...
		if ( self::$queries === null || !key_exists( $query, self::$queries ) ) {
			...
			if ( $query_id !== null ) {
				...
				if ( function_exists( 'inet_pton' ) ) {
					$remote_addr = isset( $_SERVER['REMOTE_ADDR'] ) ? $_SERVER['REMOTE_ADDR'] : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					if ( $remote_addr !== null ) {
				...
				$src_uri_id = null;
				$src_uri = isset( $_SERVER['HTTP_REFERER'] ) ? $_SERVER['HTTP_REFERER'] : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				if ( $src_uri !== null ) {
				...
				$user_agent_id = null;
				$user_agent = isset( $_SERVER['HTTP_USER_AGENT'] ) ? $_SERVER['HTTP_USER_AGENT'] : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				if ( $user_agent !== null ) {
				...
				$rows = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(hit_id) FROM $hit_table WHERE $conditions", $values ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$rows = $rows !== null ? intval( $rows ) : 0;
				if ( $rows === 0 ) {
					if ( apply_filters( 'woocommerce_product_search_record_hit', true, $query, $count, $filter_args ) ) {
						if ( $wpdb->query( $wpdb->prepare( "INSERT INTO $hit_table $columns VALUES $formats", $values ) ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	...
	private static function maybe_record_uri( $uri ) {
		...
		if ( $uri !== null && is_string( $uri ) ) {
			...
			$uri = trim( $uri );
			if ( strlen( $uri ) > 0 ) {
				$uri_id = $wpdb->get_var( $wpdb->prepare( "SELECT uri_id FROM $uri_table WHERE uri = %s", $uri ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				if ( $uri_id !== null ) {
					$uri_id = intval( $uri_id );
				} else {
					if ( $wpdb->query( $wpdb->prepare( "INSERT INTO $uri_table (uri) VALUES (%s)", $uri ) ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private static function maybe_record_query( $query ) {
		...
		if ( $query !== null && is_string( $query ) ) {
			...
			if ( strlen( $query ) > 0 ) {
				...
				$query_id = $wpdb->get_var( $wpdb->prepare( "SELECT query_id FROM $query_table WHERE query = %s", $query ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				if ( $query_id !== null ) {
					$query_id = intval( $query_id );
				} else {
					if ( $wpdb->query( $wpdb->prepare( "INSERT INTO $query_table (query) VALUES (%s)", $query ) ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private static function maybe_record_user_agent( $user_agent ) {
		...
		if ( $user_agent !== null && is_string( $user_agent ) ) {
			...
			if ( strlen( $user_agent ) > 0 ) {
				$user_agent_id = $wpdb->get_var( $wpdb->prepare( "SELECT user_agent_id FROM $user_agent_table WHERE user_agent = %s", $user_agent ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				if ( $user_agent_id !== null ) {
					$user_agent_id = intval( $user_agent_id );
				} else {
					if ( $wpdb->query( $wpdb->prepare( "INSERT INTO $user_agent_table (user_agent) VALUES (%s)", $user_agent ) ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
