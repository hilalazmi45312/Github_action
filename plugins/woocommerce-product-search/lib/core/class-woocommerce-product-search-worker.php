...
class WooCommerce_Product_Search_Worker {
	...
	public static function init() {
		...
		$lock_path = apply_filters( 'woocommerce_product_search_scheduleworkerlock_path', $lock_path, $blog_id );
		try {
			...
			if ( !self::get_next_scheduled() ) {
				...
				if ( $scheduled !== false && !( $scheduled instanceof WP_Error ) ) {
					...
					if ( WPS_DEBUG ) {
						...
						} else if ( $scheduled instanceof WP_Error ) {
							$event = wp_get_scheduled_event( 'woocommerce_product_search_work' );
							if ( $event === false ) {
								wps_log_warning(
									sprintf(
										'Worker could not schedule next work cycle. [%s] %s',
