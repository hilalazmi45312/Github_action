...
class WooCommerce_Product_Search_Service {
	...
	public static function request( $query_vars ) {
		...
		if ( isset( $_REQUEST['s'] ) ) {
			$woocommerce_product_search_s = $_REQUEST['s']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		}
	...
	public static function get_s() {
		...
		$s = null;
		if ( isset( $_REQUEST['s'] ) ) {
			$s = $_REQUEST['s']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( isset( $_REQUEST['search'] ) ) {
				...
				if ( $auto_replace_rest ) {
					$s = $_REQUEST['search']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				}
			}
		}
		return $s;
	...
	/**
	 * Handled product collections (variations of the Product Collection block).
	 *
	 * @since 5.6.0
	 *
	 * @return mixed
	 */
	public static function get_handle_collections() {
		return apply_filters(
			'woocommerce_product_search_handle_product_collections',
			array(
				'woocommerce/product-collection/product-catalog' => true,
				'woocommerce/product-collection/featured' => false,
				'woocommerce/product-collection/top-rated' => false,
				'woocommerce/product-collection/on-sale' => false,
				'woocommerce/product-collection/best-sellers' => false,
				'woocommerce/product-collection/new-arrivals' => false
			)
		);
	}
	...
	public static function wp_enqueue_scripts() {
		...
		if ( !self::$scripts_registered ) {
			...
			$handle_collections = self::get_handle_collections();
			...
			wp_localize_script(
				...
				array(
					...
					'handle_collections' => $handle_collections
				)
			);
		}
		...
		if ( !self::$styles_registered ) {
			...
			$styles = array(
				...
				'product-search' => array(
					...
					'deps' => array( 'wps-selectize', 'wps-price-slider' )
				)
			);
			$styles_registered = 0;
	...
	public static function get_product_taxonomies() {
		...
		$product_taxonomies = apply_filters( 'woocommerce_product_search_service_product_taxonomies', $product_taxonomies );
		...
		if ( 'product_cat' == $taxonomy || 'product_tag' == $taxonomy ) {
			if ( !empty( $_REQUEST['ixwpss'] ) ) {
				if ( !isset( $_REQUEST[Base::SEARCH_QUERY] ) ) {
					$_REQUEST[Base::SEARCH_QUERY] = $_REQUEST['ixwpss']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				}
				$search_query = preg_replace( '/[^\p{L}\p{N}]++/u', ' ', $_REQUEST[Base::SEARCH_QUERY] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public static function get_min_max_price() {
		...
		if ( $cached_min_max !== null ) {
			...
			if ( count( $ids ) > 0 ) {
				...
				if ( $min_max_row === null ) {
					$min_max_row = $wpdb->get_row( $query, ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
