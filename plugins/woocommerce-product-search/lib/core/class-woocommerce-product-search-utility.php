...
class WooCommerce_Product_Search_Utility {
	...
	public static function get_current_url() {
		...
		$http_host = isset( $_SERVER['HTTP_HOST'] ) ? $_SERVER['HTTP_HOST'] : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	/**
	 * Sanitize the given taxonomy name.
	 *
	 * @since 6.4.0
	 *
	...
	public static function sanitize_taxonomy( $taxonomy ) {
		...
		if ( is_string( $taxonomy ) ) {
			$taxonomy = wc_sanitize_taxonomy_name( stripslashes( $taxonomy ) );
		} else {
			$taxonomy = '';
		}
		return $taxonomy;
 ...
 * @return string
 */
function woocommerce_product_search_current_url() {
	return WooCommerce_Product_Search_Utility::get_current_url();
}
