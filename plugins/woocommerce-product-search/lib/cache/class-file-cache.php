...
class File_Cache extends Cache_Base {
					...
					foreach ( $entries as $key => $entry ) {
						if ( isset( $entry['file'] ) ) {
							...
							if ( file_exists( $file_path ) ) {
								...
								if ( WPS_CACHE_DEBUG ) {
									wps_log_warning( sprintf( 'Cache file %s referenced in index %s does not exist', $file_path, $path ) );
								}
							}
						}
						if ( !$this->guardian->is_ok() ) {
		...
		if ( $path !== null ) {
			...
			if ( is_writable( $dirname ) ) {
				...
				if ( $bytes === false ) {
					$error = error_get_last();
					$msg = $error !== null && !empty( $error['message'] ) ? $error['message'] : '';
					wps_log_error( sprintf( 'Data could not be written to cache file %s [%s]', $path, $msg ) );
					$bytes = null;
				} else {
					if ( WPS_CACHE_DEBUG ) {
	 ...
	 *
	 * @param string|false $path file contents as string or false on failure
	 */
