...
class Cache_Control {
	...
	public static function init() {
		...
		$marker_threshold = isset( $options[self::MARKER_THRESHOLD] ) ? intval( $options[self::MARKER_THRESHOLD] ) : self::MARKER_THRESHOLD_DEFAULT;

		if ( defined( 'WPS_CACHE_MARKER_THRESHOLD' ) && ( is_numeric( WPS_CACHE_MARKER_THRESHOLD ) || WPS_CACHE_MARKER_THRESHOLD === null ) ) {
			$marker_threshold = WPS_CACHE_MARKER_THRESHOLD;
		}

		$marker_threshold = apply_filters( 'woocommerce_product_search_cache_control_marker_threshold', $marker_threshold );
		if ( !is_numeric( $marker_threshold ) ) {
			$marker_threshold = null;
		} else {
			$marker_threshold = max( 0, intval( $marker_threshold ) );
		}
		if ( $marker_threshold !== null ) {
