 ...
 * Filter by attribute.
 */
class WooCommerce_Product_Search_Filter_Attribute extends Filter_Renderer {
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		if ( $render_cache ) {
			...
			if ( $data !== null ) {
				...
				$results = array(
					'elements_displayed' => $data['elements_displayed'],
		...
		if (
			$context !== null &&
			!empty( $context['taxonomy_terms'] ) &&
			is_array( $context['taxonomy_terms'] )
		) {
			$taxonomy_terms = $context['taxonomy_terms'];
			if (
				isset( $taxonomy_terms[$taxonomy] ) &&
				count( $taxonomy_terms[$taxonomy] ) > 0
			) {
				$include = array();
				if ( !empty( $atts['include'] ) ) {
					if ( is_array( $atts['include'] ) ) {
						$include = $atts['include'];
					} else if ( is_string( $atts['include'] ) ) {
						$include = explode( ',', $atts['include'] );
					}
				}
				foreach ( $taxonomy_terms[$taxonomy] as $term_id ) {
					$include[] = $term_id;
				}
			...
			switch ( $params['style'] ) {
				...
				case 'dropdown' :
					$clear_output .= sprintf(
						'<div data-term="" data-taxonomy="%s" class="attribute-item-all nav-back product-search-%s-filter-item product-search-attribute-filter-item"><a href="%s" aria-label="%s">%s</a></div>',
						esc_attr( $taxonomy ),
						esc_attr( $taxonomy ),
						esc_url( $current_url ),
				...
				default :
					$clear_output .= sprintf(
						'<li data-term="" data-taxonomy="%s" class="attribute-item-all nav-back product-search-%s-filter-item product-search-attribute-filter-item"><a href="%s" aria-label="%s">%s</a></li>',
						esc_attr( $taxonomy ),
						esc_attr( $taxonomy ),
						esc_url( $current_url ),
						esc_attr__( 'Clear', 'woocommerce-product-search' ),
		...
		if ( $has_eligible_terms ) {
			...
			$params['fields'] = 'ids';
			$term_ids = Term_Control::get_term_ids( $params, $taxonomy );
		...
		$js_object .= sprintf( ',op:"%s"', esc_attr( $params['op'] ) );
		$js_object .= '}';
		...
		if ( $render_cache && is_object( $cache ) ) {
			$data['elements_displayed'] = $results['elements_displayed'];
			$data['output'] = $output;
			$cache->set( $cache_key, $data, __CLASS__, self::get_render_cache_lifetime() ); // @phpstan-ignore variable.undefined
		}
