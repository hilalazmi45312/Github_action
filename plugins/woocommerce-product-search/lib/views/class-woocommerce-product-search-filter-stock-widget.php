<?php
/**
 * class-woocommerce-product-search-filter-stock-widget.php
 *
...
/**
 * Stock product filter widget.
 */
class WooCommerce_Product_Search_Filter_Stock_Widget extends WP_Widget {
	 ...
	 * @var string cache id
	 */
	private static $cache_id = 'woocommerce_product_search_filter_stock_widget';
	...
	public static function widgets_init() {
		register_widget( 'WooCommerce_Product_Search_Filter_Stock_Widget' );
	}
	...
	public function __construct() {
		parent::__construct(
			self::$cache_id,
			__( 'Product Filter &ndash; Stock', 'woocommerce-product-search' ),
			array(
				'description' => __( 'WooCommerce Product Search &mdash; This is the live filter for products in stock in your shop. It updates the products displayed instantly, showing products in stock when the option is chosen.', 'woocommerce-product-search' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$value = isset( $instance['has_in_stock_only'] ) ? strtolower( $instance['has_in_stock_only'] ) : 'yes';
		$has_in_stock_only = $value == 'true' || $value == 'yes' || $value == '1';
		if ( $has_in_stock_only ) {
			$counts = WooCommerce_Product_Search_Filter_Stock::get_stock_counts();
			$stock = $counts['instock'] + $counts['onbackorder'];
			if ( $stock <= 0 ) { // @phpstan-ignore-line
				return;
			}
		}

		WooCommerce_Product_Search_Filter_Stock::load_resources();
		...
		$output .= WooCommerce_Product_Search_Filter_Stock::render( $instance );
		$output .= $after_widget;
	...
	public function update( $new_instance, $old_instance ) {
		...
		$settings['has_in_stock_only'] = woocommerce_product_search_input_yn( $new_instance['has_in_stock_only'] );
	...
	public function form( $instance ) {
		...
		printf(
			...
			esc_attr( $this->get_field_id( 'show_heading' ) ),
			esc_attr( $this->get_field_name( 'show_heading' ) ),
		...
		printf(
			'<input type="checkbox" id="%s" name="%s" %s />',
			esc_attr( $this->get_field_id( 'shop_only' ) ),
			esc_attr( $this->get_field_name( 'shop_only' ) ),
		...
		$has_in_stock_only = isset( $instance['has_in_stock_only'] ) ? $instance['has_in_stock_only'] : 'yes';
		echo '<p>';
		echo sprintf( '<label title="%s">', esc_attr( __( 'Display the widget only when there are any products in stock.', 'woocommerce-product-search' ) ) );
		printf(
			'<input type="checkbox" id="%s" name="%s" %s />',
			esc_attr( $this->get_field_id( 'has_in_stock_only' ) ),
			esc_attr( $this->get_field_name( 'has_in_stock_only' ) ),
			$has_in_stock_only == 'yes' ? ' checked="checked" ' : ''
		);
		echo ' ';
		echo esc_html( __( 'Show only if any products are in stock', 'woocommerce-product-search' ) );
		echo '</label>';
		...
		printf(
			...
			esc_attr( $this->get_field_name( 'filter' ) ),
...
WooCommerce_Product_Search_Filter_Stock_Widget::init();
