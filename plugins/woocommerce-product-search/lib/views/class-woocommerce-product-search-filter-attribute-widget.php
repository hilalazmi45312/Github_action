...
class WooCommerce_Product_Search_Filter_Attribute_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			self::$cache_id,
			__( 'Product Filter &ndash; Attributes', 'woocommerce-product-search' ),
			array(
	...
	public function form( $instance ) {
		...
		echo '<p>';
		echo sprintf( '<label title="%s">', esc_attr( __( 'How attributes should be displayed.', 'woocommerce-product-search' ) ) );
		...
		printf(
			...
			esc_attr( $size ),
			esc_attr__( 'One entry', 'woocommerce-product-search' )
		);
		echo '</label>';
		echo '</p>';
		echo '</div>';
		...
		if ( !isset( $woocommerce_product_search_filter_attribute_widget_settings ) ) {
			...
			$inline_script .= sprintf(
				'jQuery( document ).on( "change", ".woocommerce-product-search-filter-attribute-widget-style", function( e ) {' .
					...
					' }' .
					'var select_dropdown_only_containers = jQuery( this ).closest( ".woocommerce-product-search-filter-attribute-widget-settings" ).find( ".woocommerce-product-search-filter-attribute-widget-style-select-dropdown-only" );' .
		...
		$inline_script .= 'if ( typeof jQuery !== "undefined" ) {';
		$inline_script .= printf( 'jQuery( "#%s" ).trigger( "change" );', esc_attr( $this->get_field_id( 'style' ) ) );
		$inline_script .= '}';
