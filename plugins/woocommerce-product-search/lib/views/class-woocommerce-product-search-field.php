...
if ( !function_exists( 'woocommerce_product_search' ) ) {
	...
	function woocommerce_product_search( $atts = array() ) {
		return WooCommerce_Product_Search_Field::woocommerce_product_search( $atts ); // nosemgrep audit.php.wp.security.xss.shortcode-attr
	}
}
...
class WooCommerce_Product_Search_Field {
	...
	public static function woocommerce_product_search( $atts = array(), $content = '' ) {
		...
		$output .= sprintf( '<input type="hidden" name="wps-title" value="%d"/>', isset( $url_params['wps-title'] ) && $url_params['wps-title'] ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-excerpt" value="%d"/>', isset( $url_params['wps-excerpt'] ) && $url_params['wps-excerpt'] ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-content" value="%d"/>', isset( $url_params['wps-content'] ) && $url_params['wps-content'] ? 1 : 0 );
		...
		$output .= sprintf( '<input type="hidden" name="wps-sku" value="%d"/>', isset( $url_params['wps-sku'] ) && $url_params['wps-sku'] ? 1 : 0 );
		...
		if ($params['show_clear'] ) {
			$output .= sprintf(
				'<span title="%s" aria-label="%s" class="product-search-field-clear" style="display:none"></span>',
		...
		wp_add_inline_script( 'product-search', $inline_script ); // nosemgrep audit.php.wp.security.xss.shortcode-attr
