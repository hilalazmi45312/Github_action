...
class WooCommerce_Product_Search_Widget extends WP_Widget {
	...
	public function __construct( $id_base = null ) {
		parent::__construct(
			...
			__( 'Product Search Field', 'woocommerce-product-search' ),
			array(
	...
	public function form( $instance ) {
		...
		echo $heading_close; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		printf(
			...
			esc_attr( $height ),
			esc_attr__( 'Full content height', 'woocommerce-product-search' )
		);
		echo '</label>';
		echo '</p>';
