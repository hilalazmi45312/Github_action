...
if ( !function_exists( 'woocommerce_product_search_filter' ) ) {
	 ...
	 */
	function woocommerce_product_search_filter( $atts = array() ) {
...
class WooCommerce_Product_Search_Filter extends Filter_Renderer {
	...
	public static function init() {
		...
		if ( apply_filters( 'woocommerce_product_filter_footer_scripts', true ) ) {

			add_action( is_admin() ? 'admin_print_footer_scripts' : 'wp_print_footer_scripts', array( __CLASS__, 'footer_scripts' ), 2 );
		}
	...
	public static function shortcode( $atts = array(), $content = '' ) {
		return self::render( $atts ); // nosemgrep audit.php.wp.security.xss.shortcode-attr
	}
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		$ixwpss     = !empty( $_REQUEST['ixwpss'] ) ? sanitize_text_field( $_REQUEST['ixwpss'] ) : '';

		$current_url = woocommerce_product_search_current_url();
		$current_url = remove_query_arg( array( 'ixwpss', 'wps-title', 'wps-excerpt', 'wps-content', 'wps-categories', 'wps-attributes', 'wps-tags', 'wps-sku', 'lang', 'paged' ), $current_url );
		$href        = $current_url;
		...
		$output .= sprintf(
			...
			esc_attr( $field_id ),
			!empty( $params['placeholder'] ) ? esc_html( $params['placeholder'] ) : esc_html__( 'Search', 'woocommerce-product-search' )
		);
		$output .= sprintf(
		...
		$output .= sprintf( '<input type="hidden" name="wps-title" value="%d"/>', isset( $url_params['wps-title'] ) && $url_params['wps-title'] || !isset( $url_params['wps-title'] ) && WooCommerce_Product_Search_Service::DEFAULT_TITLE ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-excerpt" value="%d"/>', isset( $url_params['wps-excerpt'] ) && $url_params['wps-excerpt'] || !isset( $url_params['wps-excerpt'] ) && WooCommerce_Product_Search_Service::DEFAULT_EXCERPT ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-content" value="%d"/>', isset( $url_params['wps-content'] ) && $url_params['wps-content'] || !isset( $url_params['wps-content'] ) && WooCommerce_Product_Search_Service::DEFAULT_CONTENT ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-categories" value="%d"/>', isset( $url_params['wps-categories'] ) && $url_params['wps-categories'] || !isset( $url_params['wps-categories'] ) && WooCommerce_Product_Search_Service::DEFAULT_CATEGORIES ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-attributes" value="%d"/>', isset( $url_params['wps-attributes'] ) && $url_params['wps-attributes'] || !isset( $url_params['wps-attributes'] ) && WooCommerce_Product_Search_Service::DEFAULT_ATTRIBUTES ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-tags" value="%d"/>', isset( $url_params['wps-tags'] ) && $url_params['wps-tags'] || !isset( $url_params['wps-tags'] ) && WooCommerce_Product_Search_Service::DEFAULT_TAGS ? 1 : 0 );
		$output .= sprintf( '<input type="hidden" name="wps-sku" value="%d"/>', isset( $url_params['wps-sku'] ) && $url_params['wps-sku'] || !isset( $url_params['wps-sku'] ) && WooCommerce_Product_Search_Service::DEFAULT_SKU ? 1 : 0 );
		...
		if ( $params['show_clear'] ) {
			$output .= sprintf(
				'<span class="product-search-filter-search-clear" style="%s" aria-label="%s">',
				( empty( $ixwpss ) || strlen( $ixwpss ) === 0 ? 'display:none' : '' ),
				esc_attr__( 'Clear', 'woocommerce-product-search' )
			);
			$output .= esc_html__( 'Clear', 'woocommerce-product-search' );
			$output .= '</span>';
		}
		...
		} else if ( defined( 'WPS_FILTER_BLINKER_TIMEOUT' ) ) {
			if ( is_numeric( WPS_FILTER_BLINKER_TIMEOUT ) ) {
				$blinker_timeout = max( array( 0, intval( WPS_FILTER_BLINKER_TIMEOUT ) ) );
				$js_args[] = 'blinkerTimeout:' . $blinker_timeout;
			}
		}
		$js_args[] = sprintf( 'title:%d', isset( $url_params['wps-title'] ) && $url_params['wps-title'] || !isset( $url_params['wps-title'] ) && WooCommerce_Product_Search_Service::DEFAULT_TITLE ? 1 : 0 );
		$js_args[] = sprintf( 'excerpt:%d', isset( $url_params['wps-excerpt'] ) && $url_params['wps-excerpt'] || !isset( $url_params['wps-excerpt'] ) && WooCommerce_Product_Search_Service::DEFAULT_EXCERPT ? 1 : 0 );
		$js_args[] = sprintf( 'content:%d', isset( $url_params['wps-content'] ) && $url_params['wps-content'] || !isset( $url_params['wps-content'] ) && WooCommerce_Product_Search_Service::DEFAULT_CONTENT ? 1 : 0 );
		$js_args[] = sprintf( 'categories:%d', isset( $url_params['wps-categories'] ) && $url_params['wps-categories'] || !isset( $url_params['wps-categories'] ) && WooCommerce_Product_Search_Service::DEFAULT_CATEGORIES ? 1 : 0 );
		$js_args[] = sprintf( 'attributes:%d', isset( $url_params['wps-attributes'] ) && $url_params['wps-attributes'] || !isset( $url_params['wps-attributes'] ) && WooCommerce_Product_Search_Service::DEFAULT_ATTRIBUTES ? 1 : 0 );
		$js_args[] = sprintf( 'tags:%d', isset( $url_params['wps-tags'] ) && $url_params['wps-tags'] || !isset( $url_params['wps-tags'] ) && WooCommerce_Product_Search_Service::DEFAULT_TAGS ? 1 : 0 );
		$js_args[] = sprintf( 'sku:%d', isset( $url_params['wps-sku'] ) && $url_params['wps-sku'] || !isset( $url_params['wps-sku'] ) && WooCommerce_Product_Search_Service::DEFAULT_SKU ? 1 : 0 );
		...
		if ( !apply_filters( 'woocommerce_product_search_auto_toggle_filter_widgets', true ) ) {
			$safex_inline_script .= 'if ( typeof ixwpsf !== "undefined" ) { ixwpsf.autoToggleFilterWidgets = false; }';
		}
		$containers = array(
		...
		$safex_inline_script .= 'observer.observe( document, { childList : true, subtree : true } );';

		$safex_inline_script .= sprintf(
			'jQuery( document ).on("input", "#%s", function() {' .
			'var query = jQuery(this).val();' .
			'if ((query.length < %d) && (query.trim() == "")) {' .
		...
		wp_add_inline_script( 'product-filter', $inline_script ); // nosemgrep audit.php.wp.security.xss.shortcode-attr
	...
	/**
	 * Render additional footer scripts.
	 *
	 * @since 6.5.0
	 */
	public static function footer_scripts() {
		...
		if ( self::$fields > 0 ) {
			if ( apply_filters( 'woocommerce_product_filter_footer_scripts_wc_settings', true ) ) {
				if ( wp_script_is( 'wc-settings', 'enqueued' ) ) {
					wp_add_inline_script( // nosemgrep audit.php.wp.security.xss.shortcode-attr
						'wc-settings',
	...
	public static function shortcode_products( $atts = [], $content = '' ) {
		return self::render_products( $atts ); // nosemgrep audit.php.wp.security.xss.shortcode-attr
	}
		...
		if ( self::$fields === 0 && self::$filters > 0 ) {
			if ( apply_filters( 'woocommerce_product_search_filter_auto_append', true, self::$fields, self::$filters ) ) {
				...
				if ( function_exists( 'wp_is_json_request' ) ) {
					$is_json_request = wp_is_json_request();
					if ( $is_json_request ) {
						return;
					}
				}
				...
				echo self::render( array( 'style' => 'display:none!important' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
