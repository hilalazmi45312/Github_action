...
class WooCommerce_Product_Search_Filter_Rating extends Filter_Renderer {
	...
	public static function render( $atts = array(), &$results = null) {
		...
		if ( $rating ) {
			...
			$output .= sprintf(
				'<a title="%s" aria-label="%s" data-rating="" class="product-search-filter-extra rating-filter-clear %s" href="%s">%s</a>',
		...
		for ( $_rating = self::MAX_RATING; $_rating >= self::MIN_RATING; $_rating-- ) {
			...
			$output .= sprintf(
				'<a title="%s" aria-label="%s" data-rating="%d" class="product-search-filter-extra rating-filter-option %s %s" href="%s"><span class="rating-filter-star-rating rating-%d">%s</span></a>',
