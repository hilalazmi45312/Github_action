...
class WooCommerce_Product_Search_Filter_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			self::$cache_id,
			__( 'Product Filter &ndash; Search', 'woocommerce-product-search' ),
			array(
	...
	public function form( $instance ) {
		...
		printf( '<span id="%s" style="cursor:pointer">&emsp;&hellip;</span>', esc_attr( $trigger_id ) );
		echo '</h4>';

		printf( '<div id="%s">', esc_attr( $target_id ) );
