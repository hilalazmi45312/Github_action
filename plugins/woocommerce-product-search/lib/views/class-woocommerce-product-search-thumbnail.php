...
class WooCommerce_Product_Search_Thumbnail {
		...
		if ( in_array( $size, self::get_thumbnail_size_names() ) ) {
			...
			if ( !empty( $size ) && wp_attachment_is_image( $id ) ) {
				...
				if ( isset( $img_url ) ) {
					...
					list( $width, $height ) = image_constrain_size_for_editor( $width, $height, $size ); // @phpstan-ignore variable.undefined, variable.undefined ($width and $height are defined when $img_url is set)
					$result = array( $img_url, $width, $height, $is_intermediate ); // @phpstan-ignore variable.undefined ($is_intermediate is defined when $img_url is set)
	...
	public static function get_product_taxonomies() {
		...
		return \WooCommerce_Product_Search_Service::get_product_taxonomies();
	}
	...
	public static function term_thumbnail( $term, $params = array() ) {
		...
		if ( !empty( $thumbnail_id ) && is_array( $image ) && isset( $image[0] ) ) {
		...
		if ( $thumbnail_url ) {
			$data = array(
				...
				'name'             => $term->name,
				'thumbnail_width'  => $thumbnail_width, // @phpstan-ignore variable.undefined (given with $thumbnail_url)
				'thumbnail_height' => $thumbnail_height, // @phpstan-ignore variable.undefined (given with $thumbnail_url)
			...
			if ( $image_srcset && $image_sizes ) {
				$output .= sprintf(
					...
					esc_attr( $term->name ),
					esc_attr( $term->name ),
					esc_attr( $thumbnail_width ), // @phpstan-ignore variable.undefined (given with $thumbnail_url)
					esc_attr( $thumbnail_height ), // @phpstan-ignore variable.undefined (given with $thumbnail_url)
