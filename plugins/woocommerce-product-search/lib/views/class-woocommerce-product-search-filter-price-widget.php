...
class WooCommerce_Product_Search_Filter_Price_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			self::$cache_id,
			__( 'Product Filter &ndash; Price', 'woocommerce-product-search' ),
			array(
	...
	public function form( $instance ) {
		...
		printf(
			'<input type="checkbox" id="%s" name="%s" %s />',
			esc_attr( $this->get_field_id( 'slider' ) ),
			esc_attr( $this->get_field_name( 'slider' ) ),
		...
		printf(
			'<input type="checkbox" id="%s" name="%s" %s />',
			esc_attr( $this->get_field_id( 'fields' ) ),
			esc_attr( $this->get_field_name( 'fields' ) ),
