...
class WooCommerce_Product_Search_Filter_Price extends Filter_Renderer {
	...
	public static function wp_init() {
		...
		if ( isset( $_GET['min_price'] ) ) {
			if ( strlen( trim( $_GET['min_price'] ) ) === 0 ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				unset( $_GET['min_price'] );
			}
		}
		if ( isset( $_GET['max_price'] ) ) {
			if ( strlen( trim( $_GET['max_price'] ) ) === 0 ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public static function render( $atts = array(), &$results = null) {
		...
		$min_price = isset( $_REQUEST['min_price'] ) ? WooCommerce_Product_Search_Utility::to_float( $_REQUEST['min_price'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$max_price = isset( $_REQUEST['max_price'] ) ? WooCommerce_Product_Search_Utility::to_float( $_REQUEST['max_price'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( $params['slider'] ) {
			...
			$inline_script .= 'let observer = new MutationObserver(';
			$inline_script .= '( mutations, observer ) => {';
			$inline_script .= 'for ( let mutation of mutations ) {';
			$inline_script .= 'if ( mutation.type === "childList" && mutation.addedNodes.length > 0 ) {';
			$inline_script .= 'for ( let node of mutation.addedNodes ) {';
			$inline_script .= sprintf( 'jQuery( node ).find( "#%s" ).each( function( index ) {', $slider_id );
			$inline_script .= sprintf(
				'wps_price_slider.create( "%s", %d, %d, %d, %d, %d );',
				'#' . $slider_id,
				$min_max['min_price'],
			...
			$inline_script .= '}';
			$inline_script .= ');';
			$inline_script .= 'observer.observe( document, { childList : true, subtree : true } );';
		...
		/**
		 * Filter the slider limits displayed.
		 *
		 * @since 6.7.0
		 *
		 * @param array $min_max_display min/max for display
		 * @param array $min_max limits
		 *
		 * @return array min/max for display
		 */
		$min_max_display = apply_filters(
			'woocommerce_product_search_filter_price_min_max_display',
			$min_max_display,
			$min_max
		);
		...
		$output .= sprintf(
			...
			!empty( $params['min_placeholder'] ) ? esc_html( $params['min_placeholder'] ) : esc_html__( 'Min', 'woocommerce-product-search' ),
		);
		$output .= sprintf(
		...
		$output .= sprintf(
			...
			!empty( $params['max_placeholder'] ) ? esc_html( $params['max_placeholder'] ) : esc_html__( 'Max', 'woocommerce-product-search' ),
		);
		$output .= sprintf(
		...
		if ( $params['show_clear'] ) {
			$output .= sprintf(
				'<span class="product-search-filter-price-clear" style="%s" aria-label="%s">',
		...
		$dynamic_script = 'let observer = new MutationObserver(';
		$dynamic_script .= '( mutations, observer ) => {';
		$dynamic_script .= 'for ( let mutation of mutations ) {';
		$dynamic_script .= 'if ( mutation.type === "childList" && mutation.addedNodes.length > 0 ) {';
		$dynamic_script .= 'for ( let node of mutation.addedNodes ) {';
		$dynamic_script .= sprintf( 'jQuery( node ).find( "#%s .product-search-filter-price-field" ).each( function( index ) {', esc_js( $container_id ) );
		...
		$dynamic_script .= '}';
		$dynamic_script .= ');';
		$dynamic_script .= 'observer.observe( document, { childList : true, subtree : true } );';
