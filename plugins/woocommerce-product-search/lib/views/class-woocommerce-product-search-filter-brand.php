<?php
/**
 * class-woocommerce-product-search-filter-brand.php
 *
...
if ( !function_exists( 'woocommerce_product_search_filter_brand' ) ) {
	/**
	 * Renders a product brand filter which is returned as HTML and loads
	 * required resources.
	 *
	 * @param array $atts desired filter options
	 * @return string form HTML
	 */
	function woocommerce_product_search_filter_brand( $atts = array() ) {
		return WooCommerce_Product_Search_Filter_Brand::render( $atts );
	}
}
 ...
 * Filter by brand.
 */
class WooCommerce_Product_Search_Filter_Brand extends Filter_Renderer {
	...
	public static function init() {
		add_shortcode( 'woocommerce_product_filter_brand', array( __CLASS__, 'shortcode' ) );
	}
	...
	/**
	 * [woocommerce_product_filter_brand] shortcode handler.
	 *
	 * @param array $atts
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		$atts = shortcode_atts(
			array(
				...
				'include'            => null,
				'multiple'           => 'no',
				'none_selected'      => __( 'Any Brand', 'woocommerce-product-search' ),
				'number'             => null,
				'order'              => 'ASC',
				'orderby'            => 'name',
				'shop_only'          => 'no',
		...
		$n               = self::get_n();
		$container_class = '';
		$container_id    = sprintf( 'product-search-filter-brand-%d', $n );
		$heading_class   = 'product-search-filter-terms-heading product-search-filter-brand-heading';
		$heading_id      = sprintf( 'product-search-filter-brand-heading-%d', $n );
		...
		if ( $render_cache ) {
			...
			if ( $data !== null ) {
				...
				if ( $style === 'dropdown' ) {
					wp_enqueue_script( 'wps-selectize' );
					wp_enqueue_script( 'selectize-ix' );
					wp_enqueue_style( 'wps-selectize' );
				}
		...
		if ( $taxonomy === false ) {
			...
			if ( $atts['heading'] === null || $atts['heading'] === '' ) {
				if ( !empty( $taxonomy->labels ) && !empty( $taxonomy->labels->singular_name ) ) {
					$atts['heading'] = $taxonomy->labels->singular_name;
				} else {
					$atts['heading'] = $taxonomy->label;
				}
			}
		...
		if ( !empty( $atts['child_of'] ) ) {
			if ( !( $child_of_term = get_term_by( 'id', $atts['child_of'], 'product_brand' ) ) ) {
				if ( !( $child_of_term = get_term_by( 'slug', $atts['child_of'], 'product_brand' ) ) ) {
					if ( !( $child_of_term = get_term_by( 'name', $atts['child_of'], 'product_brand' ) ) ) {
						$child_of_term = null;
					}
				}
			}
		...
		if (
			...
			is_array( $_REQUEST['ixwpst'][$taxonomy] ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		) {
			$include_term_ids = array();
			foreach ( $_REQUEST['ixwpst'][$taxonomy] as $term_id ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$query_ixwpst = isset( $_GET['ixwpst'] ) && is_array( $_GET['ixwpst'] ) ? $_GET['ixwpst'] : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$aria_label = !empty( $atts['heading'] ) ? $atts['heading'] : str_replace( 'pa_', '', $taxonomy );

		$output_prefix = apply_filters(
			"woocommerce_product_search_filter_{$taxonomy}_prefix",
			sprintf(
				'<div id="%s" class="product-search-filter-terms %s %s" data-multiple="%s" aria-label="%s">',
		...
		switch ( $params['style'] ) {
			...
			default :
				...
				$open_inside_output = sprintf(
					'<ul class="product-brands product-search-filter-items product-search-filter-brand product-search-filter-%s %s%s%s">',
					esc_attr( $taxonomy ),
					esc_attr( $list_class ),
			...
			switch ( $params['style'] ) {
				...
				case 'dropdown' :
					$clear_output .= sprintf(
						'<div data-term="" class="brand-item-all nav-back product-search-%s-filter-item"><a href="%s" aria-label="%s">%s</a></div>',
						esc_attr( $taxonomy ),
						esc_url( $current_url ),
				...
				default :
					$clear_output .= sprintf(
						'<li data-term="" class="brand-item-all nav-back product-search-%s-filter-item"><a href="%s" aria-label="%s">%s</a></li>',
						esc_attr( $taxonomy ),
						esc_url( $current_url ),
						esc_attr__( 'Clear', 'woocommerce-product-search' ),
						esc_html__( 'Clear', 'woocommerce-product-search' )
		...
		if ( $has_eligible_terms ) {
			$root_class = sprintf( 'product-brands product-search-filter-items product-search-filter-brand product-search-filter-%s %s', $taxonomy, $list_class );
			if ( $params['style'] === 'dropdown' ) {
...
WooCommerce_Product_Search_Filter_Brand::init();
