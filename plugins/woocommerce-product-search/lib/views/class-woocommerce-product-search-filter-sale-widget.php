<?php
/**
 * class-woocommerce-product-search-filter-sale-widget.php
 *
...
/**
 * Sale product filter widget.
 */
class WooCommerce_Product_Search_Filter_Sale_Widget extends WP_Widget {
	 ...
	 * @var string cache id
	 */
	private static $cache_id = 'woocommerce_product_search_filter_sale_widget';
	...
	public static function widgets_init() {
		register_widget( 'WooCommerce_Product_Search_Filter_Sale_Widget' );
	}
	...
	public function __construct() {
		parent::__construct(
			self::$cache_id,
			__( 'Product Filter &ndash; Sale', 'woocommerce-product-search' ),
			array(
				'description' => __( 'WooCommerce Product Search &mdash; This is the live filter for products on sale in your shop. It updates the products displayed instantly, showing products on sale when the option is chosen.', 'woocommerce-product-search' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		WooCommerce_Product_Search_Filter_Sale::load_resources();
		...
		$output .= WooCommerce_Product_Search_Filter_Sale::render( $instance );
		$output .= $after_widget;
	...
	public function form( $instance ) {
		...
		$has_on_sale_only = isset( $instance['has_on_sale_only'] ) ? $instance['has_on_sale_only'] : 'yes';
		echo '<p>';
		echo sprintf( '<label title="%s">', esc_attr( __( 'Display the widget only when there are any products on sale.', 'woocommerce-product-search' ) ) );
		printf(
			'<input type="checkbox" id="%s" name="%s" %s />',
			esc_attr( $this->get_field_id( 'has_on_sale_only' ) ),
			esc_attr( $this->get_field_name( 'has_on_sale_only' ) ),
			$has_on_sale_only == 'yes' ? ' checked="checked" ' : ''
		);
		echo ' ';
		echo esc_html( __( 'Show only if any products are on sale', 'woocommerce-product-search' ) );
		echo '</label>';
...
WooCommerce_Product_Search_Filter_Sale_Widget::init();
