<?php
/**
 * class-woocommerce-product-search-filter-reset-widget.php
 *
 ...
 */
class WooCommerce_Product_Search_Filter_Reset_Widget extends WP_Widget {
	 ...
	 * @var string cache id
	 */
	private static $cache_id = 'woocommerce_product_search_filter_reset_widget';
	...
	public static function widgets_init() {
		register_widget( 'WooCommerce_Product_Search_Filter_Reset_Widget' );
	}
	...
	public function __construct() {
		parent::__construct(
			self::$cache_id,
			__( 'Product Filter &ndash; Reset', 'woocommerce-product-search' ),
			array(
				'description' => __( 'WooCommerce Product Search &mdash; This conveniently clears all live filters in one go. It updates the products displayed instantly.', 'woocommerce-product-search' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		WooCommerce_Product_Search_Filter_Reset::load_resources();
		...
		$output .= WooCommerce_Product_Search_Filter_Reset::render( $instance );
		$output .= $after_widget;
	...
	public function form( $instance ) {
		...
		$submit_button_label = isset( $instance['submit_button_label'] ) ? $instance['submit_button_label'] : __( 'Clear', 'woocommerce-product-search' );
		echo '<p>';
...
WooCommerce_Product_Search_Filter_Reset_Widget::init();
