...
class WooCommerce_Product_Search_Filter_Tag extends Filter_Renderer {
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		$query_ixwpst = isset( $_GET['ixwpst'] ) ? $_GET['ixwpst'] : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( isset( $_GET['ixwpst'] ) && isset( $_GET['ixwpst'][$taxonomy] ) ) {
			...
			$clear_output .= sprintf(
				'<a class="tag-item-all nav-back tag-cloud-link product-search-%s-filter-item" data-term="" href="%s" aria-label="%s">%s</a>' . "\n",
				esc_attr( $taxonomy ),
				esc_url( $current_url ),
		...
		unset( $params['list_class'] ); // @phpstan-ignore unset.offset
	...
	public static function tag_cloud( $args = '', &$results = array() ) {
		...
		if ( ! ( empty( $tags ) || is_wp_error( $tags ) ) ) {
			...
			foreach ( $tags as $key => $tag ) {
				...
				if ( !is_wp_error( $link ) ) {
					$tags[ $key ]->link = $link; // @phpstan-ignore property.nonObject
					$tags[ $key ]->id = $tag->term_id; // @phpstan-ignore property.nonObject
	...
	public static function wp_generate_tag_cloud_data( $tags_data ) {
		...
		if ( isset( $_GET['ixwpst'] ) && isset( $_GET['ixwpst'][$woocommerce_product_search_tag_cloud_data_taxonomy] ) ) {
			$current_tags = $_GET['ixwpst'][$woocommerce_product_search_tag_cloud_data_taxonomy]; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
