...
class Admin_Settings extends \WooCommerce_Product_Search_Admin_Base {
	...
	public static function save() {
		...
		if ( isset( $_POST['submit'] ) ) {
			if ( wp_verify_nonce( $_POST[self::NONCE], 'set' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public static function woocommerce_product_search() {
		...
		printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == 'general' ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_GENERAL ) ) ); // @phpstan-ignore-line
		echo esc_html( __( 'General', 'woocommerce-product-search' ) );
		echo '</a>';
		echo '|';
		echo '</li>';
		echo '<li class="tab-header">';
		printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == 'weights' ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_WEIGHTS ) ) ); // @phpstan-ignore-line
		echo esc_html( __( 'Weights', 'woocommerce-product-search' ) );
		echo '</a>';
		echo '|';
		echo '</li>';
		echo '<li class="tab-header">';
		printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == 'thumbnails' ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_THUMBNAILS ) ) ); // @phpstan-ignore-line
		echo esc_html( __( 'Thumbnails', 'woocommerce-product-search' ) );
		echo '</a>';
		echo '|';
		echo '</li>';
		echo '<li class="tab-header">';
		printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == 'css' ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_CSS ) ) ); // @phpstan-ignore-line
		echo esc_html( __( 'CSS', 'woocommerce-product-search' ) );
		echo '</a>';
		echo '|';
		echo '</li>';
		echo '<li class="tab-header">';
		printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == self::SECTION_INDEX ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_INDEX ) ) ); // @phpstan-ignore-line
		echo esc_html( __( 'Index', 'woocommerce-product-search' ) );
		echo '</a>';
		echo '|';
		echo '</li>';
		echo '<li class="tab-header">';
		printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == self::SECTION_CACHE ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_CACHE ) ) ); // @phpstan-ignore-line
		...
		if ( current_user_can( self::ASSISTANT_CONTROL_CAPABILITY ) ) {
			...
			if ( self::uses_classic_widgets() ) {
				...
				echo '</li>';
				echo '<li class="tab-header">';
				printf( '<a class="%s" href="%s">', isset( $current_section ) && $current_section == self::SECTION_ASSISTANT ? 'current' : '', esc_url( self::get_admin_section_url( self::SECTION_ASSISTANT ) ) ); // @phpstan-ignore-line
