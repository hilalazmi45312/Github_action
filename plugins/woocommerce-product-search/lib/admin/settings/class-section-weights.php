...
class Section_Weights extends \WooCommerce_Product_Search_Admin_Base {
	...
	public static function save() {
		...
		$settings->set( \WooCommerce_Product_Search::WEIGHT_TITLE, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_TITLE] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_TITLE] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_TITLE] ) : \WooCommerce_Product_Search::WEIGHT_TITLE_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->set( \WooCommerce_Product_Search::WEIGHT_EXCERPT, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_EXCERPT] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_EXCERPT] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_EXCERPT] ) : \WooCommerce_Product_Search::WEIGHT_EXCERPT_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->set( \WooCommerce_Product_Search::WEIGHT_CONTENT, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_CONTENT] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_CONTENT] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_CONTENT] ) : \WooCommerce_Product_Search::WEIGHT_CONTENT_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->set( \WooCommerce_Product_Search::WEIGHT_TAGS, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_TAGS] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_TAGS] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_TAGS] ) : \WooCommerce_Product_Search::WEIGHT_TAGS_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->set( \WooCommerce_Product_Search::WEIGHT_CATEGORIES, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_CATEGORIES] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_CATEGORIES] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_CATEGORIES] ) : \WooCommerce_Product_Search::WEIGHT_CATEGORIES_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->set( \WooCommerce_Product_Search::WEIGHT_ATTRIBUTES, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_ATTRIBUTES] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_ATTRIBUTES] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_ATTRIBUTES] ) : \WooCommerce_Product_Search::WEIGHT_ATTRIBUTES_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->set( \WooCommerce_Product_Search::WEIGHT_SKU, isset( $_POST[\WooCommerce_Product_Search::WEIGHT_SKU] ) && strlen( trim( $_POST[\WooCommerce_Product_Search::WEIGHT_SKU] ) ) > 0 ? intval( $_POST[\WooCommerce_Product_Search::WEIGHT_SKU] ) : \WooCommerce_Product_Search::WEIGHT_SKU_DEFAULT ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$settings->save();
	}
	...
	public static function render() {
		...
		echo '<p>';
		echo '<label>';
		printf( '<input name="%s" type="checkbox" %s />', esc_attr( \WooCommerce_Product_Search::USE_WEIGHTS ), $use_weights ? ' checked="checked" ' : '' );
		echo ' ';
