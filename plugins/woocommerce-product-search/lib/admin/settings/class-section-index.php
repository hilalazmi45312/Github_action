...
class Section_Index extends \WooCommerce_Product_Search_Admin_Base {
	...
	public static function save() {
		...
		if ( current_user_can( self::INDEXER_CONTROL_CAPABILITY ) ) {
			...
			$index_order = isset( $_POST[\WooCommerce_Product_Search_Indexer::INDEX_ORDER] ) ? sanitize_text_field( $_POST[\WooCommerce_Product_Search_Indexer::INDEX_ORDER] ) : \WooCommerce_Product_Search_Indexer::INDEX_ORDER_DEFAULT;
			switch( $index_order ) {
	...
	public static function render() {
		...
		printf( '%.2f', $pct ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo '</div>';
		echo '%';
		...
		if ( function_exists( 'mb_ereg_replace' ) ) {
			...
			printf(
				/* translators: link */
				esc_html__( 'The PHP Multibyte String extension (%s) is enabled.', 'woocommerce-product-search' ),
				'<a href="https://www.php.net/manual/en/book.mbstring.php">mbstring</a>'
			);
			echo '</p>';
		} else {
			echo '<p>';
			...
			printf(
				/* translators: link */
				esc_html__( 'The PHP Multibyte String extension (%s) is not enabled.', 'woocommerce-product-search' ),
				'<a href="https://www.php.net/manual/en/book.mbstring.php">mbstring</a>'
			);
			echo '</p>';
			wps_log_warning( 'The PHP Multibyte String extension (mbstring) is not enabled' );
		}
		...
		if ( class_exists( 'IntlBreakIterator' ) ) {
			...
			printf(
				/* translators: link */
				esc_html__( 'The PHP Internationalization extension (%s) is enabled.', 'woocommerce-product-search' ),
				'<a href="https://www.php.net/manual/en/book.intl.php">intl</a>'
			);
			echo '</p>';
		} else {
			echo '<p>';
			...
			printf(
				/* translators: link */
				esc_html__( 'The PHP Internationalization extension (%s) is not enabled.', 'woocommerce-product-search' ),
				'<a href="https://www.php.net/manual/en/book.intl.php">intl</a>'
			);
			echo '</p>';
			wps_log_warning( 'The PHP Internationalization extension (intl) is not enabled' );
		}
		...
		printf(
			...
			esc_attr( \WooCommerce_Product_Search_Indexer::INDEX_PER_CYCLE_DEFAULT ),
			( $can_control ? '' : ' readonly="readonly" ' )
		);
		echo '&nbsp;';
		...
		printf(
			...
			self::get_admin_section_url( self::SECTION_INDEX ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		);
