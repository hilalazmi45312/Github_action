...
class Section_General extends \WooCommerce_Product_Search_Admin_Base {
	...
	public static function save() {
		...
		if ( WPS_EXT_PDS ) {
			...
			if ( isset( $_POST[\WooCommerce_Product_Search::JSON_LIMIT] ) ) {
				if ( trim( $_POST[\WooCommerce_Product_Search::JSON_LIMIT] ) !== '' ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( $settings->get( \WooCommerce_Product_Search::AUTO_REPLACE_FORM ) ) {
			...
			foreach( $field_names as $field_name ) {
				...
				$sub = $_POST; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$n = count( $parts );
				$i = 0;
				$value = null;
	...
	public static function render() {
		...
		echo '<p>';
		echo '<label>';
		printf( '<input name="%s" type="checkbox" %s />', esc_attr( \WooCommerce_Product_Search::AUTO_REPLACE ), $auto_replace ? ' checked="checked" ' : '' );
		echo ' ';
		...
		printf(
			...
			esc_attr( \WooCommerce_Product_Search::AUTO_REPLACE_ADMIN ),
		...
		printf(
			'<input name="%s" type="checkbox" %s %s />',
			esc_attr( \WooCommerce_Product_Search::AUTO_REPLACE_JSON ),
		...
		echo ' &mdash; ';
		printf( '<label title="%s">', esc_attr__( 'Limit JSON product search results', 'woocommerce-product-search' ) );
		...
		printf(
			'<input name="%s" type="checkbox" %s %s />',
			esc_attr( \WooCommerce_Product_Search::AUTO_REPLACE_REST ),
		...
		echo '<p>';
		echo '<label>';
		printf( '<input id="wps-auto-replace-form-checkbox" name="%s" type="checkbox" %s />', esc_attr( \WooCommerce_Product_Search::AUTO_REPLACE_FORM ), $auto_replace_form ? ' checked="checked" ' : '' );
		echo ' ';
		...
		$search_widget_instance = new \WooCommerce_Product_Search_Widget( 'wps-auto-instance' );
		$search_widget_instance->_set( 1 );
		echo $search_widget_instance->form( $auto_instance ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo '</div>';
