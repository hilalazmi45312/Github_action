...
class Section_Cache_File_Cache extends \WooCommerce_Product_Search_Admin_Base {
	...
	public static function save() {
		...
		$max_files = isset( $_POST['max_files'] ) ? trim( $_POST['max_files'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$max_size = isset( $_POST['max_size'] ) ? trim( $_POST['max_size'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$min_free_disk_space = isset( $_POST['min_free_disk_space'] ) ? trim( $_POST['min_free_disk_space'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$gc_interval = isset( $_POST['gc_interval'] ) ? trim( $_POST['gc_interval'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$gc_time_limit = isset( $_POST['gc_time_limit'] ) ? trim( $_POST['gc_time_limit'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public static function render() {
		...
		printf(
			...
			esc_html__( 'Caches', 'woocommerce-product-search' ),
		...
		echo '</div>';
		echo '<div class="cache-settings-field-input">';
		printf( '<input id="%s" name="%s" type="number" value="%s" placeholder="%s" min="0"/>', 'max_files', 'max_files', esc_attr( $max_files ), esc_attr( File_Cache::MAX_FILES_DEFAULT ) );
		...
		echo '</div>';
		echo '<div class="cache-settings-field-input">';
		printf( '<input id="%s" class="measure" name="%s" type="text" value="%s" placeholder="%s" />', 'max_size', 'max_size', esc_attr( $max_size ), esc_attr( File_Cache::MAX_SIZE_DEFAULT ) );
		...
		echo '</div>';
		echo '<div class="cache-settings-field-input">';
		printf( '<input id="%s" class="measure" name="%s" type="text" value="%s" placeholder="%s" />', 'min_free_disk_space', 'min_free_disk_space', esc_attr( $min_free_disk_space ), esc_attr( File_Cache::MIN_FREE_DISK_SPACE_DEFAULT ) );
		...
		echo '</div>';
		echo '<div class="cache-settings-field-input">';
		printf( '<input id="%s" name="%s" type="number" value="%s" placeholder="%s" min="0"/>', 'gc_interval', 'gc_interval', esc_attr( $gc_interval ), esc_attr( File_Cache::GC_INTERVAL_DEFAULT ) );
		...
		echo '</div>';
		echo '<div class="cache-settings-field-input">';
		printf( '<input id="%s" name="%s" type="number" value="%s" placeholder="%s" min="%d"/>', 'gc_time_limit', 'gc_time_limit', esc_attr( $gc_time_limit ), esc_attr( File_Cache::GC_TIME_LIMIT_DEFAULT ), esc_attr( File_Cache::GC_TIME_LIMIT_MIN ) );
