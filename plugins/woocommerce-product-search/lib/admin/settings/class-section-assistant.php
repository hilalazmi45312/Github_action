...
class Section_Assistant extends \WooCommerce_Product_Search_Admin_Base {
	...
	public static function save() {
		...
		if ( current_user_can( self::ASSISTANT_CONTROL_CAPABILITY ) ) {
			...
			$sidebar_id           = !empty( $_POST['wps-assistant-sidebar-id'] ) ? sanitize_text_field( $_POST['wps-assistant-sidebar-id'] ) : null;
			$assistant_widget_ids = null;
			if ( !empty( $_POST['wps-assistant-widget-ids'] ) && is_array( $_POST['wps-assistant-widget-ids'] ) ) {
				$assistant_widget_ids = array_map( 'sanitize_text_field', $_POST['wps-assistant-widget-ids'] );
			}
			if (
				$sidebar_id !== null &&
