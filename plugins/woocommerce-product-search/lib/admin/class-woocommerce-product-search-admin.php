...
class WooCommerce_Product_Search_Admin extends WooCommerce_Product_Search_Admin_Base {
	...
	public static function wp_init() {
		...
		if (
			...
			wp_verify_nonce( $_REQUEST['nonce'], 'wps-index-js' ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		) {
			@set_time_limit( 0 );
			...
			switch( $_REQUEST['cmd'] ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				case 'start' :
	...
	public static function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
		if ( $plugin_file == plugin_basename( WOO_PS_FILE ) ) {
			...
			$plugin_meta[] = '<a style="font-weight:bold" href="https://woocommerce.com/my-account/contact-support/">' . esc_html__( 'Support', 'woocommerce-product-search' ) . '</a>';
		}
		return $plugin_meta;
	}
