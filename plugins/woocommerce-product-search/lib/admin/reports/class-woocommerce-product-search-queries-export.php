...
class WooCommerce_Product_Search_Queries_Export {
	...
	public static function wp_init() {
		if ( self::is_export_request() ) {
			if ( wp_verify_nonce( $_REQUEST['wps-queries-export'], 'export' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				self::export();
			}
		}
	}
	...
	public static function export() {
		...
		if ( !headers_sent() ) {
			...
			printf( implode( ',', $columns ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			printf( "\n" );
			...
			foreach( $rows as $row ) {
				...
				printf( implode( ',', $entry ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				printf( "\n" );
			}
			die;
		} else {
