...
class WooCommerce_Product_Search_Report_Searches extends WC_Admin_Report {
		...
		<div class="section">
			<form method="GET">
				<div>
					<?php //?>
					<div>
					<input type="text" name="search_query" value="<?php echo esc_attr( isset( $_GET['search_query'] ) ? trim ( sanitize_text_field( $_GET['search_query'] ) ) : '' ); ?>" />
					<?php
					echo '&nbsp;';
			...
			<table cellspacing="0">
				...
				if ( ! empty( $this->top_query_ids_with_results ) && is_array( $this->top_query_ids_with_results ) ) {
					foreach ( $this->top_query_ids_with_results as $query_id => $count ) {
						$query = $wpdb->get_var( $wpdb->prepare( "SELECT query FROM $query_table WHERE query_id = %d", intval( $query_id ) ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						if ( $query ) {
	...
	private function initialize_search_data_widgets() {
		...
		$this->search_query_mode  = !empty( $_REQUEST['search_query_mode'] ) ? sanitize_text_field( $_REQUEST['search_query_mode'] ) : 'startswith';
		...
		"SELECT COUNT(DISTINCT h.ip) hits, h.query_id from $hit_table h $join WHERE $where GROUP BY h.query_id ORDER BY hits DESC, h.query_id DESC LIMIT 10", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			$values
		) );
		if ( $rows !== null ) {
			foreach ( $rows as $row ) {
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
		$rows = $wpdb->get_results( $wpdb->prepare(
		"SELECT COUNT(DISTINCT h.ip) hits, h.query_id from $hit_table h $join WHERE $where_no_results GROUP BY h.query_id ORDER BY hits DESC, h.query_id DESC LIMIT 10", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			$values
		) );
		if ( $rows !== null ) {
			foreach ( $rows as $row ) {
	...
	private function initialize_search_data_chart() {
		...
		$query = $wpdb->prepare(
		"SELECT h.date, h.query_id, COUNT(DISTINCT h.ip) searches from $hit_table h $join WHERE $where GROUP BY h.date, h.query_id", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			$values
		);

		$query_no_results = $wpdb->prepare(
		"SELECT h.date, h.query_id, COUNT(DISTINCT h.ip) searches from $hit_table h $join WHERE $where_no_results GROUP BY h.date, h.query_id", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			$values
		);

		$this->queries_results = array(
			'results'    => $wpdb->get_results( $query ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			'no_results' => $wpdb->get_results( $query_no_results ) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	...
	public function get_main_chart() {
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				var order_data = jQuery.parseJSON( '<?php echo $chart_data; ?>' );<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
