...
class WooCommerce_Product_Search_Report_Queries extends WP_List_Table {
	 ...
	 * Constructor.
	 */
	public function __construct( $args = array() ) { // @phpstan-ignore-line
		...
		if ( $user_per_page !== $per_page ) {
			...
			if ( !empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-queries' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	 ...
	 * Bulk action handler.
	 */
	function process_bulk_action() { // nosemgrep scanner.php.wp.security.csrf.process-bulk-action
		$action = $this->current_action();
		switch( $action ) {
			case 'what' :
				if ( !empty( $_REQUEST['_query_id'] ) ) {
					$query_ids = $_REQUEST['_query_id']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					if ( is_array( $query_ids ) ) {
						$query_ids = array_map( 'sanitize_text_field', $query_ids );
	...
	function prepare_items() {
		...
		$search_query_mode  = !empty( $_REQUEST['search_query_mode'] ) ? sanitize_text_field( $_REQUEST['search_query_mode'] ) : 'startswith';
		...
		$start_date = !empty( $_REQUEST['start_date'] ) ? strtotime( sanitize_text_field( $_REQUEST['start_date'] ) ) : false;
		$end_date   = !empty( $_REQUEST['end_date'] ) ? strtotime( sanitize_text_field( $_REQUEST['end_date'] ) ) : false;

		$orderby = isset( $_REQUEST['orderby'] ) ? strtolower( sanitize_text_field( $_REQUEST['orderby'] ) ) : 'hits';
		switch( $orderby ) {
			case 'hits' :
			case 'query' :
			case 'query_id' :
		...
		$query = $wpdb->prepare(
			"SELECT SQL_CALC_FOUND_ROWS q.query, q.query_id, MIN(h.date) min_date, MAX(h.date) max_date, COUNT(DISTINCT h.ip) hits ". // nosemgrep audit.php.wp.security.sqli.input-in-sinks
			"FROM $query_table q " . // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			"LEFT JOIN $hit_table h ON q.query_id = h.query_id " . // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	function extra_tablenav( $which ) {
		...
		if ( $which === 'top' && !is_singular() ) {
			...
			$query_results = isset( $_REQUEST['query_results'] ) ? sanitize_text_field( $_REQUEST['query_results'] ) : self::QUERY_RESULTS_DEFAULT;
			switch ( $query_results ) {
			...
			printf(
				...
				esc_attr( self::QUERY_RESULTS_ALL ),
			...
			printf(
				...
				esc_attr( self::QUERY_RESULTS_ONLY ),
			...
			printf(
				...
				esc_attr( self::QUERY_RESULTS_NONE ),
			...
			printf(
				...
				( ! empty( $_REQUEST['start_date'] ) ? esc_attr( wp_unslash( sanitize_text_field( $_REQUEST['start_date'] ) ) ) : '' )
			);
			echo '<span>&ndash;</span>';
			printf(
				'<input type="text" size="11" placeholder="yyyy-mm-dd" value="%s" name="end_date" class="range_datepicker to" />',
				( ! empty( $_REQUEST['end_date'] ) ? esc_attr( wp_unslash( sanitize_text_field( $_REQUEST['end_date'] ) ) ) : '' )
			);
			echo '</div>';

			echo '&emsp;';

			printf( '<input type="hidden" name="page" value="%s"/>', !empty( $_REQUEST['page'] ) ? esc_attr( sanitize_text_field( $_REQUEST['page'] ) ) : '' );
			printf( '<input type="hidden" name="tab" value="%s"/>', !empty( $_REQUEST['tab'] ) ? esc_attr( sanitize_text_field( $_REQUEST['tab'] ) ) : '' );
			printf( '<input type="hidden" name="report" value="%s"/>', !empty( $_REQUEST['report'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report'] ) ) : '' );
					...
					wp_nonce_url(
						add_query_arg(
							array(
								...
								'start_date'        => ! empty( $_REQUEST['start_date'] ) ? sanitize_text_field( $_REQUEST['start_date'] ) : '',
								'end_date'          => ! empty( $_REQUEST['end_date'] ) ? sanitize_text_field( $_REQUEST['end_date'] ) : '',
