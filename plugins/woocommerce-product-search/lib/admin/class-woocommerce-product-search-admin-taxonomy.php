...
class WooCommerce_Product_Search_Admin_Taxonomy {
	...
	public static function add_form_fields( $taxonomy ) {
		...
		<div class="form-field term-thumbnail-wrap">
			<label><?php esc_html_e( 'Search Filter Thumbnail Image', 'woocommerce-product-search' ); ?></label>
			...
			<div style="line-height: 60px;">
				...
				<button type="button" class="upload_product_search_filter_image_button button"><?php esc_html_e( 'Set image', 'woocommerce-product-search' ); ?></button>
				<button type="button" class="remove_product_search_filter_image_button button"><?php esc_html_e( 'Remove image', 'woocommerce-product-search' ); ?></button>
			</div>
	...
	public static function edit_form_fields( $term, $taxonomy ) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Search Filter Thumbnail Image', 'woocommerce-product-search' ); ?></label></th>
			<td>
	...
	public static function manage_taxonomy_custom_column( $content, $column_name, $term_id ) {
		if ( $column_name === 'product_search_image' ) {
			...
			if ( $product_search_image_id ) {
				...
				if ( empty( $title ) ) {
					$title = esc_html__( 'Search Filter Thumbnail Image', 'woocommerce-product-search' );
				}
			} else {
				$url = wc_placeholder_img_src();
				$title = esc_html__( 'No Search Filter Thumbnail Image', 'woocommerce-product-search' );
			}
			$content .= sprintf(
