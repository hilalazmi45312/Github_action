...
class WooCommerce_Product_Search_Admin_Notice {
	...
	public static function admin_init() {
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			if ( !empty( $_GET[self::SHOW_WELCOME_NOTICE] ) && isset( $_GET['wps_notice'] ) && wp_verify_nonce( $_GET['wps_notice'], 'show' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( ! (
				...
				if ( !empty( $_GET[self::HIDE_WELCOME_NOTICE] ) && isset( $_GET['wps_notice'] ) && wp_verify_nonce( $_GET['wps_notice'], 'hide' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					add_user_meta( $user_id, self::HIDE_WELCOME_NOTICE, true );
				}
				if ( !empty( $_GET[self::REMIND_WELCOME_NOTICE] ) && isset( $_GET['wps_notice'] ) && wp_verify_nonce( $_GET['wps_notice'], 'later' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( !$showing_welcome ) {
				...
				if ( !empty( $_GET[self::HIDE_REVIEW_NOTICE] ) && isset( $_GET['wps_notice'] ) && wp_verify_nonce( $_GET['wps_notice'], 'hide' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					add_user_meta( $user_id, self::HIDE_REVIEW_NOTICE, true );
				}
				if ( !empty( $_GET[self::REMIND_LATER_NOTICE] ) && isset( $_GET['wps_notice'] ) && wp_verify_nonce( $_GET['wps_notice'], 'later' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( !empty( $_GET[self::CONFIRM_UPDATE] ) && isset( $_GET['wps_notice'] ) && wp_verify_nonce( $_GET['wps_notice'], 'confirm' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public static function admin_notices_welcome( $atts = null ) {
		...
		global $pagenow, $woocommerce_product_search_welcome;
		...
		$output .= 'border-color: #720eec !important;';
		...
		$output .= esc_html__( 'Welcome to WooCommerce Product Search', 'woocommerce-product-search' );
		$output .= '</h2>';
		$output .= '<p style="font-size: 1.62em;">';
		$output .= esc_html__( 'Congrats on using the best Search Experience for WooCommerce!', 'woocommerce-product-search' );
		$output .= '</p>';
		...
		$output .= wp_kses(
			sprintf(
				...
				esc_url( 'https://woocommerce.com/my-account/contact-support/' )
			),
			array( 'a' => array( 'href' => array() ) )
		);
		$output .= ' ';
		...
		$output .= '<p>';
		$output .= esc_html__( 'Your customers will love your store for this &mdash; it helps them to find the right products quickly.', 'woocommerce-product-search' );
		$output .= '</p>';
		...
		if ( WooCommerce_Product_Search_Admin::uses_classic_widgets() ) {
			...
			$output .= '<p>';
			$output .= '<strong>';
			$output .= esc_html__( 'Shall we make it easy for your customers and add live filters to your shop pages right now?', 'woocommerce-product-search' );
			$output .= '</strong>';
			$output .= '</p>';
			$output .= '<p>';
			$output .= sprintf(
				...
				esc_html__( 'Run the Assistant', 'woocommerce-product-search' )
			);
			$output .= '</p>';
			$output .= '</li>';
		}
		...
		$output .= sprintf(
			...
			esc_url( WooCommerce_Product_Search_Admin::get_admin_section_url( WooCommerce_Product_Search_Admin::SECTION_GENERAL ) ),
			esc_html__( 'Review the Settings', 'woocommerce-product-search' )
		);
		$output .= '</p>';
		$output .= '</li>';
		...
		if ( $epilogue ) {
			...
			$output .= sprintf(
				'<a class="button" href="%s">%s</a>',
				esc_url( $remind_url ),
				esc_html__( 'Remind me later', 'woocommerce-product-search' )
			);
			$output .= '&emsp;';
			$output .= sprintf(
				'<a style="color:inherit;white-space:nowrap;" href="%s">%s</a>',
				esc_url( $hide_url ),
				esc_html__( 'I\'ve got it, let\'s hide this', 'woocommerce-product-search' )
			);
			$output .= sprintf(
	...
	public static function admin_notices() {
		...
		$output .= '<p>';
		$output .= sprintf(
			esc_html__( 'Many thanks for using %1$s%2$s%3$s!', 'woocommerce-product-search' ),
			'<strong>',
			'WooCommerce Product Search',
			'</strong>'
		);
		$output .= ' ';
		$output .= esc_html__( 'Could you please take a moment and rate this extension?', 'woocommerce-product-search' );
		$output .= ' ';
		$output .= sprintf(
			'<a title="%s" style="color:inherit;white-space:nowrap;cursor:help;opacity:0.5;" href="%s">%s</a>',
			esc_attr__( 'I have already done that or do not want to submit a review.', 'woocommerce-product-search' ),
			esc_url( $hide_url ),
			esc_html__( 'Dismiss', 'woocommerce-product-search' )
		);
		$output .= '</p>';
		$output .= '<p>';
		$output .= sprintf(
			...
			esc_url( 'https://woocommerce.com/products/woocommerce-product-search/' ),
			esc_html__( 'Submit a rating', 'woocommerce-product-search' )
		);
		$output .= '&emsp;';
		$output .= sprintf(
			'<a class="button" href="%s">%s</a>',
		...
		$output .= '</p>';
		$output .= '<p>';
		$output .= esc_html__( 'Your rating helps us to provide you with an excellent product and commit to continuous improvement.', 'woocommerce-product-search' );
		$output .= ' ';
		$output .= sprintf(
			/* translators: 1 - opening tag, 2 - closing tag */
			esc_html__( 'If you would rate any aspect below 5 stars, please %1$slet us know%2$s before you submit your rating, so we can improve it.', 'woocommerce-product-search' ),
			'<a href="https://woocommerce.com/my-account/contact-support/">',
			'</a>'
		);
		$output .= '</p>';
		$output .= '</div>';
	...
	public static function update_notice() {
		...
		$output .= '<h2>';
		$output .= esc_html__( 'WooCommerce Product Search requires a Database Update &hellip;', 'woocommerce-product-search' );
		$output .= '</h2>';
		$output .= '<p>';
		$output .= esc_html__( 'The database update will run in the background and may take a while.', 'woocommerce-product-search' );
		$output .= ' ';
		$output .= esc_html__( 'During this process, product searches and filters may produce fewer or no results until the process is completed.', 'woocommerce-product-search' );
		$output .= ' ';
		$output .= esc_html__( 'We recommend to run the update during low traffic hours.', 'woocommerce-product-search' );
		$output .= '</p>';
		$output .= '<p>';
		$output .= sprintf(
			'<a class="button button-primary" href="%s">%s</a>',
			esc_url( $confirm_url ),
			esc_html__( 'Update', 'woocommerce-product-search' )
		);
		$output .= '</p>';
		$output .= '</div>';
