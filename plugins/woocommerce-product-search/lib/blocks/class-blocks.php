...
class Blocks {
	...
	public static function init() {
		...
		require_once WOO_PS_BLOCKS_LIB . '/class-product-filter-brand-block.php';
	...
	public static function wp_init() {
		...
		$trigger_asset_file = include WOO_PS_BLOCKS_LIB . '/build/trigger.asset.php';
		$trigger_dependencies = array_merge(
			$trigger_asset_file['dependencies'],
			array( 'jquery' )
		);
		wp_register_script(
			'woocommerce-product-search-trigger',
			WOO_PS_PLUGIN_URL . '/lib/blocks/build/trigger.js',
			$trigger_dependencies,
			$trigger_asset_file['version']
		);
		...
		$constants['taxonomies'] = array(
			...
			array(
				'value' => 'product_brand',
				'label' => __( 'Product Brand', 'woocommerce-product-search' )
			)
		);
