...
class Engine_Stage_Pagination extends Engine_Stage {
	...
	public function get_matching_ids( &$ids ) {
		...
		if ( $this->engine !== null ) {
			...
			$this->count = count( $ids );
			if ( $this->caching ) {
				$this->is_cache_write = $cache->set( $cache_key, $ids, self::CACHE_GROUP, $this->get_cache_lifetime() ); // @phpstan-ignore variable.undefined, variable.undefined
			}
		}
	...
	/**
	 * This is a paginating stage.
	 *
	 * {@inheritDoc}
	 *
	 * @see \com\itthinx\woocommerce\search\engine\Engine_Stage::is_paginating()
	 */
	public function is_paginating() {
		return true;
	}

	/**
	 * Provide the pre-pagination count.
	 *
	 * @return int
	 */
