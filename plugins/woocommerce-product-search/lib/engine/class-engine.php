...
class Engine {
	...
	public function process() {
		...
		foreach ( $this->stages as $stage ) {
			...
			if ( !$stage->is_paginating() ) {
				$totals[] = count( $ids );
			} else {
				if ( method_exists( $stage, 'get_total' ) ) {
					$totals[] = $stage->get_total();
				}
			}
	...
	public function get_ids() {
		...
		if ( $this->caching ) {
			...
			if ( is_array( $ids ) ) {
				...
				if ( array_key_exists( 'total', $ids ) && array_key_exists( 'ids', $ids ) ) {
					$this->total = intval( $ids['total'] );
					$ids = $ids['ids'];
		...
		if ( $this->caching ) {
			...
			$this->is_cache_write = $cache->set( $cache_key, $store, self::CACHE_GROUP, $this->get_cache_lifetime() );
		}
