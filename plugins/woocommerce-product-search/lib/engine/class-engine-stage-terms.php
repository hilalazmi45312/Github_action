...
class Engine_Stage_Terms extends Engine_Stage {
				...
				switch ( $key ) {
					case 'taxonomy':
						if ( is_string( $value ) ) {
							...
							$value = \WooCommerce_Product_Search_Utility::sanitize_taxonomy( trim( $value ) );
							if ( strlen( $value ) === 0 ) {
								$value = null;
							}
						} else {
	...
	public function get_matching_ids( &$ids ) {
		...
		$product_taxonomies = apply_filters( 'woocommerce_product_search_engine_stage_product_taxonomies', $product_taxonomies, $this );
		...
		$term_ids = array();
		if (
			in_array( $this->taxonomy, $product_taxonomies ) &&
			$this->terms !== null && count( $this->terms ) > 0
		) {
		...
		if ( count( $term_ids ) > 0 ) {
			...
			if ( $this->op === 'or' ) {
				...
				$query =
					"SELECT object_id FROM $object_term_table WHERE object_id > 0 AND object_id NOT IN " .
					'( ' .
						"SELECT object_id FROM $object_term_table " .
						'WHERE ' .
						'term_id IN ( ' . implode( ',', $term_ids ) . ' ) ' .
					...
					') ' .
					'UNION ALL ' .
					"SELECT parent_object_id AS object_id FROM $object_term_table WHERE parent_object_id > 0 AND parent_object_id NOT IN " .
					'( ' .
						"SELECT parent_object_id AS object_id FROM $object_term_table " .
						'WHERE ' .
						'term_id IN ( ' . implode( ',', $term_ids ) . ' ) ' .
