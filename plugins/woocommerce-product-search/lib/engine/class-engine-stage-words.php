...
class Engine_Stage_Words extends Engine_Stage {
	...
	public function get_matching_ids( &$ids ) {
		...
		if ( $this->attributes ) {
			...
			$object_type_ids[] = $indexer->get_object_type_id( 'product', 'brand', 'term_taxonomy', 'taxonomy', 'product_brand' );
			$variation_object_type_ids[] = $indexer->get_object_type_id( 'product_variation', 'brand', 'term_taxonomy', 'taxonomy', 'product_brand' );
		}
		...
		if ( count( $object_type_ids ) > 0 ) {
			...
			foreach ( $search_terms as $search_term ) {
				...
				if ( $length < $match_split ) {
					...
					$query = $wpdb->prepare(
						"SELECT object_id, NULL AS post_parent FROM $index_table WHERE key_id IN ( SELECT key_id FROM $key_table WHERE `key` = %s ) AND object_type_id IN ( " . implode( ',', array_map( 'intval', $object_type_ids ) ) . " ) " . // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						"UNION ALL " .
						"SELECT ID AS object_id, post_parent FROM $wpdb->posts WHERE ID IN ( SELECT object_id FROM $index_table WHERE key_id IN ( SELECT key_id FROM $key_table WHERE `key` = %s ) AND object_type_id IN ( " . implode( ',', array_map( 'intval', $variation_object_type_ids ) ) . ") )", // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$search_term,
						$search_term
					);
					if ( $this->limit !== null ) {
					...
					// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					$query = $wpdb->prepare(
						"SELECT object_id, NULL AS post_parent FROM $index_table WHERE key_id IN ( SELECT key_id FROM $key_table WHERE `key` LIKE %s ) AND object_type_id IN ( " . implode( ',', array_map( 'intval', $object_type_ids ) ) . " ) " . // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						"UNION ALL " .
						"SELECT ID AS object_id, post_parent FROM $wpdb->posts WHERE ID IN ( SELECT object_id FROM $index_table WHERE key_id IN ( SELECT key_id FROM $key_table WHERE `key` LIKE %s ) AND object_type_id IN ( " . implode( ',', array_map( 'intval', $variation_object_type_ids ) ) . ") )", // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$like,
						$like
					);
					if ( $this->limit !== null ) {
		...
		if ( !empty( $atoms ) ) {
			...
			foreach ( $atoms as $atom ) {
				...
				if ( !is_array( $results ) ) {
					$results = $wpdb->get_results( $atom['query'] ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					if ( $this->atomic_caching ) {
						if ( is_array( $results ) ) {
							$cache->set( $atomic_cache_key, $results, self::ATOMIC_CACHE_GROUP, $this->atomic_lifetime ); // @phpstan-ignore variable.undefined ($atomic_cache_key is defined if $this->atomic_caching)
						}
					}
				}
				if ( is_array( $results ) ) {
