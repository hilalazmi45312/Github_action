				...
				switch ( $operator ) {
					case 'AND':
						...
						$query = $wpdb->prepare(
							...
							"WHERE " .
							implode( ' AND ', $conditions ) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						);
						if ( $this->variations ) {
							$query .=
								" " .
								...
								"SELECT ID FROM $wpdb->posts " .
								"WHERE " .
								implode( ' AND ', $v_conditions ); // @phpstan-ignore variable.undefined ($v_conditions is defined if $this->variations)
						}
						break;
					case 'IN':
						$query =
