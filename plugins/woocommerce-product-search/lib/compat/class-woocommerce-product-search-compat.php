...
class WooCommerce_Product_Search_Compat {
	...
	public static function init() {
		...
		if ( function_exists( 'pll_current_language' ) ) {
			if ( apply_filters( 'woocommerce_product_search_compat', true, 'plugin', 'polylang' ) ) {
				require_once WOO_PS_COMPAT_LIB . '/class-woocommerce-product-search-compat-polylang.php';
			}
		}
		...
		if ( $theme instanceof WP_Theme ) {
			if ( $theme->exists() ) {
				...
				if ( $name === 'Twenty Twenty-Five' || $parent_theme === 'Twenty Twenty-Five' ) {
					if ( apply_filters( 'woocommerce_product_search_compat', true, 'theme', 'twentytwentyfive' ) ) {
						require_once WOO_PS_COMPAT_LIB . '/class-woocommerce-product-search-compat-twentytwentyfive.php';
					}
				}
			}
		}
