...
class Product_Search_Field_Control {
	...
	private static function get_request_parameters() {
		...
		$min_price  = isset( $_REQUEST[self::MIN_PRICE] ) ? \WooCommerce_Product_Search_Utility::to_float( $_REQUEST[self::MIN_PRICE] ) : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$max_price  = isset( $_REQUEST[self::MAX_PRICE] ) ? \WooCommerce_Product_Search_Utility::to_float( $_REQUEST[self::MAX_PRICE] ) : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$search_query = isset( $_REQUEST[Base::SEARCH_QUERY] ) && is_string( $_REQUEST[Base::SEARCH_QUERY] ) ? sanitize_text_field( $_REQUEST[Base::SEARCH_QUERY] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$search_query = trim( preg_replace( '/\s+/', ' ', $search_query ) );

		$limit = isset( $_REQUEST[self::LIMIT] ) && is_numeric( $_REQUEST[self::LIMIT] ) ? intval( $_REQUEST[self::LIMIT] ) : self::DEFAULT_LIMIT; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$limit = max( 0, intval( apply_filters( 'product_search_limit', min( $limit, self::MAX_LIMIT ) ) ) );

		$order = isset( $_REQUEST[self::ORDER] ) ? strtoupper( trim( sanitize_text_field( $_REQUEST[self::ORDER] ) ) ) : self::DEFAULT_ORDER;
		switch ( $order ) {
			case 'DESC' :
			case 'ASC' :
				break;
		...
		$order_by = isset( $_REQUEST[self::ORDER_BY] ) ? strtolower( trim( sanitize_text_field( $_REQUEST[self::ORDER_BY] ) ) ) : self::DEFAULT_ORDER_BY;
		switch ( $order_by ) {
			case 'date' :
			case 'title' :
			case 'ID' :
	...
	public static function get_product_categories_for_request( $params ) {
		...
		if ( count( $ids ) > 0 ) {
			...
			$cat_query =
				...
				')';
			$categories = $wpdb->get_results( $cat_query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public static function request_results() {
		...
		foreach ( $ids as $id ) {
			...
			if ( $thumbnail_id = get_post_thumbnail_id( $id ) ) {
				$image = wp_get_attachment_image_src( $thumbnail_id, \WooCommerce_Product_Search_Thumbnail::thumbnail_size_name(), false );
				if ( is_array( $image ) && isset( $image[0] ) && isset( $image[1] ) && isset( $image[2] ) ) {
			...
			if ( $product_thumbnails ) {
				if ( $thumbnail_url !== null ) {
					...
					$results[$id]['thumbnail_width']  = $thumbnail_width; // @phpstan-ignore variable.undefined (if $thumbnail_url is defined, so is this)
					$results[$id]['thumbnail_height'] = $thumbnail_height; // @phpstan-ignore variable.undefined (if $thumbnail_url is defined, so is this)
					if ( !empty( $thumbnail_alt ) ) {
		...
		if ( self::$total > $limit ) {
			$url_query_args = array(
				...
				'post_type'  => 'product',
				'ixwps'      => 1,
				'wps-title'      => $params['title'] ? 1 : 0,
				'wps-excerpt'    => $params['excerpt'] ? 1 : 0,
				'wps-content'    => $params['content'] ? 1: 0,
				'wps-categories' => $params['categories'] ? 1 : 0,
				'wps-attributes' => $params['attributes'] ? 1 : 0,
				'wps-tags'       => $params['tags'] ? 1 : 0,
				'wps-sku'        => $params['sku'] ? 1 : 0
			);
	...
	public static function single_term_title( $title ) {
		$language = isset( $_REQUEST['lang'] ) ? sanitize_text_field( $_REQUEST['lang'] ) : null;
		if ( $language !== null ) {
	...
	private static function wpml_maybe_switch_lang() {
		...
		if ( isset( $sitepress ) && is_object( $sitepress ) && method_exists( $sitepress, 'get_current_language' ) && method_exists( $sitepress, 'switch_lang' ) ) {
			if ( !empty( $_REQUEST['lang'] ) ) {
				if ( $sitepress->get_current_language() !== $_REQUEST['lang'] ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	private static function maybe_remove_wpml_query_filter() {
		...
		if ( isset( $wpml_query_filter ) ) {
			$language = !empty( $_REQUEST['lang'] ) ? sanitize_text_field( $_REQUEST['lang'] ) : null;
			if ( $language === null ) {
				$wps_removed_wpml_query_filter = remove_filter( 'posts_where', array( $wpml_query_filter, 'posts_where_filter' ), 10 );
			}
		}
	}
	...
	private static function shorten( $content, $what = 'description' ) {
		...
		if ( $max_characters > 0 ) {
			if ( function_exists( 'mb_substr' ) ) {
				...
				$output = html_entity_decode( $output, ENT_COMPAT | ENT_HTML401 );
