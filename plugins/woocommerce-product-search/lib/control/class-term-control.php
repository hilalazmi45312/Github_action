...
class Term_Control {
	...
	public static function wp_init() {
		$source = Query_Control::get_source();
		if (
			isset( $source['ixwpss'] ) ||
			isset( $source['ixwpst'] ) ||
			isset( $source['ixwpsp'] ) ||
	...
	public static function get_request_ixwpst() {
		...
		return isset( $source['ixwpst'] ) && is_array( $source['ixwpst'] ) ? $source['ixwpst'] : array();
	}
	...
	public static function get_ixwpst( $query = null ) {
		...
		$cache_context = array(
			...
			'get'      => !empty( $_GET ) ? $_GET : null, // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			'post'     => !empty( $_POST ) ? $_POST : null, // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			'ixwpst'   => $ixwpst,
			'context'  => $context
		);
	...
	public static function get_terms_args( $args, $taxonomies ) {
		...
		if ( $process_terms ) {
			...
			foreach ( $taxonomies as $taxonomy ) {
				if (
					isset( $source['ixwpsf'] ) &&
					isset( $source['ixwpsf']['taxonomy'] ) &&
					isset( $source['ixwpsf']['taxonomy'][$taxonomy] ) &&
					isset( $source['ixwpsf']['taxonomy'][$taxonomy]['filter'] )
				) {
					if ( strval( $source['ixwpsf']['taxonomy'][$taxonomy]['filter'] ) === '0' ) {
	...
	public static function get_term_ids( $args, $taxonomies ) {
		...
		if ( isset( $source['ixwpsf'] ) && isset( $source['ixwpsf']['taxonomy'] ) ) {
			foreach ( $taxonomies as $taxonomy ) {
				if ( isset( $source['ixwpsf']['taxonomy'][$taxonomy] ) ) {
					if (
						isset( $source['ixwpsf']['taxonomy'][$taxonomy]['multiple'] ) &&
						intval( $source['ixwpsf']['taxonomy'][$taxonomy]['multiple'] ) === 1
					) {
		...
		$cache_context = apply_filters( 'woocommerce_product_search_term_control_get_term_ids_cache_context', $cache_context, $args, $taxonomies );
		$cache_key = self::get_cache_key( $cache_context );
		$cache = Cache::get_instance();
		...
		if ( $hide_empty ) {
			...
			if ( $post_ids !== null ) {
				...
				Tools::int( $post_ids );
				$post_ids_condition = "ot.term_id IN ( SELECT DISTINCT term_id FROM $object_term_table WHERE object_id IN (" . implode( ',', $post_ids ) . ') )'; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			}
		}
		...
		if ( has_filter( 'woocommerce_product_search_term_control_get_term_ids_where_clauses' ) !== false ) {
			$where = apply_filters( 'woocommerce_product_search_term_control_get_term_ids_where_clauses', $where, $query_control, $params, $taxonomies );
		}
	...
	public static function get_term_counts( $taxonomy ) {
		...
		if ( count( $post_ids ) > 0 ) {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$query = $wpdb->prepare( "SELECT object_id, parent_object_id, term_id, parent_term_id, object_type, inherit FROM $object_term_table WHERE taxonomy = %s ", $taxonomy ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$query .= ' AND object_id IN (' . implode( ',', $post_ids ) . ')';
			$object_terms = $wpdb->get_results( $query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			if ( $object_terms ) {
