...
class W3ExABulkEditAjaxHandler{
	...
	private static $failsafe_active = false;
	private static $failsafe_total = 0;
	...
	public static function CallWooAction($productid,$oldpost = null,$proddata = null)
	{
		...
		do_action( 'woocommerce_update_product',  (int) $productid, $product );
		if(self::$bwoosave)
		{
			if(!empty($product) && is_object($product))
			{
				...
				if ( ! is_null( $proddata ) && isset( $proddata['product_manufacturer'] ) ) {
					$term = get_term( (int)$proddata['product_manufacturer'], 'product_manufacturer' );
					$manufacturer_slug = ( $term && ! is_wp_error( $term ) ) ? $term->slug : '';

					$product->update_meta_data( '_manufacturer_slug', sanitize_title( $manufacturer_slug ) );
	...
	public static function is_global_unique_id_unique( $product_id, $global_unique_id)
	{
		global $wpdb;
		...
		$existing_product_id = $wpdb->get_var( $query );
		
		return ( ! $existing_product_id || $existing_product_id == $product_id );
	}
	...
	public static function loadProducts( $titleparam, $catparams, $attrparams, $priceparam, $saleparam, $customparam, &$total, $ispagination, $isnext, &$hasnext, &$isbegin, $categoryor, $skuparam, $guidparam, $tagsparams, $descparam, $shortdescparam, $custsearchparam, $arrduplicate = null, $reserved = null ) {
		try {
			// Reset failsafe flags at the start of each load
			self::$failsafe_active = false;
			self::$failsafe_total = 0;
			...
			// Count active filters for failsafe auto-expansion
			$activeFilterCount = 0;
			$original_limit = null;

			if (!empty($titleparam)) $activeFilterCount++;
			if (!empty($catparams) && is_array($catparams) && count($catparams) > 0) $activeFilterCount++;
			if (!empty($attrparams) && is_array($attrparams) && count($attrparams) > 0) $activeFilterCount++;
			if (!empty($priceparam)) $activeFilterCount++;
			if (!empty($saleparam)) $activeFilterCount++;
			//if (!empty($customparam) && is_array($customparam) && count($customparam) > 0) $activeFilterCount++;
			if (!empty($skuparam)) $activeFilterCount++;
			if (!empty($tagsparams) && is_array($tagsparams) && count($tagsparams) > 0) $activeFilterCount++;
			if (!empty($descparam)) $activeFilterCount++;
			if (!empty($shortdescparam)) $activeFilterCount++;
			if (!empty($custsearchparam) && is_array($custsearchparam) && count($custsearchparam) > 0) $activeFilterCount++;

			// Check if failsafe filtering is enabled (moved earlier to be used for limit expansion)
			$apply_failsafe_filter = true; // Default to enabled for backward compatibility

			if (class_exists('W3ExABulkEdit_ConfigLoader')) {
				$apply_failsafe_filter = W3ExABulkEdit_ConfigLoader::is_failsafe_filtering_enabled();
			} else {
				// Fallback to database settings if config loader not available
				$settings = get_option('w3exabe_settings', array());
				$apply_failsafe_filter = isset($settings['failsafe_filtering_enabled']) ? $settings['failsafe_filtering_enabled'] : true;
			}

			// Auto-expand limit when 2+ filters are active and failsafe filtering is enabled
			if ($activeFilterCount >= 2 && $apply_failsafe_filter) {
				$original_limit = $LIMIT;

				// Override limit for filtered queries
				if (defined('WCABE_FAILSAFE_FETCH_LIMIT')) {
					$LIMIT = (int)WCABE_FAILSAFE_FETCH_LIMIT;
				} else {
					// Default behavior: remove limit entirely to fetch all products
					$bgettotalnumber = true; // This removes the LIMIT clause
				}

				// Log for debugging if needed
				if (defined('WCABE_FAILSAFE_DEBUG') && WCABE_FAILSAFE_DEBUG) {
					error_log("WCABE: Auto-expanding limit due to $activeFilterCount active filters. Original limit: " . ($original_limit ?? 'none') . ", New limit: " . ($bgettotalnumber ? 'unlimited' : $LIMIT));
				}
			}
			...
			if ( $arrduplicate === null ) {
				...
				if ( false === $ret ) {
					return new WP_Error( 'db_query_error',
						esc_html__( 'Could not execute query' ), $wpdb->last_error );
				}
				...
				if ( $guidparam != null && $guidparam !== "" ) {
					if ( $reserved !== null ) {
						foreach ( $reserved as $reserveitem ) {
							if ( isset( $reserveitem['action'] ) && $reserveitem['action'] === 'multiple' && $reserveitem['id'] === '_global_unique_id' ) {
								$ismultiple = true;
								break;
							}
						}
					}
					$guidquery = " INNER JOIN {$meta} meta3 ON {$posts}.ID=meta3.post_id
		                            AND CASE WHEN meta3.meta_key='_global_unique_id' THEN meta3.meta_value";
					switch ( $guidparam['value'] ) {
						case "con":
							{
								if ( $ismultiple ) {
									$searchstring = $guidparam['title'];
									$searchstring = trim( $searchstring );
									if ( $searchstring == "" )
										break;
									$arrstrings = explode( ',', $searchstring );
									...
									if ( count( $arrstrings ) > 1 ) {
										...
										foreach ( $arrstrings as $arrstring ) {
											...
											$counter ++;
											if ( $counter === 1 ) {
												$guidquery .= " LIKE '%" . $arrstring . "%'";
												continue;
											}
											$guidquery .= " OR meta3.meta_value LIKE '%" . $arrstring . "%'";
										}
									}
								} else {
									$guidquery .= " LIKE '%" . $guidparam['title'] . "%'";
								}
							}
							break;
						case "isexactly":
								...
								if ( $ismultiple ) {
									...
									if ( count( $arrstrings ) > 1 ) {
										...
										foreach ( $arrstrings as $arrstring ) {
											...
											$guidquery .= " OR meta3.meta_value = '" . $arrstring . "'";
										}
									}
								} else {
									$guidquery .= " = '" . $guidparam['title'] . "'";
								}
								...
								if ( $ismultiple ) {
									...
									if ( count( $arrstrings ) > 1 ) {
										...
										foreach ( $arrstrings as $arrstring ) {
											...
											$counter ++;
											if ( $counter === 1 ) {
												$guidquery .= " NOT LIKE '%" . $arrstring . "%'";
												continue;
											}
											$guidquery .= " AND meta3.meta_value NOT LIKE '%" . $arrstring . "%'";
										}
									}
								} else {
									$guidquery .= " NOT LIKE '%" . $guidparam['title'] . "%'";
								}
                ...
                foreach ( $custsearchparam as $custitem ) {
                    $innercounter ++;
                    if ( isset( $custitem['id'] ) && $custitem['id'] === 'post_author' ) {
                        $author_ids = array_map('intval', $custitem['array']);
                        $shortdesclike = " AND {$posts}.post_author IN (" . implode(',', $author_ids) . ") ";
                        continue;
                    }
...
///////////////////////////////////////
				...
				if ( $bgettotalnumber ) {
					...
					$query = "INSERT INTO {$temptable} (
								...
								FROM {$posts}
								{$catsquery}{$pricequery}{$salequery}{$skuquery}{$guidquery}{$custommetasearch}
				...
				if ( $catsquery === '' ) {//let's get products without product_type'
					if ( $bgettotalnumber ) {
						...
						$query = "INSERT INTO {$temptable} (ID, type, post_parent, useit)
                                    SELECT
                                    {$posts}.ID, 0 AS type, 0 AS post_parent,0 as useit
                                    FROM {$posts}{$pricequery}{$salequery}{$skuquery}{$guidquery}{$custommetasearch}
                                    WHERE 
				...
				if ( (int) $ret > $LIMIT ) {
					...
					if ( $minid !== null && $maxid !== null ) {
						if ( $ispagination ) {
							if ( $isnext && $maxused !== "" ) {
								$idlimitquery = " AND p1.ID < {$maxused}";
							} else {
								if ( $minused == "" )
									$idlimitquery = " AND p1.ID > {$minused}";
							}
						}
				...
				if ( $guidquery != "" && $bgetvariations ) {
					$guidquery = " INNER JOIN {$meta} meta ON p1.ID=meta.post_id
		                            AND CASE WHEN meta.meta_key='_global_unique_id' THEN meta.meta_value";
					switch ( $guidparam['value'] ) {
						case "con":
							{
								if ( $ismultiple ) {
									...
									if ( count( $arrstrings ) > 1 ) {
										...
										foreach ( $arrstrings as $arrstring ) {
											...
											$guidquery .= " OR meta.meta_value LIKE '%" . $arrstring . "%'";
										}
									}
								} else {
									$guidquery .= " LIKE '%" . $guidparam['title'] . "%'";
								...
								if ( $ismultiple ) {
									...
									if ( count( $arrstrings ) > 1 ) {
										...
										foreach ( $arrstrings as $arrstring ) {
											...
											$guidquery .= " OR meta.meta_value = '" . $arrstring . "'";
										}
									}
								} else {
									$guidquery .= " = '" . $guidparam['title'] . "'";
								...
								if ( $ismultiple ) {
									...
									if ( count( $arrstrings ) > 1 ) {
										...
										foreach ( $arrstrings as $arrstring ) {
											...
											$guidquery .= " AND meta.meta_value NOT LIKE '%" . $arrstring . "%'";
										}
									}
								} else {
									$guidquery .= " NOT LIKE '%" . $guidparam['title'] . "%'";
					...
					if ( $attrsquery != "" ) {
						{
							$query = "INSERT INTO {$temptable}(
                                        SELECT p1.ID, 1 AS type,p1.post_parent,0 AS useit
                                        FROM {$posts} p1{$attrsquery}{$skuquery}{$guidquery}
                                        WHERE (p1.post_type='product_variation'){$idlimitquery} ORDER BY p1.ID DESC {$limitquery}
                                        )";
							$ret   = $wpdb->query( $query );
							if ( is_wp_error( $ret ) ) {
						...
						foreach ( $custsearchparam as $custitem ) {
							if ( isset( $custitem['type'] ) && $custitem['type'] === 'attribute' ) {
								...
								}
								{
									...
									$notin     = "NOT";
									$id        = "p1.ID";
									if ( $skuquery !== "" || $guidquery !== "" || $titlelike !== "" || $desclike !== "" || $shortdesclike !== "" ) {
										$notin = "";
										$id    = "p1.post_parent";
									}
									$query = "INSERT INTO {$temptable}(
		...
		// FAIL-SAFE FILTER: Apply secondary verification of filters
		// This ensures all filters are properly applied to the result set
		if (!defined('WCABE_DISABLE_FAILSAFE_FILTER') || !WCABE_DISABLE_FAILSAFE_FILTER) {

			// Load the fail-safe filter class
			$failsafe_file = __DIR__ . '/includes/class-failsafe-filter.php';
			if (file_exists($failsafe_file)) {
				require_once($failsafe_file);

				// Check if we should enable debug mode
				$debug_mode = defined('WCABE_FAILSAFE_DEBUG') && WCABE_FAILSAFE_DEBUG;

				// Recalculate active filter count if not already set
				if (!isset($activeFilterCount)) {
					$activeFilterCount = 0;
					if (!empty($titleparam)) $activeFilterCount++;
					if (!empty($catparams) && is_array($catparams) && count($catparams) > 0) $activeFilterCount++;
					if (!empty($attrparams) && is_array($attrparams) && count($attrparams) > 0) $activeFilterCount++;
				...
				// Note: $apply_failsafe_filter is already set earlier in the function (around line 835)
				// Load config loader for debug mode if needed
				...
				$debug_mode = false;
				if (class_exists('W3ExABulkEdit_ConfigLoader')) {
					$debug_mode = W3ExABulkEdit_ConfigLoader::is_failsafe_debug_enabled();
				}

				if ($apply_failsafe_filter && $activeFilterCount >= 2) {
					// Initialize the fail-safe filter (only for multi-filter scenarios)
					$failsafeFilter = new W3ExABulkEdit_FailsafeFilter($debug_mode);

					// Prepare filter parameters for the fail-safe check
					$filterParams = [
						'title' => $titleparam,
						...
						'short_description' => $shortdescparam,
						'custom_search' => $custsearchparam,
						'category_or' => $categoryor
					];

					// Apply fail-safe filtering
					$originalCount = count($info);
					$info = $failsafeFilter->applyFilters($info, $filterParams);
					$filteredCount = count($info);

					// Log discrepancies for debugging and monitoring
					if ($originalCount != $filteredCount) {
						$discrepancy_msg = sprintf(
							"WCABE Failsafe Filter: Adjusted results from %d to %d products. Filters: %s",
							$originalCount,
							$filteredCount,
							json_encode($filterParams)
						);

						error_log($discrepancy_msg);

						// Check if notices should be shown using config loader
						$show_notices = false;
						if (class_exists('W3ExABulkEdit_ConfigLoader')) {
							$show_notices = W3ExABulkEdit_ConfigLoader::should_show_notices();
						} elseif (defined('WCABE_FAILSAFE_SHOW_NOTICES')) {
							$show_notices = WCABE_FAILSAFE_SHOW_NOTICES;
						}

						// Optionally store for admin notice
						if ($show_notices) {
							update_option('wcabe_failsafe_last_discrepancy', [
				...
				// Restore pagination if we auto-expanded the limit
				if (isset($original_limit) && $original_limit !== null && $activeFilterCount >= 2) {
					// Initialize filteredCount if not already set (in case failsafe filter wasn't applied)
					if (!isset($filteredCount)) {
						$filteredCount = count($info);
					}

					// Set failsafe flags for the AJAX response
					// This indicates that all products were loaded at once due to failsafe filtering
					if ($apply_failsafe_filter) {
						self::$failsafe_active = true;
						self::$failsafe_total = $filteredCount;
					}

					// Update total count to reflect filtered results
					$total = $filteredCount;

					// Apply original pagination limit to filtered results
					if ($ispagination && count($info) > $original_limit) {
						// Keep only the number of products specified by original limit
						// The -1 is because the original LIMIT is incremented by 1 to check for more results
						$info = array_slice($info, 0, $original_limit - 1);
						$hasnext = ($filteredCount > $original_limit - 1);

						if (defined('WCABE_FAILSAFE_DEBUG') && WCABE_FAILSAFE_DEBUG) {
							error_log("WCABE: Restored pagination. Filtered count: $filteredCount, Showing: " . count($info) . " products, Has next: " . ($hasnext ? 'yes' : 'no'));
						}
					}
				}
			}
	...
	{
		/**
		 * Fires before bulk save operations begin.
		 * Useful for setting constants like WP_IMPORTING to prevent reindexing.
		 */
		do_action('wcabe_before_bulk_save');
								...
								if($sel_fields[$i]['type'] === 'customh')
								{
									if(taxonomy_exists($i))
									{
										...
										if ( $i === 'product_manufacturer' ) {
											$term = get_term( (int)$Row, 'product_manufacturer' );
											$manufacturer_slug = ( $term && ! is_wp_error( $term ) ) ? $term->slug : '';
											if ( $theproduct ) {
												$theproduct->update_meta_data( '_manufacturer_slug', sanitize_title( $manufacturer_slug ) );
											} else {
												update_post_meta( $ID, '_manufacturer_slug', sanitize_title( $manufacturer_slug ) );
											}
											continue;
										}
						...
						case "_global_unique_id":
							{
								...
								{
									try{
										if ( self::is_global_unique_id_unique( $ID, $Row ) ) {
											$theproduct->update_meta_data( '_global_unique_id', $Row );
										} else {
											if (self::$debugmode) {
												wcabe_log("The GTIN/UPC/EAN/ISBN value $Row for product $ID is diplicate, so it was not saved.", 'gtin');
											}
										}
										$theproduct->update_meta_data( '_global_unique_id', $Row );
									}catch(Exception $e) {
										$newvar = new stdClass();
										$newvar->ID = (string)$ID;
										$newvar->_global_unique_id = '';
										$retarray[] = $newvar;
									}
								}else
								{
									$save_guid = wc_clean($Row);
									
									if ($save_guid === '')
									{
										update_post_meta($ID, '_global_unique_id', '');
									}else
									{
										$isunique = self::is_global_unique_id_unique($ID, $save_guid);
										if($isunique)
										{
											update_post_meta( $ID, '_global_unique_id', $save_guid );
										}else
										{
											$newvar = new stdClass();
											$newvar->ID = (string)$ID;
...
//										if($slug != $Row)
		...
		/**
		 * Fires after all bulk save operations complete.
		 * Useful for cleanup after setting WP_IMPORTING.
		 */
		do_action('wcabe_after_bulk_save');
	...
	public static function addProducts($prodcount)
	{
		...
		for($i = 0; $i < $prodcount; $i++)
		{
			...
			do_action( 'woocommerce_new_product', $post_id, wc_get_product($post_id) );
	...
	public static function duplicateProducts(&$data,$count=1)
	{
		...
		self::loadProducts(null,null,null,null,null,null,$total,false,false,$hasnext,$isbegin,false,null,null,null,null,null,null,$retarray);
		return $retarray;
	}
	...
	public static function duplicate_post_meta( $id, $new_id, &$postobject) 
	{
		...
		$post_meta_infos = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value FROM $wpdb->postmeta WHERE post_id=%d AND meta_key NOT IN ( 'total_sales', '_sku', '_global_unique_id' );", absint( $id ) ) );
	...
	public static function LoadProductsFields(&$dataids,&$retarray,$customparam = NULL)
	{
		...
		self::loadProducts(null,null,null,null,null,$customparam,$total,false,false,$hasnext,$isbegin,false,null,null,null,null,null,null,$retarray);
		return true;
	}
    ...
    public static function ajax()
    {
		...
		switch($type){
			...
			case 'loadproducts':
			{
				...
				$guidparam = NULL;
				if(isset($_POST['guidparam']))
				   $guidparam = $_POST['guidparam'];
				$tagsparams = NULL;
				...
				$ret = self::loadProducts($titleparam,$catparams,$attrparams,$priceparam,$saleparam,$customparam,$total,$ispagination,$isnext,$hasnext,$isbegin,$categoryor,$skuparam,$guidparam,$tagsparams,$descparam,$shortdescparam,$custsearchparam,NULL,$reserved);
				...
				// Add flag when Failsafe filtering is active with 2+ filters
				// This helps the frontend display appropriate pagination info
				if (self::$failsafe_active) {
					$arr['failsafe_active'] = true;
					$arr['failsafe_total'] = self::$failsafe_total;
				}
			}break;
			case 'getdebuginfo':
			{
			...
			case 'loadgroupedproducts':
			{
				...
				self::loadProducts(null,null,null,null,null,null,$total,false,false,$hasnext,$isbegin,false,null,null,null,null,null,null,$arrgrouped);
			...
			case 'savesettings':
			{
				...
				if(is_array($curr_settings))
				{
					...
					if(isset($data['show_guid']))
						$curr_settings['show_guid'] = $data['show_guid'];
					...
					if(isset($data['disable_update_check'])) {
						$curr_settings['disable_update_check'] = $data['disable_update_check'];
					}
