...
if ( ! class_exists( 'WFACP_Compatibility_With_Astra' ) ) {
	...
	class WFACP_Compatibility_With_Astra {
		...
		public function internal_css() {
			...
			$cssHtml .= "body:not(.cartflows-canvas):not(.cartflows-default):not(.wfacp_pre_built) .woocommerce form .form-row label:not(.checkbox):not(.woocommerce-form__label-for-checkbox){opacity: inherit;max-width: 100%;position: relative;margin: 0;padding: 0;white-space: inherit;line-height: 1.5;}";
		...
		public function remove_script() {
			if ( ! class_exists( 'WFACP_Common' ) || WFACP_Common::get_id() == 0 || !is_checkout()) {
				return;
			}
			?>
            <script>
