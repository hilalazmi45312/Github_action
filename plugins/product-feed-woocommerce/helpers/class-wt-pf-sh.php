 ...
 * @link
 *
 * @package Webtoffee_Product_Feed_Sync_Pro\Helpers\SecurityHelper
 */
		...
		public static function verify_nonce( $plugin_id, $nonce_id = '' ) {
			...
			if (!$nonce || !(wp_verify_nonce($nonce, $nonce_id))) { //verifying nonce
				if (defined('DOING_AJAX') && DOING_AJAX) {
					wp_send_json_error([
						'status' => 0,
						'msg' => esc_html__('Access denied.', 'product-feed-woocommerce')
					], 403);
				} else {
					wp_die(esc_html__('Access denied.', 'product-feed-woocommerce'), '', ['response' => 403]);
				}
            	exit;
			}
		    return true;
			...
			if (!$is_allowed) {
				if (defined('DOING_AJAX') && DOING_AJAX) {
					wp_send_json_error(array(
						'status' => 0,
						'msg' => esc_html__('You do not have sufficient permissions to perform this operation.', 'product-feed-woocommerce')
					), 403);					
				} else {
					wp_die(esc_html__('You do not have sufficient permissions to perform this operation.', 'product-feed-woocommerce'), '', ['response' => 403]);
				}
	    		exit;
	    	}
