<?php
/**
 * Handles the Bing export actions.
 *
 * @package   Webtoffee_Product_Feed_Sync_Pro\Admin\Modules\Bing
 * @version   1.0.0
 */
...
if ( ! class_exists( 'Webtoffee_Product_Feed_Sync_Pro_Bing_Export' ) ) {
	...
	/**
	 * Class for handling Bing product feed export functionality
	 */
	class Webtoffee_Product_Feed_Sync_Pro_Bing_Export extends Product_Feed_For_Woocommerce_Product {
		...
		public function prepare_data_to_export( $form_data, $batch_offset, $step ) {
			...
			if ( $batch_offset < $export_limit ) {
				...
				foreach ( $products_ids as $key => $product_id ) {
					...
					if ( $product->is_type( 'variation' ) && ! empty( $prod_categories ) ) {

						$parent_id = $product->get_parent_id();
						if ( has_term( $prod_categories, 'product_cat', $parent_id ) ) {
							continue;
						}
					}
		 ...
		 * Generate row data
		 *
		 * @param type $product_object Product.
		 * @return array
		 */
		...
		public function shipping( $catalog_attr, $product_attr, $export_columns, $key = '' ) {
			...
			if ( isset( $this->form_data['advanced_form_data']['wt_pf_file_as'] ) && 'xml' === $this->form_data['advanced_form_data']['wt_pf_file_as'] ) {
				/**
				 * Filters the Bing product shipping XML.
				 *
				 * @since 1.0.0
		...
		/**
		 * Get shipping information with country and price for Bing
		 *
		 * @param string $catalog_attr Catalog attribute.
		 * @param string $product_attr Product attribute.
		 * @param array  $export_columns Export columns.
		 * @param string $key Optional shipping key.
		 * @return string|array Shipping information with country and price.
		 */
		public function shipping_cp( $catalog_attr, $product_attr, $export_columns, $key = '' ) {
			...
			/**
			 * Filters the Bing product shipping CP.
			 *
			 * @since 1.0.0
			 * @param string     $shipping_str     Formatted shipping string.
			 * @param array      $shipping_details Raw shipping details.
		...
		/**
		 * Get shipping information with country, service and price for Bing
		 *
		 * @param string $catalog_attr Catalog attribute.
		 * @param string $product_attr Product attribute.
		 * @param array  $export_columns Export columns.
		 * @param string $key Optional shipping key.
		 * @return string|array Shipping information with country, service and price.
		 */
		public function shipping_csp( $catalog_attr, $product_attr, $export_columns, $key = '' ) {
			...
			/**
			 * Filters the Bing product shipping CSP.
			 *
			 * @since 1.0.0
			 * @param string     $shipping_str     Formatted shipping string.
			 * @param array      $shipping_details Raw shipping details.
