				...
				<tr class="treegrid-1 ">
					...
					<td><!--suppress HtmlUnknownAttribute -->
						...
						<select id= "cat_mapping_<?php echo esc_attr( $cat->term_id ); ?>" name="map_to[<?php echo esc_attr( $cat->term_id ); ?>]">
								...
								echo wp_kses( wt_fb_feed_category_dropdown(), $allowed_tags );
								?>
							</select>
					</td>
				</tr>
...
if ( ! function_exists( 'wt_fb_feed_category_dropdown' ) ) {
	...
	function wt_fb_feed_category_dropdown( $selected = '' ) {
		...
		$category_dropdown = wp_cache_get( 'iiit_fbfeed_dropdown_product_categories' );
		...
		if ( false === $category_dropdown ) {
			...
			wp_cache_set( 'iiit_fbfeed_dropdown_product_categories', $category_dropdown, '', WEEK_IN_SECONDS );
		}
...
<div class="wt-wrap">
	...
	<?php
	esc_html_e(
		'Map WooCommerce categories with Facebook categories.',
		'product-feed-woocommerce'
	);
	?>
	</h4>
	<span>Facebook has a pre-defined set of <a target="_blank" href="https://www.facebook.com/products/categories/en_US.txt">categories</a>. It is important that you map the categories defined within your store with the Facebook categories respectively so that the products will be synched accordingly. Everytime we come across a new category that has not been mapped prior we will produce it in the below section for you to verify. You can always edit the prior mapping under the respective <a target="_blank" href="<?php echo esc_url( admin_url( 'edit-tags.php?taxonomy=product_cat&post_type=product' ) ); ?>">categories</a>.</span>
