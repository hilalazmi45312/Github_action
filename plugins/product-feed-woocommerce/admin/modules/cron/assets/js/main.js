var wt_productfeed_cron_js = ( function ( $ ) {
    ...
    var wt_productfeed_cron_js =
	{
	    ...
	    {
		jQuery( '.wt_productfeed_cron_url' ).on( 'click', function () {
		    window.prompt( wt_productfeed_cron_params.msgs.use_url, jQuery( this ).attr( 'data-href' ) );
		} );
	    },
	    reg_delete: function ()
	    {
		jQuery( '.wt_productfeed_delete_cron' ).on( 'click', function () {
	...
	{
		jQuery('.wt_productfeed_update_schedule').off('click').on('click', function(){
		...
		jQuery.ajax( {
		    ...
		    success: function ( response )
		    {
				...
				if(response.data.id) {
					...
					jQuery( '[name="wt_productfeed_cron_interval"]' ).off( 'click' ).on( 'click', function () {
						var vl = jQuery( this ).val();
						wt_productfeed_cron.toggle_interval_fields( vl );
					} );

					jQuery( '[name="wt_productfeed_schedule_type"]' ).off( 'click' ).on( 'click', function () {
						wt_productfeed_cron.hide_url_cron_fields();
					} );
		...
		{
			cron_settings_el.find('.wt_productfeed_sub_tab li').on('click', function(){
		...
		jQuery('.wt_productfeed_field_group_hd').off('click').on('click', function(){
