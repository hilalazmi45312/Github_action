<?php
/**
 * Handles the Tiktok export actions.
 *
 * @package   Webtoffee_Product_Feed_Sync_Pro\Admin\Modules\Tiktok
 * @version   1.0.0
 */
...
if ( ! class_exists( 'Webtoffee_Product_Feed_Sync_Pro_Tiktok_Export' ) ) {
	/**
	 * Webtoffee_Product_Feed_Sync_Pro_Tiktok_Export Class.
	 */
	class Webtoffee_Product_Feed_Sync_Pro_Tiktok_Export extends Product_Feed_For_Woocommerce_Product {
		/**
		 ...
		 * Prepare CSV header
		 *
		 * @return array Export columns.
		 */
		public function prepare_header() {

			$export_columns = $this->parent_module->get_selected_column_names();
			/**
			 * Filter the export columns for CSV header.
			 *
			 * @since 1.0.0
			 *
		...
		public function prepare_data_to_export( $form_data, $batch_offset, $step ) {
			...
			/**
			 * Filter the batch count limit per request.
			 *
			 * @since 1.0.0
			 *
			 * @param int $batch_count Batch count limit.
			 */
			...
			if ( $batch_offset < $export_limit ) {
				...
				/**
				 * Filter the product catalog query arguments.
				 *
				 * @since 1.0.0
				 *
		...
		protected function generate_row_data_wc_lower( $product_object ) {
			...
			/**
			 * Filter the batch product feed row data.
			 *
			 * @since 1.0.0
			 *
		...
		* @return string
		...
		public function google_product_category( $catalog_attr, $product_attr, $export_columns ) {
				...
				foreach ( $category_path as $category ) {
					...
					if ( $tiktok_category_id ) {
						...
						if ( false === $tiktok_category_list ) {
							$tiktok_category_list            = Webtoffee_Product_Feed_Sync_Pro_Tiktok::get_category_array();
			...
			/**
			 * Filter the product TikTok category.
			 *
			 * @since 1.0.0
			 *
			 * @param string $tiktok_product_category Product TikTok category.
			 * @param object $product Product object.
			 */
		...
		public function parent_link( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the product parent URL.
			 *
			 * @since 1.0.0
			 *
			 * @param string $link Product parent URL.
			 * @param object $product Product object.
			 */
		...
		public function canonical_link( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the product canonical URL.
			 *
			 * @since 1.0.0
			 *
			 * @param string $canonical_link Product canonical URL.
			 * @param object $product Product object.
			 */
		...
		public function image_link( $catalog_attr, $product_attr, $export_columns ) {
			...
			if ( $this->product->is_type( 'variation' ) ) {
				...
				if ( has_post_thumbnail( $this->product->get_id() ) ) {
					...
					if ( is_array( $get_image ) && isset( $get_image[0] ) ) {
						$image    = self::wt_feed_get_formatted_url( $get_image[0] );
					}
				}
		...
		public static function get_product_gallery( $product ) {
			...
			if ( $attachment_ids && is_array( $attachment_ids ) ) {
				...
				foreach ( $attachment_ids as $attachment_id ) {
					$img_urls[ $m_key ] = Wt_Pf_Catalog_Export_Helper::feed_get_formatted_url( wp_get_attachment_url( $attachment_id ) );
					$m_key++;
				}
			}
		 ...
		 * @param array $product_attr Product attributes.
		 * @param array $export_columns Export columns.
		 * @param string  $additional_img Additional images.
		 * @return string
		 */
		...
		public function gender( $catalog_attr, $product_attr, $export_columns ) {
			...
			if ( '' == $gender ) {
				$gender = get_post_meta( $this->product->get_id(), '_wt_tiktok_gender', true );
			}
			/**
			 ...
			 * @param object $product Product object.
			 */
			return apply_filters( 'wt_feed_tiktok_product_gender', $gender, $this->product );
		}
		/**
		 * Get product size.
		 *
		...
		public function size( $catalog_attr, $product_attr, $export_columns ) {
			...
			if ( '' == $size ) {
				$size = get_post_meta( $this->product->get_id(), '_wt_tiktok_size', true );
			}
			/**
			 ...
			 * @param object $product Product object.
			 */
			return apply_filters( 'wt_feed_tiktok_product_size', $size, $this->product );
		}
		/**
		 * Get product color.
		 *
		...
		public function color( $catalog_attr, $product_attr, $export_columns ) {
			...
			if ( '' == $color ) {
				$color = get_post_meta( $this->product->get_id(), '_wt_tiktok_color', true );
			}
			/**
			 ...
			 * @param object $product Product object.
			 */
			return apply_filters( 'wt_feed_product_tiktok_color', $color, $this->product );
		}
		/**
		...
		* @return string
		...
		public function multipack( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the product is multipack.
			 *
			 * @since 1.0.0
			 *
			 * @param string $multi_pack Product is multipack.
			 * @param object $product Product object.
			 */
		...
		public function item_group_id( $catalog_attr, $product_attr, $export_columns ) {
			 ...
			 * @since 1.0.0
			 *
			 * @param int $id Product item group ID.
			 * @param object $product Product object.
			 */
		...
		public function availability( $catalog_attr, $product_attr, $export_columns ) {
			 ...
			 * @since 1.0.0
			 *
			 * @param string $status Product availability status.
			 * @param object $product Product object.
			 */
		 ...
		 * @param array  $product_attr Product attributes.
		 * @param array  $export_columns Export columns.
		 * @param string $key Export column key.
		 * @return string
		 */
		public function shipping( $catalog_attr, $product_attr, $export_columns, $key = '' ) {
			...
			if ( isset( $this->form_data['advanced_form_data']['wt_pf_file_as'] ) && 'xml' === $this->form_data['advanced_form_data']['wt_pf_file_as'] ) {
				/**
				 * Filter the TikTok product shipping XML.
				 *
				 * @since 1.0.0
				 *
				 * @param array $shipping_details_xml Product shipping details XML.
			...
			/**
			 * Filter the TikTok product shipping.
			 *
			 * @since 1.0.0
			 *
			 * @param string $shipping_str Product shipping string.
			 * @param array $shipping_details Product shipping details.
			 * @param object $product Product object.
			 */
		...
		public function custom_label_0( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the TikTok product custom label 0.
			 *
			 * @since 1.0.0
			 *
		...
		public function custom_label_1( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the TikTok product custom label 1.
			 *
			 * @since 1.0.0
			 *
		...
		public function custom_label_2( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the TikTok product custom label 2.
			 *
			 * @since 1.0.0
			 *
		...
		public function custom_label_3( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the TikTok product custom label 3.
			 *
			 * @since 1.0.0
			 *
		...
		public function custom_label_4( $catalog_attr, $product_attr, $export_columns ) {
			...
			/**
			 * Filter the TikTok product custom label 4.
			 *
			 * @since 1.0.0
			 *
		...
		public function tax_status( $catalog_attr, $product_attr, $export_columns ) {
			...
			return apply_filters( 'wt_feed_filter_product_tax_status', $this->product->get_tax_status(), $this->product );
				 ...
				 * @param array $query_vars The query args.
				 * @return array
				 */
