var wt_pf_basic_history=(function( $ ) {
	...
	var wt_pf_basic_history=
	{
				...
				success:function(data)
				{
					...
					if(data.status==1)
					{
						...
						}else
						{
							if(offset==0)
							{
								$('.wt_pf_history_loadmore_btn').off('click').on('click', function(){
									wt_pf_basic_history.load_page(history_id);
								});
							}
						}
		...
		reg_delete:function()
		{
			jQuery('.wt_pf_delete_history, .wt_pf_delete_log').on('click', function(){
		...
		{
			jQuery('.wt_pf_export_refresh_btn').on('click', function(){
		...
		{
			jQuery('.wt_pf_copy').on('click', function(){
		...
		reg_bulk_action:function()
		{
			...
			checkbox_main.off('click').on('click', function()
			{
				if($(this).is(':checked'))
				{
			...
			checkbox_sub.off('click').on('click', function()
			{
			...
			$('.wt_pf_bulk_action_btn').on('click', function(){
            ...
            $('.wt_pf_bulk_action_logs_btn').on('click', function(){
				if($('.wt_pf_history_checkbox_sub:checked').length>0 && $('.wt_pf_bulk_action option:selected').val()!="")
				{
					var cr_action=$('.wt_pf_bulk_action option:selected').val();
					if(cr_action=='delete')
