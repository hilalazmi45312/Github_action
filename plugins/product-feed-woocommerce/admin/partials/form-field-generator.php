...
foreach ( $form_fields as $key => $value ) {
	...
	*/
	...
	if ( isset( $value['help_text_conditional'] ) && is_array( $value['help_text_conditional'] ) ) {
		foreach ( $value['help_text_conditional'] as $help_text_config ) {
			if ( is_array( $help_text_config ) ) {
				...
				$conditional_help_html .= '<span class="wt-pf_form_help wt-pf_conditional_help_text" data-pf-help-condition="' . esc_attr( $condition_attr ) . '">' . $help_text_config['help_text'] . '</span>';
			}
		}
	}
	...
	if ( '' == $tr_html ) {
		...
		$form_data_key = ( substr( $field_name, 0, 8 ) !== 'wt_pf_' ? 'wt_pf_' : '' ) . $field_name;
		...
		<tr <?php echo wp_kses_post( $tr_id . $field_group_attr ); ?> class="<?php echo wp_kses_post( $tr_class ); ?>">
			<td colspan="3" class="wt_pf_field_group">
				<div class="wt_pf_field_group_hd">
					...
					<div class="wt_pf_field_group_toggle_btn" data-id="<?php echo isset( $value['group_id'] ) ? wp_kses_post( $value['group_id'] ) : ''; ?>" data-visibility="<?php echo wp_kses_post( $visibility ); ?>"><span class="dashicons dashicons-arrow-<?php echo ( 1 == $visibility ? 'down' : 'right' ); ?>"></span></div>
				</div>
		...
		<tr <?php echo wp_kses_post( $tr_id . $field_group_attr ); ?> class="<?php echo wp_kses_post( $tr_class ); ?>"  <?php echo wp_kses_post( $form_toggler_child ); ?>>
			...
			<td <?php echo wp_kses_post( $colspan_attr ); ?> class="<?php echo wp_kses_post( $td_class2 ); ?>">
				...
				if ( '' == $field_html ) {
					...
					if ( 'textarea' == $form_element_type ) {
						...
						<div class="wt_form_radio_block">
							<?php
							$radio_fields = array(
								'Yes' => __( 'Yes', 'product-feed-woocommerce' ),
								'No' => __( 'No', 'product-feed-woocommerce' ),
							);
							foreach ( $radio_fields as $rad_vl => $rad_label ) {
								?>
							<input <?php echo wp_kses_post( $css_attr ); ?> type="radio" id="<?php echo wp_kses_post( $form_data_key . '_' . $rad_vl ); ?>" name="<?php echo wp_kses_post( $form_data_key ); ?>" value="<?php echo wp_kses_post( $rad_vl ); ?>" <?php echo ( $vl == $rad_vl ) ? ' checked="checked"' : ''; ?> <?php echo wp_kses_post( $attr ); ?> <?php echo wp_kses_post( $form_toggler_register ); ?> /> <?php echo wp_kses_post( $rad_label ); ?>
						...
						<div class="wt_pf_file_attacher_dv">
							...
							<input type="button" name="upload_file" data-wt_pf_file_attacher_title="<?php esc_html_e( 'Choose a file.', 'product-feed-woocommerce' ); ?>"  data-wt_pf_file_attacher_button_text="<?php esc_html_e( 'Select', 'product-feed-woocommerce' ); ?>" class="wf_button button button-primary wt_pf_file_attacher" wt_pf_file_attacher_target="#<?php echo wp_kses_post( $field_id ); ?>" value="<?php esc_html_e( 'Upload', 'product-feed-woocommerce' ); ?>" />
						</div>
						<?php
						...
						<div id="<?php echo wp_kses_post( $dropzone_id ); ?>" class="wt_pf_dropzone" wt_pf_dropzone_target="#<?php echo wp_kses_post( $field_id ); ?>">
							<div class="dz-message">
								<?php esc_html_e( 'Drop files here or click to upload', 'product-feed-woocommerce' ); ?>
				...
				if ( '' != $help_text ) {
					?>
					<span class="wt-pf_form_help"><?php echo wp_kses_post( $help_text ); ?></span>
					<?php
				}
