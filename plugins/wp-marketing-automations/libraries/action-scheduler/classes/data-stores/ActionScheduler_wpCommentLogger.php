...
class ActionScheduler_wpCommentLogger extends ActionScheduler_Logger {
	...
	protected function get_comment_count() {
		...
		if ( ! $stats ) {
			$stats    = array();
			$count    = $wpdb->get_results( "SELECT comment_approved, COUNT( * ) AS num_comments FROM {$wpdb->comments} WHERE comment_type NOT IN('order_note','action_log') GROUP BY comment_approved", ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$total    = 0;
			$stats    = array();
			$approved = array(
				'0'            => 'moderated',
