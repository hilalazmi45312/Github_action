...
class ActionScheduler_HybridStore extends Store {
	...
	private function set_demarkation_id( $id = null ) {
		if ( empty( $id ) ) {
			...
			$id = (int) $wpdb->get_var( "SELECT MAX(ID) FROM $wpdb->posts" ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$id++;
		}
