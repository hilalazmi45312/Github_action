...
abstract class ActionScheduler_Store extends ActionScheduler_Store_Deprecated {
	...
	protected function validate_args( $args, $action_id ) {
		...
		if ( ! is_array( $args ) ) {
			throw ActionScheduler_InvalidActionException::from_decoding_args( $action_id ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}

		// Validate JSON decoding if possible.
		if ( function_exists( 'json_last_error' ) && JSON_ERROR_NONE !== json_last_error() ) {
			throw ActionScheduler_InvalidActionException::from_decoding_args( $action_id, $args ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped, WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
	}
	...
	protected function validate_schedule( $schedule, $action_id ) {
		if ( empty( $schedule ) || ! is_a( $schedule, 'ActionScheduler_Schedule' ) ) {
			throw ActionScheduler_InvalidActionException::from_schedule( $action_id, $schedule ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped, WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
	}
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		if ( strlen( wp_json_encode( $action->get_args() ) ) > static::$max_args_length ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'ActionScheduler_Action::$args too long. To ensure the args column can be indexed, action args should not be more than %d characters when encoded as JSON.', 'woocommerce' ), static::$max_args_length ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.Security.EscapeOutput.OutputNotEscaped, WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
	}
	...
	public function get_status_labels() {
		return array(
			self::STATUS_COMPLETE => __( 'Complete', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			self::STATUS_PENDING  => __( 'Pending', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			self::STATUS_RUNNING  => __( 'In-progress', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			self::STATUS_FAILED   => __( 'Failed', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			self::STATUS_CANCELED => __( 'Canceled', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		);
	}
