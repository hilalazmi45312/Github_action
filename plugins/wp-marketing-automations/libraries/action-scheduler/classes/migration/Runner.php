...
class Runner {
	...
	public function run( $batch_size = 10 ) {
		...
		if ( $this->progress_bar ) {
			...
			$this->progress_bar->set_message( sprintf( _n( 'Migrating %d action', 'Migrating %d actions', $batch_size, 'woocommerce' ), $batch_size ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			...
			if ( $destination_action_id ) {
				$this->destination_logger->log(
					...
					sprintf(
						...
						__( 'Migrated action with ID %1$d in %2$s to ID %3$d in %4$s', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						$source_action_id,
