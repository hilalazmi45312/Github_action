...
class Config {
	...
	public function get_source_store() {
		if ( empty( $this->source_store ) ) {
			throw new \RuntimeException( esc_html__( 'Source store must be configured before running a migration', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
	...
	public function get_source_logger() {
		if ( empty( $this->source_logger ) ) {
			throw new \RuntimeException( esc_html__( 'Source logger must be configured before running a migration', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
	...
	public function get_destination_store() {
		if ( empty( $this->destination_store ) ) {
			throw new \RuntimeException( esc_html__( 'Destination store must be configured before running a migration', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
	...
	public function get_destination_logger() {
		if ( empty( $this->destination_logger ) ) {
			throw new \RuntimeException( esc_html__( 'Destination logger must be configured before running a migration', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
