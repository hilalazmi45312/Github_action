...
class ActionScheduler_ActionFactory {
	...
	public function repeat( $action ) {
		...
		if ( is_null( $next ) || ! $schedule->is_recurring() ) {
			throw new InvalidArgumentException( __( 'Invalid action - must be a recurring action.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
		...
		try {
			...
			error_log(
				sprintf(
					...
					__( 'Caught exception while enqueuing action "%1$s": %2$s', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
					$options['hook'],
					$e->getMessage()
				)
			);
