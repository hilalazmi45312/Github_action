		...
		if ( ! has_action( $hook ) ) {
			throw new Exception(
				sprintf(
					...
					__( 'Scheduled action for %1$s will not be executed as no callbacks are registered.', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.Security.EscapeOutput.ExceptionNotEscaped
					$hook // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
				)
			);
		}
