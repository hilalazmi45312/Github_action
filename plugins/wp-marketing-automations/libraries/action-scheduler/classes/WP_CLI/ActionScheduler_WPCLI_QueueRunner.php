...
class ActionScheduler_WPCLI_QueueRunner extends ActionScheduler_Abstract_QueueRunner {
	...
	public function __construct( ?ActionScheduler_Store $store = null, ?ActionScheduler_FatalErrorMonitor $monitor = null, ?ActionScheduler_QueueCleaner $cleaner = null ) {
		if ( ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
			...
			throw new Exception( sprintf( esc_html__( 'The %s class can only be run within WP CLI.', 'woocommerce' ), __CLASS__ ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
	...
	public function setup( $batch_size, $hooks = array(), $group = '', $force = false ) {
		...
		if ( $this->has_maximum_concurrent_batches() ) {
			if ( $force ) {
				WP_CLI::warning( esc_html__( 'There are too many concurrent batches, but the run is forced to continue.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			} else {
				WP_CLI::error( esc_html__( 'There are too many concurrent batches.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			}
		}
	...
	protected function setup_progress_bar() {
		...
		$this->progress_bar = new ProgressBar(
			...
			sprintf( _n( 'Running %d action', 'Running %d actions', $count, 'woocommerce' ), $count ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			$count
		);
	}
	...
	public function run( $context = 'WP CLI' ) {
		...
		foreach ( $this->actions as $action_id ) {
			...
			if ( ! in_array( $action_id, $this->store->find_actions_by_claim_id( $this->claim->get_id() ), true ) ) {
				WP_CLI::warning( esc_html__( 'The claim has been lost. Aborting current batch.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				break;
			}
	...
	public function before_execute( $action_id ) {
		...
		WP_CLI::log( sprintf( esc_html__( 'Started processing action %s', 'woocommerce' ), $action_id ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
	}
	...
	public function after_execute( $action_id, $action = null ) {
		...
		WP_CLI::log( sprintf( esc_html__( 'Completed processing action %1$s with hook: %2$s', 'woocommerce' ), $action_id, $action->get_hook() ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
	}
	...
	public function action_failed( $action_id, $exception ) {
		WP_CLI::error(
			...
			sprintf( esc_html__( 'Error processing action %1$s: %2$s', 'woocommerce' ), $action_id, $exception->getMessage() ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			false
		);
	}
