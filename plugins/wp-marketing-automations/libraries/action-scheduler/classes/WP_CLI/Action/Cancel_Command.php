...
class Cancel_Command extends \ActionScheduler_WPCLI_Command {
	...
	protected function cancel_single( $hook, $callback_args, $group ) {
		if ( empty( $hook ) ) {
			\WP_CLI::error( __( 'Please specify hook of action to cancel.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
		...
		if ( null === $result ) {
			$e = new \Exception( __( 'Unable to cancel scheduled action: check the logs.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			$this->print_error( $e, false );
		}
	...
	protected function cancel_all( $hook, $callback_args, $group ) {
		if ( empty( $hook ) && empty( $group ) ) {
			\WP_CLI::error( __( 'Please specify hook and/or group of actions to cancel.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
		}
		 ...
		 */
		\WP_CLI::success( __( 'Request to cancel scheduled actions completed.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
	}
	...
	protected function print_success() {
		\WP_CLI::success( __( 'Scheduled action cancelled.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
	}
	...
	protected function print_error( \Exception $e, $multiple ) {
		\WP_CLI::error(
			sprintf(
				...
				__( 'There was an error cancelling the %1$s: %2$s', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				$multiple ? __( 'scheduled actions', 'woocommerce' ) : __( 'scheduled action', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				$e->getMessage()
			)
		);
	}
