	...
	protected function print_total_batches( int $batches_processed ) {
		WP_CLI::log(
			sprintf(
				...
				_n( '%d batch processed.', '%d batches processed.', $batches_processed, 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				$batches_processed
			)
		);
	}
	...
	protected function print_error( Exception $e ) {
		WP_CLI::error(
			sprintf(
				...
				__( 'There was an error deleting an action: %s', 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				$e->getMessage()
			)
		);
	}
	...
	protected function print_success( int $actions_deleted ) {
		WP_CLI::success(
			sprintf(
				...
				_n( '%d action deleted.', '%d actions deleted.', $actions_deleted, 'woocommerce' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				$actions_deleted
			)
		);
	}
