 ...
 * @since       2.2.0
 */
if ( ! class_exists( 'WFOCUKirki_Output_Property_Background_Position' ) ) {
	/**
	 * Output overrides.
	 */
	class WFOCUKirki_Output_Property_Background_Position extends WFOCUKirki_Output_Property {
		...
		protected function process_value() {
			...
			if ( ! is_null( $this->value ) && false !== strpos( $this->value, 'calc' ) ) {
				return;
			}
			...
			if ( ! is_null( $this->value ) && false !== strpos( $this->value, ' ' ) ) {
			...
			foreach ( $x_dimensions as $x_dimension ) {
				if ( ! is_null( $this->value ) && false !== strpos( $this->value, $x_dimension ) ) {
					$x = $x_dimension;
				}
			}
			$y = 'center';
			foreach ( $y_dimensions as $y_dimension ) {
				if ( ! is_null( $this->value ) && false !== strpos( $this->value, $y_dimension ) ) {
					$y = $y_dimension;
				}
			}
			$this->value = $x . ' ' . $y;
