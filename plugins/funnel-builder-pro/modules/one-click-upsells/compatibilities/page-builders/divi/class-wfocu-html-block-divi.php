...
if ( ! class_exists( 'WFOCU_Divi_HTML_BLOCK' ) ) {
	abstract class WFOCU_Divi_HTML_BLOCK extends WFOCU_Divi_Field {
		...
		final public function render( $attrs, $content = null, $render_slug = '' ) {
			...
			if ( apply_filters( 'wfocu_print_divi_widget', true, $this->get_slug(), $this ) ) {
		...
		public function localize_array() {
			...
			foreach ( $fields as $key => $field ) {
				if ( isset( $field['c_type'] ) && 'wfocu_start_border' == $field['c_type'] ) {
					$border_start                       = true;
					$border_data[ $field['field_key'] ] = $field['selector'];
					continue;
				}
				if ( isset( $field['c_type'] ) && 'wfocu_end_border' == $field['c_type'] ) {
					$border_start = false;
					continue;
				}
				...
				if ( isset( $field['c_type'] ) && 'wfocu_end_box_shadow' == $field['c_type'] ) {
					$box_start = false;
					continue;
				}
