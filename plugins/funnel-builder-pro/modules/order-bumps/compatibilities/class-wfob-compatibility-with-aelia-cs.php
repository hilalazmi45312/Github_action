...
if ( ! class_exists( 'WFOB_Compatibility_With_Aelia_CS' ) ) {
	class WFOB_Compatibility_With_Aelia_CS {
		...
		public function wfob_discount_regular_price_data( $price ) {
		...
		public function wfob_discount_price_data( $price ) {
			if ( false == $this->is_enable() ) {
				return $price;
			}
		...
		public function wfob_product_raw_data( $raw_data, $product ) {
			...
			if ( in_array( $product->get_type(), WFOB_Common::get_variation_product_type() ) ) {
				...
				$temp_sale = get_post_meta( $product->get_id(), 'variable_sale_currency_prices', true );
				if ( '' !== $temp_sale ) {
