...
class WooFunnel_WFOB {
	...
	public static function register() {
		$configuration = array(
			'basename'    => plugin_basename( WFOB_PLUGIN_FILE ),
			'version'     => self::$version,
			'plugin_path' => dirname( WFOB_PLUGIN_FILE ),
			'class'       => __CLASS__,
		);
	...
	public static function load_files() {
		 ...
		 */
		require_once dirname( WFOB_PLUGIN_FILE ) . '/woofunnels/' . 'includes/class-woofunnels-dashboard-loader.php';

		if ( self::$version === BWF_VERSION ) {
			do_action( 'woofunnels_loaded', $get_global_path );
		} elseif ( ( defined( 'WFOB_IS_DEV' ) && true === WFOB_IS_DEV ) || ( defined( 'BWF_DEV' ) && true === BWF_DEV ) ) {
