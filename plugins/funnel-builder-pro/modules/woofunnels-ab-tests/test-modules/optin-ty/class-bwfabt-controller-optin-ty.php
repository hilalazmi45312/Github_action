...
if ( ! class_exists( 'BWFABT_Controller_Optin_Ty' ) ) {
	/**
	 * Class contains all the optin_ty related ab testing functionality
	 * Class BWFABT_Controller_Optin_Ty
	 */
	#[AllowDynamicProperties]
	class BWFABT_Controller_Optin_Ty extends BWFABT_Controller {
		private static $ins = null;
		private $control_query;

		/**
		 * BWFABT_Controller_Optin_Ty constructor.
		 */
		public function __construct() {
			parent::__construct();
			add_filter( 'bwfabt_get_supported_controllers', array( $this, 'bwfabt_add_optin_ty_controller' ) );
			add_filter( 'wffn_oty_post_type_args', array( $this, 'wffn_add_control_meta_query' ), 10, 1 );
			add_action( 'wp', array( $this, 'maybe_redirect_to_ab_funnel' ), 1, 1 );
			add_action( 'wffn_event_step_viewed_optin_ty', array( $this, 'update_ab_optin_thankyou_visited' ) );
			add_action( 'wffn_event_step_converted_optin_ty', array( $this, 'update_ab_optin_thankyou_converted' ) );

			$this->control_query = false;
		}

		/**
		 * @return BWFABT_Controller_Optin_Ty|null
		 */
		 ...
		 * @return mixed
		 */
		public function bwfabt_add_optin_ty_controller( $controllers ) {
			$controllers['optin_ty'] = 'BWFABT_Controller_Optin_Ty';

			return $controllers;
		}

		/**
		 * Return title of optin_ty controller
		 */
		public function get_title() {
			return __( 'Optin Thank You', 'woofunnels-ab-tests' );
		}
		...
		public function duplicate_variant( $variant_data ) {
			...
			if ( $variant_id > 0 && $experiment_id > 0 ) {
				$new_post_id = WFOPP_Core()->optin_ty_pages->duplicate_oty_page( $variant_id );
		...
		/**
		 *
		 * Redirecting to ab test decided optin-ty page.
		 */
		public function maybe_redirect_to_ab_funnel() {
			...
			if ( $wffn_post_id > 0 && true === $this->validate_run( $wffn_post_id ) && WFOPP_Core()->optin_ty_pages->get_post_type_slug() === $post->post_type ) {
				$active_test_id = $this->get_running_test_id( $wffn_post_id );
				if ( $active_test_id > 0 ) {
					$experiment = BWFABT_Core()->admin->get_experiment( $active_test_id );
					$this->maybe_render_js_for_ab_experiment( $experiment, $wffn_post_id );
		...
		/**
		 * @param $optin_thankyou_id
		 */
		public function update_ab_optin_thankyou_visited( $optin_thankyou_id ) {
			$running_ab_test_id = $this->get_running_test_id_on_step( $optin_thankyou_id );
			WFFN_Core()->logger->log( "Updating AB optin thankyou id: $optin_thankyou_id visited, running AB test id: $running_ab_test_id" );
			if ( $optin_thankyou_id > 0 && $running_ab_test_id > 0 && class_exists( 'WFCO_Model_Report_views' ) ) {
				WFCO_Model_Report_views::update_data( date( 'Y-m-d', current_time( 'timestamp' ) ), $optin_thankyou_id, 17 );
				WFFN_Core()->logger->log( 'Updated AB optin thankyou id: ' . $optin_thankyou_id . ' visited (17) @: ' . current_time( 'mysql' ) );
			}
		}
		 ...
		 * @param $optin_thankyou_id
		 */
		public function update_ab_optin_thankyou_converted( $optin_thankyou_id ) {
			$running_ab_test_id = $this->get_running_test_id_on_step( $optin_thankyou_id );
			WFFN_Core()->logger->log( "Updating AB optin thankyou id: $optin_thankyou_id converted, running AB test id: $running_ab_test_id" );
			if ( $optin_thankyou_id > 0 && $running_ab_test_id > 0 && class_exists( 'WFCO_Model_Report_views' ) ) {
				WFCO_Model_Report_views::update_data( date( 'Y-m-d', current_time( 'timestamp' ) ), $optin_thankyou_id, 18 );
				WFFN_Core()->logger->log( 'Updated AB optin thankyou id: ' . $optin_thankyou_id . ' converted (18) @: ' . current_time( 'mysql' ) );
			}
		}
		...
		public function reset_stats( $experiment ) {
			$types = array( 17, 18 );
			$this->delete_ab_report_views( $experiment, $types );
		...
		public function get_analytics_data( $step_ids, $experiment_id, $is_interval = '', $int_request = '' ) {
			...
			$get_query = "SELECT object_id, SUM(CASE WHEN type = 17 THEN `no_of_sessions` END) AS `viewed` ,SUM(CASE WHEN type = 18 THEN `no_of_sessions` END) AS `converted` " . $params . " " . $interval_query . " FROM  `" . $wpdb->prefix . "wfco_report_views`  WHERE object_id IN (" . $step_ids . ") " . $date_query . " " . $group_by . " ORDER BY object_id ASC";
			...
			if ( is_array( $get_data ) && count( $get_data ) > 0 ) {
				foreach ( $get_data as $item ) {
					if ( isset( $data[ $item['object_id'] ] ) ) {
						$data[ $item['object_id'] ]['conversions']     = is_null( $item['converted'] ) ? 0 : intval( $item['converted'] );
	...
	BWFABT_Controller_Optin_Ty::get_instance();
}