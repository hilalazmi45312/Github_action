...
if ( ! class_exists( 'BWFABT_Report_Landing' ) ) {
	/**
	 * This class will render Landing tests reports
	 * Class BWFABT_Report_Landing
	 */
	#[AllowDynamicProperties]
	class BWFABT_Report_Landing extends BWFABT_Report {
		...
		/**
		 * BWFABT_Report_Landing constructor.
		 */
		...
		/**
		 * @return BWFABT_Report_Landing|null
		 */
		...
		public function bwfabt_add_order_bump_report( $reports ) {
			$reports['landing'] = 'BWFABT_Report_Landing';
		...
		public function get_performance_data( $experiment, $type ) {
			...
			$landing_views    = array();
			$landing_accepted = array();
			$landing_revenue  = array();
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$landing_views[ $variant_id ]    = isset( $landing_views[ $variant_id ] ) ? $landing_views[ $variant_id ] : 0;
				$landing_accepted[ $variant_id ] = isset( $landing_accepted[ $variant_id ] ) ? $landing_accepted[ $variant_id ] : 0;
				$landing_revenue[ $variant_id ]  = isset( $landing_revenue[ $variant_id ] ) ? $landing_revenue[ $variant_id ] : floatval( 0 );
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$landing_views[ $variant_id ] = array_sum( wp_list_pluck( wp_list_filter( $data, array( 'object_id' => $variant_id, 'type' => 13 ) ), 'no_of_sessions' ) );

				$landing_revenue[ $variant_id ]  = 0;
				$landing_accepted[ $variant_id ] = array_sum( wp_list_pluck( wp_list_filter( $data, array( 'object_id' => $variant_id, 'type' => 14 ) ), 'no_of_sessions' ) );;

				foreach ( $data as $view_data ) {
					if ( intval( $view_data['type'] ) !== 13 ) {
						continue;
					}
					foreach ( $dates_weekly as $weekly_date ) {
				...
				foreach ( $data as $landing_data ) {
					$landing_id   = intval( $landing_data['object_id'] );
					$landing_date = date( 'Y-m-d', strtotime( $landing_data['date'] ) );

					$revenue_daily[ $variant_id ][ $landing_date ] += 0;
					$revenue_daily[ $variant_id ][ $landing_date ] = round( $revenue_daily[ $variant_id ][ $landing_date ], 2 );

					$views_daily[ $variant_id ][ $landing_date ] = array_sum( wp_list_pluck( wp_list_filter( $data, array(
						'object_id' => $variant_id,
						'date'      => $landing_date,
						'type'      => 13
					) ), 'no_of_sessions' ) );

					$accepted_daily[ $variant_id ][ $landing_date ] = array_sum( wp_list_pluck( wp_list_filter( $data, array(
						'object_id' => $variant_id,
						'date'      => $landing_date,
						'type'      => 14
					) ), 'no_of_sessions' ) );

					if ( intval( $landing_data['type'] ) !== 13 ) {
						continue;
					}
					foreach ( $dates_weekly as $weekly_date ) {
						if ( ( $weekly_date['to'] >= strtotime( $landing_data['date'] ) ) && ( strtotime( $landing_data['date'] ) > $weekly_date['from'] ) ) {
							$revenue_weekly[ $variant_id ][ $weekly_date['label'] ]  += 0;
							$accepted_weekly[ $variant_id ][ $weekly_date['label'] ] += ( $landing_id === $variant_id ) ? $landing_data['no_of_sessions'] : 0;
						}
					}

					foreach ( $dates_monthly as $monthly_date ) {
						if ( ( $monthly_date['to'] >= strtotime( $landing_data['date'] ) ) && ( strtotime( $landing_data['date'] ) > $monthly_date['from'] ) ) {
							$revenue_monthly[ $variant_id ][ $monthly_date['label'] ]  += 0;
							$accepted_monthly[ $variant_id ][ $monthly_date['label'] ] += ( $landing_id === $variant_id ) ? $landing_data['no_of_sessions'] : 0;
						}
					}
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$table_data[ $new_variant_id ] = array(
					...
					'landing_views'         => $landing_views[ $variant_id ],
					'landing_accepted'      => $landing_accepted[ $variant_id ],
					'avg_revenue_per_visit' => '-',
					'conversion_rate'       => ( $landing_views[ $variant_id ] > 0 ) ? round( ( ( $landing_accepted[ $variant_id ] / $landing_views[ $variant_id ] ) * 100 ), 2 ) : 0,
					'total_revenue'         => $landing_revenue[ $variant_id ],
				);
			...
			$accepted      = array_sum( wp_list_pluck( $table_data, 'landing_accepted' ) );
			$total_views   = array_sum( wp_list_pluck( $table_data, 'landing_views' ) );
                    ...
                    <div class="accordionItemHeading">
                        <div class="wfabt_full_accord" style="border-left: 4px solid <?php echo esc_attr( $variant_colors[ $color_index ] ); ?>">
                            ...
                            <div class="wfabt_right_accord wfabt_right_accord_bnt">
                                <ul>
                                    <li>
                                        <span><?php echo sprintf( esc_html__( '%1$s %2$s ', 'woofunnels-ab-tests' ), esc_attr( $data['landing_views'] ), '<span class="bwfabt-label">' . esc_html__( 'Views', 'woofunnels-ab-tests' ) . '</span>' ) ?></span>
                                    </li>
                                    <li>
                                        <span><?php echo sprintf( esc_html__( '%1$s %2$s ', 'woofunnels-ab-tests' ), esc_attr( $data['landing_accepted'] ), '<span class="bwfabt-label">' . esc_html__( 'Conversion', 'woofunnels-ab-tests' ) . '</span>' ) ?></span>
                                    </li>
                                    <li>
		...
		public function get_choose_winner_table( $experiment ) {
            ...
            <table>
                <tr>
                    <th class="left"><?php esc_html_e( 'Sales-Pages/Variants', 'woofunnels-ab-tests' ); ?></th>
				...
				<?php foreach ( $table_data as $landing_id => $landing_data ) {
					if ( ! array_key_exists( $landing_id, $active_variants ) ) {
						continue;
					} ?>
                    <tr>
                        <td class="left">
							<?php echo ( true === $landing_data['control'] ) ? '<span class="bwfabt-control">' . esc_html__( 'Original', 'woofunnels-ab-tests' ) . '</span>' : ''; ?><br>
							<?php echo esc_html( $landing_data['title'] ); ?>
                        </td>
                        <td><?php echo esc_html( $landing_data['landing_views'] ) ?></td>
                        <td><?php echo esc_html( $landing_data['landing_accepted'] ) ?></td>
                        <td><?php echo esc_html( $landing_data['conversion_rate'] . '%' ) ?></td>
                        <td><?php echo wffn_price( $landing_data['total_revenue'] ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
                        <td>
                            <a data-funnel_id="<?php echo esc_attr( $landing_id ); ?>" class="choose_ab_winner wfabt_bg_no_act"><img src="<?php echo esc_url( BWFABT_PLUGIN_URL ) ?>/assets/img/trophy.png">
                            </a>
                        </td>
	...
	BWFABT_Report_Landing::get_instance();
