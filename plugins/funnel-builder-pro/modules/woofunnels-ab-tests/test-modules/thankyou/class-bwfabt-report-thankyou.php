...
if ( ! class_exists( 'BWFABT_Report_Thank_You' ) ) {
	/**
	 * This class will render Thank-You tests reports
	 * Class BWFABT_Report_Thank_You
	 */
	#[AllowDynamicProperties]
	class BWFABT_Report_Thank_You extends BWFABT_Report {
		...
		/**
		 * BWFABT_Report_Thank_You constructor.
		 */
		...
		/**
		 * @return BWFABT_Report_Thank_You|null
		 */
		...
		public function bwfabt_add_order_bump_report( $reports ) {
			$reports['thank_you'] = 'BWFABT_Report_Thank_You';
		...
		public function get_performance_data( $experiment, $type ) {
			...
			$query = "SELECT * FROM {table_name} WHERE `object_id` IN (" . esc_sql( implode( ',', $query_variant_ids ) ) . ") AND type=15 " . $date_query;
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$ty_views[ $variant_id ]    = isset( $ty_views[ $variant_id ] ) ? $ty_views[ $variant_id ] : 0;
				$ty_accepted[ $variant_id ] = isset( $ty_accepted[ $variant_id ] ) ? $ty_accepted[ $variant_id ] : 0;
				$ty_revenue[ $variant_id ]  = isset( $ty_revenue[ $variant_id ] ) ? $ty_revenue[ $variant_id ] : floatval( 0 );
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$ty_views[ $variant_id ] = array_sum( wp_list_pluck( wp_list_filter( $data, array( 'object_id' => $variant_id, 'type' => 15 ) ), 'no_of_sessions' ) );

				$ty_revenue[ $variant_id ]  = 0;
				$ty_accepted[ $variant_id ] = 0;
				...
				foreach ( $data as $ty_data ) {

					$ty_id   = intval( $ty_data['object_id'] );
					$ty_date = date( 'Y-m-d', strtotime( $ty_data['date'] ) );

					$revenue_daily[ $variant_id ][ $ty_date ] += 0;
					$revenue_daily[ $variant_id ][ $ty_date ] = round( $revenue_daily[ $variant_id ][ $ty_date ], 2 );

					$views_daily[ $variant_id ][ $ty_date ]    = array_sum( wp_list_pluck( wp_list_filter( $data, array( 'object_id' => $variant_id, 'date' => $ty_date ) ), 'no_of_sessions' ) );
					$accepted_daily[ $variant_id ][ $ty_date ] += ( $ty_id === $variant_id ) ? 1 : 0;

					foreach ( $dates_weekly as $weekly_date ) {
						if ( ( $weekly_date['to'] >= strtotime( $ty_data['date'] ) ) && ( strtotime( $ty_data['date'] ) > $weekly_date['from'] ) ) {
							$revenue_weekly[ $variant_id ][ $weekly_date['label'] ]  += 0;
							$accepted_weekly[ $variant_id ][ $weekly_date['label'] ] += ( $ty_id === $variant_id ) ? 1 : 0;
						}
					}

					foreach ( $dates_monthly as $monthly_date ) {
						if ( ( $monthly_date['to'] >= strtotime( $ty_data['date'] ) ) && ( strtotime( $ty_data['date'] ) > $monthly_date['from'] ) ) {
							$revenue_monthly[ $variant_id ][ $monthly_date['label'] ]  += 0;
							$accepted_monthly[ $variant_id ][ $monthly_date['label'] ] += ( $ty_id === $variant_id ) ? 1 : 0;
						}
					}
			...
			foreach ( $variant_ids as $variant_id ) {
				$control_id                    = get_post_meta( $variant_id, '_bwf_ab_control', true );
				$control_data                  = ( is_array( $control_data ) && isset( $control_data['control_id'] ) ) ? intval( $control_data['control_id'] ) : 0;
				...
				$table_data[ $new_variant_id ] = array(
					...
					'ty_views'              => $ty_views[ $variant_id ],
					'ty_accepted'           => $ty_accepted[ $variant_id ],
					'avg_revenue_per_visit' => '-',
					'conversion_rate'       => ( $ty_views[ $variant_id ] > 0 ) ? round( ( ( $ty_accepted[ $variant_id ] / $ty_views[ $variant_id ] ) * 100 ), 2 ) : 0,
					'total_revenue'         => $ty_revenue[ $variant_id ],
				);
			...
			$accepted      = array_sum( wp_list_pluck( $table_data, 'ty_accepted' ) );
			$total_views   = array_sum( wp_list_pluck( $table_data, 'ty_views' ) );
                    ...
                    <div class="accordionItemHeading">
                        <div class="wfabt_full_accord" style="border-left: 4px solid <?php echo esc_attr( $variant_colors[ $color_index ] ); ?>">
                            ...
                            <div class="wfabt_right_accord wfabt_right_accord_bnt">
                                <ul>
                                    <li>
                                        <span><?php echo sprintf( esc_html__( '%1$s %2$s ', 'woofunnels-ab-tests' ), esc_attr( $data['ty_views'] ), '<span class="bwfabt-label">' . esc_html__( 'Views', 'woofunnels-ab-tests' ) . '</span>' ) ?></span>
                                    </li>
                                    <li>
                                        <span><?php echo sprintf( esc_html__( '%1$s %2$s ', 'woofunnels-ab-tests' ), esc_attr( $data['ty_accepted'] ), '<span class="bwfabt-label">' . esc_html__( 'Conversion', 'woofunnels-ab-tests' ) . '</span>' ) ?></span>
                                    </li>
                                    <li>
		...
		public function get_choose_winner_table( $experiment ) {
            ...
            <table>
                <tr>
                    <th class="left"><?php esc_html_e( 'Thank-You-Pages/Variants', 'woofunnels-ab-tests' ); ?></th>
				...
				<?php foreach ( $table_data as $ty_id => $ty_data ) {
					if ( ! array_key_exists( $ty_id, $active_variants ) ) {
						continue;
					} ?>
                    <tr>
                        <td class="left">
							<?php echo ( true === $ty_data['control'] ) ? '<span class="bwfabt-control">' . esc_html__( 'Original', 'woofunnels-ab-tests' ) . '</span>' : ''; ?><br>
							<?php echo esc_html( $ty_data['title'] ); ?>
                        </td>
                        <td><?php echo esc_html( $ty_data['ty_views'] ) ?></td>
                        <td><?php echo esc_html( $ty_data['ty_accepted'] ) ?></td>
                        <td><?php echo esc_html( $ty_data['conversion_rate'] . '%' ) ?></td>
                        <td><?php echo wc_price( $ty_data['total_revenue'] ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
                        <td>
                            <a data-funnel_id="<?php echo esc_attr( $ty_id ); ?>" class="choose_ab_winner wfabt_bg_no_act"><img src="<?php echo esc_url( BWFABT_PLUGIN_URL ) ?>/assets/img/trophy.png">
                            </a>
                        </td>
	...
	BWFABT_Report_Thank_You::get_instance();
}