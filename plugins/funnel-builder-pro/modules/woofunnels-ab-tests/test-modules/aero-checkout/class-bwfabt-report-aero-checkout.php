...
/**
 * This class will render Aero tests reports
 * Class BWFABT_Report_Aero_Checkout
 */
if ( ! class_exists( 'BWFABT_Report_Aero_Checkout' ) ) {
	#[AllowDynamicProperties]
	class BWFABT_Report_Aero_Checkout extends BWFABT_Report {
		...
		/**
		 * BWFABT_Report_Aero_Checkout constructor.
		 */
		...
		/**
		 * @return BWFABT_Report_Aero_Checkout|null
		 */
		...
		public function bwfabt_add_order_bump_report( $reports ) {
			$reports['aero'] = 'BWFABT_Report_Aero_Checkout';
		...
		public function get_performance_data( $experiment, $type ) {
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$aero_views[ $variant_id ]    = ( isset( $aero_views[ $variant_id ] ) && $aero_views[ $variant_id ] > 0 ) ? $aero_views[ $variant_id ] : 0;
				$aero_accepted[ $variant_id ] = isset( $aero_accepted[ $variant_id ] ) ? $aero_accepted[ $variant_id ] : 0;
				$aero_revenue[ $variant_id ]  = isset( $aero_revenue[ $variant_id ] ) ? $aero_revenue[ $variant_id ] : floatval( 0 );
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$variant_filter = array( 'wfacp_id' => $variant_id );

				$aero_views[ $variant_id ]    = array_sum( wp_list_pluck( wp_list_filter( $data, array( 'object_id' => $variant_id ) ), 'no_of_sessions' ) );
				$aero_revenue[ $variant_id ]  = array_sum( wp_list_pluck( wp_list_filter( $aero_datum, $variant_filter ), 'total_revenue' ) );
				$aero_accepted[ $variant_id ] = count( wp_list_filter( $aero_datum, $variant_filter ) );
				...
				foreach ( $aero_datum as $aero_data ) {
					$aero_id   = intval( $aero_data['wfacp_id'] );
					$aero_date = date( 'Y-m-d', strtotime( $aero_data['date'] ) );

					$revenue_daily[ $variant_id ][ $aero_date ] += ( $aero_id === $variant_id ) ? floatval( $aero_data['total_revenue'] ) : floatval( 0 );
					$revenue_daily[ $variant_id ][ $aero_date ] = round( $revenue_daily[ $variant_id ][ $aero_date ], 2 );

					$views_daily[ $variant_id ][ $aero_date ]    = array_sum( wp_list_pluck( wp_list_filter( $data, array( 'object_id' => $variant_id, 'date' => $aero_date ) ), 'no_of_sessions' ) );
					$accepted_daily[ $variant_id ][ $aero_date ] += ( $aero_id === $variant_id ) ? 1 : 0;

					foreach ( $dates_weekly as $weekly_date ) {
						if ( ( $weekly_date['to'] >= strtotime( $aero_data['date'] ) ) && ( strtotime( $aero_data['date'] ) > $weekly_date['from'] ) ) {
							$revenue_weekly[ $variant_id ][ $weekly_date['label'] ]  += ( $aero_id === $variant_id ) ? floatval( $aero_data['total_revenue'] ) : floatval( 0 );
							$accepted_weekly[ $variant_id ][ $weekly_date['label'] ] += ( $aero_id === $variant_id ) ? 1 : 0;
						}
					}

					foreach ( $dates_monthly as $monthly_date ) {
						if ( ( $monthly_date['to'] >= strtotime( $aero_data['date'] ) ) && ( strtotime( $aero_data['date'] ) > $monthly_date['from'] ) ) {
							$revenue_monthly[ $variant_id ][ $monthly_date['label'] ]  += ( $aero_id === $variant_id ) ? floatval( $aero_data['total_revenue'] ) : floatval( 0 );
							$accepted_monthly[ $variant_id ][ $monthly_date['label'] ] += ( $aero_id === $variant_id ) ? 1 : 0;
						}
					}
				}
			}
			...
			foreach ( $variant_ids as $variant_id ) {
				...
				$table_data[ $new_variant_id ] = array(
					...
					'aero_views'            => ( $aero_views[ $variant_id ] > 0 ) ? $aero_views[ $variant_id ] : 0,
					'aero_accepted'         => $aero_accepted[ $variant_id ],
					'avg_revenue_per_visit' => ( $aero_views[ $variant_id ] > 0 ) ? ( $aero_revenue[ $variant_id ] / $aero_views[ $variant_id ] ) : 0,
					'conversion_rate'       => ( $aero_views[ $variant_id ] > 0 ) ? round( ( ( $aero_accepted[ $variant_id ] / $aero_views[ $variant_id ] ) * 100 ), 2 ) : 0,
					'total_revenue'         => $aero_revenue[ $variant_id ],
				);
			...
			$this->chart_data['dates']['weekly']  = ( $dates_weekly_label > 0 ) ? $dates_weekly_label : 0;
			$this->chart_data['dates']['monthly'] = $dates_monthly_label;

			$this->chart_data['revenue']['daily']    = $revenue_daily;
			$this->chart_data['views']['daily']      = ( $views_daily > 0 ) ? $views_daily : 0;
			...
			$this->chart_data['views']['weekly']      = ( $views_weekly > 0 ) ? $views_weekly : 0;
			...
			$this->chart_data['views']['monthly']      = ( $views_monthly < 0 ) ? 0 : $views_monthly;
			$this->chart_data['accepted']['monthly']   = $accepted_monthly;
			$this->chart_data['conversion']['monthly'] = $conversion_monthly;

			$this->table_data = $table_data;

			$accepted      = array_sum( wp_list_pluck( $table_data, 'aero_accepted' ) );
			$total_views   = array_sum( wp_list_pluck( $table_data, 'aero_views' ) );
			$total_revenue = array_sum( wp_list_pluck( $table_data, 'total_revenue' ) );

			$analytics['total_views']           = ( $total_views > 0 ) ? $total_views : 0;
			$analytics['total_conversion']      = $accepted;
			$analytics['avg_revenue_per_visit'] = ( $total_views > 0 ) ? wc_price( $total_revenue / $total_views ) : wc_price( 0 );
                    ...
                    <div class="accordionItemHeading">
                        <div class="wfabt_full_accord" style="border-left: 4px solid <?php echo esc_attr( $variant_colors[ $color_index ] ); ?>">
                            ...
                            <div class="wfabt_right_accord wfabt_right_accord_bnt">
                                <ul>
                                    <li>
                                        <span><?php echo sprintf( esc_html__( '%1$s %2$s ', 'woofunnels-ab-tests' ), esc_attr( $data['aero_views'] ), '<span class="bwfabt-label">' . esc_html__( 'Views', 'woofunnels-ab-tests' ) . '</span>' ) ?></span>
                                    </li>
                                    <li>
                                        <span><?php echo sprintf( esc_html__( '%1$s %2$s ', 'woofunnels-ab-tests' ), esc_attr( $data['aero_accepted'] ), '<span class="bwfabt-label">' . esc_html__( 'Conversion', 'woofunnels-ab-tests' ) . '</span>' ) ?></span>
                                    </li>
                                    <li>
		...
		public function get_choose_winner_table( $experiment ) {
            ...
            <table>
                <tr>
                    <th class="left"><?php esc_html_e( 'Aero-Pages/Variants', 'woofunnels-ab-tests' ); ?></th>
				...
				<?php foreach ( $table_data as $aero_id => $aero_data ) {
					if ( ! array_key_exists( $aero_id, $active_variants ) ) {
						continue;
					} ?>
                    <tr>
                        <td class="left">
							<?php echo ( true === $aero_data['control'] ) ? '<span class="bwfabt-control">' . esc_html__( 'Original', 'woofunnels-ab-tests' ) . '</span>' : ''; ?><br>
							<?php echo esc_html( $aero_data['title'] ); ?>
                        </td>
                        <td><?php echo esc_html( $aero_data['aero_views'] ) ?></td>
                        <td><?php echo esc_html( $aero_data['aero_accepted'] ) ?></td>
                        <td><?php echo esc_html( $aero_data['conversion_rate'] . '%' ) ?></td>
                        <td><?php echo wc_price( $aero_data['total_revenue'] ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
                        <td>
                            <a data-funnel_id="<?php echo esc_attr( $aero_id ); ?>" class="choose_ab_winner wfabt_bg_no_act"><img src="<?php echo esc_url( BWFABT_PLUGIN_URL ) ?>/assets/img/trophy.png">
                            </a>
                        </td>
	...
	BWFABT_Report_Aero_Checkout::get_instance();
}