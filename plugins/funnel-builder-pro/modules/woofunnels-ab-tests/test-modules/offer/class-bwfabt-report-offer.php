...
if ( ! class_exists( 'BWFABT_Report_Offer' ) ) {
	/**
	 * This class will render offer tests reports
	 * Class BWFABT_Report_Offer
	 */
	#[AllowDynamicProperties]
	class BWFABT_Report_Offer extends BWFABT_Report {
		/**
		 * @var null $ins
		 */
		...
		/**
		 * BWFABT_Report_Offer constructor.
		 */
		public function __construct() {
			parent::__construct();
			$this->table_data = array();

			add_filter( 'bwfabt_get_supported_reports', array( $this, 'bwfabt_add_offer_report' ) );
		}

		/**
		 * @return BWFABT_Report_Offer|null
		 */
		 ...
		 * @return mixed
		 */
		public function bwfabt_add_offer_report( $reports ) {
			$reports['offer'] = 'BWFABT_Report_Offer';
		...
		public function get_performance_data( $experiment, $type ) {
			...
			foreach ( array_keys( $variants ) as $funnel_id ) {
				...
				$table_data[ $funnel_id ] = array(
					...
					'avg_revenue_per_visit' => ( $funnel_triggered > 0 ) ? ( $total_upsells / $funnel_triggered ) : 0,
	...
	BWFABT_Report_Offer::get_instance();
}