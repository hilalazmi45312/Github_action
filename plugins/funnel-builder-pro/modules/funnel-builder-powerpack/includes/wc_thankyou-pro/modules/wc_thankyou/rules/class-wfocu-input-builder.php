<?php
if ( ! class_exists( 'wfty_Input_Builder' ) ) {
	/**
	 * Helper class to render input field types.
	 */
	class wfty_Input_Builder {
		...
		public static function create_input_field( $field_args, $value = null ) {
			...
			$field_args   = apply_filters( 'wfty_rules_get_input_defaults', self::get_input_field_defaults( $field_args ) );
			$field_value  = apply_filters( 'wfty_rules_get_input_value', $value, $field_args );
			$input_object = WFTY_Rules::get_instance()->woocommerce_wfty_rule_get_input_object( $field_args['input'] );
