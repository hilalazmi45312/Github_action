...
if ( ! class_exists( 'WFACP_GutenBerg' ) ) {
	...
	WFACP_GutenBerg {
		...
		public function get_cart_html() {
			...
			$settings = file_get_contents( 'php://input' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_php_input -- Reading request body
		...
		public function get_form_html() {
			...
			$json = file_get_contents( 'php://input' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_php_input -- Reading request body
			if ( '' !== $json ) {
			...
			include $template->wfacp_get_form(); // phpcs:ignore WordPressVIPMinimum.Files.IncludingFile.UsingVariable -- Template file inclusion
		...
		public function enqueue_block_editor_assets() {
			...
			if ( class_exists( 'WFACP_Common' ) && WFACP_Common::get_post_type_slug() === $post->post_type && 'post.php' === $pagenow && isset( $_GET['post'] ) && intval( $_GET['post'] ) > 0 ) { //phpcs:ignore
				...
				$assets     = file_exists( $assets_path ) ? include $assets_path : array( // phpcs:ignore WordPressVIPMinimum.Files.IncludingFile.UsingVariable -- Asset file inclusion
					'dependencies' => array(
						'wp-plugins',
						'wp-element',
						'wp-edit-post',
		...
		public static function mini_cart_default_attrs() {
			return [
				...
				'mini_cart_coupon_button_text'          => __( 'Apply', 'woocommerce' ),
				'mini_cart_enable_strike_through_price' => false,
				'mini_cart_enable_low_stock_trigger'    => false,
		...
		public function checkout_botton_icon_list() {
			$icon_list = [
				...
				'\e908' => __( 'Checkmark', 'woofunnels-aero-checkout' ),
		...
		public static function form_attributes() {
			...
			$attributes['collapsible_mini_cart_enable_strike_through_price'] = false;
			$attributes['collapsible_mini_cart_enable_low_stock_trigger']    = false;
			$attributes['collapsible_mini_cart_low_stock_message']           = __( '{{quantity}} LEFT IN STOCK', 'woofunnels-aero-checkout' );
			$attributes['collapsible_mini_cart_enable_saving_price_message'] = false;
			$attributes['collapsible_mini_cart_saving_price_message']        = __( 'You saved {{saving_amount}} ({{saving_percentage}}) on this order', 'woofunnels-aero-checkout' );
			$attributes['order_summary_field_enable_strike_through_price']   = false;
			$attributes['order_summary_field_enable_low_stock_trigger']      = false;
			$attributes['order_summary_field_low_stock_message']             = __( '{{quantity}} LEFT IN STOCK', 'woofunnels-aero-checkout' );
