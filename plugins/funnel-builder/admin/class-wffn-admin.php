...
if ( ! class_exists( 'WFFN_Admin' ) ) {
	...
	/**
	 * Check if any promotional campaign is active
	 * Uses the same logic as BF submenu registration
	 *
	 ...
	 *
	 * @return bool True if any campaign is active, false otherwise
	 */
	public function is_promotional_campaign_active() {
		...
		if ( WFFN_Core()->admin_notifications->show_bf_header_notification() ) {
			return true;
		}

		if ( WFFN_Core()->admin_notifications->show_small_business_saturday_header_notification() ) {
			return true;
		}

		if ( WFFN_Core()->admin_notifications->show_bfext_header_notification() ) {
			return true;
		}

		// CM group campaigns (2 campaigns)
		if ( WFFN_Core()->admin_notifications->show_cm_header_notification() ) {
			return true;
		}

		if ( WFFN_Core()->admin_notifications->show_cmext_header_notification() ) {
			return true;
		}
		...
		public function admin_enqueue_assets( $hook_suffix ) {
			...
			if ( $this->is_wffn_flex_page( 'all' ) ) {
				...
				if ( WFFN_Core()->admin->is_wffn_flex_page() ) {
					$this->load_react_app( 'main-20251127153058' ); //phpcs:ignore WordPressVIPMinimum.Security.Mustache.OutputNotation
